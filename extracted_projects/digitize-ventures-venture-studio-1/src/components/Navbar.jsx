...
export default function Navbar() {
  return (
    <nav className="fixed top-4 left-0 right-0 z-50 px-8 lg:px-16 py-3 flex items-center justify-between pointer-events-none">
      {/* Logo */}
      <div className="pointer-events-auto h-12 px-4 flex items-center justify-center gap-2 rounded-full bg-white/5 backdrop-blur-md border border-white/10">
        <iconify-icon icon="solar:box-minimalistic-bold" width="20" height="20" className="text-white"></iconify-icon>
        <span className="text-white font-body font-medium text-sm tracking-tight">Digitize</span>
      </div>

      {/* Center Links (Desktop) */}
      <div className="pointer-events-auto hidden md:flex items-center liquid-glass rounded-full px-1.5 py-1">
        {['Portfolio', 'Services', 'The Lab', 'Funding', 'Founder'].map((item) => (
          <Link
            key={item}
            to="/"

      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 h-full" style={{ paddingTop: '150px' }}>
        ...
        >
          ...
          <span className="text-white/80 font-body text-sm font-medium">
            Webs que traen clientes, no solo cumplen.
          </span>
        </motion.div>

        {/* Heading */}
        <BlurText 
          text="La web que tu negocio merece." 
          className="hero-heading text-6xl md:text-7xl lg:text-[6.5rem] font-heading italic text-foreground leading-[0.85] max-w-4xl tracking-tight justify-center"
          delay={0.1}
          stagger={0.1}
        />
        ...
        >
          Diseño profesional, rendimiento real y mantenimiento incluido. Hecha en días, lista para vender desde el primer clic.
        </motion.p>
        ...
        >
          <button className="liquid-glass-strong hover:scale-105 transition-transform duration-300 rounded-full px-6 py-3.5 flex items-center gap-2 text-white font-medium text-sm">
            Reservar llamada
            <iconify-icon icon="solar:arrow-right-up-linear" width="16" height="16" className="opacity-70"></iconify-icon>
          </button>
        ...
        <div className="mt-auto pb-12 pt-16 flex flex-col items-center w-full">
          ...
          <motion.div 
            ...
            className="flex flex-wrap justify-center gap-8 md:gap-14 opacity-70"
          >
            {[
              { name: 'Google', icon: 'simple-icons:google' },
              { name: 'Stripe', icon: 'simple-icons:stripe' },
              { name: 'WhatsApp Business', icon: 'simple-icons:whatsapp' },
              { name: 'Meta', icon: 'simple-icons:meta' },
              { name: 'WordPress', icon: 'simple-icons:wordpress' }
            ].map((partner) => (
              <div key={partner.name} className="flex items-center gap-2 text-xl md:text-2xl font-heading italic text-white hover:opacity-100 hover:scale-105 transition-all cursor-default">

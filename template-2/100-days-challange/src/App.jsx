...
<button className="md:hidden text-slate-400 hover:text-slate-900 transition-colors" onClick={(e) => { toggleMobileMenu() }}>
...
<select className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium py-1.5 pl-3 pr-8 rounded-md cursor-pointer outline-none transition-colors border-none tracking-tight" id="week-selector" onChange={(e) => { switchWeek(e.currentTarget.value) }}>
...
<button className="group flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-medium px-4 py-2 rounded-lg transition-colors shadow-sm w-full sm:w-auto" onClick={(e) => { openModal() }}>
...
<div className="absolute inset-0 bg-slate-900/20 backdrop-blur-sm transition-opacity" onClick={(e) => { closeModal() }}></div>
...
<button className="text-slate-400 hover:text-slate-600" onClick={(e) => { closeModal() }}>
<iconify-icon icon="solar:close-circle-linear" width="24"></iconify-icon>
</button>
</div>
<form className="space-y-4" onSubmit={(e) => { handleAddNew(event) }}>
<div>

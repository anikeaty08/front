...
<input accept="video/*" id="bg-video-upload" onChange={(e) => { (function(input){var f=input.files&&input.files[0];if(!f)return;var url=URL.createObjectURL(f);var v=document.getElementById('bg-video');var s=document.getElementById('bg-video-source');if(v&&s){s.src=url;s.type=f.type||'video/mp4';v.load();v.play().catch(function(){});}})(e.currentTarget); }} style={{display: `none`}} type="file" />
</div>
...
<button className="inline-flex transition hover:bg-amber-300 cursor-pointer text-sm font-semibold text-slate-950 bg-amber-400 w-full rounded-xl mt-2 pt-3 pr-5 pb-3 pl-5 shadow-lg gap-x-2 gap-y-2 items-center justify-center" onClick={(e) => { window.location.href='/mailto:gregory.dearham@gmail.com' }} role="button" type="submit">
                    Send Message

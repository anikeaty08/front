...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
      ...
      const genPill = document.getElementById('genPill'); // removed in DOM; safe no-op
      ...
      // Ensure song loops automatically until explicitly paused
      ...
      // NEW: Immediate UI feedback for generation
      let genUiEarlyTimer = null;
      function beginGeneratingUI() {
        hide(readyPill);
        show(genPill); // safe; no-op since pill removed
        loadingOverlay.classList.remove('hidden');
        const startId = genState.id;
        if (genUiEarlyTimer) clearTimeout(genUiEarlyTimer);
        genUiEarlyTimer = setTimeout(() => {
          if (genState.id === startId) {
      ...
      // ---------- LIVE tempo analyzer (real-time BPM -> swipe speed) ----------
      let bpmLive = null;
      const tempo = {
        started: false,
        source: null,
        ...
        interval: null,
        hopSec: 0.05,
        envBuf: new Float32Array(200),
        idx: 0,
        count: 0,
        ema: 0,
        td: null,
        routed: false // NEW: track routing to audio output
      };
      function startTempoAnalyzer() {
        try { ensureAudio(); } catch(e){}
        if (!audioCtx || !previewAudio) return;
        if (!tempo.source) {
          try { tempo.source = audioCtx.createMediaElementSource(previewAudio); } catch(e) { /* already created */ }
        }
        if (!tempo.analyser) {
          tempo.analyser = audioCtx.createAnalyser();
          tempo.analyser.fftSize = 2048;
          tempo.analyser.smoothingTimeConstant = 0.8;
          tempo.td = new Float32Array(tempo.analyser.fftSize);
        }
        // Ensure audio is actually heard: route analyser to master gain once
        if (tempo.source && tempo.analyser && !tempo.routed) {
          try { tempo.source.connect(tempo.analyser); } catch(e) {}
          try { if (masterGain) tempo.analyser.connect(masterGain); } catch(e) {}
          tempo.routed = true;
        }
        if (tempo.started) return;
        tempo.started = true;
        const hopMs = Math.max(10, Math.round(tempo.hopSec * 1000));
        tempo.interval = setInterval(sampleTempoFrame, hopMs);
      }
      function stopTempoAnalyzer() {
        if (tempo.interval) { clearInterval(tempo.interval); tempo.interval = null; }
        tempo.started = false;
        // Clean routing to avoid duplicate connections on next start
        if (tempo.analyser && tempo.routed) {
          try { tempo.analyser.disconnect(); } catch(e) {}
          tempo.routed = false;
        }
      }
      function sampleTempoFrame() {
        if (!tempo.analyser || !tempo.td) return;
        tempo.analyser.getFloatTimeDomainData(tempo.td);
        let e = 0;
        for (let i=0; i<tempo.td.length; i++) {
          const s = tempo.td[i];
          e += s*s;
        }
        e /= tempo.td.length;
        const alpha = 0.2;
        tempo.ema = (1 - alpha) * tempo.ema + alpha * e;
        const onset = Math.max(0, e - tempo.ema) / (tempo.ema + 1e-7);

        tempo.envBuf[tempo.idx] = onset;
        tempo.idx = (tempo.idx + 1) % tempo.envBuf.length;
        tempo.count = Math.min(tempo.count + 1, tempo.envBuf.length);

        const est = estimateBpmFromEnv();
        if (est) {
          if (bpmLive == null) bpmLive = est;
          else bpmLive = bpmLive * 0.8 + est * 0.2;
        }
      }
      function estimateBpmFromEnv() {
        const count = tempo.count;
        if (count < 40) return null;
        const buf = new Float32Array(count);
        for (let i=0; i<count; i++) {
          const idx = (tempo.idx - count + i + tempo.envBuf.length) % tempo.envBuf.length;
          buf[i] = tempo.envBuf[idx];
        }
        let mean = 0; for (let i=0;i<count;i++) mean += buf[i]; mean /= count;
        let varSum = 0; for (let i=0;i<count;i++) { const d = buf[i]-mean; varSum += d*d; }
        const std = Math.sqrt(varSum / Math.max(1, count-1)) || 1;
        for (let i=0;i<count;i++) buf[i] = (buf[i]-mean)/std;

        const minLag = Math.max(2, Math.round((60/180)/tempo.hopSec));
        const maxLag = Math.min(count-3, Math.round((60/60)/tempo.hopSec));
        ...
        let bpm = 60 / (bestLag * tempo.hopSec);
        while (bpm < 60) bpm *= 2;
        while (bpm > 180) bpm /= 2;
        return isFinite(bpm) ? bpm : null;
      }
      function getLiveRevealDuration() {
        if (bpmLive == null) return 5000;
        return bpmToRevealDuration(bpmLive);
      }
      // ---------- END LIVE tempo analyzer ----------

      function startBrushReveal(initialDuration = 5000) {
        ...
        function frame(now) {
          ...
          const dynDuration = getLiveRevealDuration() || initialDuration;
          p += dt / dynDuration;
          const pp = clamp(p, 0, 1);

          const n = Math.floor(strokes * (pp*pp));
          octx.save();
      ...
      // ---------- NEW: BPM-driven swipe duration ----------
      const bpmCache = new Map(); // key: trackId or src -> bpm
      ...
      function estimateBPMFromBuffer(buffer) {
        try {
          ...
          const hop = 512;
          const nFrames = Math.floor((data.length - frameSize) / hop);
          ...
          const winFrames = Math.max(2, Math.floor(0.20 / hopTime));
          const env = smooth(flux, winFrames);
          ...
          const maxLag = Math.min(env.length - 2, Math.round((60/60) / hopTime));
      ...
      function bpmToRevealDuration(bpm) {
        if (!isFinite(bpm) || bpm <= 0) return 5000;
        const minBPM = 60, maxBPM = 180;
        const minDur = 2600, maxDur = 8000;
        const b = clamp(bpm, minBPM, maxBPM);
        const t = (b - minBPM) / (maxBPM - minBPM);
        const dur = maxDur + (minDur - maxDur) * t;
        return Math.round(dur);
      }

      async function getRevealDuration(opts = {}) {
        const defaultDur = 5000;
        if (!previewAudio || previewAudio.paused || !previewAudio.src) return defaultDur;
        const key = currentTrack?.trackId || previewAudio.src;
        if (bpmCache.has(key)) return bpmToRevealDuration(bpmCache.get(key));
        const timeoutMs = typeof opts.timeoutMs === 'number' ? opts.timeoutMs : 300;
        const est = estimateBPMFromUrl(previewAudio.src).then(bpm => {
          if (bpm) bpmCache.set(key, bpm);
          return bpm;
        }).catch(() => null);
        const bpm = await Promise.race([est, new Promise(res => setTimeout(() => res(null), timeoutMs))]);
        return bpm ? bpmToRevealDuration(bpm) : defaultDur;
      }
      // ---------- END BPM ----------
      ...
      async function doGenerate(fullPrompt, opts = {}) {
        ...
        if (now - lastGenAt < 2500) return false;
        lastGenAt = now;
        ...
        try {
          ...
          imgBitmap = bitmap;
          resizeCanvases();
          const duration = await getRevealDuration({ timeoutMs: 400 });
          startBrushReveal(duration);
          ...
          try {
            ...
            const blob = await await fallback.blob();
          ...
          if (genUiEarlyTimer) { clearTimeout(genUiEarlyTimer); genUiEarlyTimer = null; }
        }
        return true;
      }
      ...
      function togglePlayPause() {
        if (previewAudio.paused) {
          if (currentTrack && selectedStyle) beginGeneratingUI();
          try { ensureAudio(); audioCtx && audioCtx.resume(); } catch(e){}
      ...
      function setPlayIcon(isPlaying) {
        playPauseBtn.innerHTML = `<i data-lucide="${isPlaying ? 'pause' : 'play'}" class="h-5 w-5 text-zinc-100" style="stroke-width: 1.5"></i>`;
        if (window.lucide) lucide.createIcons();
      }
      ...
      // If loop were to be disabled elsewhere, ensure end continues playback unless paused manually
      previewAudio.addEventListener('ended', () => {
        if (previewAudio.loop) {
          try { previewAudio.play(); } catch(e) {}
          return;
        }
        setPlayIcon(false);
        if (playHint) playHint.classList.remove('hidden');
      ...
      async function generateFromSong(track, opts = {}) {
        ...
        try {
          ...
          lyrics = await Promise.race([fetchLyrics(artist, title), timeout]);
        } catch(e) {
          lyrics = '';
        }
        ...
        if (seedKey === lastGenKey && opts.reason !== 'style-change') return;
        const seed = hashSeed(seedKey);

        const started = await doGenerate(basePrompt + stylePhrase, { seed });
        if (started) lastGenKey = seedKey;
      }
...
<div className="mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-center">
...
<h1 className="text-xl md:text-2xl tracking-tight font-semibold text-zinc-100">Painted Music</h1>
</div>
</div>
...
<button className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-gradient-to-br from-[#D1B97A] via-[#A6853A] to-[#6A5525] hover:brightness-110 border border-black/30 shadow-sm transition" id="playPauseBtn">
<i className="h-5 w-5 text-zinc-100" data-lucide="play" style={{strokeWidth: `1.5`}}></i>
</button>
</div>
</div>
</div>

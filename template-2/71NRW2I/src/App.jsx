...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
      ...
      // State
      // tokens now store objects: { value: string, op: "AND" | "OR" }
      let tokens = [];
      let suggestions = [];
      let chart;
      let chartMode = "pie"; // "pie" | "bubble"
      let lastSeedForReset = "";
      let logicalOperator = "AND"; // default operator for new tokens
      ...
      // Global default operator toggle
      function setOperator(op) {
        logicalOperator = op;
        if (andBtn && orBtn) {
          if (op === "AND") {
            andBtn.className = "inline-flex items-center rounded-md px-2.5 py-1.5 text-xs text-white bg-indigo-600 hover:bg-indigo-500 transition";
            orBtn.className = "inline-flex items-center rounded-md px-2.5 py-1.5 text-xs text-slate-300 hover:text-white hover:bg-slate-800 transition";
          } else {
            orBtn.className = "inline-flex items-center rounded-md px-2.5 py-1.5 text-xs text-white bg-indigo-600 hover:bg-indigo-500 transition";
            andBtn.className = "inline-flex items-center rounded-md px-2.5 py-1.5 text-xs text-slate-300 hover:text-white hover:bg-slate-800 transition";
          }
        }
      }

      // Tokens UI (with per-token operators)
      function renderTokens() {
        const existingChips = Array.from(tokenInput.querySelectorAll(".token-chip"));
        existingChips.forEach(c => c.remove());

        const hasManualSeed = !!seedInput.value.trim();

        tokens.forEach((t, idx) => {
          const op = t.op || logicalOperator;
          const isFirstNoSeed = !hasManualSeed && idx === 0;
          ...
          chip.innerHTML = `
            ...
            <div class="flex items-center">
              <div class="flex items-center gap-1 bg-slate-950 ring-1 ring-white/10 rounded-md p-0.5 ${isFirstNoSeed ? 'opacity-50 pointer-events-none' : ''}">
                <button class="op-and ${op==='AND'?'text-white bg-indigo-600':'text-slate-300 hover:text-white hover:bg-slate-800'} rounded px-2 py-0.5 transition">AND</button>
                <button class="op-or ${op==='OR'?'text-white bg-indigo-600':'text-slate-300 hover:text-white hover:bg-slate-800'} rounded px-2 py-0.5 transition">OR</button>
              </div>
            </div>
            <span>${titleCase(t.value)}</span>
            <button class="remove-btn p-0.5 rounded hover:bg-slate-800">
              <i data-lucide="x" class="w-3.5 h-3.5 text-slate-400"></i>
            </button>
          `;

          // Operator events
          const andEl = chip.querySelector(".op-and");
          const orEl = chip.querySelector(".op-or");
          andEl.addEventListener("click", (e) => {
            e.stopPropagation();
            tokens[idx].op = "AND";
            renderTokens();
            triggerGenerate();
          });
          orEl.addEventListener("click", (e) => {
            e.stopPropagation();
            tokens[idx].op = "OR";
            renderTokens();
            triggerGenerate();
        ...
        // Selected summary with operators between terms
        selectedTokens.innerHTML = "";
        const manual = seedInput.value.trim();
        if (manual) {
          const first = document.createElement("span");
          first.className = "inline-flex items-center gap-2 rounded-md px-2 py-1 text-[11px] bg-slate-950 ring-1 ring-white/10 text-slate-300";
          first.innerHTML = `<i data-lucide="circle" class="w-3 h-3 text-slate-500"></i>${titleCase(manual)}`;
          selectedTokens.appendChild(first);
        }
        tokens.forEach((t, idx) => {
          const sWrap = document.createElement("div");
          sWrap.className = "inline-flex items-center gap-2";
          if (manual || idx > 0) {
            const opPill = document.createElement("span");
            opPill.className = "inline-flex items-center rounded-md px-2 py-1 text-[10px] bg-slate-900 ring-1 ring-white/10 text-slate-300";
            opPill.textContent = t.op || logicalOperator;
            sWrap.appendChild(opPill);
          }
          ...
          s.innerHTML = `<i data-lucide="circle" class="w-3 h-3 text-slate-500"></i>${titleCase(t.value)}`;
          sWrap.appendChild(s);
          selectedTokens.appendChild(sWrap);
        });
      ...
      function addToken(t) {
        ...
        const exists = tokens.some(x => x.value.toLowerCase() === normalized);
        if (!exists) {
          // Use current global operator as default for the new token
          tokens.push({ value: normalized, op: logicalOperator });
          renderTokens();
        }
      }
      ...
      function triggerGenerate(shuffle=false) {
        ...
        if (!seed) return;
        const seedForSuggestions = seed.replace(/\s+(AND|OR)\s+/gi, " ");
        const data = buildSuggestions(seedForSuggestions);
        if (shuffle) {
        ...
        renderChart(data);
        lastSeedForReset = seedForSuggestions;
      }
      ...
      // Global default operator toggle events
      if (andBtn && orBtn) {
        andBtn.addEventListener("click", () => { setOperator("AND"); triggerGenerate(); });
        orBtn.addEventListener("click", () => { setOperator("OR"); triggerGenerate(); });
      }
...
<button className="inline-flex items-center rounded-md px-2.5 py-1.5 text-xs text-white bg-indigo-600 hover:bg-indigo-500 transition" id="andBtn">AND</button>
<button className="inline-flex items-center rounded-md px-2.5 py-1.5 text-xs text-slate-300 hover:text-white hover:bg-slate-800 transition" id="orBtn">OR</button>
</div>
</div>

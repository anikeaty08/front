      ...
      window.addEventListener('DOMContentLoaded', () => {
        ...
        if (langBtn && langPopover) {
          langBtn.addEventListener('click', () => {
            ...
            langBtn.setAttribute('aria-expanded', langPopover.classList.contains('hidden') ? 'false' : 'true');
          });
        ...
        const declineDisclaimer = document.getElementById('decline-disclaimer');
        ...
        function toggleModal(show) {
          ...
          disclaimerModal.classList.toggle('hidden', !show);
        }

        openDisclaimer?.addEventListener('click', () => toggleModal(true));
        closeDisclaimer?.addEventListener('click', () => toggleModal(false));
        declineDisclaimer?.addEventListener('click', () => toggleModal(false));
        document.getElementById('disclaimer-backdrop')?.addEventListener('click', () => toggleModal(false));
        ...
        docFile?.addEventListener('change', () => {
          const name = docFile.files && docFile.files[0] ? docFile.files[0].name : '';
          if (docFileName) docFileName.textContent = name;
        });
      });
...
<div className="flex items-center gap-3">
<img alt="user" className="h-9 w-9 rounded-full object-cover" src="https://images.unsplash.com/photo-1621619856624-42fd193a0661?w=1080&q=80" />
<div>
...
<div className="flex items-center justify-between">
<div className="flex items-center gap-3">
<img alt="chat user" className="h-9 w-9 rounded-full object-cover" src="https://images.unsplash.com/photo-1635151227785-429f420c6b9d?w=1080&q=80" />
<div>
...
<form className="rounded-2xl ring-1 p-5 sm:p-6 bg-white/5 ring-white/10 space-y-4" id="signup-form">
...
<select className="w-full bg-transparent outline-none text-sm" id="country">
...
<input accept="image/*" capture="environment" className="hidden" id="doc-file" type="file" />
...
<input className="peer sr-only" id="terms-checkbox" type="checkbox" />
...
<input className="flex-1 text-sm rounded-md ring-1 px-3 py-2 outline-none placeholder:text-neutral-500 focus:ring-indigo-500/40 bg-black/20 ring-white/10" placeholder="Seu e-mail" type="email" />
...
<img alt="" className="h-8 w-8 rounded-full object-cover" src="https://images.unsplash.com/photo-1635151227785-429f420c6b9d?w=1080&q=80" />
...
<div className="flex items-center gap-3 p-2 rounded-md ring-1 bg-black/20 ring-white/10">
<img alt="" className="h-8 w-8 rounded-full object-cover" src="https://images.unsplash.com/photo-1621619856624-42fd193a0661?w=1080&q=80" />
<div className="text-sm">
...
<p>© <span id="year">2025</span> Conexão. Todos os direitos reservados.</p>
<div className="flex items-center gap-4">
<a className="transition hover:text-white" data-i18n="footer.terms" href="#">Termos</a>
<a className="transition hover:text-white" data-i18n="footer.privacy" href="#">Privacidade</a>
<a className="transition flex items-center gap-1 hover:text-white" href="#">
<i className="h-3.5 w-3.5" data-lucide="mail"></i> <span data-i18n="footer.support">Suporte</span>
</a>
</div>
</div>
</div>
...
<div className="fixed inset-0 z-50 hidden" id="disclaimer-modal">
<div className="absolute inset-0 bg-black/70 backdrop-blur-sm" id="disclaimer-backdrop"></div>
<div className="relative mx-auto max-w-lg w-full px-4 sm:px-0">
<div className="mt-24 sm:mt-28 rounded-2xl ring-1 bg-neutral-950 ring-white/10 shadow-2xl shadow-black/50 overflow-hidden">
<div className="px-5 sm:px-6 py-4 flex items-center justify-between bg-white/5">
<div className="flex items-center gap-2">
<i className="h-5 w-5 text-blue-300" data-lucide="file-text"></i>
<h4 className="text-lg font-medium tracking-tight">Termo de Responsabilidade</h4>
</div>
<button aria-label="Fechar" className="inline-flex items-center justify-center h-8 w-8 rounded-md hover:bg-white/10 transition" id="close-disclaimer">
<i className="h-4 w-4" data-lucide="x"></i>
</button>
</div>
<div className="px-5 sm:px-6 py-5 space-y-3 text-sm text-neutral-300">
<p>• O aplicativo atua exclusivamente como meio de conexão entre pessoas viajantes e solicitantes de um favor/envio. Não prestamos serviço de transporte, logística, guarda de bens ou intermediação financeira.</p>
<p>• Toda negociação, entrega, valores e itens combinados são de responsabilidade única e exclusiva dos usuários envolvidos.</p>
<p>• O aplicativo não é e não se responsabiliza por perdas, danos, extravios, roubos, fraudes ou quaisquer eventos decorrentes da interação entre usuários.</p>
<p>• Para maior eficiência e segurança, recomendamos ativar a conta Premium, com mais informações de perfil, histórico e sinais de confiabilidade.</p>
<p>• Evite fraudes: verifique documentos, combine pontos públicos, registre evidências no chat e utilize o checklist de segurança.</p>
</div>
<div className="px-5 sm:px-6 py-4 flex items-center justify-between bg-white/5">
<a className="inline-flex items-center gap-2 px-3 py-2 rounded-md ring-1 ring-white/10 hover:bg-white/10 text-sm" href="#cadastro">
<i className="h-4 w-4" data-lucide="user-round-plus"></i>
              Voltar ao cadastro
            </a>
<div className="flex items-center gap-2">
<button className="inline-flex items-center gap-2 px-3 py-2 rounded-md bg-white/10 hover:bg-white/20 ring-1 ring-white/10 text-sm" id="decline-disclaimer">
                Fechar
              </button>
<button className="inline-flex items-center gap-2 px-3 py-2 rounded-md bg-indigo-500 hover:bg-indigo-400 text-white text-sm" id="accept-disclaimer">

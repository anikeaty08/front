...

{
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Правильный селектор для кнопки "Назначить звонок"
    const scheduleCallBtn = document.querySelector('footer a[href="#"]');
    
    console.log('Найдена кнопка:', scheduleCallBtn); // Для отладки
    ...
    if (scheduleCallBtn) {
      ...
      console.error('Кнопка "Назначить звонок" не найдена!');
    }
    ...
    if (scheduleForm) {
      scheduleForm.addEventListener('submit', function(e) {
        ...
        // Отправка через EmailJS - ЗАМЕНИТЕ эти значения на свои!
...
<div className="fixed left-6 top-1/2 -translate-y-1/2 z-40 hidden xl:block" id="statusIndicator" style={{transform: `translateY(calc(-50% - 101.3px)) scale(0.976255)`, filter: `drop-shadow(rgba(34, 197, 94, 0.55) 0px 0px 34.4554px)`}}>
...
<div className="fixed z-[60] pointer-events-none rounded-full" id="spotlight" style={{width: `220px`, height: `220px`, borderRadius: `9999px`, background: `white`, mixBlendMode: `difference`, opacity: `0`, transform: `translate(-50%, -50%)`, left: `1751px`, top: `239px`}}></div>
...
<div className="mt-4 rounded-lg overflow-hidden border border-white/10 transition-all duration-300 group-hover:border-white/20">
<img alt="Дизайн‑системы — превью" className="aspect-video w-full object-cover transition-transform duration-500 group-hover:scale-110" src="/assets/9d25c2d1-8a41-471b-bac8-c69712d13bb1_800w.jpg" style={{}} />
</div>
</div>
<div className="flex text-sm text-[#ffffff] mt-6 items-center">
<span>Learn more</span>
...
<div className="mt-4 min-h-[160px] font-mono text-sm leading-relaxed text-gray-300" id="codeContainer"><div style={{whiteSpace: `pre`, fontFamily: `ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace`, lineHeight: `1.6`, fontSize: `0.9rem`, borderRight: `none`}}><span style={{color: `#60A5FA`}}>// Дизайн‑система</span></div><div style={{whiteSpace: `pre`, fontFamily: `ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace`, lineHeight: `1.6`, fontSize: `0.9rem`, borderRight: `none`}}><span style={{color: `#60A5FA`}}>const</span> <span style={{color: `#FBBF24`}}>components</span> = <span style={{color: `#E5E7EB`}}>{"{"}</span></div><div style={{whiteSpace: `pre`, fontFamily: `ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace`, lineHeight: `1.6`, fontSize: `0.9rem`, borderRight: `2px solid rgb(16, 185, 129)`}}><span style={{color: `#E5E7EB`}}> </span></div></div>
</div>
...
<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 opacity-0 pointer-events-none transition-opacity duration-300" id="scheduleModal">
<div className="relative bg-gray-900 border border-white/20 rounded-2xl max-w-md w-full p-6 backdrop-blur-xl" onClick={(e) => { event.stopPropagation() }}>
<button className="absolute top-4 right-4 text-gray-400 hover:text-white" id="closeModal">
<svg className="lucide lucide-x" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path></path><path></path></svg>
</button>
<h3 className="text-xl font-semibold text-white mb-2">Запись на звонок</h3>
<p className="text-gray-400 mb-6">Выберите удобную дату и время для звонка</p>
<form className="space-y-4" id="scheduleForm">
<div>
<label className="block text-sm font-medium text-gray-300 mb-1" htmlFor="callDate">Дата *</label>
<input className="w-full bg-gray-800 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" id="callDate" name="date" required type="date" />
</div>
<div>
<label className="block text-sm font-medium text-gray-300 mb-1" htmlFor="callTime">Время *</label>
<select className="w-full bg-gray-800 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" id="callTime" name="time" required>
...
<div>
<label className="block text-sm font-medium text-gray-300 mb-1" htmlFor="contactInfo">Email или телефон *</label>
<input className="w-full bg-gray-800 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" id="contactInfo" name="contact" placeholder="Ваш email или номер телефона" required type="text" />
</div>
<div>
<label className="block text-sm font-medium text-gray-300 mb-1" htmlFor="message">Сообщение (необязательно)</label>
<textarea className="w-full bg-gray-800 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" id="message" name="message" placeholder="Кратко опишите тему разговора" rows="3"></textarea>
</div>
<button className="w-full bg-blue-600 text-white py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors" type="submit">
        Отправить заявку
      </button>
</form>
</div>
</div>
<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 opacity-0 pointer-events-none transition-opacity duration-300" id="successMessage">
<div className="relative bg-gray-900 border border-white/20 rounded-2xl max-w-md w-full p-6 backdrop-blur-xl text-center" onClick={(e) => { event.stopPropagation() }}>
<svg className="lucide lucide-check-circle mx-auto mb-4" fill="none" height="48" stroke="#10B981" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" width="48" xmlns="http://www.w3.org/2000/svg">
<path></path>
<path></path>
</svg>
<h3 className="text-xl font-semibold text-white mb-2">Заявка отправлена!</h3>
<p className="text-gray-400 mb-4">Я свяжусь с вами в ближайшее время для подтверждения звонка</p>
<button className="bg-blue-600 text-white py-2 px-6 rounded-lg font-medium hover:bg-blue-700 transition-colors" id="closeSuccess">
      Хорошо
    </button>
</div>
</div>

  ...
  useEffect(() => {
    ...
    const steps = [
      ...
      },
      {
        label: 'Temps Réel',
        privacy: '🔐 100 % Local',
        ideal:   'Parfait pour réunion live de toute durée (latence < 1 s).',
    ...
    /* -------------------------  DOM elements -------------------------- */
    const sliderThumb = document.getElementById('sliderThumb'); // <-- fix
...
<span className="flex items-center gap-1">Temps Réel
...
<span className="dot absolute left-1/2 -translate-x-1/2 -top-6 w-3 h-3 rounded-full border-2 border-amber-500 bg-gray-800 group-hover:bg-amber-400 transition"></span>
<span className="flex items-center gap-1">Cloud
              <span className="text-[10px] border border-amber-400 px-1.5 py-0.5 rounded-full">PRO</span>
</span>
</button>
</div>

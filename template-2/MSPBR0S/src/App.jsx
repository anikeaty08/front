...
// Функция для снижения качества на мобильных
function optimizeSplineForMobile() {
if (isMobileDevice()) {
const splineIframe = document.getElementById('aura-spline');
if (splineIframe) {
// Добавляем параметры для снижения качества
const originalSrc = splineIframe.src;
if (!originalSrc.includes('quality=')) {
splineIframe.src = originalSrc + (originalSrc.includes('?') ? '&' : '?') + 'quality=low';
}
// Дополнительные параметры для оптимизации
splineIframe.setAttribute('loading', 'lazy');
splineIframe.setAttribute('importance', 'low');
// Уменьшаем размеры для мобильных
setTimeout(() => {
splineIframe.style.transform = 'scale(0.5)';
splineIframe.style.transformOrigin = 'center center';
}, 1000);
}
}
}
// Запускаем после загрузки DOM
document.addEventListener('DOMContentLoaded', optimizeSplineForMobile);
...
<div className="aura-background-component top-0 w-full -z-10 absolute h-[975px] hue-rotate-15"><div className="spline-container absolute top-0 left-0 w-full h-full -z-10"><iframe allowtransparency="true" frameborder="0" height="100%" id="aura-spline" importance="low" loading="lazy" src="https://my.spline.design/retrofuturismbganimation-Lb3VtL1bNaYUnirKNzn0FvaW" width="100%"></iframe></div></div>
...
<img alt="UX/UI дизайн — превью" className="aspect-video w-full object-cover transition-transform duration-500 group-hover:scale-110" src="/assets/70c9e818-8e08-4e5c-ae2f-4fc0f3b2b30e_800w.jpg" />
</div>
</div>

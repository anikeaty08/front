...
<template x-if="user"><div className="flex items-center space-x-4"><span className="text-gray-600 text-sm"><span x-text="user.email"></span></span><button className="text-gray-500 hover:text-blue-600 text-sm"></button></div></template>
</nav>
</header>
...
<form className="bg-white rounded-lg shadow px-6 py-4 flex flex-col md:flex-row items-start md:items-end gap-4 w-full md:w-auto md:min-w-[400px]">
<div>
...
<template x-for="status in statuses"><option x-text="status"></option></template>
</select>
...
<template x-for="status in statuses"><div className="flex-1 bg-white rounded-lg shadow min-w-[250px] h-[540px] flex flex-col"><div className="px-4 py-3 border-b flex items-center justify-between"><span className="font-semibold text-gray-700" x-text="status"></span><span className="text-xs text-gray-400" x-text="filteredLeads.filter(l => l.status === status).length"></span></div><div className="flex-1 px-2 py-2 space-y-3 overflow-y-auto"><template x-for="lead in filteredLeads.filter(l => l.status === status)"><div className="bg-blue-50 border border-blue-100 rounded px-3 py-2 cursor-move shadow-sm hover:shadow-md transition relative group"><div className="font-semibold text-blue-700 truncate" x-text="lead.name"></div><div className="text-xs text-gray-500 truncate" x-text="lead.company"></div><div className="absolute top-1 right-2 text-gray-400 text-xs group-hover:opacity-100 opacity-0 transition"><svg className="inline w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path></path></svg></div></div></template></div></div></template>
</div>
</div>
...
<button className="absolute top-2 right-3 text-gray-400 hover:text-blue-600">
<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeWidth="2"></path></svg>
</button>
...
<select className="border rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-200" x-model="selectedLead.status">
<template x-for="status in statuses"><option x-text="status"></option></template>
</select>
</div>
<div>
...
<textarea className="border rounded px-2 py-1 w-full min-h-[60px] focus:outline-none focus:ring-1 focus:ring-blue-200" x-model="selectedLead.notes"></textarea>
</div>
<div className="mt-6 flex justify-between items-center">
<button className="text-red-600 hover:underline text-sm">Delete Lead</button>
<button className="bg-blue-600 text-white px-5 py-2 rounded font-semibold hover:bg-blue-700 transition">Close</button>
</div>
</div>
</div>
</main>

...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
    ...
    function renderDays() {
      ...
      days.forEach((d, i)=>{
        ...
        el.innerHTML = `
          ...
          <div class="text-[10px] mt-1 text-slate-500">${d.toLocaleDateString(undefined,{weekday:'short'})}</div>
        `;
    ...
    function placeTasks(grid, laneName) {
      ...
      if (laneName) tasks = tasks.filter(t=>t.lane===laneName);
    ...
    function renderLanes() {
      ...
      // Single track timeline (no category headers)
    ...
    function attachInteractions() {
      ...
      fitBtn?.addEventListener('click', ()=>{
        ...
        state.range.start = minStart;
        state.range.end = maxEnd;
        startInput.value = state.range.start;
        endInput.value = state.range.end;
        renderAll();
      });
      ...
      // Zoom controls
      const getDayW = () => parseInt(getComputedStyle(document.documentElement).getPropertyValue('--day-width')) || 86;
      const setDayW = (px) => {
        document.documentElement.style.setProperty('--day-width', px + 'px');
        // Reposition cursor after layout change
        requestAnimationFrame(()=>positionCursorLine());
      };
      const Z = { min: 44, max: 120, step: 8 };
      zoomOutBtn?.addEventListener('click', ()=>{
        const next = Math.max(Z.min, getDayW() - Z.step);
        setDayW(next);
      });
      zoomInBtn?.addEventListener('click', ()=>{
        const next = Math.min(Z.max, getDayW() + Z.step);
        setDayW(next);
      });
    }
...
<button className="hidden xs:inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 text-sm" id="fitBtn">
<i className="w-4 h-4" data-lucide="scan"></i>
<span>Fit</span>
</button>

<div className="inline-flex-center rounded-xl border border-slate-200 overflow-hidden">
<button className="px-2 py-1.5 hover:bg-slate-50 text-slate-700" id="zoomOutBtn">
<i className="w-4 h-4" data-lucide="minus"></i>
</button>
<div className="h-5 w-px bg-slate-200"></div>
<button className="px-2 py-1.5 hover:bg-slate-50 text-slate-700" id="zoomInBtn">

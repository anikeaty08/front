...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
    ...
    function loadInvestments(){
      ...
      investments.forEach(inv=>{
        ...
        card.className='w-full border border-[#A8A8A8] rounded-[12px] p-4 flex flex-col text-left gap-3';
...
<header className="h-14 flex items-center px-4 py-3 border-b border-gray-100 bg-[#ffffff] mb-6">
<button className="-ml-2 p-2 hidden" id="backBtn" onclick="prevScreen()">
<i className="ph ph-caret-left w-6 h-6 text-[#161616]"></i>
</button>
<h1 className="ml-3 text-[18px] font-normal tracking-tight" id="headerTitle">Inversión+</h1>
</header>

<main className="flex-1 relative">

<section className="w-full h-full flex flex-col px-4 pt-6 pb-6 gap-6" data-screen="0">
...
<button className="bg-[#00D180] text-[#161616] h-14 w-full rounded-[12px] font-semibold" onclick="nextScreen()">Comenzar</button>
</div>
</section>

<section className="hidden w-full h-full flex flex-col px-4 pt-6 pb-6 gap-6" data-screen="1">
...
<input className="w-full h-14 border border-[#A8A8A8] rounded-[12px] px-4 text-[15px] focus:outline-none focus:ring-2 focus:ring-[#00D180]" id="amountInput" min="100" oninput="updateSimulation()" step="100" type="number" value="1000"/>
</label>

<label className="flex flex-col gap-2">
<span className="text-[14px]">Plazo (días)</span>
<select className="w-full h-14 border border-[#A8A8A8] rounded-[12px] px-4 text-[15px] focus:outline-none focus:ring-2 focus:ring-[#00D180]" id="termSelect" onchange="updateSimulation()">
...
<div className="mt-auto">
<button className="bg-[#00D180] text-[#161616] h-14 w-full rounded-[12px] font-semibold" onclick="goToConfirm()">Continuar</button>
</div>
</section>

<section className="hidden w-full h-full flex flex-col px-4 pt-6 pb-6 gap-6" data-screen="2">
...
<label className="flex items-start gap-3 mt-2 text-[14px] select-none">
<input className="w-6 h-6 accent-[#00D180] rounded-md" id="termsCheck" type="checkbox"/>
            Acepto los Términos y Condiciones de Inversión+
          </label>
</div>
<div className="mt-auto">
<button className="bg-[#00D180] text-[#161616] h-14 w-full rounded-[12px] font-semibold opacity-50" disabled="" id="confirmBtn" onclick="createInvestment()">Confirmar inversión</button>
</div>
</section>
...
<section className="hidden w-full h-full flex flex-col items-center px-4 pt-6 pb-6 gap-6" data-screen="4">
...
<button className="bg-[#00D180] text-[#161616] h-14 w-full rounded-[12px] font-semibold" onclick="nextScreen()">Continuar</button>
</div>
</section>

<section className="hidden w-full h-full flex flex-col px-4 pt-6 pb-6 gap-6" data-screen="5">
<h2 className="text-[22px] font-semibold tracking-tight">Al finalizar tu inversión</h2>
<div className="flex flex-col gap-4">
<button className="post-option w-full border border-[#A8A8A8] rounded-[12px] h-14 flex items-center justify-between px-4 text-[14px] transition hover:border-[#00D180]/60" onclick="selectPostAction(0)">Reinvertir monto + ganancias</button>
<button className="post-option w-full border border-[#A8A8A8] rounded-[12px] h-14 flex items-center justify-between px-4 text-[14px] transition hover:border-[#00D180]/60" onclick="selectPostAction(1)">Reinvertir solo el monto</button>
<button className="post-option w-full border border-[#A8A8A8] rounded-[12px] h-14 flex items-center justify-between px-4 text-[14px] transition hover:border-[#00D180]/60" onclick="selectPostAction(2)">Mover todo a Stori Cuenta+</button>
</div>
<div className="mt-auto">
<button className="bg-[#00D180] text-[#161616] h-14 w-full rounded-[12px] font-semibold opacity-50" disabled="" id="postContinue" onclick="nextScreen()">Guardar Preferencia</button>
</div>
</section>

<section className="hidden w-full h-full flex flex-col px-4 pt-6 pb-6 gap-4" data-screen="6">
<h2 className="text-[22px] font-semibold tracking-tight">Tus inversiones</h2>
<p className="text-[14px] mb-2" id="totalInv"></p>
<div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-1" id="invList"></div>
<div>
<button className="bg-[#00D180] text-[#161616] h-14 w-full rounded-[12px] font-semibold" onclick="resetToSim()">Crear nueva inversión</button>
</div>
</section>

<section className="hidden w-full h-full flex flex-col px-4 pt-6 pb-6 gap-6" data-screen="7">
...
<div className="mt-auto">
<button className="bg-[#00D180] text-[#161616] h-14 w-full rounded-[12px] font-semibold" onclick="backToList()">Volver</button>
</div>
</section>
</main>
</div>

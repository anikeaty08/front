...
export default function App() {
  useEffect(() => {
    ...
    qs('#joinBtn').onclick = () => {
      ...
      if (!emailRe.test(email)) { qs('#emailInput').focus(); return; }
    ...
    function buildQuestion() {
      ...
      qs('#stepIndicator').textContent = `Step ${currentQ + 1} of ${survey.length}`;
      qs('#progressBar').style.width = `${(currentQ + 1) / survey.length * 100}%`;
    ...
    qs('#nextQ').onclick = () => {
      ...
      if (!item.validate(val)) { qs('#questionWrapper input').classList.add('border-red-500'); qs('#questionWrapper input').focus(); return; }
    ...
    function buildLeaderboard(myCode) {
      ...
      qs('#lbList').innerHTML = mock.map((p,i)=>`<div><span class="text-neutral-400 mr-2">#${i+1}</span>${p.name} <span class="text-neutral-500">– ${p.count}</span></div>`).join('');
...
<div className="text-sm text-neutral-400" id="stepIndicator"></div>

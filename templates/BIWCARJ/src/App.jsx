...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Subject/CC/BCC editor toggle
      const subjectToggle = document.querySelector('[data-action="toggle-subject"]');
      const subjectPanel = document.getElementById('subject-panel');
      const subjectChevron = subjectToggle ? subjectToggle.querySelector('[data-lucide="chevron-down"]') : null;
      if (subjectToggle && subjectPanel) {
        subjectToggle.addEventListener('click', () => {
          const isOpen = subjectPanel.classList.toggle('hidden') === false;
          if (subjectChevron) subjectChevron.classList.toggle('rotate-180', isOpen);
          subjectToggle.setAttribute('aria-expanded', String(isOpen));
        });
      }

      // Files coverage donut chart (removed canvas; safe guard retained)
      ...
      if (filesChart) {
        ...
        new Chart(ctx, {
          type: 'doughnut',
          data: {
            labels: ['Q4_Marketing_Budget.xlsx', 'Campaign_Strategy.pdf'],
            datasets: [{
              data: [65, 35],
...
<i className="w-3.5 h-3.5" data-lucide="files"></i>
<span>Files Analyzed</span>
</div>
<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">

<div className="sm:col-span-2 space-y-2">
<div className="border border-neutral-800 rounded-md p-2 bg-neutral-900/40">
<div className="flex items-center justify-between text-xs">
<div className="flex items-center gap-1.5">
<i className="w-3.5 h-3.5 text-neutral-300" data-lucide="file-spreadsheet"></i>
<span className="font-medium text-neutral-100">Q4_Marketing_Budget.xlsx</span>
</div>
<span className="px-1.5 py-0.5 rounded border border-neutral-800 text-[10px] text-neutral-300">Attached</span>
</div>
<div className="mt-2 space-y-1 text-xs text-neutral-300">
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 text-emerald-400 mt-0.5" data-lucide="check-circle-2"></i>
<span>Decision: Approve Q4 allocation ($2.3M)</span>
</div>
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 text-teal-400 mt-0.5" data-lucide="circle-dollar-sign"></i>
<span>Proposed split: 70% digital (prev. 65%)</span>
</div>
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 text-cyan-400 mt-0.5" data-lucide="table"></i>
<span>Relevant: Benchmarks; Sensitivity D18–F24</span>
</div>
</div>
</div>
<div className="border border-neutral-800 rounded-md p-2 bg-neutral-900/40">
<div className="flex items-center justify-between text-xs">
<div className="flex items-center gap-1.5">
<i className="w-3.5 h-3.5 text-neutral-300" data-lucide="file-text"></i>
<span className="font-medium text-neutral-100">Campaign_Strategy.pdf</span>
</div>
<span className="px-1.5 py-0.5 rounded border border-neutral-800 text-[10px] text-neutral-300">Attached</span>
</div>
<div className="mt-2 space-y-1 text-xs text-neutral-300">
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 text-indigo-400 mt-0.5" data-lucide="megaphone"></i>
<span>Initiatives aligned to increased digital spend</span>
</div>
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 text-amber-400 mt-0.5" data-lucide="clock"></i>
<span>Timing dependencies: creative and channel rollouts</span>
</div>
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 text-emerald-400 mt-0.5" data-lucide="check-circle-2"></i>
<span>Use to validate spend rationale before approval</span>
</div>
</div>
</div>
</div>
...
<div className="border border-neutral-800 rounded-md p-2 bg-neutral-900/40">
<div className="text-[11px] text-neutral-400 mb-1.5">Why these files matter</div>
<div className="space-y-1 text-xs text-neutral-300">
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 mt-0.5" data-lucide="message-square"></i>
<span>They directly support Sarah's urgent approval request.</span>
</div>
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 text-rose-400 mt-0.5" data-lucide="alert-triangle"></i>
<span>Pay attention to ROI sensitivity (cells D18–F24).</span>
</div>
<div className="flex items-start gap-1.5">
<i className="w-3.5 h-3.5 text-neutral-400 mt-0.5" data-lucide="users"></i>
<span>Loop in Alex (benchmarks) and Mark (ROI notes) if questions.</span>
</div>
</div>
</div>
</div>
...
<button aria-expanded="false" className="w-full bg-neutral-50 px-3 py-2 flex items-center justify-between border border-neutral-200 rounded-md" data-action="toggle-subject">
<div className="flex items-center gap-2 text-xs font-medium text-neutral-700">
<i className="w-3.5 h-3.5" data-lucide="sliders-horizontal"></i>
<span>Edit Subject / CC / BCC</span>
</div>
<i className="w-4 h-4 transition-transform" data-lucide="chevron-down"></i>
</button>
<div className="hidden border border-neutral-200 border-t-0 rounded-b-md p-3 bg-white" id="subject-panel">
<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
<div className="md:col-span-3">
<label className="text-[11px] text-neutral-600 font-medium">Subject</label>
<input className="mt-1 w-full text-sm border border-neutral-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Add a subject" type="text"/>
</div>
<div>
<label className="text-[11px] text-neutral-600 font-medium">CC</label>
<input className="mt-1 w-full text-sm border border-neutral-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Add CC recipients" type="text"/>
</div>
<div>
<label className="text-[11px] text-neutral-600 font-medium">BCC</label>
<input className="mt-1 w-full text-sm border border-neutral-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Add BCC recipients" type="text"/>
</div>
</div>
</div>
</div>
...
<textarea className="w-full resize-none border border-neutral-300 rounded-md px-3 pr-12 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm" placeholder="Type your response..." rows="2"></textarea>
<button aria-label="Attach file" className="absolute right-2 bottom-2 inline-flex items-center justify-center h-7 w-7 rounded-md text-neutral-600 hover:text-neutral-900 bg-white border border-neutral-200 hover:bg-neutral-50" type="button">
...
<div className="border border-neutral-200 rounded-lg divide-y divide-neutral-200">
<div className="flex items-center justify-between px-3 py-2 text-sm">
<span>Q4 Budget Planning</span>Active
</div>
<div classflex="" items-center="" justify-between="" px-3="" py-2="" text-sm"="">

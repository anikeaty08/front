    ...
    try {
      ...
      function onCU1Toggle(value) {
        ...
        if (value === 'yes') {
          ...
          // hide hint remains showing
        }
        validateCU();
      }
      function onCU2Toggle(value) {
        validateCU();
      }
      function validateCU() {
        const field = document.querySelector('[data-field-id="c4-cu"]');
        if (!field) return;
        const cu1Yes = document.querySelector('[data-toggle-group="cu1"] .seg-btn[data-value="yes"][data-state="on"]');
        const cu2Yes = document.querySelector('[data-toggle-group="cu2"] .seg-btn[data-value="yes"][data-state="on"]');
        const cu1Date = document.querySelector('[data-cu="cu1-date"]')?.value?.trim();
        const cu1Lieu = document.querySelector('[data-cu="cu1-lieu"]')?.value?.trim();
        const cu2Date = document.querySelector('[data-cu="cu2-date"]')?.value?.trim();
        const cu2Lieu = document.querySelector('[data-cu="cu2-lieu"]')?.value?.trim();

        let ok = true;
        if (cu1Yes) {
          ok = ok && !!cu1Date && !!cu1Lieu;
        }
        if (cu2Yes) {
          ok = ok && !!cu2Date && !!cu2Lieu;
        }
        field.dataset.status = ok ? 'valid' : 'warning';
        updateProgress();
      }
      document.querySelectorAll('[data-cu]').forEach(inp => {
        inp.addEventListener('input', validateCU);
        inp.addEventListener('change', validateCU);
      });

      // Cadre 6 — Dérogations / écarts: choices and drawer
      document.querySelectorAll('.choice-card').forEach(btn => {
        btn.addEventListener('click', () => {
          const choice = btn.getAttribute('data-choice');
          const field = document.querySelector('[data-field-id="c6-derogation"]');
          const area = document.getElementById('c6-textarea');
          const wrap = document.getElementById('c6-derogation-field');
          if (choice === 'respect') {
            wrap?.classList.add('hidden');
            field.dataset.status = 'valid';
            updateProgress();
          } else if (choice === 'modify') {
            document.getElementById('cadre-2')?.setAttribute('open','');
            document.getElementById('c2-textarea')?.focus();
          } else if (choice === 'justify') {
            wrap?.classList.remove('hidden');
            field.dataset.status = (area?.value?.trim().length > 20) ? 'valid' : 'blocking';
            updateProgress();
            document.getElementById('openJustifyDrawer')?.focus();
          }
        });
      });
      document.getElementById('openJustifyDrawer')?.addEventListener('click', () => {
        document.getElementById('justifyDrawer')?.classList.remove('hidden');
      });
      document.querySelectorAll('[data-justify-close]').forEach(el => {
        el.addEventListener('click', () => document.getElementById('justifyDrawer')?.classList.add('hidden'));
      });

      // Drawer library examples
      const EXAMPLES = {
        harmonie: [
          'Le projet reprend l’alignement, l’épannelage et les teintes des constructions voisines afin d’assurer une lecture urbaine cohérente.',
          'Les matériaux choisis présentent une finition mate et des teintes naturelles limitant l’impact visuel dans le site.'
        ],
        integration: [
          'L’implantation préserve les vues lointaines, les arbres remarquables et les éléments structurels du paysage.',
          'La gestion des eaux pluviales est assurée à la parcelle (noues, citerne) afin d’éviter tout ruissellement vers le domaine public.'
        ],
        codt: [
          'Conformément aux objectifs du CoDT, le projet favorise une utilisation parcimonieuse du sol et une qualité architecturale pérenne.',
          'Le projet veille à la compatibilité des fonctions et à la protection du cadre de vie, conformément aux objectifs généraux.'
        ]
      };
      function renderLibList(key) {
        const list = document.getElementById('libList');
        if (!list) return;
        list.innerHTML = '';
        (EXAMPLES[key] || []).forEach((txt, i) => {
          const row = document.createElement('div');
          row.className = 'rounded-md border border-neutral-200 p-2 flex items-start gap-2';
          row.innerHTML = `
            <i data-lucide="bookmark" class="w-4 h-4 text-neutral-500 mt-1"></i>
            <div class="flex-1 text-neutral-800">${txt}</div>
            <button class="inline-flex items-center gap-1.5 rounded-md border border-neutral-200 bg-white px-2 py-1 text-xs text-neutral-700 hover:bg-neutral-50" data-insert-lib="${i}">
              <i data-lucide="plus" class="w-3.5 h-3.5"></i> Insérer
            </button>
          `;
          list.appendChild(row);
        });
      ...
      document.querySelectorAll('.lib-chip').forEach(chip => {
        chip.addEventListener('click', () => {
          const key = chip.getAttribute('data-lib');
          renderLibList(key);
        });
      });
      document.getElementById('libList')?.addEventListener('click', (e) => {
        const btn = e.target.closest('[data-insert-lib]');
        if (!btn) return;
        const idx = parseInt(btn.getAttribute('data-insert-lib'), 10);
        const key = document.querySelector('.lib-chip[data-lib].ring-1')?.getAttribute('data-lib');
        const area = document.getElementById('c6-textarea');
        const sourceKey = key || 'harmonie';
        const text = EXAMPLES[sourceKey]?.[idx] || '';
        insertAtCursor(area, text + '\n');
        // Update blocking -> valid if enough content
        const field = document.querySelector('[data-field-id="c6-derogation"]');
        field.dataset.status = (area.value.trim().length > 20) ? 'valid' : 'blocking';
        updateProgress();
      });
      // Visual selection of lib chips
      document.querySelectorAll('.lib-chip').forEach(chip => {
        chip.addEventListener('click', () => {
          document.querySelectorAll('.lib-chip').forEach(c => c.classList.remove('ring-1','ring-neutral-300','bg-neutral-50'));
          chip.classList.add('ring-1','ring-neutral-300','bg-neutral-50');
        });
      });

      // Text insert helpers (chips and tags)
      ...
      document.querySelectorAll('.ia-suggest').forEach(btn => {
        btn.addEventListener('click', () => {
          const targetId = btn.getAttribute('data-target');
          const text = btn.getAttribute('data-insert') || '';
          const el = document.getElementById(targetId);
          insertAtCursor(el, (el.value && !el.value.endsWith('\n') ? '\n' : '') + text);
        });
      });
      document.querySelectorAll('.tag-insert').forEach(btn => {
        btn.addEventListener('click', () => {
          const targetId = btn.getAttribute('data-target');
          const text = btn.getAttribute('data-insert') || '';
          const el = document.getElementById(targetId);
          insertAtCursor(el, (el.value && !el.value.endsWith(' ') ? ' ' : '') + text + ' ');
        });
      });
      document.getElementById('applySuggestion')?.addEventListener('click', () => {
        const txt = document.getElementById('iaSuggestion')?.value || '';
        if (!txt.trim()) return;
        insertAtCursor(document.getElementById('c6-textarea'), txt + '\n');
        const field = document.querySelector('[data-field-id="c6-derogation"]');
        field.dataset.status = 'valid';
        updateProgress();
      });
      document.getElementById('c6-textarea')?.addEventListener('input', (e) => {
        const field = document.querySelector('[data-field-id="c6-derogation"]');
        field.dataset.status = (e.target.value.trim().length > 20) ? 'valid' : 'blocking';
        updateProgress();
      });

      // Cadre 10 — PEB logic
      const selectedTags = new Set();
      const pebTagsWrap = document.getElementById('peb-tags');
      const q1Group = document.querySelector('[data-toggle-group="peb-q1"]');
      const q1Block = document.getElementById('peb-q1-block');
      const q2Group = document.querySelector('[data-toggle-group="peb-q2"]');
      const slider = document.getElementById('pebSlider');
      const percentLabel = document.getElementById('pebPercent');
      const pebLite = document.getElementById('peb-lite');
      const pebPro = document.getElementById('peb-pro');

      function toggleTagVisual(btn, on) {
        btn.classList.toggle('bg-neutral-900', on);
        btn.classList.toggle('text-white', on);
        btn.classList.toggle('border-neutral-900', on);
      }

      pebTagsWrap?.addEventListener('click', (e) => {
        const tagBtn = e.target.closest('.peb-tag');
        if (!tagBtn) return;
        const tag = tagBtn.getAttribute('data-tag');
        if (selectedTags.has(tag)) selectedTags.delete(tag);
        else selectedTags.add(tag);
        toggleTagVisual(tagBtn, selectedTags.has(tag));

        // Enable or disable Q1
        const hasTags = selectedTags.size > 0;
        if (hasTags) {
          q1Block.classList.remove('opacity-50');
          q1Group?.removeAttribute('aria-disabled');
          q1Group?.querySelectorAll('.seg-btn').forEach(b => b.disabled = false);
          // Ensure default NO active and show proper block
          q1Group?.querySelectorAll('.seg-btn').forEach(b => b.removeAttribute('data-state'));
          const noBtn = q1Group?.querySelector('[data-value="no"]');
          if (noBtn) noBtn.setAttribute('data-state','on');
          onPEBQ1Toggle('no');
          // Visually update [data-when] for 'no'
          document.querySelectorAll('[data-when="peb-q1:no"]').forEach(el => { el.classList.remove('hidden'); el.removeAttribute('hidden'); });
          document.querySelectorAll('[data-when="peb-q1:yes"]').forEach(el => { el.classList.add('hidden'); el.setAttribute('hidden',''); });
        } else {
          // Reset to disabled
          q1Block.classList.add('opacity-50');
          q1Group?.setAttribute('aria-disabled','true');
          q1Group?.querySelectorAll('.seg-btn').forEach(b => { b.disabled = true; b.removeAttribute('data-state'); });
          document.querySelectorAll('[data-when^="peb-q1:"]').forEach(el => { el.classList.add('hidden'); el.setAttribute('hidden',''); });
          document.querySelectorAll('[data-when^="peb-q2:"]').forEach(el => { el.classList.add('hidden'); el.setAttribute('hidden',''); });
          pebLite?.classList.add('hidden');
          pebPro?.classList.add('hidden');
          document.querySelector('[data-field-id="c10-peb"]').dataset.status = 'warning';
          updateProgress();
        }
      });

      function onPEBQ1Toggle(value) {
        const field = document.querySelector('[data-field-id="c10-peb"]');
        if (value === 'no') {
          // Hide Q2 and outputs, show résumé
          document.querySelectorAll('[data-when="peb-q1:yes"]').forEach(el => { el.classList.add('hidden'); el.setAttribute('hidden',''); });
          document.querySelectorAll('[data-when="peb-q1:no"]').forEach(el => { el.classList.remove('hidden'); el.removeAttribute('hidden'); });
          pebLite?.classList.add('hidden');
          pebPro?.classList.add('hidden');
          ...
          document.querySelectorAll('[data-when="peb-q1:no"]').forEach(el => { el.classList.add('hidden'); el.setAttribute('hidden',''); });
          document.querySelectorAll('[data-when="peb-q1:yes"]').forEach(el => { el.classList.remove('hidden'); el.removeAttribute('hidden'); });
          // Ensure Q2 default is NO
          if (q2Group) {
            q2Group.querySelectorAll('.seg-btn').forEach(b => b.removeAttribute('data-state'));
            q2Group.querySelector('[data-value="no"]')?.setAttribute('data-state','on');
          }
          onPEBQ2Toggle('no');
          field.dataset.status = 'valid';
        }
      ...
      function onPEBQ2Toggle(value) {
        if (value === 'yes') {
          pebLite?.classList.add('hidden');
          pebPro?.classList.remove('hidden');
        } else {
          pebPro?.classList.add('hidden');
          pebLite?.classList.remove('hidden');
        }
        updateProgress();
      }
      ...
      slider?.addEventListener('input', (e) => {
        ...
        percentLabel.textContent = String(val);
        // Auto-set Q2 according to 25% threshold
        const want = val > 25 ? 'yes' : 'no';
        const btnWant = q2Group?.querySelector(`.seg-btn[data-value="${want}"]`);
        if (btnWant && !btnWant.hasAttribute('data-state')) {
          q2Group.querySelectorAll('.seg-btn').forEach(b => b.removeAttribute('data-state'));
          btnWant.setAttribute('data-state','on');
          onPEBQ2Toggle(want);
        }
      });
      ...
      document.getElementById('btnExport')?.addEventListener('click', () => {
        const data = {
          description: document.getElementById('c2-textarea')?.value || '',
          phasage: steps,
          servitude: {
            has: document.querySelector('[data-toggle-group="servitude"] .seg-btn[data-value="yes"][data-state="on"]') ? true : false,
            description: document.getElementById('servitude-desc')?.value || '',
            files: Array.from(document.querySelectorAll('#thumbs .truncate')).map(n => n.textContent.trim())
          },
          cu: {
            cu1: {
              yes: !!document.querySelector('[data-toggle-group="cu1"] .seg-btn[data-value="yes"][data-state="on"]'),
              date: document.querySelector('[data-cu="cu1-date"]')?.value || '',
              lieu: document.querySelector('[data-cu="cu1-lieu"]')?.value || '',
              ref: document.querySelector('[data-cu="cu1-ref"]')?.value || ''
            },
            cu2: {
              yes: !!document.querySelector('[data-toggle-group="cu2"] .seg-btn[data-value="yes"][data-state="on"]'),
              date: document.querySelector('[data-cu="cu2-date"]')?.value || '',
              lieu: document.querySelector('[data-cu="cu2-lieu"]')?.value || '',
              ref: document.querySelector('[data-cu="cu2-ref"]')?.value || ''
            }
          },
          derogation: document.getElementById('c6-textarea')?.value || '',
          peb: {
            types: Array.from(selectedTags),
            modifEner: !!document.querySelector('[data-toggle-group="peb-q1"] .seg-btn[data-value="yes"][data-state="on"]'),
            proportion: parseInt(document.getElementById('pebPercent')?.textContent || '0', 10),
            over25:document.querySelector('[data-toggle-group="peb-q2"] .seg-btn[data-value="yes"][data-state="on"]')
          },
          progress: document.getElementById('progress-value')?.textContent || '0%'
        };
        const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = 'annexe-9-export.json';
      ...
      // Section badges summary (per <details>)
      function updateSectionBadges() {
        document.querySelectorAll('details').forEach(d => {
          const fields = Array.from(d.querySelectorAll('.field'));
          if (fields.length === 0) return;
          let status = 'valid';
          if (fields.some(f => f.dataset.status === 'blocking')) status = 'blocking';
          else if (fields.some(f => f.dataset.status === 'warning')) status = 'warning';

          const badge = d.querySelector('.field-status-badge');
          if (!badge) return;
          if (status === 'valid') {
            badge.className = 'inline-flex items-center gap-1.5 rounded-full border border-green-200 bg-green-50 px-2.5 py-1 text-xs font-medium text-green-700 field-status-badge';
            badge.innerHTML = '<i data-lucide="check-circle-2" class="w-3.5 h-3.5"></i> Validé';
          } else if (status === 'warning') {
            badge.className = 'inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 field-status-badge';
            badge.innerHTML = '<i data-lucide="alert-triangle" class="w-3.5 h-3.5"></i> À vérifier';
          } else {
            badge.className = 'inline-flex items-center gap-1.5 rounded-full border border-red-200 bg-red-50 px-2.5 py-1 text font-medium text-red-700 field-status-badge';
            badge.innerHTML = '<i data-lucide="octagon-x" class="w-3.5 h-3.5"></i> Bloquant';
          }
        });
        refreshIcons();
      }

      // Keep progress and badges in sync when fields change
      const observer = new MutationObserver(() => {
        updateProgress();
        updateSectionBadges();
      });
      observer.observe(document.body, { attributes: true, subtree: true, attributeFilter: ['data-status'] });

      // Open details when clicking validation list link
      document.getElementById('validationList')?.addEventListener('click', (e) => {
        const a = e.target.closest('a[href^="#"]');
        if (!a) return;
        const id = a.getAttribute('href')?.slice(1);
        const d = document.getElementById(id);
        if (d?.tagName === 'DETAILS') d.setAttribute('open','');
      });
      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        validateCU();
        // Default selected library
        renderLibList('harmonie');
      });

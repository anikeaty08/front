...
<button @click="applyPromo('PANINO50')" className="px-3 py-1.5 bg-white/15 hover:bg-white/25 rounded-lg text-sm font-medium">Активировать</button>
</div>
</div>
</div>
...
<template :key="cat.slug" x-htmlFor="cat in categories">
<button :class="activeCategory === cat.slug
                  ? 'bg-gradient-to-r from-orange-500 to-rose-500 border-transparent text-white'
                  : 'bg-neutral-900 border-neutral-800 text-neutral-300 hover:bg-neutral-800'" @click="activeCategory = cat.slug" className="flex-shrink-0 px-4 py-2 rounded-xl border text-sm">
...
<div className="px-6 mt-3 h-[355px] md:h-[430px] overflow-y-auto" style={{scrollbarWidth: 'none', MsOverflowStyle: 'none'}}>
<div className="grid grid-cols-2 gap-4">
<template :key="item.id" x-htmlFor="item in filteredItems()">
<article className="rounded-2xl overflow-hidden bg-neutral-900/70 border border-neutral-800 hover:bg-neutral-900 transition">
<a @click.prevent="openProduct(item)" className="block relative" href="#">
<img :alt="item.name" :src="https://images.unsplash.com/photo-1642615835477-d303d7dc9ee9?w=1080&amp;q=80" className="w-full h-32 md:h-36 object-cover" loading="lazy"/>
<div className="absolute top-2 left-2 flex items-center gap-1 bg-black/70 px-2 py-1 rounded-full text-xs">
<i className="w-3.5 h-3.5 text-yellow-400" data-lucide="star"></i>
<span x-text="item.rating.toFixed(1)"></span>
</div>
</a>
<div className="p-3">
<h3 className="text-[15px] font-semibold tracking-tight mb-0.5" x-text="item.name"></h3>
<p className="text-xs text-neutral-400 line-clamp-2 mb-2" x-text="item.short"></p>
<div className="flex items-center justify-between">
<span className="text-rose-400 font-semibold" x-text="item.price + ' руб.'"></span>
<button @click="addToCartQuick(item)" className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-gradient-to-r from-orange-500 to-rose-500 text-white text-sm font-medium">
<i className="w-4 h-4" data-lucide="plus"></i>В корзину
                    </button>
</div>
</div>
</article>
...
<button @click="backToHome()" className="w-10 h-10 rounded-xl bg-neutral-900 border border-neutral-800 flex items-center justify-center hover:bg-neutral-800 transition">
...
<img :src="https://images.unsplash.com/photo-1635151227785-429f420c6b9d?w=1080&amp;q=80" alt="" className="w-full h-56 object-cover transition"/>
<button @click="prev()" className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-xl bg-black/50 backdrop-blur border border-white/10 flex items-center justify-center hover:bg-black/60">
<i className="w-5 h-5" data-lucide="chevron-left"></i>
</button>
<button @click="next()" className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-xl bg-black/50 backdrop-blur border border-white/10 flex items-center justify-center hover:bg-black/60">
<i className="w-5 h-5" data-lucide="chevron-right"></i>
</button>
<div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1">
<template :key="i" x-htmlFor="(img, i) in images">
<div :className="i===index?'w-6 bg-white':'w-2 bg-white/50'" className="h-1.5 rounded-full"></div>
</template>
</div>
</div>
</div>

<div className="px-6 mt-4 flex-1 overflow-y-auto" style={{scrollbarWidth: 'none', MsOverflowStyle: 'none'}}>
...
<button :className="detailTab==='desc' ? 'bg-neutral-800 text-white' : 'text-neutral-300 hover:text-white'" @click="detailTab='desc'" className="px-4 py-2 rounded-xl text-sm">Описание</button>
<button :className="detailTab==='ing' ? 'bg-neutral-800 text-white' : 'text-neutral-300 hover:text-white'" @click="detailTab='ing'" className="px-4 py-2 rounded-xl text-sm">Состав</button>
</div>
<div className="mt-3">
...
<template :key="ing" x-htmlFor="ing in selectedProduct.ingredients">
<li x-text="ing"></li>
</template>
</ul>
</div>
...
<template :key="extra.code" x-htmlFor="extra in selectedProduct.extras">
<label className="flex items-center justify-between p-3 rounded-xl bg-neutral-900 border border-neutral-800 cursor-pointer">
<div className="flex items-center gap-3">
<span className="relative inline-flex w-5 h-5">
<input :value="extra.code" @change="toggleExtra(extra)" className="peer absolute inset-0 opacity-0" type="checkbox"/>
<span className="w-5 h-5 rounded-md border border-neutral-700 bg-neutral-800 peer-checked:border-orange-400 peer-checked:bg-gradient-to-r from-orange-500 to-rose-500"></span>
<i className="w-3.5 h-3.5 text-white absolute left-1 top-1 opacity-0 peer-checked:opacity-100 transition" data-lucide="check"></i>
</span>
<div>
<p className="text-sm font-medium" x-text="extra.name"></p>
<p className="text-xs text-neutral-400" x-text="extra.desc"></p>
</div>
</div>
<span className="text-sm font-semibold text-rose-300" x-text="'+' + extra.price + ' руб.'"></span>
</label>
</template>
</div>
</div>
...
<div className="flex items-center gap-3">
<button @click="decQty()" className="w-11 h-11 rounded-xl bg-neutral-900 border border-neutral-800 flex items-center justify-center hover:bg-neutral-800">
<i className="w-5 h-5 text-neutral-300" data-lucide="minus"></i>
</button>
<div className="w-12 text-center font-semibold" x-text="qty"></div>
<button @click="incQty()" className="w-11 h-11 rounded-xl bg-neutral-900 border border-neutral-800 flex items-center justify-center hover:bg-neutral-800">
<i className="w-5 h-5 text-neutral-300" data-lucide="plus"></i>
</button>
<button @click="addToCartDetail()" className="flex-1 h-12 rounded-2xl bg-gradient-to-r from-orange-500 to-rose-500 text-white font-semibold flex items-center justify-center gap-2 hover:from-orange-600 hover:to-rose-600 transition shadow-lg shadow-orange-500/20">
...
<button @click="$store.cart.clear()" className="text-sm text-neutral-300 hover:text-white">Очистить</button>
</div>

<div className="px-6 mt-4 h-[360px] md:h-[440px] overflow-y-auto" style={{scrollbarWidth: 'none', MsOverflowStyle: 'none'}}>
<template x-if="$store.cart.items.length === 0">
<div className="flex flex-col items-center justify-center text-center text-neutral-400 h-44">
<i className="w-8 h-8 mb-2" data-lucide="shopping-bag"></i>
<p>Ваша корзина пуста</p>
</div>
</template>
<div className="space-y-3">
<template :key="line.key" x-htmlFor="line in $store.cart.items">
<div className="p-3 rounded-2xl bg-neutral-900 border border-neutral-800">
<div className="flex items-start gap-3">
<img :src="https://images.unsplash.com/photo-1621619856624-42fd193a0661?w=1080&amp;q=80" alt="" className="w-16 h-16 rounded-xl object-cover border border-neutral-800"/>
<div className="flex-1">
<div className="flex items-start justify-between">
<h3 className="text-sm font-semibold tracking-tight" x-text="line.name"></h3>
<button @click="$store.cart.remove(line.key)" className="text-neutral-400 hover:text-white">
<i className="w-5 h-5" data-lucide="x"></i>
</button>
</div>
<p className="text-xs text-neutral-400 mt-0.5" x-text="line.extras &amp;&amp; line.extras.length ? 'Допы: ' + line.extras.map(e=&gt;e.name).join(', ') : 'Без допов'"></p>
<div className="mt-2 flex items-center justify-between">
<div className="inline-flex items-center gap-2">
<button @click="$store.cart.dec(line.key)" className="w-8 h-8 rounded-lg bg-neutral-950 border border-neutral-800 flex items-center justify-center hover:bg-neutral-800">
<i className="w-4 h-4" data-lucide="minus"></i>
</button>
<span className="w-6 text-center text-sm font-semibold" x-text="line.qty"></span>
<button @click="$store.cart.inc(line.key)" className="w-8 h-8 rounded-lg bg-neutral-950 border border-neutral-800 flex items-center justify-center hover:bg-neutral-800">
<i className="w-4 h-4" data-lucide="plus"></i>
</button>
</div>
<div className="text-right">
<p className="text-sm text-neutral-300">за единицу: <span className="font-semibold" x-text="line.unit + ' руб.'"></span></p>
<p className="text-sm font-semibold text-rose-300" x-text="(line.unit*line.qty) + ' руб.'"></p>
</div>
</div>
</div>
</div>
...
<button @click="$store.cart.applyPromo()" className="px-3 h-10 rounded-xl bg-gradient-to-r from-orange-500 to-rose-500 text-white text-sm font-medium">Применить</button>
</div>
...
<div className="absolute bottom-0 left-0 right-0 bg-neutral-950/90 backdrop-blur border-t border-neutral-800 p-4">
<template x-if="!$store.cart.placed">
<button @click="$store.cart.checkout()" className="w-full h-12 rounded-2xl bg-gradient-to-r from-orange-500 to-rose-500 text-white font-semibold flex items-center justify-center gap-2 hover:from-orange-600 hover:to-rose-600 transition shadow-lg shadow-orange-500/20">
              Перейти к оплате
...
<i className="w-5 h-5 text-emerald-400" data-lucide="check-circle2"></i>
<span className="font-medium">Подтвержден</span>
</div>
<div :className="$store.cart.status &gt;= 2 ? 'text-white' : 'text-neutral-400'" className="flex items-center gap-2 text-sm">
<i className="w-5 h-5 text-orange-300" data-lucide="chef-hat"></i>
<span className="font-medium">Готовится</span>
<span className="ml-auto text-xs text-neutral-400" x-text="'~' + $store.cart.timerPrep + ' мин'"></span>
</div>
<div :className="$store.cart.status &gt;= 3 ? 'text-white' : 'text-neutral-400'" className="flex items-center gap-2 text-sm">
<i className="w-5 h-5 text-sky-300" data-lucide="car"></i>
<span className="font-medium">В пути</span>
<span className="ml-auto text-xs text-neutral-400" x-text="'~' + $store.cart.timerDrive + ' мин'"></span>
</div>
<div :className="$store.cart.status &gt;= 4 ? 'text-white' : 'text-neutral-400'" className="flex items-center gap-2 text-sm">
<i className="w-5 h-5 text-emerald-400" data-lucide="circle-check"></i>

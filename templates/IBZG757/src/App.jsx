...
<button @click="open = !open" className="md:hidden relative" x-data="{open:false}">
<span :className="{'opacity-0':open}" className="fa-solid fa-bars text-xl text-teal-600 transition"></span>
<span :className="{'opacity-0':!open}" className="fa-solid fa-xmark text-xl text-teal-600 transition absolute inset-0"></span>
...
<template :key="index" x-htmlFor="(slide, index) in slides">
<div className="absolute inset-0" x-show="activeSlide === index" x-transition:enter="transition ease-out duration-700" x-transition:enter-end="opacity-100 scale-100" x-transition:enter-start="opacity-0 scale-105">
<img :src="slide.img" alt="" className="w-full h-full object-cover object-center"/>
<div className="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>
<div className="absolute inset-0 flex flex-col items-end justify-center px-8 md:px-16 text-white">
<h2 className="text-3xl md:text-4xl lg:text-5xl font-semibold tracking-tight mb-4" x-text="slide.title"></h2>
<p className="text-sm md:text-base max-w-md" x-text="slide.subtitle"></p>
</div>
</div>
</template>

<button @click="prev" className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/70 backdrop-blur rounded-full p-2 hover:bg-white shadow">
<i className="fa-solid fa-chevron-left text-teal-600"></i>
</button>
<button @click="next" className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/70 backdrop-blur rounded-full p-2 hover:bg-white shadow">
...
<template :key="index" x-htmlFor="(slide, index) in slides">
<button :className="activeSlide === index ? 'bg-teal-600' : 'bg-gray-300'" @click="activeSlide = index" className="w-2.5 h-2.5 rounded-full"></button>
</template>
</div>
</div>

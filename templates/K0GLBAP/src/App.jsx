...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
      ...
      const showToast = (msg, type='default') => {
        ...
        el.className = 'px-3 py-2 rounded-md border text-[13px] shadow-lg flex items-center gap-2 ' + (type==='error' ? 'border-red700/60 bg-red-900/20 text-red-200' : type==='success' ? 'border-emerald-700/60 bg-emerald-900/20 text-emerald-200' : 'border-neutral-800/80 bg-neutral-900/80 text-neutral-200');
        ...
        el.innerHTML = `
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="mt-0.5 text-[11px] text-neutral-500 capitalize">${node.category}</div>
              <div class="mt-2">
      ...
      function updateConnections() {
        ...
        // Build chain: trigger -> rules... -> actions
        if (triggers.length) {
          const tEl = qs(`[data-node-id="${triggers[0].id}"]`);
          if (!tEl) return;
          let prevRight = getCenterRight(tEl);

          if (rules.length) {
            rules.forEach((r, idx) => {
              const rEl = qs(`[data-node-id="${r.id}"]`);
              if (!rEl) return;
              const rLeft = getCenterLeft(rEl);
              addPath(pathBetween(prevRight, rLeft));
              prevRight = getCenterRight(rEl);
            });
          }
          ...
          if (actions.length) {
            actions.forEach(aNode => {
              ...
              addPath(pathBetween(prevRight, aLeft));
            });
          }
        }
      }
      ...
      function initTabs() {
        ...
        qsa('.inspector-tab').forEach(b => {
          b.addEventListener('click', () => {
            ...
            qs('#inspectorMeta').classList.toggle('hidden', state.inspectorTab !== 'meta');
            qs('#inspectorForm').classList.toggle('hidden', state.inspectorTab !== 'configure');
            buildInspector();
          });
        });
      }
      ...
      function initHeaderActions() {
        qs('#btnDeleteSelected').addEventListener('click', () => {
          if (!state.selectedId) return;
          deleteNode(state.selectedId);
        });
        qs('#btnExportNode').addEventListener('click', () => {
          ...
          a.href = url; a.download = `${n.id}.json`; a.click();
        ...
        qs('#btnPreview').addEventListener('click', () => {
          if (!state.nodes.length) return showToast('Nothing to preview yet.', 'error');
          const steps = state.nodes.map(n => `${n.category.toUpperCase()}: ${n.label}`).join(' → ');
          showToast(`Preview: ${steps}`, 'default');
        });
        qs('#btnValidate').addEventListener('click', () => {
          const errors = [];
          if (!state.nodes.some(n => n.category === 'trigger')) errors.push('A Trigger is required.');
          if (!state.nodes.some(n => n.category === 'action')) errors.push('At least one Action is required.');
          const triggerCount = state.nodes.filter(n => n.category === 'trigger').length;
          if (triggerCount > 1) errors.push('Only one Trigger is allowed.');
          if (errors.length) {
      ...
      // Search inputs
      function initSearch() {
        qs('#paletteSearch').addEventListener('input', renderPalette);
        qs('#globalSearch').addEventListener('keydown', (e) => {
          if (e.key === 'Enter') {
            e.preventDefault();
            showToast('Global search is illustrative in this demo.', 'default');
          }
        });
      }
...
<p className="text-[11px] text-neutral-500">Send emails, update opps, start automations</p>
</div>
</div>

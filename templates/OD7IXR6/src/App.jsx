...
<button aria-label="Fechar formulário" className="absolute top-4 right-4 p-2 rounded-full hover:bg-neutral-800 focus:outline-none transition-colors" onclick="closePopup()" style={{animationDelay: '180ms'}}>
...
<input autocomplete="name" className="focus:border-emerald-400 outline-none transition-all placeholder-neutral-500 text-base font-normal text-neutral-100 bg-slate-50 border rounded-lg pt-3 pr-4 pb-3 pl-4" id="nome" name="nome" oninput="validateForm()" placeholder="Digite o seu nome" required="" type="text"/>
</div>

<div className="flex flex-col gap-2">
<label className="text-sm font-medium text-slate-50" htmlFor="telefone">Telefone / WhatsApp</label>
<input autocomplete="tel" className="focus:border-emerald-400 outline-none transition-all placeholder-neutral-500 text-base font-normal text-neutral-100 bg-slate-50 border-neutral-700 border rounded-lg pt-3 pr-4 pb-3 pl-4" id="telefone" maxlength="11" name="telefone" oninput="validateForm()" pattern="^\d{10,11}$" placeholder="DDD+ número" required="" type="tel"/>
</div>

<div className="flex flex-col gap-2">
<label className="text-sm font-medium text-slate-50" htmlFor="cidade">Cidade</label>
<input autocomplete="address-level2" className="focus:border-emerald-400 outline-none transition-all placeholder-neutral-500 text-base font-normal text-neutral-100 bg-slate-50 border-neutral-700 border rounded-lg pt-3 pr-4 pb-3 pl-4" id="cidade" name="cidade" oninput="validateForm()" placeholder="Sua cidade" required="" type="text"/>
</div>
<div className="h-px bg-neutral-800 my-2"></div>

<button className="flex items-center justify-center gap-3 w-full bg-emerald-500 hover:bg-emerald-600 transition-colors text-white font-semibold text-base px-4 py-3 rounded-xl focus:outline-none shadow-xl hover:shadow-emerald-900/30 disabled:opacity-50 disabled:cursor-not-allowed outline outline-0 hover:outline-2 hover:outline-emerald-400/40 animate-fade-in" disabled="" id="whatsappButton" onclick="redirectWhatsapp()" style={{animationDelay: '350ms', opacity: '1', transition: 'opacity 0.5s cubic-bezier(0.4, 0, 0.2, 1), transform 0.7s cubic-bezier(0.4, 0, 0.2, 1)'}} type="button">
...
<style>
    ...
    @keyframes slide-up-fade { from { opacity: 0; transform: translateY(24px) scale(.97); } to { opacity: 1; transform: translateY(0) scale(1); } }
  </style>

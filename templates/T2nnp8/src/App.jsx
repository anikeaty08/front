    ...
    function render() {
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          ...
          // Calculate base colors with purple tone
          const r = 0.4 + 0.4 * Math.cos(u.x * d);
          const g = 0.2 + 0.2 * Math.cos(u.y * a);
          const b = 0.6 + 0.4 * Math.cos(a + d);
          
          // Apply final transformation
          const finalR = Math.cos(0.5 + 0.5 * Math.cos(d) * r) * 0.7;
          const finalG = Math.cos(0.5 + 0.5 * Math.cos(a) * g) * 0.3;

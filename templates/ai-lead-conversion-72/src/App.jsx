...
'100%': { transform: 'translateX(-100%)' },
      ...
      vec3 mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; } vec4 mod289(vec4 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; } vec4 permute(vec4 x) { return mod289(((x*34.0)+1.0)*x); } vec4 taylorInvSqrt(vec4 r) { return 1.79284291400159 - 0.85373472095314 * r; } float snoise(vec3 v) { const vec2 C = vec2(1.0/6.0, 1.0/3.0); const vec4 D = vec4(0.0, 0.5, 1.0, 2.0); vec3 i = floor(v + dot(v, C.yyy)); vec3 x0 = v - i + dot(i, C.xxx); vec3 g = step(x0.yzx, x0.xyz); vec3 l = 1.0 - g; vec3 i1 = min(g.xyz, l.zxy); vec3 i2 = max(g.xyz, l.zxy); vec3 x1 = x0 - i1 + C.xxx; vec3 x2 = x0 - i2 + 2.0 * C.xxx; vec3 x3 = x0 - 1.0 + 3.0 * C.xxx; i = mod289(i); vec4 p = permute(permute(permute(i.z + vec4(0.0, i1.z, i2.z, 1.0)) + i.y + vec4(0.0, i1.y, i2.y, 1.0)) + i.x + vec4(0.0, i1.x, i2.x, 1.0)); float n_ = 1.0/7.0; vec3 ns = n_ * D.wyz - D.xzx; vec4 j = p - 49.0 * floor(p * ns.z * ns.z); vec4 x_ = floor(j * ns.z); vec4 y_ = floor(j - 7.0 * x_); vec4 x = x_ * ns.x + ns.yyyy; vec4 y = y_ * ns.x + ns.yyyy; vec4 h = 1.0 - abs(x) - abs(y); vec4 b0 = vec4(x.xy, y.xy); vec4 b1 = vec4(x.zw, y.zw); vec4 s0 = floor(b0)*2.0 + 1.0; vec4 s1 = floor(b1)*2.0 + 1.0; vec4 sh = -step(h, vec4(0.0)); vec4 a0 = b0.xzyw + s0.xzyw*sh.xxyy; vec4 a1 = b1.xzyw + s1.xzyw*sh.zzww; vec3 p0 = vec3(a0.xy,h.x); vec3 p1 = vec3(a0.zw,h.y); vec3 p2 = vec3(a1.xy,h.z); vec3 p3 = vec3(a1.zw,h.w); vec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2,p2), dot(p3,p3))); p0 *= norm.x; p1 *= norm.y; p2 *= norm.z; p3 *= norm.w; vec4 m = max(0.6 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0); m = m * m; return 42.0 * dot(m*m, vec4(dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3))); }
      ...
      function route(viewId) {
        ...
        document.querySelectorAll('.nav-link').forEach(function(link) {
          ...
          link.classList.add('text-neutral-600', 'dark:text-white/70');
          if (link.dataset.target === viewId) {
            link.classList.remove('text-neutral-600', 'dark:text-white/70');
      ...
      try {
        ...
        var systemsGroup = new THREE.Group();
        systemsGroup.position.x = 4.5;
        scene.add(systemsGroup);

        var geometry = new THREE.IcosahedronGeometry(4.5, 30);
        uniforms = {
          uTime: { value: 0 },
...
<a className="nav-link px-5 py-2 text-sm font-medium text-emerald-600 dark:text-emerald-400 bg-neutral-100 dark:bg-white/10 transition-all rounded-full" data-target="home" href="#" onclick="route('home'); return false;">Home</a>
...
<svg className="w-5 h-5 dark:hidden" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"></path></svg>
...
<svg fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M4 5h16"></path><path d="M4 12h16"></path><path d="M4 19h16"></path></svg>
</button>
</div>
</div>
</div>
...
<a className="text-lg font-medium text-neutral-600 dark:text-white/70" href="#" onclick="route('home'); document.getElementById('mobile-menu').classList.add('hidden'); return false;">Home</a>
<a className="text-lg font-medium text-neutral-600 dark:text-white/70" href="#" onclick="route('services'); document.getElementById('mobile-menu').classList.add('hidden'); return false;">Services</a>
<a className="text-lg font-medium text-neutral-600 dark:text-white/70" href="#" onclick="route('portfolio'); document.getElementById('mobile-menu').classList.add('hidden'); return false;">Portfolio</a>
<a className="text-lg font-medium text-neutral-600 dark:text-white/70" href="#" onclick="route('team'); document.getElementById('mobile-menu').classList.add('hidden'); return false;">Team</a>
<a className="text-lg font-medium text-neutral-600 dark:text-white/70" href="#" onclick="route('contact'); document.getElementById('mobile-menu').classList.add('hidden'); return false;">Contact</a>
</div>
</div>
</header>
...
<section className="relative pt-16 sm:pt-28 pb-28 sm:pb-36 px-6 text-center max-w-5xl mx-auto">
...
<h1 className="text-5xl sm:text-7xl md:text-8xl tracking-tighter font-display font-semibold text-neutral-900 dark:text-white mb-6 leading-[1.05]">
              Building the Digital<br className="hidden sm:block"/> Future of Ethiopia
            </h1>
<p className="text-base sm:text-lg text-neutral-500 dark:text-white/50 max-w-2xl mx-auto mb-10 font-light leading-relaxed">
...
<button className="w-full sm:w-auto group relative inline-flex items-center justify-center bg-transparent border border-neutral-300 dark:border-white/25 text-neutral-900 dark:text-white h-12 px-7 rounded-full text-sm font-medium transition-all hover:bg-white/10 hover:border-white/40" onclick="route('contact')">
<span>Start Your Project</span>
<svg className="ml-2 w-4 h-4 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</button>
<button className="w-full sm:w-auto inline-flex items-center justify-center text-neutral-500 dark:text-white/50 h-12 px-4 text-sm font-medium hover:text-neutral-900 dark:hover:text-white transition-colors" onclick="route('portfolio')">
                View Our Work
              </button>
</div>
</div>
...
<div className="flex items-center gap-3 text-neutral-500 dark:text-neutral-400 font-bold text-lg"><span className="w-6 h-6 text-emerald-500">⬡</span> WEB DEVELOPMENT</div>
<div className="flex items-center gap-3 text-neutral-500 dark:text-neutral-400 font-bold text-lg"><span className="w-6 h-6 text-blue-500">◈</span> MOBILE APPS</div>
<div className="flex items-center gap-3 text-neutral-500 dark:text-neutral-400 font-bold text-lg"><span className="w-6 h-6 text-purple-500">◉</span> SEO OPTIMIZATION</div>
<div className="flex items-center gap-3 text-neutral-500 dark:text-neutral-400 font-bold text-lg"><span className="w-6 h-6 text-orange-500">◎</span> E-COMMERCE</div>
<div className="flex items-center gap-3 text-neutral-500 dark:text-neutral-400 font-bold text-lg"><span className="w-6 h-6 text-pink-500">◇</span> UI/UX DESIGN</div>
<div className="flex items-center gap-3 text-neutral-500 dark:text-neutral-400 font-bold text-lg"><span className="w-6 h-6 text-indigo-500">◆</span> CYBER SECURITY</div>
</div>
<div className="flex animate-marquee whitespace-nowrap min-w-full shrink-0 items-center justify-around gap-16 px-8">
<div className="flex items-center gap-3 text-neutral-500 dark:text-neutral-400 font-bold text-lg"><span className="w-6 h-6 text-emerald-500">⬡</span> WEB DEVELOPMENT</div>
<div className="flex items-center gap-3 text-neutral-500 dark:text-neutral-400 font-bold text-lg"><span className="w-6 h-6 text-blue-500">◈</span> MOBILE APPS</div>
...
<h3 className="text-4xl sm:text-5xl font-display font-semibold tracking-tight text-neutral-900 dark:text-white">Specialized Digital Solutions</h3>
</div>
<button className="hidden md:inline-flex items-center text-sm font-semibold text-neutral-900 dark:text-white border-b border-neutral-300 dark:border-white/30 hover:border-emerald-500 pb-1 transition-colors mt-4 md:mt-0" onclick="route('services')">
                View All Services
                <svg className="ml-2 w-4 h-4" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</button>
</div>
...
<svg fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><rect height="18" rx="2" ry="2" width="18" x="3" y="3"></rect><line x1="3" x2="21" y1="9" y2="9"></line><line x1="9" x2="9" y1="21" y2="9"></line></svg>
</div>
<h4 className="text-xl font-bold tracking-tight text-neutral-900 dark:text-white mb-2">Web Development</h4>
...
<svg fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10"></path></svg>
</div>
<h4 className="text-xl font-bold tracking-tight text-neutral-900 dark:text-white mb-2">Secure Systems</h4>
...
<svg fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg>
</div>
<h4 className="text-xl font-bold tracking-tight text-neutral-900 dark:text-white mb-2">Real Estate Solutions</h4>
...
<h3 className="text-4xl sm:text-5xl font-display font-semibold tracking-tight text-neutral-900 dark:text-white">Delivering Excellence</h3>
</div>
...
<img alt="Real Estate Project" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" src="https://images.unsplash.com/photo-1564013799919-ab600027ffc6?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=1200&amp;q=80"/>
</div>
...
<h3 className="text-4xl font-display font-semibold tracking-tight text-neutral-900 dark:text-white mb-6">Grand Horizon</h3>
<p className="text-base text-neutral-600 dark:text-white/70 mb-8 leading-relaxed">We built a custom Next.js platform allowing Grand Horizon to list 500+ properties with advanced filtering and map integration.</p>
...
<img alt="Hotel Project" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" src="https://images.unsplash.com/photo-1566073771259-6a8506099945?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=1200&amp;q=80"/>
</div>
...
<h3 className="text-4xl font-display font-semibold tracking-tight text-neutral-900 dark:text-white mb-6">Lalibela Resort</h3>
<p className="text-base text-neutral-600 dark:text-white/70 mb-8 leading-relaxed">A 5-star digital experience featuring direct booking, room virtual tours, and local experience guides.</p>
<button className="inline-flex items-center text-sm font-semibold text-emerald-600 dark:text-emerald-400 hover:text-emerald-500 transition-colors" onclick="route('portfolio')">
                  View Case Study
                  <svg className="ml-2 w-4 h-4" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</button>
...
<h2 className="text-3xl sm:text-5xl font-display font-semibold tracking-tight mb-8 reveal-on-scroll">Meet the Experts</h2>
<p className="text-lg text-neutral-400 max-w-2xl mx-auto mb-12 reveal-on-scroll delay-100">Our team consists of Ethiopia's finest developers, designers, and strategists.</p>
<button className="inline-flex items-center justify-center bg-white text-black h-12 px-8 rounded-full text-sm font-semibold transition-transform hover:-translate-y-1 reveal-on-scroll delay-200" onclick="route('team')">Meet the Whole Team</button>
</div>
</section>

<section className="py-32 px-6 max-w-3xl mx-auto text-center">
<div className="reveal-on-scroll">
<h2 className="text-4xl sm:text-6xl font-display font-semibold tracking-tight text-neutral-900 dark:text-white mb-6">Ready to Start?</h2>
<p className="text-lg text-neutral-600 dark:text-white/60 mb-10">We are currently accepting new projects for Q2 2025.</p>
<button className="group inline-flex items-center justify-center bg-emerald-500 text-white h-14 px-10 rounded-full text-base font-semibold shadow-lg shadow-emerald-500/30 transition-all hover:bg-emerald-600 hover:scale-105" onclick="route('contact')">
              Get a Free Quote
              <svg className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</button>
</div>
</section>
</div>
...
<h1 className="text-5xl sm:text-6xl font-display font-semibold tracking-tight text-neutral-900 dark:text-white mb-6">Our Services</h1>
<p className="text-lg text-neutral-600 dark:text-white/60">Comprehensive digital solutions tailored for the Ethiopian market.</p>
</div>
...
<h3 className="text-xl font-semibold tracking-tight text-neutral-900 dark:text-white mb-3">Custom Development</h3>
...
<h3 className="text-xl font-semibold tracking-tight text-neutral-900 dark:text-white mb-3">Premium WordPress</h3>
...
<h3 className="text-xl font-semibold tracking-tight text-neutral-900 dark:text-white mb-3">Hotel Booking Engines</h3>
...
<h3 className="text-xl font-semibold tracking-tight text-neutral-900 dark:text-white mb-3">E-Commerce</h3>
...
<div className="p-8 rounded-3xl bg-neutral-50 dark:bg-white/5 border border-neutral-200 dark:border-white/10 hover:border-emerald-500/50 transition-all duration-300 reveal-on-scroll delay-200">
<div className="w-12 h-12 bg-pink-500/10 rounded-lg flex items-center justify-center mb-6 text-pink-500">
...
<h3 className="text-xl font-semibold tracking-tight text-neutral-900 dark:text-white mb-3">UI/UX Design</h3>
...
<div className="p-8 rounded-3xl bg-neutral-50 dark:bg-white/5 border border-neutral-200 dark:border-white/10 hover:border-emerald-500/50 transition-all duration-300 reveal-on-scroll delay-300">
<div className="w-12 h-12 bg-indigo-500/10 rounded-lg flex items-center justify-center mb-6 text-indigo-500">
<svg fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><circle cx="11" cy="11" r="8"></circle><path d="m21 21-4.3-4.3"></path></svg>
</div>
<h3 className="text-xl font-semibold tracking-tight text-neutral-900 dark:text-white mb-3">SEO &amp; Marketing</h3>
...
<section className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
<div className="text-center max-w-3xl mx-auto mb-20 reveal-on-scroll">
<h1 className="text-5xl sm:text-6xl font-display font-semibold tracking-tight text-neutral-900 dark:text-white mb-6">Selected Works</h1>
<p className="text-lg text-neutral-600 dark:text-white/60">A showcase of our finest digital products.</p>
</div>
...
<h3 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-white">Grand Horizon Properties</h3>
<p className="text-neutral-500 mt-2 text-sm">Real Estate Platform • Next.js</p>
</div>
...
<h3 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-white">Lalibela Heights</h3>
<p className="text-neutral-500 mt-2 text-sm">Hotel Booking • WordPress</p>
</div>
<div className="group cursor-pointer reveal-on-scroll delay-200">
<div className="rounded-2xl overflow-hidden aspect-video border border-neutral-200 dark:border-white/10 mb-6 relative">
<div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors z-10"></div>
<img alt="Work" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" src="https://images.unsplash.com/photo-1523050854058-8df90110c476?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=1200&amp;q=80"/>
</div>
<h3 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-white">Addis International School</h3>
<p className="text-neutral-500 mt-2 text-sm">Education Portal • React</p>
</div>
<div className="group cursor-pointer reveal-on-scroll delay-300">
<div className="rounded-2xl overflow-hidden aspect-video border border-neutral-200 dark:border-white/10 mb-6 relative">
<div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors z-10"></div>
<img alt="Work" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=1200&amp;q=80"/>
</div>
<h3 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-white">EthioPay Integration</h3>
<p className="text-neutral-500 mt-2 text-sm">Fintech • Node.js</p>
</div>
</div>
</section>
</div>
...
<section className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
<div className="text-center max-w-3xl mx-auto mb-20 reveal-on-scroll">
<h1 className="text-5xl sm:text-6xl font-display font-semibold tracking-tight text-neutral-900 dark:text-white mb-6">The Team</h1>
<p className="text-lg text-neutral-600 dark:text-white/60">Creative minds building digital experiences.</p>
</div>
...
<div className="w-32 h-32 mx-auto rounded-full overflow-hidden mb-6 border-4 border-white dark:border-white/10 shadow-lg bg-neutral-200 dark:bg-neutral-800">
<img alt="Team" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=400&amp;q=80"/>
</div>
<h3 className="text-lg font-semibold text-neutral-900 dark:text-white">Dawit Alemu</h3>
...
<div className="w-32 h-32 mx-auto rounded-full overflow-hidden mb-6 border-4 border-white dark:border-white/10 shadow-lg bg-neutral-200 dark:bg-neutral-800">
<img alt="Team" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=400&amp;q=80"/>
</div>
<h3 className="text-lg font-semibold text-neutral-900 dark:text-white">Sara Kebede</h3>
...
<div className="w-32 h-32 mx-auto rounded-full overflow-hidden mb-6 border-4 border-white dark:border-white/10 shadow-lg bg-neutral-200 dark:bg-neutral-800">
<img alt="Team" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=400&amp;q=80"/>
</div>
<h3 className="text-lg font-semibold text-neutral-900 dark:text-white">Abebe Tadesse</h3>
...
<div className="w-32 h-32 mx-auto rounded-full overflow-hidden mb-6 border-4 border-white dark:border-white/10 shadow-lg bg-neutral-200 dark:bg-neutral-800">
<img alt="Team" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" src="https://images.unsplash.com/photo-1438761681033-6461ffad8d80?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=400&amp;q=80"/>
</div>
<h3 className="text-lg font-semibold text-neutral-900 dark:text-white">Helen Bekele</h3>
...
<p className="text-base text-neutral-600 dark:text-white/60">Tell us about your project. We'll get back to you within 24 hours.</p>
</div>
<form className="bg-white dark:bg-neutral-900/50 border border-neutral-200 dark:border-white/10 p-8 sm:p-12 rounded-3xl shadow-2xl dark:shadow-none reveal-on-scroll delay-100 backdrop-blur-sm" onsubmit="return false;">
...
<select className="w-full bg-neutral-50 dark:bg-black/50 border border-neutral-200 dark:border-white/10 rounded-xl px-4 py-3 text-neutral-900 dark:text-white text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition appearance-none cursor-pointer outline-none">
...
<textarea className="w-full bg-neutral-50 dark:bg-black/50 border border-neutral-200 dark:border-white/10 rounded-xl px-4 py-3 text-neutral-900 dark:text-white text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition placeholder-neutral-400 dark:placeholder-white/20 outline-none" placeholder="Tell us about your project goals..." rows="4"></textarea>
</div>
</div>
<div className="mt-8">
<button className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-neutral-900 dark:bg-emerald-500 px-8 py-4 text-sm font-semibold text-white dark:text-neutral-900 hover:opacity-90 transition shadow-lg" type="button">Send Inquiry</button>
</div>
</form>
</div>
</section>
...
<a className="text-neutral-400 hover:text-emerald-500 transition-colors" href="#">
<svg className="w-5 h-5" fill="currentColor" viewbox="0 0 24 24"><path clip-rule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" fill-rule="evenodd"></path></svg>
</a>
<a className="text-neutral-400 hover:text-emerald-500 transition-colors" href="#">
<svg className="w-5 h-5" fill="currentColor" viewbox="0 0 24 24"><path clip-rule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.468 2.373c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" fill-rule="evenodd"></path></svg>
</a>
</div>
</div>
<div>

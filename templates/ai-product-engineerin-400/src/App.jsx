...
gtag('config', 'G-2M6V79H761');
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            gsap.to('.shimmer-effect', {
                x: '300%', skewX: -20, duration: 3.5, repeat: -1, ease: "none",
                startAt: { x: '-150%', skewX: -20 }
            });
            gsap.to('.border-flow-anim', {
                backgroundPositionX: '200%', duration: 3, repeat: -1, ease: "linear"
            });
            gsap.fromTo('.matrix-line-anim',
                { strokeDashoffset: 1000 },
                { strokeDashoffset: 0, duration: 40, repeat: -1, ease: "linear" }
            );
            gsap.fromTo('.matrix-line-anim-reverse',
                { strokeDashoffset: 0 },
                { strokeDashoffset: 1000, duration: 40, repeat: -1, ease: "linear" }
            );
            ...
            heroTitleSpans.forEach(span => {
                ...
                text.split('').forEach(char => {
                    ...
                    charSpan.className = 'hero-char inline-block';
            ...
            gsap.to('.hero-char', {
                rotationY: 360, y: -5, color: "#ea580c", duration: 1.5,
                stagger: { each: 0.05, repeat: -1, repeatDelay: 5, yoyo: true },
                ease: "power2.inOut", delay: 2.5
            });
            ...
            capTl.to('.cap-anim-word', {y: "0%", opacity: 1, duration: 0.8, ease: "power4.out"})
                 .to('.cap-anim-word', { color: "#fdba74", duration: 0.6, ease: "power2.out" }, "+=0.1")
                 .to('#cap-sub', {opacity: 1, y: 0, duration: 0.8, ease: "power3.out"}, "-=0.6");

            // Capabilities Cards Scrolling Animation
            const cards = gsap.utils.toArray('.dm-card-wrapper');
            cards.forEach((card, index) => {
                // Entrance Scroll Animation per card
                gsap.fromTo(card, 
                    { opacity: 0, y: 100, scale: 0.95, rotationX: 10 }, 
                    { 
            ...
            // High-End Footer Text Animation
            gsap.set(".footer-brand", { gap: "2rem" });
            gsap.set(".footer-letter", { opacity: 0, y: 150, rotationX: -90, scale: 0.5, filter: "blur(20px)" });
            ...
            footerTl.to(".footer-letter", { 
                opacity: 1, y: 0, rotationX: 0, scale: 1, filter: "blur(0px)", 
                duration: 1.8, stagger: 0.1, ease: "power4.out", color: "rgba(255, 255, 255, 0.9)" 
            })
            .to(".footer-brand", {
                gap: "0rem",
            ...
            .to(".footer-letter", {
                color: "#C2410C",
                textShadow: "0px 0px 40px rgba(194,65,12,0.8)",
                duration: 2,
                stagger: 0.1,
                ease: "sine.inOut",
                repeat: -1,
            ...
            if (heroParticles) {
                for (let i = 0; i < 40; i++) {
                    ...
                    gsap.to(particle, {
                        y: `-${Math.random() * 200 + 100}`, x: `+=${Math.random() * 100 - 50}`,
                        opacity: 0, duration: Math.random() * 5 + 3, repeat: -1, ease: "linear", delay: Math.random() * 3
                    });
                }
            }
        });
        ...
        function openCaseStudy(title) {
            ...
            gsap.fromTo(overlay, { y: "100%" }, { y: "0%", duration: 0.8, ease: "expo.inOut" });
            gsap.fromTo(".case-anim", { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, stagger: 0.1, delay: 0.4, ease: "power3.out" });
        }
...
<span className="text-xl tracking-tighter font-light uppercase flex relative" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>
<span className="text-slate-900 transition-colors duration-300 group-hover:text-[#C2410C] relative z-10 tracking-tighter">TECH</span>
<span className="text-[#C2410C] relative z-0 tracking-tighter">NAMAH</span>
</span>
</div>
...
<a className="text-xs font-light text-slate-500 hover:text-[#C2410C] uppercase tracking-widest transition-colors nav-link relative group" href="#services">
                    Capabilities
                    <span className="absolute -bottom-1 left-0 w-0 h-px bg-[#C2410C] group-hover:w-full transition-all duration-300"></span>
</a>
<a className="text-xs font-light text-slate-500 hover:text-[#C2410C] uppercase tracking-widest transition-colors nav-link relative group" href="#work">
                    Work
                    <span className="absolute -bottom-1 left-0 w-0 h-px bg-[#C2410C] group-hover:w-full transition-all duration-300"></span>
</a>
<a className="text-xs font-light text-slate-500 hover:text-[#C2410C] uppercase tracking-widest transition-colors nav-link relative group" href="#process-wrapper">
                    Process
                    <span className="absolute -bottom-1 left-0 w-0 h-px bg-[#C2410C] group-hover:w-full transition-all duration-300"></span>
</a>
</div>
<div className="flex items-center gap-2 relative z-10">
<button className="nav-btn relative flex items-center justify-center px-6 py-2.5 text-xs font-light text-white bg-slate-900 rounded-full overflow-hidden group shadow-sm hover:shadow-md transition-shadow duration-300">
...
<span className="text-xs font-light text-slate-600 uppercase tracking-widest">Future-Ready AI Engineering</span>
</div>
<h1 className="text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-extralight tracking-tighter text-slate-900 leading-[1] mb-6 relative z-20" style={{fontFamily: '\'Space Grotesk\', sans-serif', perspective: '1000px'}}>
<span className="block"><span className="hero-title-text block [transform-style:preserve-3d] text-slate-900">Architecting</span></span>
...
<span className="hero-title-text block [transform-style:preserve-3d]">Tomorrow's</span>
...
<span className="block whitespace-nowrap"><span className="hero-title-text block [transform-style:preserve-3d] text-slate-900">Digital Reality.</span></span>
</h1>
...
<button className="relative overflow-hidden flex items-center justify-center gap-3 px-8 py-3.5 text-sm font-light text-white bg-slate-900 rounded-full shadow-md hover:shadow-lg transition-all duration-300 group z-10 flex-1 sm:flex-none tracking-tight">
<div className="absolute inset-0 w-0 bg-gradient-to-r from-[#C2410C] to-[#ea580c] transition-all duration-500 ease-out group-hover:w-full z-0"></div>
...
<button className="relative flex items-center justify-center gap-2 px-8 py-3.5 text-sm font-light text-slate-600 bg-white/50 backdrop-blur-md border border-slate-200/80 hover:border-[#C2410C]/30 hover:bg-white rounded-full transition-all duration-300 flex-1 sm:flex-none group tracking-tight">
...
<div className="relative w-full h-[32rem] flex items-center justify-center lg:justify-end" style={{perspective: '1000px'}}>
...
<div className="text-xs font-light text-[#C2410C] uppercase tracking-widest border border-[#C2410C]/20 px-3 py-1 rounded-full bg-[#C2410C]/5 flex items-center gap-2">
...
<div className="absolute top-3 left-4 text-xs font-mono font-light text-slate-500 z-10 tracking-tight">Neural Net Output</div>
<div className="absolute top-3 right-4 text-xs font-mono font-light text-emerald-400/80 z-10 flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span> Live</div>
...
<div className="text-xs text-slate-500 mb-2 uppercase tracking-widest font-mono font-light">Process Matrix</div>
<div className="relative h-full overflow-hidden mask-image-bottom" style={{maskImage: 'linear-gradient(to bottom, black 50%, transparent 100%)'}}>
<div className="text-xs font-mono font-light text-emerald-400/60 leading-relaxed whitespace-nowrap tracking-tight" id="hero-scrolling-code">
...
<div className="text-xs text-orange-200/50 mb-1 font-mono font-light tracking-tight">Inference</div>
<div className="text-sm font-light text-orange-100/90 flex items-center gap-2 tracking-tight">Processing <iconify-icon className="animate-spin text-xs" icon="solar:refresh-linear"></iconify-icon></div>
</div>
</div>
</div>
</div>
...
<div>
<div className="text-xs font-light text-slate-800 tracking-tight">Clean Architecture</div>
<div className="text-xs font-mono font-light text-slate-500 mt-0.5 flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> 0 Tech Debt</div>
</div>
</div>
...
<span className="text-xs font-mono font-light text-slate-400 uppercase tracking-widest">Data Sync</span>
<span className="text-xs font-light text-emerald-400 font-mono tracking-tight" id="hero-percent">99.9%</span>
</div>
...
<div className="max-w-6xl mx-auto px-6 mb-24 relative z-10" style={{perspective: '1000px'}}>
<h2 className="text-4xl md:text-5xl lg:text-6xl font-extralight tracking-tighter text-white mb-6 relative z-10 inline-block overflow-hidden" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>
<span className="cap-anim-word inline-block translate-y-full opacity-0 relative transition-colors duration-500">
                    Capabilities Matrix
                    <div className="shimmer-effect absolute inset-0 w-[50%] bg-gradient-to-r from-transparent via-white/40 to-transparent skew-x-[-20deg]"></div>
</span>
</h2>
<p className="text-lg font-thin text-slate-400 max-w-2xl tracking-tight opacity-0 translate-y-4 relative z-10" id="cap-sub">We leverage the latest in AI and modern web technologies to engineer scalable, high-performance digital products.</p>
</div>
<div className="max-w-6xl mx-auto px-6 relative cards-stack-container pb-[20vh] z-10">

<div className="dm-card-wrapper sticky top-24 lg:top-24 w-full origin-top mb-12 transform-gpu">
...
<div className="absolute inset-0 opacity-0 group-hover/matrixcard:opacity-100 transition-opacity duration-500 bg-[linear-gradient(90deg,transparent,rgba(59,130,246,0.6),transparent)] border-flow-anim pointer-events-none z-20" style={{borderRadius: '2rem', padding: '1px', WebkitMask: 'linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0)', WebkitMaskComposite: 'xor', maskComposite: 'exclude', backgroundSize: '200% 100%'}}></div>
<div className="lg:w-5/12 flex flex-col justify-between relative z-10 h-full py-2">
<div>
<div className="text-slate-400 text-xs font-mono font-light tracking-widest uppercase mb-6 flex items-center gap-2"><span className="w-6 h-px bg-slate-600"></span> Node 01</div>
<h3 className="text-4xl md:text-5xl font-extralight tracking-tighter text-white mb-6 leading-tight" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>AI SaaS <br/>Platforms</h3>
<p className="text-base font-thin text-slate-300 max-w-sm leading-relaxed tracking-tight">Scalable, secure, and intelligent SaaS applications powered by advanced machine learning models and LLMs.</p>
</div>
<div className="mt-8 lg:mt-12">
<button className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white font-light hover:bg-white/10 hover:border-white/20 transition-all group cursor-pointer text-sm shadow-sm tracking-tight">
...
<div className="flex justify-between items-center text-slate-500 text-xs font-mono font-light tracking-tight"><span>Model Performance</span><span className="flex items-center gap-2 text-blue-400"><div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></div> Live Sync</span></div>
...
<div className="text-xl tracking-tight text-white font-light" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Scalable Base</div>
</div>
...
<div className="text-2xl tracking-tighter text-blue-400 font-light mb-1" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>&lt;50ms</div>
<div className="text-xs text-slate-500 font-mono font-light tracking-tight">Inference Latency</div>
</div>
</div>
</div>
</div>

<div className="dm-card-wrapper sticky top-32 lg:top-32 w-full origin-top mb-12 transform-gpu">
<div className="dm-card w-full min-h-[32rem] bg-white/[0.03] backdrop-blur-[12px] border border-white/[0.08] rounded-[2rem] p-8 lg:p-12 flex flex-col lg:flex-row gap-10 overflow-hidden shadow-2xl transition-all duration-500 hover:-translate-y-2 hover:scale-[1.02] hover:bg-white/[0.06] hover:border-white/[0.15] group/matrixcard cursor-pointer relative">
...
<div className="absolute inset-0 opacity-0 group-hover/matrixcard:opacity-100 transition-opacity duration-500 bg-[linear-gradient(90deg,transparent,rgba(139,92,246,0.6),transparent)] border-flow-anim pointer-events-none z-20" style={{borderRadius: '2rem', padding: '1px', WebkitMask: 'linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0)', WebkitMaskComposite: 'xor', maskComposite: 'exclude', backgroundSize: '200% 100%'}}></div>
<div className="lg:w-5/12 flex flex-col justify-between relative z-10 h-full py-2">
<div>
<div className="text-slate-400 text-xs font-mono font-light tracking-widest uppercase mb-6 flex items-center gap-2"><span className="w-6 h-px bg-slate-600"></span> Node 02</div>
<h3 className="text-4xl md:text-5xl font-extralight tracking-tighter text-white mb-6 leading-tight" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>UI/UX &amp; <br/>Vibe Coding</h3>
<p className="text-base font-thin text-slate-300 max-w-sm leading-relaxed tracking-tight">Crafting future-ready product experiences specifically for conversational interfaces.</p>
</div>
<div className="mt-8 lg:mt-12">
<button className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white font-light hover:bg-white/10 hover:border-white/20 transition-all group cursor-pointer text-sm shadow-sm tracking-tight">
                                Explore Capabilities <iconify-icon className="text-lg transform group-hover:translate-x-1 transition-transform" icon="solar:arrow-right-linear" strokeWidth="1.5"></iconify-icon>
...
<div className="text-xs text-slate-500 font-mono font-light tracking-tight">Design System</div>
...
<div className="text-sm tracking-tight text-white font-light" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Generative UI</div>
</div>
</div>
</div>
</div>

<div className="dm-card-wrapper sticky top-40 lg:top-40 w-full origin-top mb-12 transform-gpu">
<div className="dm-card w-full min-h-[32rem] bg-white/[0.03] backdrop-blur-[12px] border border-white/[0.08] rounded-[2rem] p-8 lg:p-12 flex flex-col lg:flex-row gap-10 overflow-hidden shadow-2xl transition-all duration-500 hover:-translate-y-2 hover:scale-[1.02] hover:bg-white/[0.06] hover:border-white/[0.15] group/matrixcard cursor-pointer relative">
...
<div className="absolute inset-0 opacity-0 group-hover/matrixcard:opacity-100 transition-opacity duration-500 bg-[linear-gradient(90deg,transparent,rgba(6,182,212,0.6),transparent)] border-flow-anim pointer-events-none z-20" style={{borderRadius: '2rem', padding: '1px', WebkitMask: 'linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0)', WebkitMaskComposite: 'xor', maskComposite: 'exclude', backgroundSize: '200% 100%'}}></div>
<div className="lg:w-5/12 flex flex-col justify-between relative z-10 h-full py-2">
<div>
<div className="text-slate-400 text-xs font-mono font-light tracking-widest uppercase mb-6 flex items-center gap-2"><span className="w-6 h-px bg-slate-600"></span> Node 03</div>
<h3 className="text-4xl md:text-5xl font-extralight tracking-tighter text-white mb-6 leading-tight" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Mobile <br/>Applications</h3>
<p className="text-base font-thin text-slate-300 max-w-sm leading-relaxed tracking-tight">High-performance native and cross-platform mobile apps built with modern frameworks.</p>
</div>
<div className="mt-8 lg:mt-12">
<button className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white font-light hover:bg-white/10 hover:border-white/20 transition-all group cursor-pointer text-sm shadow-sm tracking-tight">
                                Explore Capabilities <iconify-icon className="text-lg transform group-hover:translate-x-1 transition-transform" icon="solar:arrow-right-linear" strokeWidth="1.5"></iconify-icon>
...
<div className="text-lg tracking-tight text-white font-light" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Cross-Platform</div>
</div>
...
<div>
<div className="text-xs text-slate-500 font-mono font-light mb-1 tracking-tight">FPS</div>
<div className="text-2xl tracking-tighter text-cyan-400 font-light" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>60.0</div>
</div>
...
<div className="dm-card-wrapper sticky top-48 lg:top-48 w-full origin-top mb-12 transform-gpu">
<div className="dm-card w-full min-h-[32rem] bg-white/[0.03] backdrop-blur-[12px] border border-white/[0.08] rounded-[2rem] p-8 lg:p-12 flex flex-col lg:flex-row gap-10 overflow-hidden shadow-2xl transition-all duration-500 hover:-translate-y-2 hover:scale-[1.02] hover:bg-white/[0.06] hover:border-white/[0.15] group/matrixcard cursor-pointer relative">
...
<div className="absolute inset-0 opacity-0 group-hover/matrixcard:opacity-100 transition-opacity duration-500 bg-[linear-gradient(90deg,transparent,rgba(236,72,153,0.6),transparent)] border-flow-anim pointer-events-none z-20" style={{borderRadius: '2rem', padding: '1px', WebkitMask: 'linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0)', WebkitMaskComposite: 'xor', maskComposite: 'exclude', backgroundSize: '200% 100%'}}></div>
<div className="lg:w-5/12 flex flex-col justify-between relative z-10 h-full py-2">
<div>
<div className="text-slate-400 text-xs font-mono font-light tracking-widest uppercase mb-6 flex items-center gap-2"><span className="w-6 h-px bg-slate-600"></span> Node 04</div>
<h3 className="text-4xl md:text-5xl font-extralight tracking-tighter text-white mb-6 leading-tight" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Webflow <br/>Enterprise</h3>
<p className="text-base font-thin text-slate-300 max-w-sm leading-relaxed tracking-tight">Pixel-perfect, GSAP-animated enterprise websites built natively in Webflow for ultimate marketing agility and soothing aesthetics.</p>
</div>
<div className="mt-8 lg:mt-12">
<button className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white font-light hover:bg-white/10 hover:border-white/20 transition-all group cursor-pointer text-sm shadow-sm tracking-tight">
                                Explore Capabilities <iconify-icon className="text-lg transform group-hover:translate-x-1 transition-transform" icon="solar:arrow-right-linear" strokeWidth="1.5"></iconify-icon>
...
<div className="absolute top-4 left-6 text-xs font-mono font-light text-pink-400/80 tracking-tight">Smooth Rendering</div>
...
<div className="text-lg tracking-tight text-white font-light" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Visual Dev</div>
</div>
...
<div>
<div className="text-xs text-slate-500 font-mono font-light mb-1 tracking-tight">Lighthouse</div>
<div className="text-3xl tracking-tighter text-pink-400 font-light" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>100</div>
</div>
...
<div className="max-w-7xl mx-auto px-6" style={{perspective: '1000px'}}>
<div className="mb-16">
<h2 className="text-4xl md:text-5xl lg:text-6xl font-extralight tracking-tighter text-slate-900 mb-4" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>
...
<p className="text-lg font-thin tracking-tight text-slate-500 max-w-lg opacity-0 translate-y-4" id="work-sub">A glimpse into the digital products we've engineered.</p>
</div>
<div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-start relative">
<div className="w-full lg:w-1/2 flex flex-col gap-0 border-t border-slate-200/40">
<div className="work-trigger group cursor-pointer py-10 border-b border-slate-200/40 relative border-l-4 border-transparent px-4 hover:pl-8 hover:border-[#C2410C] hover:bg-slate-50/50 transition-all duration-300" data-target="visual-1" onclick="openCaseStudy('Nexus Data')">
<div className="text-xs font-light text-slate-400 mb-3 uppercase tracking-widest group-hover:text-[#C2410C] transition-colors">01 / AI SaaS Platform</div>
<div className="flex items-center justify-between">
<h3 className="text-3xl md:text-4xl font-extralight tracking-tighter text-slate-800 group-hover:text-[#C2410C] transition-colors duration-300" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Nexus Data</h3>
...
<p className="text-base font-thin text-slate-500 mt-6 max-w-md tracking-tight">Predictive analytics dashboard powered by custom machine learning models, allowing enterprises to forecast trends accurately.</p>
</div>
</div>
</div>
<div className="work-trigger group cursor-pointer py-10 border-b border-slate-200/40 relative border-l-4 border-transparent px-4 hover:pl-8 hover:border-[#C2410C] hover:bg-slate-50/50 transition-all duration-300" data-target="visual-2" onclick="openCaseStudy('FinAI Assistant')">
<div className="text-xs font-light text-slate-400 mb-3 uppercase tracking-widest group-hover:text-[#C2410C] transition-colors">02 / Mobile Application</div>
<div className="flex items-center justify-between">
<h3 className="text-3xl md:text-4xl font-extralight tracking-tighter text-slate-800 group-hover:text-[#C2410C] transition-colors duration-300" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>FinAI Assistant</h3>
<div className="w-10 h-10 rounded-full border border-slate-200/80 flex items-center justify-center group-hover:bg-[#C2410C] group-hover:border-[#C2410C] transition-all duration-300">
<iconify-icon className="text-slate-400 group-hover:text-white text-lg" icon="solar:arrow-right-up-linear" strokeWidth="1.5"></iconify-icon>
</div>
</div>
<div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-[grid-template-rows] duration-500 ease-in-out">
<div className="overflow-hidden">
<p className="text-base font-thin text-slate-500 mt-6 max-w-md tracking-tight">Personal finance app with a conversational AI interface designed to help Gen Z manage investments effortlessly.</p>
</div>
</div>
</div>
<div className="work-trigger group cursor-pointer py-10 border-b border-slate-200/40 relative border-l-4 border-transparent px-4 hover:pl-8 hover:border-[#C2410C] hover:bg-slate-50/50 transition-all duration-300" data-target="visual-3" onclick="openCaseStudy('Aura Health')">
<div className="text-xs font-light text-slate-400 mb-3 uppercase tracking-widest group-hover:text-[#C2410C] transition-colors">03 / Webflow Enterprise</div>
<div className="flex items-center justify-between">
<h3 className="text-3xl md:text-4xl font-extralight tracking-tighter text-slate-800 group-hover:text-[#C2410C] transition-colors duration-300" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Aura Health</h3>
<div className="w-10 h-10 rounded-full border border-slate-200/80 flex items-center justify-center group-hover:bg-[#C2410C] group-hover:border-[#C2410C] transition-all duration-300">
<iconify-icon className="text-slate-400 group-hover:text-white text-lg" icon="solar:arrow-right-up-linear" strokeWidth="1.5"></iconify-icon>
</div>
</div>
<div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-[grid-template-rows] duration-500 ease-in-out">
<div className="overflow-hidden">
<p className="text-base font-thin text-slate-500 mt-6 max-w-md tracking-tight">High-converting, GSAP-animated marketing website for a leading AI healthcare startup, built natively in Webflow.</p>
</div>
</div>
</div>
</div>
...
<div className="w-full h-[70%] bg-[#C2410C]/80 rounded-t-sm relative"><div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs px-1.5 py-0.5 rounded shadow tracking-tight">+84%</div></div>
...
<div className="text-xs font-extralight opacity-80 mb-1 tracking-tight">Total Balance</div>
<div className="text-2xl font-extralight tracking-tighter">$24,500.00</div>
</div>
...
<section className="overflow-hidden bg-[#050505] text-white relative h-screen border-t border-slate-900 z-10" id="process-wrapper">
...
<div className="h-full flex flex-col justify-center relative z-10" style={{perspective: '1000px'}}>
<div className="w-full absolute top-12 md:top-24 left-0 px-6 max-w-7xl mx-auto right-0 flex justify-between items-end z-20">
<h2 className="text-4xl md:text-5xl lg:text-6xl font-extralight tracking-tighter" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>How We Build</h2>
<div className="hidden md:flex text-xs text-slate-500 items-center gap-2 tracking-widest uppercase font-mono font-light">
...
<div className="relative w-full aspect-[4/3] md:aspect-[16/9] bg-[#0A0A0A]/80 backdrop-blur-xl border border-white/5 rounded-[2rem] p-6 flex flex-col overflow-hidden cursor-pointer hover:border-[#C2410C]/30 transition-all duration-500 group/card shadow-xl hover:-translate-y-1">
<div className="grid grid-cols-2 grid-rows-2 gap-4 h-full w-full">
<div className="col-span-2 row-span-1 bg-white/[0.02] rounded-xl p-6 relative overflow-hidden flex flex-col justify-between border border-white/5">
<div className="absolute -right-6 -bottom-6 text-7xl md:text-8xl font-extralight tracking-tighter text-white/5 group-hover/card:text-[#C2410C] group-hover/card:opacity-10 transition-all duration-700 pointer-events-none" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>01</div>
<div className="relative z-10">
<div className="text-[#C2410C] font-mono font-light tracking-widest text-xs mb-2 uppercase flex items-center gap-2"><span className="w-4 h-px bg-[#C2410C]"></span> Discover</div>
<h3 className="text-3xl md:text-4xl font-extralight text-white mb-2 tracking-tighter" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Validation</h3>
</div>
</div>
...
<p className="text-slate-400 text-xs font-thin leading-relaxed tracking-tight mt-2 md:mt-4">We analyze your vision, validate assumptions, and architect the AI foundation.</p>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 flex items-center justify-between group-hover/card:bg-[#C2410C]/5 transition-colors">
<div className="text-xs text-white/80 font-light tracking-tight">Phase 1/5</div>
</div>
</div>
</div>
</div>
...
<div className="grid grid-cols-2 grid-rows-2 gap-4 h-full w-full">
<div className="col-span-2 row-span-1 bg-white/[0.02] rounded-xl p-6 relative overflow-hidden flex flex-col justify-between border border-white/5">
<div className="absolute -right-6 -bottom-6 text-7xl md:text-8xl font-extralight tracking-tighter text-white/5 group-hover/card:text-[#C2410C] group-hover/card:opacity-10 transition-all duration-700 pointer-events-none" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>02</div>
<div className="relative z-10">
<div className="text-[#C2410C] font-mono font-light tracking-widest text-xs mb-2 uppercase flex items-center gap-2"><span className="w-4 h-px bg-[#C2410C]"></span> Strategy</div>
<h3 className="text-3xl md:text-4xl font-extralight text-white mb-2 tracking-tighter" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Architecture</h3>
</div>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 relative group/inner overflow-hidden">
...
<p className="text-slate-400 text-xs font-thin leading-relaxed tracking-tight mt-2 md:mt-4">Crafting the blueprint. We select the optimal machine learning models and stack.</p>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 flex items-center justify-between group-hover/card:bg-[#C2410C]/5 transition-colors">
<div className="text-xs text-white/80 font-light tracking-tight">Phase 2/5</div>
</div>
</div>
</div>
</div>
...
<div className="grid grid-cols-2 grid-rows-2 gap-4 h-full w-full">
<div className="col-span-2 row-span-1 bg-white/[0.02] rounded-xl p-6 relative overflow-hidden flex flex-col justify-between border border-white/5">
<div className="absolute -right-6 -bottom-6 text-7xl md:text-8xl font-extralight tracking-tighter text-white/5 group-hover/card:text-[#C2410C] group-hover/card:opacity-10 transition-all duration-700 pointer-events-none" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>03</div>
<div className="relative z-10">
<div className="text-[#C2410C] font-mono font-light tracking-widest text-xs mb-2 uppercase flex items-center gap-2"><span className="w-4 h-px bg-[#C2410C]"></span> Design</div>
<h3 className="text-3xl md:text-4xl font-extralight text-white mb-2 tracking-tighter" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Prototyping</h3>
</div>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 relative group/inner overflow-hidden">
...
<p className="text-slate-400 text-xs font-thin leading-relaxed tracking-tight mt-2 md:mt-4">Crafting intuitive, AI-driven interfaces and frictionless user experiences.</p>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 flex items-center justify-between group-hover/card:bg-[#C2410C]/5 transition-colors">
<div className="text-xs text-white/80 font-light tracking-tight">Phase 3/5</div>
</div>
</div>
</div>
</div>
...
<div className="grid grid-cols-2 grid-rows-2 gap-4 h-full w-full">
<div className="col-span-2 row-span-1 bg-white/[0.02] rounded-xl p-6 relative overflow-hidden flex flex-col justify-between border border-white/5">
<div className="absolute -right-6 -bottom-6 text-7xl md:text-8xl font-extralight tracking-tighter text-white/5 group-hover/card:text-[#C2410C] group-hover/card:opacity-10 transition-all duration-700 pointer-events-none" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>04</div>
<div className="relative z-10">
<div className="text-[#C2410C] font-mono font-light tracking-widest text-xs mb-2 uppercase flex items-center gap-2"><span className="w-4 h-px bg-[#C2410C]"></span> Develop</div>
<h3 className="text-3xl md:text-4xl font-extralight text-white mb-2 tracking-tighter" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Engineering</h3>
</div>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 relative group/inner overflow-hidden">
...
<p className="text-slate-400 text-xs font-thin leading-relaxed tracking-tight mt-2 md:mt-4">Building scalable infrastructure and integrating complex machine learning models.</p>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 flex items-center justify-between group-hover/card:bg-[#C2410C]/5 transition-colors">
<div className="text-xs text-white/80 font-light tracking-tight">Phase 4/5</div>
</div>
</div>
</div>
</div>
...
<div className="grid grid-cols-2 grid-rows-2 gap-4 h-full w-full">
<div className="col-span-2 row-span-1 bg-white/[0.02] rounded-xl p-6 relative overflow-hidden flex flex-col justify-between border border-white/5">
<div className="absolute -right-6 -bottom-6 text-7xl md:text-8xl font-extralight tracking-tighter text-white/5 group-hover/card:text-[#C2410C] group-hover/card:opacity-10 transition-all duration-700 pointer-events-none" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>05</div>
<div className="relative z-10">
<div className="text-[#C2410C] font-mono font-light tracking-widest text-xs mb-2 uppercase flex items-center gap-2"><span className="w-4 h-px bg-[#C2410C]"></span> Launch</div>
<h3 className="text-3xl md:text-4xl font-extralight text-white mb-2 tracking-tighter" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Deployment</h3>
</div>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 relative group/inner overflow-hidden">
...
<p className="text-slate-400 text-xs font-thin leading-relaxed tracking-tight mt-2 md:mt-4">Rigorous QA testing and deploying the product to global edge networks.</p>
</div>
<div className="col-span-1 row-span-1 bg-white/[0.02] rounded-xl p-6 border border-white/5 flex items-center justify-between group-hover/card:bg-[#C2410C]/5 transition-colors">
<div className="text-xs text-white/80 font-light tracking-tight">Phase 5/5</div>
</div>
</div>
</div>
</div>
...
<section className="bg-[#020202] text-white overflow-hidden relative z-20">
...
<div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-[#C2410C]/20 bg-[#C2410C]/5 text-xs font-light text-[#C2410C] tracking-widest uppercase mb-8 interest-element">
<span className="w-1.5 h-1.5 rounded-full bg-[#C2410C] animate-pulse"></span> Have we caught your interest?
                </div>
<h2 className="text-5xl md:text-7xl font-extralight tracking-tighter text-white mb-12 leading-[1.1] interest-element" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>
...
<div className="text-sm font-light text-white leading-tight mb-0.5 tracking-tight">Jonah Neuss</div>
<div className="text-xs text-slate-400 tracking-widest uppercase font-mono font-light">CEO, Founder</div>
</div>
</div>
<button className="flex items-center gap-3 bg-transparent hover:bg-white/10 transition-colors rounded-full py-1.5 pl-5 pr-1.5 group cursor-pointer tracking-tight">
<span className="text-sm font-light text-white">Let's Connect</span>
...
<div className="pt-24 pb-12 relative z-10 bg-[#000000]">
<div className="max-w-7xl mx-auto px-6">
<div className="flex justify-end mb-24">
<div className="max-w-md text-left">
<div className="inline-flex items-center px-3 py-1.5 rounded-full border border-[#C2410C]/20 bg-transparent text-xs font-mono font-light text-[#C2410C] tracking-widest uppercase mb-6">Get in touch</div>
<h3 className="text-4xl md:text-5xl font-extralight tracking-tighter text-white mb-8" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Let's talk about your project</h3>
<button className="inline-flex items-center gap-2 px-8 py-3.5 bg-white text-slate-900 hover:bg-[#C2410C] hover:text-white rounded-full text-sm font-light transition-colors group tracking-wide shadow-md">
...
<div className="mb-20 overflow-hidden w-full flex justify-center" style={{perspective: '1000px'}}>
<h2 className="footer-brand flex justify-center w-full px-4 text-7xl md:text-9xl leading-[0.8] font-light uppercase" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>
<span className="footer-letter inline-block [transform-style:preserve-3d]">T</span>
<span className="footer-letter inline-block [transform-style:preserve-3d]">E</span>
<span className="footer-letter inline-block [transform-style:preserve-3d]">C</span>
<span className="footer-letter inline-block [transform-style:preserve-3d]">H</span>
<span className="footer-letter inline-block [transform-style:preserve-3d]">N</span>
<span className="footer-letter inline-block [transform-style:preserve-3d]">A</span>
<span className="footer-letter inline-block [transform-style:preserve-3d]">M</span>
<span className="footer-letter inline-block [transform-style:preserve-3d]">A</span>
<span className="footer-letter inline-block [transform-style:preserve-3d]">H</span>
</h2>
</div>
...
<div>
<h5 className="text-xs font-light text-slate-500 mb-8 uppercase tracking-widest">Navigation</h5>
<ul className="flex flex-col gap-4">
<li><a className="text-sm font-thin text-slate-400 hover:text-white transition-colors tracking-tight" href="#">Home</a></li>
<li><a className="text-sm font-thin text-slate-400 hover:text-white transition-colors tracking-tight" href="#">About Us</a></li>
<li><a className="text-sm font-thin text-slate-400 hover:text-white transition-colors tracking-tight" href="#">Services</a></li>
<li><a className="text-sm font-thin text-slate-400 hover:text-white transition-colors tracking-tight" href="#">Portfolio</a></li>
</ul>
</div>
<div>
<h5 className="text-xs font-light text-slate-500 mb-8 uppercase tracking-widest">Contact</h5>
<ul className="flex flex-col gap-4">
<li><a className="text-sm font-thin text-slate-400 hover:text-white transition-colors tracking-tight" href="mailto:hello@technamah.com">hello@technamah.com</a></li>
</ul>
</div>
<div>
<h5 className="text-xs font-light text-slate-500 mb-8 uppercase tracking-widest">Socials</h5>
<ul className="flex flex-col gap-4">
<li><a className="text-sm font-thin text-slate-400 hover:text-white transition-colors tracking-tight" href="#">LinkedIn</a></li>
<li><a className="text-sm font-thin text-slate-400 hover:text-white transition-colors tracking-tight" href="#">Twitter</a></li>
</ul>
</div>
<div>
<h5 className="text-xs font-light text-slate-500 mb-8 uppercase tracking-widest">Location</h5>
<p className="text-sm font-thin text-slate-400 leading-relaxed max-w-xs tracking-tight">123 Innovation Drive<br/>Tech City, TC 90210</p>
</div>
</div>
...
<div className="text-sm font-thin text-slate-500 tracking-tight">© 2026 TechNamah. All rights reserved.</div>
...
<div className="text-white font-light text-xl tracking-tighter case-anim pointer-events-auto uppercase" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>TECH<span className="text-[#C2410C]">NAMAH</span></div>
...
<div className="text-[#C2410C] font-mono font-light tracking-widest uppercase mb-6 text-xs case-anim flex items-center gap-3"><span className="w-8 h-px bg-[#C2410C]"></span> Case Study</div>
<h1 className="text-6xl md:text-8xl lg:text-9xl leading-none font-extralight tracking-tighter text-slate-900 mb-8 case-anim" id="case-study-title" style={{fontFamily: '\'Space Grotesk\', sans-serif'}}>Project Name</h1>
<p className="text-xl md:text-2xl text-slate-500 max-w-3xl leading-relaxed case-anim font-thin tracking-tight" id="case-study-desc">An indepth look into how we architected a scalable, AI-powered solution for a market-leading enterprise.</p>
</div>
<div className="h-[60vh] w-full bg-slate-50 flex items-center justify-center case-anim border-y border-slate-100/50">
<div className="text-slate-400 font-mono font-light text-xs uppercase tracking-widest"><iconify-icon className="text-2xl block mx-auto mb-2 text-center" icon="solar:gallery-linear"></iconify-icon> Project Visual Showcase</div>
</div>
</div>

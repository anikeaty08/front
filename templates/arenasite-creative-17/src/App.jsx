    ...
    !function(){if(!window.UnicornStudio){window.UnicornStudio={isInitialized:!1};var i=document.createElement("script");i.src="https://cdn.jsdelivr.net/gh/hiunicornstudio/unicornstudio.js@v1.4.29/dist/unicornStudio.umd.js",i.onload=function(){window.UnicornStudio.isInitialized||(UnicornStudio.init(),window.UnicornStudio.isInitialized=!0)},(document.head || document.body).appendChild(i)}}();
        ...
        if(portfolioItems.length > 0) {
            ...
            function updatePortfolioCarousel() {
                ...
                // Shift dynamically controls the overlap spacing. 
                // Using 75/85 guarantees elements overlap continuously inside the container with no background gaps.
                const shiftAmount = isMobile ? 85 : 75; 

                portfolioItems.forEach((item, index) => {
                    let offset = index - currentPortIndex;
                    
                    // Normalize for continuous loop math seamlessly
                    if (offset > Math.floor(totalPortItems / 2)) offset -= totalPortItems;
                    if (offset < -Math.floor(totalPortItems / 2)) offset += totalPortItems;

                    const absOffset = Math.abs(offset);
                    
                    // Core 3D Transformation Math
                    let tx = -50 + (offset * shiftAmount); 
                    let tz = absOffset * -100; // Pushes inactive items far back in 3D perspective
                    let scale = 1 - (absOffset * 0.15); // Scale down peripheral depth items
                    
                    if (offset === 0) {
                        scale = 1.05; // Center item takes visual dominance
                    }

                    // Aesthetic Controls (Brightness, Layers, Glow)
                    let zIndex = 30 - absOffset * 10;
                    let brightness = offset === 0 ? 1.15 : 0.3 - (absOffset * 0.1); // Subdues side items
                    let opacity = absOffset > 2 ? 0 : 1; 
                    
                    // Tightens mobile view by hiding furthest items
                    ...
                    // Expose textual context only on active focused item
                    if (offset === 0) {
...
<div className="flex group-hover:rotate-12 transition-transform font-bold text-purple-400 bg-zinc-900 w-8 h-8 border-purple-500/30 border rounded shadow-[0_0_15px_rgba(168,85,247,0.2)] items-center justify-center"></div>
<span className="self-center text-xl font-medium whitespace-nowrap tracking-tight text-zinc-100 group-hover:text-purple-300 transition-colors">ArenaSite</span>
</a>
...
<li className=""><a className="interactive-target hover:text-purple-300 transition-colors" href="#portfolio">Portfólio</a></li>
<li className=""><a className="interactive-target hover:text-purple-300 transition-colors" href="#processo">Processo</a></li>
...
<span className="group-hover:text-purple-100 transition-colors scale-105">Começar Projeto</span>
</button>
</div>
</nav>
...
<span className="text-sweep block pb-2">Sites impactantes de alta conversão. </span>
</h1>

<p className="md:text-xl reveal-base anim-blur delay-300 is-visible text-lg text-zinc-400 max-w-2xl mr-auto mb-10 ml-auto">Especialistas em criar experiências digitais imersivas. Com a <span className="[--fx-filter:blur(12px)_liquid-glass(0,10)_saturate(1.25)_noise(0.5,1,0.15)] font-medium text-purple-400/90 font-roboto border-0 rounded-none backdrop-blur-xl">ArenaSite</span> combinamos estratégia, design e tecnologia para elevar sua presença online.</p>

<div className="flex flex-col sm:flex-row gap-4 justify-center items-center reveal-base anim-pop delay-400 is-visible">
<a className="interactive-target shiny-cta text-xl font-bold text-white text-right rounded-full pt-3 pr-6 pb-3 pl-6 shadow-lg" href="#contato">Começar Projeto</a>
</div>
</div>
...
<div className="w-2 h-2 rounded-full bg-yellow-500/50 group-hover:bg-yellow-400 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="text-sm font-medium uppercase tracking-widest text-gray-300 group-hover:text-white transition-colors">Gestão completa</span>
</div>
<div className="flex items-center gap-4 group">
<div className="w-2 h-2 rounded-full bg-yellow-500/50 group-hover:bg-yellow-400 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="uppercase group-hover:text-white transition-colors text-sm font-medium text-gray-300 tracking-widest">Análise comportamental</span>
</div>
<div className="flex gap-4 group gap-x-4 gap-y-4 items-center">
<div className="group-hover:bg-yellow-400 transition-colors bg-[conic-gradient(from_180deg,var(--tw-gradient-stops))] from-amber-400 via-amber-600 to-amber-400 w-2 h-2 rounded-full shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="uppercase group-hover:text-white transition-colors text-sm font-medium text-gray-300 tracking-widest">Design premium</span>
</div>
<div className="flex items-center gap-4 group">
<div className="w-2 h-2 rounded-full bg-yellow-500/50 group-hover:bg-yellow-400 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="text-sm font-medium uppercase tracking-widest text-gray-300 group-hover:text-white transition-colors">Acompanhamento diário</span>
</div>
<div className="flex items-center gap-4 group">
<div className="w-2 h-2 rounded-full bg-yellow-500/50 group-hover:bg-yellow-400 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="text-sm font-medium uppercase tracking-widest text-gray-300 group-hover:text-white transition-colors">Branding</span>
</div>
</div>
...
<div className="flex items-center gap-4 group">
<div className="w-2 h-2 rounded-full bg-yellow-500/50 group-hover:bg-yellow-400 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="text-sm font-medium uppercase tracking-widest text-gray-300 group-hover:text-white transition-colors">Estratégia Personalizada</span>
</div>
<div className="flex items-center gap-4 group">
<div className="w-2 h-2 rounded-full bg-yellow-500/50 group-hover:bg-yellow-400 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="text-sm font-medium uppercase tracking-widest text-gray-300 group-hover:text-white transition-colors">Acompanhamento diário</span>
...
<div className="flex items-center gap-4 group">
<div className="w-2 h-2 rounded-full bg-yellow-500/50 group-hover:bg-yellow-400 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="text-sm font-medium uppercase tracking-widest text-gray-300 group-hover:text-white transition-colors">Tráfego Pago</span>
</div>
<div className="flex items-center gap-4 group">
<div className="w-2 h-2 rounded-full bg-yellow-500/50 group-hover:bg-yellow-400 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.3)]"></div>
<span className="text-sm font-medium uppercase tracking-widest text-gray-300 group-hover:text-white transition-colors">Estratégia Personalizada</span>
...
<h2 className="text-3xl md:text-4xl font-semibold mb-4">Nossas Especialidades</h2>
<p className="text-zinc-400 max-w-2xl mx-auto">Soluções completas da ArenaSite para transformar visitantes em clientes fiéis.</p>
</div>
...
<iconify-icon icon="ph:desktop-duotone" width="24"></iconify-icon>
</div>
<h3 className="text-xl font-medium mb-3 group-hover:text-purple-100 transition-colors">Websites &amp; Landing Pages</h3>
<p className="text-sm text-zinc-400 leading-relaxed group-hover:text-zinc-300">Desenvolvimento de sites de alta performance, focados em conversão e experiência do usuário (UX/UI).</p>
</div>
...
<iconify-icon icon="ph:paint-brush-broad-duotone" width="24"></iconify-icon>
</div>
<h3 className="text-xl font-medium mb-3 group-hover:text-purple-100 transition-colors">Design de Mídia</h3>
<p className="text-sm text-zinc-400 leading-relaxed group-hover:text-zinc-300">Criação de assets visuais para redes sociais, banners e campanhas publicitárias que capturam a atenção.</p>
</div>
...
<div className="absolute inset-0 bg-gradient-to-br from-purple-500/0 via-purple-500/0 to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
<div className="w-12 h-12 bg-zinc-800 rounded-lg flex items-center justify-center mb-6 text-zinc-300 group-hover:bg-purple-600 group-hover:text-white transition-all duration-300 group-hover:shadow-[0_0_15px_rgba(168,85,247,0.4)]">
<iconify-icon icon="ph:fingerprint-duotone" width="24"></iconify-icon>
</div>
<h3 className="text-xl font-medium mb-3 group-hover:text-purple-100 transition-colors">Branding</h3>
<p className="text-sm text-zinc-400 leading-relaxed group-hover:text-zinc-300">Construção de identidade visual completa, desde o logotipo até o manual da marca e tom de voz.</p>
</div>
...
<h3 className="text-xl font-medium mb-3 group-hover:text-purple-100 transition-colors">SEO &amp; Performance</h3>
<p className="text-sm text-zinc-400 leading-relaxed group-hover:text-zinc-300">Otimização técnica para motores de busca e melhoria de velocidade para garantir o topo do Google.</p>
</div>

<div className="interactive-target group relative overflow-hidden p-8 rounded-2xl bg-zinc-900/50 border border-white/5 transition-all duration-300 hover:-translate-y-1 hover:border-purple-500/40 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] reveal-base anim-pop delay-200">
<div className="absolute inset-0 bg-gradient-to-br from-purple-500/0 via-purple-500/0 to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
<div className="w-12 h-12 bg-zinc-800 rounded-lg flex items-center justify-center mb-6 text-zinc-300 group-hover:bg-purple-600 group-hover:text-white transition-all duration-300 group-hover:shadow-[0_0_15px_rgba(168,85,247,0.4)]">
<iconify-icon icon="ph:chat-circle-text-duotone" width="24"></iconify-icon>
</div>
<h3 className="text-xl font-medium mb-3 group-hover:text-purple-100 transition-colors">Consultoria Digital</h3>
<p className="text-sm text-zinc-400 leading-relaxed group-hover:text-zinc-300">Análise de presença digital e planejamento estratégico para crescimento sustentável do seu negócio.</p>
</div>

<div className="interactive-target group relative overflow-hidden p-8 rounded-2xl bg-zinc-900/50 border border-white/5 transition-all duration-300 hover:-translate-y-1 hover:border-purple-500/40 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] reveal-base anim-pop delay-300">
<div className="absolute inset-0 bg-gradient-to-br from-purple-500/0 via-purple-500/0 to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
<div className="w-12 h-12 bg-zinc-800 rounded-lg flex items-center justify-center mb-6 text-zinc-300 group-hover:bg-purple-600 group-hover:text-white transition-all duration-300 group-hover:shadow-[0_0_15px_rgba(168,85,247,0.4)]">
<iconify-icon icon="ph:film-strip-duotone" width="24"></iconify-icon>
</div>
<h3 className="text-xl font-medium mb-3 group-hover:text-purple-100 transition-colors">Motion Graphics</h3>
<p className="text-sm text-zinc-400 leading-relaxed group-hover:text-zinc-300">Animações e vídeos curtos que explicam seu produto e aumentam o engajamento nas redes.</p>
</div>
</div>
</div>
</section>
...
<h2 className="text-3xl md:text-4xl font-medium mb-4 tracking-tight">Nosso Portfólio</h2>
<p className="text-zinc-400 max-w-2xl mx-auto">Experiências digitais premium criadas com precisão e estética inovadora.</p>
</div>
...
<h3 className="text-xl md:text-2xl font-medium text-white tracking-tight drop-shadow-md">Sushi bar</h3>
<p className="text-xs md:text-sm text-zinc-200 mt-1 drop-shadow-md"></p>
</div>
</div>
</div>
...
<div className="absolute inset-0 flex items-end p-4 md:p-6 pointer-events-none">
<div className="portfolio-content max-w-sm">
<h3 className="text-xl md:text-2xl font-medium text-white tracking-tight drop-shadow-md">Barbershop</h3>
<p className="text-xs md:text-sm text-zinc-200 mt-1 drop-shadow-md"></p>
</div>
</div>
</div>
...
<div className="absolute inset-0 flex items-end p-4 md:p-6 pointer-events-none">
<div className="portfolio-content max-w-sm">
<h3 className="text-xl md:text-2xl font-medium text-white tracking-tight drop-shadow-md">Restaurante</h3>
<p className="text-xs md:text-sm text-zinc-200 mt-1 drop-shadow-md"></p>
</div>
</div>
</div>
...
<div className="absolute inset-0 flex items-end p-4 md:p-6 pointer-events-none">
<div className="portfolio-content max-w-sm">
<h3 className="text-xl md:text-2xl font-medium text-white tracking-tight drop-shadow-md">Moda</h3>
<p className="text-xs md:text-sm text-zinc-200 mt-1 drop-shadow-md"></p>
</div>
</div>
</div>
...
<div className="absolute inset-0 flex items-end p-4 md:p-6 pointer-events-none">
<div className="portfolio-content max-w-sm">
<h3 className="text-xl md:text-2xl font-medium text-white tracking-tight drop-shadow-md">Imobiliária</h3>
<p className="text-xs md:text-sm text-zinc-200 mt-1 drop-shadow-md"></p>
</div>
</div>
</div>
...
<section className="pt-0 pb-24 relative overflow-hidden border-t border-white/5" id="processo">
<div className="container mx-auto px-6 max-w-4xl mt-12 md:mt-16">
<div className="text-center mb-20 reveal-base anim-slide-up">
<h2 className="text-3xl md:text-4xl font-semibold mb-4 tracking-tight">Processo ArenaSite</h2>
<p className="text-zinc-400">Como transformamos sua ideia em realidade.</p>
</div>
...
<h3 className="font-medium text-white group-hover:text-purple-200 transition-colors">Descoberta</h3>
</div>
<p className="text-sm text-zinc-400">Entendemos seu negócio, público-alvo e objetivos através de um briefing detalhado e pesquisa de mercado.</p>
</div>
</div>
...
<h3 className="font-medium text-white group-hover:text-purple-200 transition-colors">Estratégia &amp; Wireframe</h3>
</div>
<p className="text-sm text-zinc-400">Planejamos a arquitetura da informação e criamos o esqueleto visual do projeto focando na jornada do usuário.</p>
</div>
</div>
...
<h3 className="font-medium text-white group-hover:text-purple-200 transition-colors">Design Visual</h3>
</div>
<p className="text-sm text-zinc-400">Aplicamos a identidade visual, tipografia e cores, criando interfaces modernas e atraentes.</p>
</div>
</div>
...
<h3 className="font-medium text-white group-hover:text-purple-200 transition-colors">Desenvolvimento</h3>
</div>
<p className="text-sm text-zinc-400">Codificação limpa e otimizada, garantindo que o design funcione perfeitamente em todos os dispositivos.</p>
</div>
</div>
</div>
</div>
...
<h2 className="reveal-base anim-blur md:text-5xl text-4xl font-semibold tracking-tight font-montserrat mb-6">Vamos criar algo incrível.</h2>
<p className="reveal-base anim-slide-up delay-100 text-zinc-400 max-w-xl mr-auto mb-10 ml-auto">Contate para conversar sobre como a ArenaSite pode ajudar sua empresa a crescer no ambiente digital.</p>
<div className="reveal-base anim-pop delay-200">
<a className="interactive-target shiny-cta inline-flex items-center justify-center text-base font-bold text-white rounded-full pt-4 pr-8 pb-4 pl-8 group" href="mailto:contato@arenasite.com">
                    Solicitar Orçamento
                </a>
</div>
</div>
...
<p className="hover:text-zinc-300 transition-colors">© 2024 ArenaSite. Todos os direitos reservados.</p>
...
<a className="interactive-target hover:text-purple-400 transition-colors" href="#">Behance</a>
</div>
</div>
</footer>

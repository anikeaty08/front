...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
    ...
    (function () {
      ...
      ensureChartJs(initArkChart);

      function initArkChart() {
        // Elements
        const ctx = document.getElementById('arkChart').getContext('2d');
        const priceEl = document.getElementById('arkPrice');
        const changeEl = document.getElementById('arkChange');
        const mcapEl = document.getElementById('arkMcap');
        const mcapBarEl = document.getElementById('arkMcapBar');
        const volEl = document.getElementById('arkVol');
        const tradesEl = document.getElementById('arkTrades');
        const highEl = document.getElementById('arkHigh');
        const lowEl = document.getElementById('arkLow');

        // Config
        const CIRC_SUPPLY = 740_000_000; // from UI context
        const USD_PER_PLS = 0.00012; // mock conversion
        let currency = '$'; // '$' or 'PLS'
        ...
        function generateSeries(points, base = 0.00028, drift = 0.00002, vol = 0.18) {
          const rand = rng(points + 1337);
          const data = [];
          let last = base;
          for (let i = 0; i < points; i++) {
            // random walk
            const step = (rand() - 0.5) * vol * base;
            last = Math.max(0.0000001, last + step + (i % 40 === 0 ? drift : 0));
            data.push(+last.toFixed(8));
          }
          return data;
        }
        ...
        function format(val, cur) {
          if (cur === 'PLS') return `${(val / USD_PER_PLS).toFixed(6)} PLS`;
          // USD
        ...
        function updateStats(series) {
          ...
          priceEl.textContent = format(last, currency);
          // mock market cap and volume
          const mcap = last * CIRC_SUPPLY;
          mcapEl.textContent = format(mcap, '$');
          mcapBarEl.style.width = `${Math.min(95, Math.max(10, (mcap / 50_000_000) * 100))}%`;
          const vol = mcap * 0.07;
          volEl.textContent = format(vol, '$');

          const hi = Math.max(...series);
          const lo = Math.min(...series);
          highEl.textContent = format(hi, currency);
          lowEl.textContent = format(lo, currency);
          tradesEl.textContent = Math.round(series.length * 42).toLocaleString();
        }
        ...
        function buildGradient(ctx, area) {
          ...
          g.addColorStop(1, 'rgba(124,58,237,0.14)');
          return g;
        }

        const dataset = generateSeries(ranges[currentRange]);
        const chart = new Chart(ctx, {
          type: 'line',
          data: {
            labels: makeLabels(ranges[currentRange]),
            datasets: [{
              data: dataset,
              ...
              backgroundColor: (context) => {
                ...
                if (!gradient) gradient = buildGradient(ctx, chartArea);
                return gradient;
              },
              pointRadius: 0,
              tension: 0.35
          ...
          options: {
            ...
            plugins: {
              ...
              tooltip: {
                ...
                callbacks: {
                  label: (ctx) => `${format(ctx.parsed.y, currency)}`
                }
              }
            },
            scales: {
              x: {
                ...
                ticks: { color: 'rgba(229,231,235,0.6)', maxTicksLimit: 8 }
              },
              y: {
                grid: { color: 'rgba(255,255,255,0.06)', drawBorder: false },
                ticks: {
                  color: 'rgba(229,231,235,0.6)',
                  callback: (v) => currency === 'PLS' ? `${(v / USD_PER_PLS).toFixed(4)}` : `${v}`
                }
              }
            }
          }
        ...
        rangeButtons.forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            rangeButtons.forEach(b => b.classList.remove('bg-emerald-500/10','ring-1','ring-emerald-500/20','text-white'));
            btn.classList.add('bg-emerald-500/10','ring-1','ring-emerald-500/20','text-white');

            const points = ranges[currentRange];
            const series = generateSeries(points, 0.00028, 0.00002, currentRange === '24h' ? 0.10 : 0.18);
            chart.data.labels = makeLabels(points);
            chart.data.datasets[0].data = series;
            gradient = undefined; // rebuild on next draw
            chart.update();
            updateStats(series);
          });
        });
        ...
        currencyButtons.forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            currencyButtons.forEach(b => b.classList.remove('bg-white/10','ring-1','ring-white/10','text-white'));
            btn.classList.add('bg-white/10','ring-1','ring-white/10','text-white');
            // Trigger tooltip/tick updates via a redraw
            chart.update();
            // Refresh price text from current data
            updateStats(chart.data.datasets[0].data);
          });
        });

        // Handle resize to refresh gradient
        new ResizeObserver(() => { gradient = undefined; chart.update(); }).observe(ctx.canvas);
      }
    })();
...
<button className="px-2.5 py-1.5 text-[11px] rounded-full text-zinc-300 hover:text-white" data-range="30d">30d</button>
<button className="px-2.5 py-1.5 text-[11px] rounded-full text-zinc-300 hover:text-white" data-range="3m">3m</button>
<button className="px-2.5 py-1.5 text-[11px] rounded-full text-zinc-300 hover:text-white" data-range="6m">6m</button>
<button className="px-2.5 py-1.5 text-[11px] rounded-full text-white bg-emerald-500/10 ring-1 ring-emerald-500/20" data-range="1y">1y</button>
<button className="px-2.5 py-1.5 text-[11px] rounded-full text-zinc-300 hover:text-white" data-range="atl">ATL</button>
</div>
<div className="inline-flex items-center gap-1 rounded-full bg-white/5 ring-1 ring-white/10 p-1 ml-1">
<button className="px-2.5 py-1.5 text-[11px] rounded-full text-white bg-white/10 ring-1 ring-white/10" data-currency="$">$</button>
<button className="px-2.5 py-1.5 text-[11px] rounded-full text-zinc-300 hover:text-white" data-currency="PLS">PLS</button>
</div>
</div>
</div>
<div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4">
<div className="md:col-span-3 rounded-xl ring-1 ring-white/10 bg-neutral-950/40 p-3 sm:p-4 relative">
<div className="absolute inset-0 pointer-events-none" style={{background: 'radial-gradient(600px 140px at 60% 0%, rgba(16,185,129,0.10), transparent 70%)'}}></div>
<div className="flex items-center justify-between px-1">
<div className="text-sm">
<span className="text-zinc-400">Price</span>
<span className="ml-2 text-zinc-100" id="arkPrice">$0.000503</span>
</div>
<div className="text-sm text-emerald-400" id="arkChange">+80.83%</div>
</div>

<div className="mt-2 relative h-64 sm:h-80">
<canvas className="absolute inset-0" height="320" id="arkChart" style={{display: 'block', boxSizing: 'border-box', height: '320px', width: '834px'}} width="834"></canvas>
</div>
<div className="mt-3 flex items-center justify-end gap-2">
<a className="inline-flex items-center gap-2 rounded-lg bg-white/5 hover:bg-white/10 ring-1 ring-white/10 px-3 py-2 text-xs text-zinc-100" href="https://pulsex.com" target="_blank">
                Buy
                <svg fill="none" height="14" stroke="currentColor" strokeWidth="1.5" viewbox="0 0 24 24" width="14" xmlns="http://www.w3.org/2000/svg"><path d="m6 6 12 12"></path><path d="M18 6v12H6"></path></svg>
</a>
<a className="inline-flex items-center gap-2 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 ring-1 ring-emerald-500/20 px-3 py-2 text-xs text-emerald-300" href="https://pulsex.com" target="_blank">
                Trade
...
<div className="text-lg tracking-tight text-zinc-100 mt-1" id="arkMcap">$372516.00</div>
<div className="mt-2 h-1.5 rounded bg-white/5 overflow-hidden">
<div className="h-full bg-emerald-500/80" id="arkMcapBar" style={{width: '10%'}}></div>
</div>
<div className="text-[11px] text-zinc-500 mt-1">Est. from price × circ.</div>
</div>
<div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
<div className="text-xs text-zinc-400">24h Volume</div>
<div className="text-lg tracking-tight text-emerald-300 mt-1" id="arkVol">$26076.12</div>
<div className="mt-2 grid grid-cols-3 gap-2">
<div className="rounded-lg bg-white/5 p-2 text-center">
<div className="text-[11px] text-zinc-400">Trades</div>
<div className="text-sm text-zinc-100" id="arkTrades">15,330</div>
</div>
<div className="rounded-lg bg-white/5 p-2 text-center">
<div className="text-[11px] text-zinc-400">High</div>
<div className="text-sm text-zinc-100" id="arkHigh">$0.000574</div>
</div>
<div className="rounded-lg bg-white/5 p-2 text-center">
<div className="text-[11px] text-zinc-400">Low</div>
<div className="text-sm text-zinc-100" id="arkLow">$0.000108</div>
</div>
</div>
</div>
<div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
<div className="flex items-center gap-2">
<span className="inline-flex h-6 w-6 items-center justify-center rounded-md bg-emerald-500/15 ring-1 ring-emerald-500/20">
<svg fill="none" height="14" stroke="currentColor" strokeWidth="1.5" viewbox="0 0 24 24" width="14" xmlns="http://www.w3.org/2000/svg"><path d="m3 17 6-6 4 4 8-8"></path></svg>
</span>
<div className="text-sm">Status</div>
</div>
<div className="mt-2 text-xs text-zinc-400">Data is simulated for demo. Connect a price feed to go live.</div>
</div>
</div>
</div>
</div>
...
<div className="rounded-lg bg-white/5 ring-1 ring-white/10 p-4 transition-all duration-700" data-animate="">
<div className="flex items-center gap-2 mb-2">
...
<span className="text-sm">Carry what matters. Burn the rest.</span>
</div>
<p className="text-sm text-neutral-300">Jettison 3% as starlit ash—supply tightens, ascent accelerates.</p>
</div>
<div className="rounded-lg bg-white/5 ring-1 ring-white/10 p-4 transition-all duration-700" data-animate="">
<div className="flex items-center gap-2 mb-2">
...
<span className="text-sm">Share the voyage. Strengthen the waters.</span>
</div>
<p className="text-sm text-neutral-300">2% reflections to holders; 3% to liquidity calms the seas.</p>
</div>
<div className="rounded-lg bg-white/5 ring-1 ring-white/10 p-4 transition-all duration-700" data-animate="">
<div className="flex items-center gap-2 mb-2">
<svg fill="none" height="16" stroke="currentColor" strokeWidth="1.5" viewbox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg"><path d="M3 3v18h18"></path><path d="m19 9-5 5-4-4-3 3"></path></svg>
<span className="text-sm">Guard the vault. Earn the future.</span>
</div>
<p className="text-sm text-neutral-300">Stewards who lock receive 3% in ongoing rewards.</p>
</div>
</div>
</div>
</div>

...
'100%': { transform: 'translateY(100%)' },
      ...
      window.totalSteps=2;window.currentStep=1;window.updateModalUI=function(){const t=document.querySelectorAll(".form-step"),e=document.getElementById("audit-progress"),n=document.getElementById("btn-back"),o=document.getElementById("btn-next"),r=document.getElementById("step-indicator"),l=document.getElementById("audit-footer");"results"===window.currentStep?(t.forEach(t=>t.classList.add("hidden")),document.querySelector('[data-step="results"]').classList.remove("hidden"),l&&(l.style.display="none"),e&&(e.style.width="100%")):(t.forEach(t=>{t.dataset.step==window.currentStep?t.classList.remove("hidden"):t.classList.add("hidden")}),l&&(l.style.display="flex"),e&&(e.style.width=`${window.currentStep/window.totalSteps*100}%`),n&&(1===window.currentStep?n.classList.add("hidden"):n.classList.remove("hidden")),o&&(window.currentStep===window.totalSteps?o.innerHTML='Get My Business Snapshot <iconify-icon icon="lucide:check-circle" width="14"></iconify-icon>':o.innerHTML='Next: Operations <iconify-icon icon="lucide:arrow-right" width="14"></iconify-icon>'),r&&(r.textContent=`Step ${window.currentStep} of ${window.totalSteps}`))},window.nextStep=function(){const t=document.querySelector(`.form-step[data-step="${window.currentStep}"]`),e=t.querySelectorAll("input[required], textarea[required]");let n=!0;if(e.forEach(t=>{if("radio"===t.type){if(!document.querySelector(`input[name="${t.name}"]:checked`)){n=!1;const e=t.closest("div").querySelector("label");e.classList.add("text-red-500"),t.addEventListener("change",()=>e.classList.remove("text-red-500"),{once:!0})}}else t.value.trim()||(n=!1,t.classList.add("border-red-500","ring-1","ring-red-500"),t.addEventListener("input",()=>t.classList.remove("border-red-500","ring-1","ring-red-500"),{once:!0}))}),!n)return;window.currentStep<window.totalSteps?(window.currentStep++,window.updateModalUI()):(window.currentStep="results",window.calculateSnapshot(),window.updateModalUI())},window.prevStep=function(){window.currentStep>1&&(window.currentStep--,window.updateModalUI())},window.calculateSnapshot=function(){const t=[{k:"q_website",l:"Website"},{k:"q_gbp",l:"Google Business Profile"},{k:"q_volume",l:"Lead Flow",v:1},{k:"q_review_sys",l:"Reviews System"},{k:"q_review_vol",l:"Review Volume (20+)"},{k:"q_crm",l:"Lead Capture System"},{k:"q_followup",l:"Automated Follow-Up"}],e={present:[],weak:[],missing:[]};t.forEach(t=>{const n=document.querySelector(`input[name="${t.k}"]:checked`)?.value;if(t.v){if("15+"===n||"5-15"===n)e.present.push(t.l);else if("0-5"===n)e.weak.push(t.l);else e.missing.push(t.l)}else "yes"===n?e.present.push(t.l):"sometimes"===n?e.weak.push(t.l):e.missing.push(t.l)});const n=document.getElementById("scorecard-container");let o="";e.present.length>0&&(o+=`<div class="bg-emerald-50/50 dark:bg-emerald-900/10 rounded-xl p-4 border border-emerald-100 dark:border-emerald-500/20"><h5 class="text-xs font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400 mb-3 flex items-center gap-2"><iconify-icon icon="lucide:check-circle" width="14"></iconify-icon> What's Working</h5><ul class="space-y-2">${e.present.map(t=>`<li class="text-sm text-slate-700 dark:text-slate-300 flex items-start gap-2"><span class="text-emerald-500">✓</span> ${t}</li>`).join("")}</ul></div>`),e.weak.length>0&&(o+=`<div class="bg-amber-50/50 dark:bg-amber-900/10 rounded-xl p-4 border border-amber-100 dark:border-amber-500/20"><h5 class="text-xs font-bold uppercase tracking-widest text-amber-600 dark:text-amber-400 mb-3 flex items-center gap-2"><iconify-icon icon="lucide:alert-triangle" width="14"></iconify-icon> What's Weak</h5><ul class="space-y-2">${e.weak.map(t=>`<li class="text-sm text-slate-700 dark:text-slate-300 flex items-start gap-2"><span class="text-amber-500">!</span> ${t}</li>`).join("")}</ul></div>`),e.missing.length>0&&(o+=`<div class="bg-red-50/50 dark:bg-red-900/10 rounded-xl p-4 border border-red-100 dark:border-red-500/20"><h5 class="text-xs font-bold uppercase tracking-widest text-red-600 dark:text-red-400 mb-3 flex items-center gap-2"><iconify-icon icon="lucide:x-circle" width="14"></iconify-icon> What's Missing</h5><ul class="space-y-2">${e.missing.map(t=>`<li class="text-sm text-slate-700 dark:text-slate-300 flex items-start gap-2"><span class="text-red-500">×</span> ${t}</li>`).join("")}</ul></div>`),n.innerHTML=o||'<div class="text-center text-slate-500">No data collected.</div>'};
...
<p className="text-slate-600 dark:text-slate-400">
            Systems that convert demand into booked revenue — without hiring
            staff.
          </p>
</div>
...
<p className="text-xs font-medium text-[#8f7459] dark:text-[#a88b70] h-8 leading-snug">
                  For businesses getting conversion systems in place fast.
                </p>
</div>
<div className="mb-8">
...
<span>High-converting landing page (template-based)</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-[#a88b70] shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Conversational AI + calendar booking</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-[#a88b70] shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Missed-call &amp; enquiry SMS follow-up</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-[#a88b70] shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
...
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-[#a88b70] shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Referral &amp; repeat-customer automation</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-[#a88b70] shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>CRM + Sora app access</span>
</li>
</ul>
...
<p className="text-xs font-medium text-slate-600 dark:text-slate-400 h-8 leading-snug">
                  For businesses ready for predictable monthly bookings.
                </p>
</div>
<div className="mb-8">
<div className="flex items-baseline gap-1">
...
<span>Advanced automation (SMS + Email)</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-slate-500 dark:text-slate-400 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>AI qualification &amp; smart lead routing</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-slate-500 dark:text-slate-400 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Optimised booking &amp; sales pipeline</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-slate-500 dark:text-slate-400 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Review &amp; social proof flywheel</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-slate-500 dark:text-slate-400 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Conversion tracking &amp; reporting</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-slate-500 dark:text-slate-400 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
...
<span>AI receptionist &amp; virtual setter</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-yellow-600 dark:text-yellow-500 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Multi-location or multi-brand systems</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-yellow-600 dark:text-yellow-500 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Advanced AI behaviours &amp; logic</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-yellow-600 dark:text-yellow-500 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Authority &amp; content automation engine</span>
</li>
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-yellow-600 dark:text-yellow-500 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
...
<li className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
<iconify-icon className="text-yellow-600 dark:text-yellow-500 shrink-0 mt-0.5" icon="lucide:check" width="16"></iconify-icon>
<span>Priority deployment &amp; support</span>
</li>
</ul>
...
<p className="text-sm text-slate-500 dark:text-slate-400">
                  Let's snapshot your core digital assets.
                </p>
</div>
...
<input className="peer sr-only" name="q_website" required="" type="radio" value="yes"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10">
                        Yes
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_website" type="radio" value="no"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        No
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_website" type="radio" value="unsure"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        Unsure
                      </div>
</label>
...
<div className="grid grid-cols-3 gap-2">
<label className="cursor-pointer">
<input className="peer sr-only" name="q_gbp" required="" type="radio" value="yes"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10">
                        Yes
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_gbp" type="radio" value="no"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        No
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_gbp" type="radio" value="unsure"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        Unsure
                      </div>
</label>
...
<div>
<label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">
                    Where do new enquiries currently come from?
...
<input className="peer sr-only" name="q_source" type="checkbox" value="google"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-[11px] font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10 flex items-center justify-center h-full">
                        Google Search / Maps
                      </div>
</label>
<label className="cursor-pointer relative">
<input className="peer sr-only" name="q_source" type="checkbox" value="website"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-[11px] font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10 flex items-center justify-center h-full">
                        Website contact form
                      </div>
</label>
<label className="cursor-pointer relative">
<input className="peer sr-only" name="q_source" type="checkbox" value="calls"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-[11px] font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10 flex items-center justify-center h-full">
                        Phone calls from Google
                      </div>
</label>
<label className="cursor-pointer relative">
<input className="peer sr-only" name="q_source" type="checkbox" value="social"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-[11px] font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10 flex items-center justify-center h-full">
                        Social media messages
                      </div>
</label>
<label className="cursor-pointer relative">
<input className="peer sr-only" name="q_source" type="checkbox" value="referrals"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-[11px] font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10 flex items-center justify-center h-full">
                        Referrals only
                      </div>
</label>
<label className="cursor-pointer relative">
<input className="peer sr-only" name="q_source" type="checkbox" value="unsure"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-[11px] font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10 flex items-center justify-center h-full">
                        We’re not sure
                      </div>
</label>
</div>
...
<div>
<label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">
                    Do you have 20+ 5-star reviews?
                  </label>
<div className="grid grid-cols-3 gap-2">
<label className="cursor-pointer">
<input className="peer sr-only" name="q_review_vol" required="" type="radio" value="yes"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10">
                        Yes
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_review_vol" type="radio" value="no"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        No
                      </div>
</label>
...
<div>
<label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">
                    Do you have a system to request reviews?
                  </label>
<div className="grid grid-cols-3 gap-2">
<label className="cursor-pointer">
<input className="peer sr-only" name="q_review_sys" required="" type="radio" value="yes"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10">
                        Yes
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_review_sys" type="radio" value="sometimes"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-amber-500 peer-checked:text-white peer-checked:border-amber-500 hover:bg-slate-100 dark:hover:bg-white/10">
                        Sometimes
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_review_sys" type="radio" value="no"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        No
                      </div>
</label>
...
<div>
<label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">
                    Do all leads go to a central CRM/Inbox?
                  </label>
<div className="grid grid-cols-3 gap-2">
<label className="cursor-pointer">
<input className="peer sr-only" name="q_crm" required="" type="radio" value="yes"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10">
                        Yes
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_crm" type="radio" value="no"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        No
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_crm" type="radio" value="unsure"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        Unsure
                      </div>
</label>
...
<div>
<label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">
                    Do you have automated lead follow-up?
                  </label>
<div className="grid grid-cols-3 gap-2">
<label className="cursor-pointer">
<input className="peer sr-only" name="q_followup" required="" type="radio" value="yes"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-slate-100 dark:hover:bg-white/10">
                        Yes
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_followup" type="radio" value="no"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        No
                      </div>
</label>
<label className="cursor-pointer">
<input className="peer sr-only" name="q_followup" type="radio" value="unsure"/>
<div className="text-center py-2.5 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all peer-checked:bg-slate-700 peer-checked:text-white peer-checked:border-slate-700 hover:bg-slate-100 dark:hover:bg-white/10">
                        Unsure
                      </div>
</label>
...
<input className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-md text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-400" name="business_name" required="" type="text"/>
</div>
<div className="space-y-1">
<label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
                      Your Name
                    </label>
<input className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-md text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-400" name="contact_name" required="" type="text"/>
</div>
<div className="col-span-2 space-y-1">
<label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
                      Email Address
                    </label>
<input className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-md text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-400" name="contact_email" required="" type="email"/>
</div>
<div className="space-y-1">
<label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
                      Website URL
                    </label>
<input className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-md text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-400" name="website_url" placeholder="If applicable" type="url"/>
</div>
<div className="space-y-1">
<label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
                      Social Media
                    </label>
<input className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-md text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-400" name="social_url" placeholder="e.g. Instagram" type="text"/>
</div>
<div className="col-span-2 space-y-1">
<label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
                      What is your biggest bottleneck?
                    </label>
<textarea className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-md text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-400" name="bottleneck" placeholder="e.g. Missing calls, quoting takes too long..." rows="2"></textarea>
</div>
</div>
</div>
</div>
<div className="form-step hidden transition-all duration-300" data-step="results">
<div className="text-center mb-6">
<h4 className="text-2xl font-bold text-slate-900 dark:text-white">
                  Business Snapshot
                </h4>
<p className="text-sm text-slate-500">
                  Your digital capability scorecard.
                </p>
</div>
<div className="space-y-6 mb-8 overflow-y-auto max-h-[50vh] pr-1 custom-scrollbar" id="scorecard-container"></div>
<div className="flex flex-col gap-3">
<button className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded-lg shadow-lg shadow-blue-500/20 active:scale-95 transition-all" onclick="closeAuditModal()" type="button">
                  Fix Missing Items
                </button>
...
<div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] flex justify-between shrink-0 transition-all" id="audit-footer">

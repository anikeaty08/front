...
'50%': { opacity: 0.3 },
        ...
        // --- 1. INTERACTIVE NEURAL GRID CANVAS ---
        ...
        // Enhanced Mouse Tracker with Higher Sensitivity
        document.addEventListener('mousemove', (e) => {
            // Normalize -1 to 1, but multiple by 2 for stronger rotation effect
            targetMouseX = ((e.clientX - cx) / cx) * 2; 
            targetMouseY = ((e.clientY - cy) / cy) * 2;
        });
        ...
        const rows = 20; // Depth
        const cols = 20; // Width
        const spacing = 180; // Space between points
        ...
        for(let r = 0; r < rows; r++) {
            for(let c = -cols/2; c < cols/2; c++) {
                points.push({
                    ...
                    y: 300, // Floor level
                    z: r * spacing,
                    ox: c * spacing, // Original positions
                    oy: 300,
                    oz: r * spacing
                });
                // Ceiling mirror
        ...
        function draw() {
            ...
            // Interpolate mouse for smooth rotation
            mouseX += (targetMouseX - mouseX) * 0.1;
            mouseY += (targetMouseY - mouseY) * 0.1;

            // Move forward
            zOffset -= speed;
            if(zOffset < -spacing) zOffset = 0;

            // Rotation angles based on mouse
            const rotateY = mouseX * 0.2; // Yaw
            const rotateX = mouseY * 0.1; // Pitch

            points.forEach(p => {
                // 1. Position with infinite loop logic
                let originalZ = p.z + zOffset;
                // Wrap around
                while (originalZ < -perspective) originalZ += rows * spacing;
                while (originalZ > (rows * spacing) - perspective) originalZ -= rows * spacing;

                let px = p.x;
                let py = p.y;
                let pz = originalZ;

                // 2. Apply Rotation (Order: X then Y)
                // Rotate around X (Pitch)
                let y1 = py * Math.cos(rotateX) - pz * Math.sin(rotateX);
                let z1 = py * Math.sin(rotateX) + pz * Math.cos(rotateX);
                
                // Rotate around Y (Yaw)
                let x2 = px * Math.cos(rotateY) - z1 * Math.sin(rotateY);
                let z2 = px * Math.sin(rotateY) + z1 * Math.cos(rotateY);
                ...
                // 3. Project to 2D
                if(pz > -perspective + 10 && pz < 3000) {
                    const scale = perspective / (perspective + pz);
                    const screenX = cx + px * scale;
                    const screenY = cy + py * scale;

                    // Visibility (Fog)
                    const alpha = Math.max(0, 1 - (pz / 2000));
                    
                    // Draw Node
                    const size = Math.max(1, 3.5 * scale); // Larger size
                    
                    if(Math.random() > 0.995) {
                        ctx.fillStyle = '#FF3B00'; // Active node
                        ctx.shadowBlur = 15;
                        ctx.shadowColor = '#FF3B00';
                    } else {
                        // Much brighter standard nodes
                    ...
                    // Draw Line to "virtual next" to simulate grid
                    // We calculate where the neighbor (in depth) would be
                    let nextPz = pz + spacing; 
                    // Apply simpler projection to next point for performance
                    // (Skipping full rotation recalc for neighbor to save frames, usually acceptable approximation for straight lines)
                    // Better visual: Calculate rotated neighbor
                    
                    // Neighbor in Z
                    let npy = p.y; let npz = originalZ + spacing; let npx = p.x;
                    
                    // Rotate Neighbor
                    let ny1 = npy * Math.cos(rotateX) - npz * Math.sin(rotateX);
                    let nz1 = npy * Math.sin(rotateX) + npz * Math.cos(rotateX);
                    let nx2 = npx * Math.cos(rotateY) - nz1 * Math.sin(rotateY);
                    let nz2 = npx * Math.sin(rotateY) + nz1 * Math.cos(rotateY);
                    
                    const nextScale = perspective / (perspective + nz2);
                    const nextScreenX = cx + nx2 * nextScale;
                    const nextScreenY = cy + ny1 * nextScale;

                    if(nz2 > -perspective + 10) {
                        ctx.beginPath();
                        ctx.strokeStyle = `rgba(255, 255, 255, ${alpha * 0.15})`; // More visible lines
...
<canvas className="fixed top-0 left-0 w-full h-full -z-10" id="neuralCanvas"></canvas>
...
<span className="font-display font-bold text-xl tracking-tighter text-white group-hover:text-accent transition-colors">NEXUS<span className="text-accent group-hover:text-white transition-colors">//</span>OS</span>
</a>
<div className="hidden md:flex gap-8 text-xs font-mono tracking-widest text-gray-400">
<a className="hover:text-white transition-colors hover-trigger" href="#features">[01] MODULES</a>
<a className="hover:text-white transition-colors hover-trigger" href="#developers">[02] DEVELOPERS</a>
<a className="hover:text-white transition-colors hover-trigger" href="#pipeline">[03] PIPELINE</a>
</div>
...
<h1 className="font-display font-bold text-6xl md:text-9xl tracking-tighter mb-6 leading-[0.9] text-white scramble-text">
                    SYNTHETIC <br/>
<span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-gray-400 to-gray-600">COGNITION</span>
</h1>
<p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed mb-10 font-light tracking-wide">
...
<button className="bg-accent text-black px-8 py-4 font-bold text-sm uppercase tracking-widest hover:bg-white transition-all hover-trigger w-full sm:w-auto btn-magnetic shadow-[0_0_20px_rgba(255,59,0,0.3)]">
                        Start Instance
                    </button>
...
<div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1620712943543-bcc4688e7485?q=80&amp;w=2560&amp;auto=format&amp;fit=crop')] bg-cover bg-center opacity-20 mix-blend-screen group-hover:scale-110 transition-transform duration-1000"></div>
...
<div className="w-10 h-10 bg-accent flex items-center justify-center mb-4 text-black font-bold shadow-[0_0_15px_rgba(255,59,0,0.5)]">
...
<div className="h-full bg-gradient-to-r from-accent to-yellow-500 w-[82%] rounded-full shadow-[0_0_10px_rgba(255,59,0,0.5)]"></div>
</div>
</div>
</div>

...
<div className="absolute inset-0 bg-emerald-500 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.8)]"></div>
</div>
<h1 className="text-xs md:text-sm font-medium tracking-tight uppercase text-white/90">Aether<span className="opacity-40">.OS</span></h1>
</div>
<div className="flex flex-col xs:flex-row xs:items-center gap-1 xs:gap-4 text-[10px] font-mono text-sci-dim tracking-wider">
<span className="text-white/70 transition-colors duration-500" id="sim-mode">NEBULA CLOUD</span>
<span className="hidden xs:inline w-px h-2 bg-white/10"></span>
<span className="opacity-60" id="particle-count">120K NODES</span>
</div>
</div>

<div className="flex gap-2 pointer-events-auto">
<button aria-label="Microphone" className="sci-panel w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-md hover:bg-white/5 hover:border-white/20 transition-all group" id="btn-audio">
<i className="w-3.5 h-3.5 text-white/60 group-hover:text-white transition-colors" data-lucide="mic"></i>
</button>
<button aria-label="Camera" className="sci-panel w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-md hover:bg-white/5 hover:border-white/20 transition-all group relative overflow-hidden" id="btn-cam">
<i className="w-3.5 h-3.5 text-white/60 group-hover:text-white transition-colors z-10 relative" data-lucide="video"></i>
<div className="absolute inset-0 bg-emerald-500/10 opacity-0 transition-opacity" id="cam-active-indicator"></div>
</button>
<button aria-label="Capture" className="sci-panel w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-md hover:bg-white/5 hover:border-white/20 transition-all group" id="btn-rec">
<i className="w-3.5 h-3.5 text-white/60 group-hover:text-white transition-colors" data-lucide="aperture"></i>
</button>
</div>
</header>
...
<h1 className="text-5xl md:text-7xl font-semibold tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white via-white/90 to-white/40 pb-2 leading-[1.1]">
                Design your vision with<br className="hidden md:block"/> creative excellence
            </h1>
<p className="mt-6 text-lg md:text-xl text-white/60 max-w-2xl font-medium leading-relaxed tracking-tight">
                Crafting digital experiences that merge art and technology. From <br className="hidden md:block"/>
                branding to web development, I build it all.
            </p>
<div className="mt-10 flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto pointer-events-auto">

<button className="group relative px-6 py-3.5 rounded-full bg-black/40 backdrop-blur-sm border border-white/10 hover:bg-white/5 transition-all duration-300 flex items-center gap-2.5 overflow-hidden shadow-lg shadow-black/20">
<span className="text-sm font-medium text-white/90">View Portfolio</span>
<i className="w-4 h-4 text-white/70 group-hover:text-white transition-transform group-hover:translate-x-1" data-lucide="arrow-right"></i>

<div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full group-hover:animate-[shimmer_1.5s_infinite]"></div>
</button>

<button className="px-8 py-3.5 rounded-full border border-orange-500/40 text-white/80 hover:text-white hover:bg-orange-500/5 hover:border-orange-500/70 transition-all duration-300 text-sm font-medium shadow-[0_0_15px_-5px_rgba(249,115,22,0.15)] hover:shadow-[0_0_20px_-5px_rgba(249,115,22,0.3)]">
                    Contact Me
                </button>
</div>
</div>
...
<video className="fixed top-0 left-0 w-1 h-1 opacity-0 pointer-events-none z-[-1]" id="input-video" playsinline=""></video>
...
<div className="sci-panel p-3 rounded-xl pointer-events-auto w-full md:w-64 max-w-[300px] backdrop-blur-xl border border-white/5">
<div className="flex justify-between items-center mb-3 px-1">
<div className="flex items-center gap-2">
<i className="w-3 h-3 text-white/40" data-lucide="cpu"></i>
<span className="text-[10px] font-medium uppercase tracking-widest text-white/50">Simulation</span>
</div>

<div className="opacity-0 transition-opacity duration-300 flex items-center gap-1.5 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20" id="hand-status">
<div className="w-1 h-1 rounded-full bg-emerald-400 animate-pulse"></div>
<span className="text-[9px] font-mono text-emerald-400 tracking-tight">TRACKING</span>
</div>
</div>
<div className="grid grid-cols-2 gap-1.5">
<button className="mode-btn active group relative flex items-center justify-between px-2.5 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition-all border border-transparent hover:border-white/10 text-left" onclick="window.setMode(0)">
<span className="text-[10px] text-white/80 group-hover:text-white font-medium">Nebula</span>
<span className="text-[9px] font-mono text-white/30">01</span>
<div className="absolute inset-0 border border-white/10 rounded-lg opacity-0 group-[.active]:opacity-100 pointer-events-none"></div>
</button>
<button className="mode-btn group relative flex items-center justify-between px-2.5 py-2 rounded-lg bg-transparent hover:bg-white/5 transition-all border border-transparent hover:border-white/10 text-left" onclick="window.setMode(1)">
<span className="text-[10px] text-white/60 group-hover:text-white transition-colors">Torus</span>
<span className="text-[9px] font-mono text-white/20">02</span>
</button>
<button className="mode-btn group relative flex items-center justify-between px-2.5 py-2 rounded-lg bg-transparent hover:bg-white/5 transition-all border border-transparent hover:border-white/10 text-left" onclick="window.setMode(2)">
<span className="text-[10px] text-white/60 group-hover:text-white transition-colors">Lattice</span>
<span className="text-[9px] font-mono text-white/20">03</span>
</button>
<button className="mode-btn group relative flex items-center justify-between px-2.5 py-2 rounded-lg bg-transparent hover:bg-white/5 transition-all border border-transparent hover:border-white/10 text-left" onclick="window.setMode(3)">
<span className="text-[10px] text-white/60 group-hover:text-white transition-colors">Vortex</span>
<span className="text-[9px] font-mono text-white/20">04</span>
</button>
</div>
</div>
...
<div className="sci-panel p-1.5 rounded-full flex md:flex-col flex-row gap-2">
<button className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 ring-1 ring-white/10 hover:scale-110 transition-transform duration-300" onclick="window.cycleColor(0)"></button>
<button className="w-6 h-6 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-600 ring-1 ring-white/10 hover:scale-110 transition-transform duration-300" onclick="window.cycleColor(1)"></button>
<button className="w-6 h-6 rounded-full bg-gradient-to-br from-orange-400 to-rose-600 ring-1 ring-white/10 hover:scale-110 transition-transform duration-300" onclick="window.cycleColor(2)"></button>
</div>
<div className="hidden md:block text-[9px] font-mono text-right text-white/20 leading-tight">
<p>INTERACTION ENGINE V2.0</p>
<p className="mt-0.5">SAYANTAN BARIK</p>
</div>
</div>
</div>

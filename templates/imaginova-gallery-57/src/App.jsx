import React, { useEffect } from 'react';

export default function App() {
  useEffect(() => {
    try {
      
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}



      lucide.createIcons();

      // Column slider interaction
      const slider = document.getElementById('colSlider');
      const gallery = document.getElementById('gallery');
      const colCount = document.getElementById('colCount');

      slider.addEventListener('input', (e) => {
          const val = e.target.value;
          gallery.style.setProperty('--cols', val);
          colCount.textContent = val;

          // Update responsive grid
          if (window.innerWidth < 640) {
              gallery.style.setProperty('--cols', Math.min(val, 2));
          } else if (window.innerWidth < 768) {
              gallery.style.setProperty('--cols', Math.min(val, 3));
          }
      });

      // Responsive grid handling
      window.addEventListener('resize', () => {
          const val = slider.value;
          if (window.innerWidth < 640) {
              gallery.style.setProperty('--cols', Math.min(val, 2));
          } else if (window.innerWidth < 768) {
              gallery.style.setProperty('--cols', Math.min(val, 3));
          } else {
              gallery.style.setProperty('--cols', val);
          }
      });

      // Tabs interaction
      document.querySelectorAll('[data-tab]').forEach(btn => {
          btn.addEventListener('click', () => {
              document.querySelectorAll('[data-tab]').forEach(b => {
                  b.classList.remove('tab-active', 'bg-gradient-to-r', 'from-cyan-500', 'to-sky-500');
                  b.classList.add('hover:bg-white/5');
              });
              btn.classList.add('tab-active', 'bg-gradient-to-r', 'from-cyan-500', 'to-sky-500');
              btn.classList.remove('hover:bg-white/5');
          });
      });

      // Initialize responsive grid
      window.dispatchEvent(new Event('resize'));
    
    } catch (error) {
      console.error("Error executing template scripts:", error);
    }
  }, []);

  return (
    <>
      

<section className="max-w-7xl sm:px-6 sm:mt-16 mt-10 px-4">
</section>

<div className="absolute inset-0 -z-10" style={{visibility: 'hidden'}}>
<img alt="" className="w-full h-full object-cover opacity-50" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/d73444d7-6d9e-4d7a-bd0f-2e1a6f620522_3840w.webp" style={{}}/>
<div className="absolute inset-0 bg-gradient-to-b from-[#0b0b16]/70 via-[#0b0b16]/80 to-[#0b0b16]"></div>
</div>

<header className="animate-fade-in opacity-0 w-full">
</header>

<div className="sm:px-6 lg:px-8 [animation:fadeSlideIn_0.8s_ease-out_0.2s_both] max-w-7xl mr-auto ml-auto pr-4 pl-4">
<div className="lg:col-span-5">
</div>
</div>

<section className="sm:px-6 max-w-7xl mt-16 mr-auto mb-16 ml-auto pt-10 pr-4 pl-4" id="contact">
<header className="relative w-full">
<nav className="mx-auto max-w-7xl px-6 py-5 flex items-center justify-between">
<a className="flex items-center gap-3 group" href="#">
<div className="h-8 w-8 rounded-md bg-white/10 ring-1 ring-white/15 flex items-center justify-center text-sm tracking-tight font-semibold group-hover:bg-white/15 transition-colors" style={{letterSpacing: '-0.02em'}}>
        ΛN
      </div>
<span className="text-base font-medium tracking-tight group-hover:opacity-90 transition-opacity">Nova</span>
</a>
<div className="hidden md:flex items-center gap-8">
<a className="text-sm font-medium text-neutral-300 hover:text-white transition-colors" href="#">Features</a>
<a className="text-sm font-medium text-neutral-300 hover:text-white transition-colors" href="#">Pricing</a>
<a className="text-sm font-medium text-neutral-300 hover:text-white transition-colors" href="#">Docs</a>
<a className="text-sm font-medium text-neutral-300 hover:text-white transition-colors" href="#">Contact</a>
</div>
<div className="hidden md:flex items-center gap-3">
<a className="px-3.5 py-2 rounded-lg bg-white/5 hover:bg-white/10 ring-1 ring-white/10 text-sm font-medium transition-colors" href="#">Sign
        in</a>
<a className="px-3.5 py-2 rounded-lg bg-emerald-500/90 hover:bg-emerald-500 text-neutral-900 text-sm font-semibold transition-colors" href="#">Get
        started</a>
</div>
<button aria-label="Open menu" className="md:hidden p-2 rounded-lg bg-white/5 hover:bg-white/10 ring-1 ring-white/10 transition-colors" id="mobileOpen">
<svg className="lucide lucide-menu h-5 w-5" data-lucide="menu" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" style={{strokeWidth: '1.5'}} viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M4 5h16"></path><path d="M4 12h16"></path><path d="M4 19h16"></path></svg>
</button>
</nav>

<div className="hidden fixed inset-0 z-50 bg-neutral-950/80 backdrop-blur-md" id="mobileMenu">
<div className="absolute right-4 top-4">
<button aria-label="Close menu" className="p-2 rounded-lg bg-white/5 hover:bg-white/10 ring-1 ring-white/10 transition-colors" id="mobileClose">
<svg className="lucide lucide-x h-5 w-5" data-lucide="x" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" style={{strokeWidth: '1.5'}} viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg>
</button>
</div>
<div className="mx-auto mt-20 w-11/12 max-w-sm rounded-2xl bg-neutral-900/70 ring-1 ring-white/10 p-6">
<div className="flex flex-col gap-3">
<a className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-white/5 transition-colors" href="#">
<span className="text-sm font-medium">Features</span>
<svg className="lucide lucide-chevron-right h-4 w-4 text-neutral-400" data-lucide="chevron-right" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" style={{strokeWidth: '1.5'}} viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="m9 18 6-6-6-6"></path></svg>
</a>
<a className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-white/5 transition-colors" href="#">
<span className="text-sm font-medium">Pricing</span>
<svg className="lucide lucide-chevron-right h-4 w-4 text-neutral-400" data-lucide="chevron-right" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" style={{strokeWidth: '1.5'}} viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="m9 18 6-6-6-6"></path></svg>
</a>
<a className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-white/5 transition-colors" href="#">
<span className="text-sm font-medium">Docs</span>
<svg className="lucide lucide-chevron-right h-4 w-4 text-neutral-400" data-lucide="chevron-right" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" style={{strokeWidth: '1.5'}} viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="m9 18 6-6-6-6"></path></svg>
</a>
<a className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-white/5 transition-colors" href="#">
<span className="text-sm font-medium">Contact</span>
<svg className="lucide lucide-chevron-right h-4 w-4 text-neutral-400" data-lucide="chevron-right" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" style={{strokeWidth: '1.5'}} viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="m9 18 6-6-6-6"></path></svg>
</a>
</div>
<div className="mt-5 flex items-center gap-2">
<a className="flex-1 px-3.5 py-2 rounded-lg bg-white/5 hover:bg-white/10 ring-1 ring-white/10 text-sm font-medium text-center transition-colors" href="#">Sign
          in</a>
<a className="flex-1 px-3.5 py-2 rounded-lg bg-emerald-500/90 hover:bg-emerald-500 text-neutral-900 text-sm font-semibold text-center transition-colors" href="#">Get
          started</a>
</div>
</div>
</div>
</header>
</section><section className="sm:px-6 max-w-7xl mt-16 mr-auto mb-16 ml-auto pt-10 pr-4 pl-4" id="contact">
<section className="sm:px-6 sm:mt-16 max-w-7xl mt-10 pr-4 pl-4">
<div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
<div className="lg:col-span-7">
<h1 className="leading-none text-white tracking-tight">
<span className="block text-[12vw] sm:text-[10vw] md:text-[8vw] lg:text-[7vw] font-semibold">
<span className="tracking-tighter" data-letter="" style={{display: 'inline-block', transform: 'translateY(0px)', opacity: '1'}}>Maya</span>
<span className="block"></span>
<span className="tracking-tighter" data-letter="" style={{display: 'inline-block', transform: 'translateY(0px)', opacity: '1'}}>Chen</span>
</span>
</h1>
<p className="sm:mt-5 sm:text-3xl leading-relaxed max-w-2xl text-base text-white/70 tracking-tight mt-4">
            AI Engineer &amp; Frontend — shipping agentic systems, RAG pipelines, and developer UX. I blend product intuition with systems engineering to build fast, reliable LLM apps.
          </p>
<div className="mt-6 flex flex-col sm:flex-row gap-3">
<a className="inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-medium tracking-tight text-neutral-900 bg-white hover:bg-white/90 border border-white/10" href="#work">
<svg className="lucide lucide-arrow-right w-4 h-4" data-lucide="arrow-right" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
<span>View Work</span>
</a>
<a className="inline-flex items-center justify-center gap-2 hover:bg-white/15 text-sm font-medium text-white tracking-tight bg-white/10 border-white/10 border rounded-full pt-3 pr-5 pb-3 pl-5 shadow-sm backdrop-blur" href="mailto:hello@mayachen.dev">
<svg className="lucide lucide-mail w-4 h-4" data-lucide="mail" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="m22 7-8.991 5.727a2 2 0 0 1-2.009 0L2 7"></path><rect height="16" rx="2" width="20" x="2" y="4"></rect></svg>
<span className="">hello@mayachen.dev</span>
</a>
</div>
<div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
<div className="flex items-start gap-3 border-t border-white/10 pt-4">
<svg className="lucide lucide-map-pin w-[18px] h-[18px] text-white/50 mt-0.5" data-lucide="map-pin" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"></path><circle cx="12" cy="10" r="3"></circle></svg>
<div className="">
<p className="text-sm font-medium tracking-tight">Based in San Francisco</p>
<p className="text-xs text-white/60 mt-0.5">Open to remote work</p>
</div>
</div>
<div className="flex items-start gap-3 border-t border-white/10 pt-4">
<svg className="lucide lucide-cpu w-[18px] h-[18px] text-white/50 mt-0.5" data-lucide="cpu" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M12 20v2"></path><path d="M12 2v2"></path><path d="M17 20v2"></path><path d="M17 2v2"></path><path d="M2 12h2"></path><path d="M2 17h2"></path><path d="M2 7h2"></path><path d="M20 12h2"></path><path d="M20 17h2"></path><path d="M20 7h2"></path><path d="M7 20v2"></path><path d="M7 2v2"></path><rect height="16" rx="2" width="16" x="4" y="4"></rect><rect height="8" rx="1" width="8" x="8" y="8"></rect></svg>
<div>
<p className="text-sm font-medium tracking-tight">AI Systems + Frontend</p>
<p className="text-xs text-white/60 mt-0.5">RAG, agents, benchmarks</p>
</div>
</div>
<div className="flex items-start gap-3 border-t border-white/10 pt-4">
<svg className="lucide lucide-check w-[18px] h-[18px] text-white/50 mt-0.5" data-lucide="check" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M20 6 9 17l-5-5"></path></svg>
<div className="">
<p className="text-sm font-medium tracking-tight">Currently available</p>
<p className="text-xs text-white/60 mt-0.5">Starting mid‑September</p>
</div>
</div>
</div>
</div>
<div className="lg:col-span-5">
<div className="relative aspect-[4/5] sm:aspect-[5/6] overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.35)] bg-white/5 rounded-3xl border border-white/10">
<img alt="Maya at work" className="absolute inset-0 w-full h-full object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/9b22c33a-b017-42bd-bab5-89be63576edd_800w.jpg" style={{filter: 'grayscale(100%) saturate(0.7) contrast(1.1)'}}/>
<div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>
<div className="absolute bottom-4 left-4 right-4 grid grid-cols-3 gap-3">
<div className="rounded-xl bg-white/10 backdrop-blur-md border border-white/15 p-3 shadow-lg">
<div className="flex items-center gap-2 mb-1">
<div className="w-2 h-2 rounded-full bg-white/50"></div>
<div className="text-lg font-semibold tracking-tight text-white">82%</div>
</div>
<p className="text-[11px] text-white/70">pass@1 eval</p>
</div>
<div className="rounded-xl bg-white/10 backdrop-blur-md border border-white/15 p-3 shadow-lg">
<div className="flex items-center gap-2 mb-1">
<div className="w-2 h-2 rounded-full bg-white/50"></div>
<div className="text-lg font-semibold tracking-tight text-white">780ms</div>
</div>
<p className="text-[11px] text-white/70">p95 latency</p>
</div>
<div className="rounded-xl bg-white/10 backdrop-blur-md border border-white/15 p-3 shadow-lg">
<div className="flex items-center gap-2 mb-1">
<div className="w-2 h-2 rounded-full bg-white/50"></div>
<div className="text-lg font-semibold tracking-tight text-white">1.2k</div>
</div>
<p className="text-[11px] text-white/70">tests</p>
</div>
</div>
</div>
</div>
</div>
</section>
</section><section className="sm:px-6 max-w-7xl mt-16 mr-auto mb-16 ml-auto pt-10 pr-4 pl-4" id="contact">
<div className="overflow-hidden sm:p-8 text-white bg-cyan-950 border-white/10 border rounded-3xl pt-6 pr-6 pb-6 pl-6 relative">
<div className="absolute inset-0 pointer-events-none">
<div className="absolute inset-0 bg-[radial-gradient(1200px_600px_at_20%_-20%,rgba(255,255,255,0.07),transparent_60%)]"></div>
<div className="absolute inset-0 bg-[radial-gradient(1200px_600px_at_80%_120%,rgba(255,255,255,0.06),transparent_60%)]"></div>
<div className="absolute inset-0 bg-[radial-gradient(#ffffff0d_1px,transparent_1px)] [background-size:20px_20px] opacity-[0.15]"></div>
</div>
<div className="relative">
<section className="z-10 pt-16 pb-16 relative">
<div className="sm:px-6 lg:px-8 max-w-6xl mr-auto ml-auto pr-4 pl-4">
<div className="text-center space-y-12">
<div className="space-y-4">
<h2 className="sm:text-4xl text-3xl tracking-tight font-sans font-semibold text-white">20+ лтет опыта в сфере дизайна</h2>
<p className="text-lg max-w-2xl mx-auto font-sans text-white/70" style={{}}>I've partnered with startups and Fortune 500s to design and build digital products that scale</p>
<div className="mt-8 pt-8 pr-6 pb-6 pl-6">
<div className="text-center">
<p className="uppercase text-sm font-medium tracking-wide font-sans text-zinc-400">Работа с такими компаниями</p>
</div>
<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6 mt-8">
<svg className="" fill="none" height="36" viewbox="0 0 99 36" width="99" xmlns="http://www.w3.org/2000/svg">
<path className="" d="M15.4102 15.04C14.2236 13.4933 11.7836 13.7066 10.8902 15.44L4.82358 27.3333H12.8236L17.3436 17.5733L15.4102 15.04Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M8.81023 19.5067L11.3036 21.28L16.2769 19.8667H16.2902" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M31.4902 27.3333L25.6502 10.5067C24.8502 8.18668 21.7702 8.02668 20.7436 10.24L12.8236 27.3333H31.4902Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M17.9969 16.1733L21.8503 14.8266L24.7969 17.3866L27.4769 15.7866" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M14.4369 23.84L18.6103 22.2L20.6503 23.9067L23.2236 22.8933L27.2636 27.3333" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M95.8754 9.07596V9.58229C95.3184 9.68355 94.753 9.89452 94.1792 10.2152C93.6053 10.5359 93.0146 11.0591 92.407 11.7848L87.6222 17.6582L88.7615 15.8354L94.8121 24.9494C95.0821 25.3375 95.3606 25.6498 95.6475 25.8861C95.9345 26.1224 96.3058 26.3249 96.7615 26.4937V27C96.2383 26.9662 95.6391 26.9494 94.964 26.9494C94.2889 26.9325 93.6729 26.924 93.1159 26.924C92.7952 26.924 92.3902 26.9325 91.9007 26.9494C91.4281 26.9494 90.829 26.9662 90.1032 27V26.4937C90.7783 26.4599 91.1834 26.3671 91.3184 26.2152C91.4534 26.0464 91.3944 25.7595 91.1412 25.3544L87.445 19.5316C87.175 19.0928 86.9302 18.7637 86.7108 18.5443C86.5083 18.3249 86.2973 18.1814 86.0779 18.1139C85.8585 18.0295 85.5885 17.9789 85.2678 17.962V17.4557C85.9767 17.4219 86.6096 17.2363 87.1665 16.8987C87.7235 16.5612 88.2805 16.0633 88.8374 15.4051L90.6349 13.1519C91.2594 12.4093 91.6391 11.7764 91.7741 11.2532C91.9091 10.73 91.8332 10.3249 91.5463 10.038C91.2594 9.75106 90.7783 9.59916 90.1032 9.58229V9.07596C90.4745 9.09284 90.8459 9.10971 91.2172 9.12659C91.5885 9.12659 91.9598 9.13503 92.3311 9.15191C92.7193 9.15191 93.1243 9.15191 93.5463 9.15191C94.0357 9.15191 94.4745 9.14347 94.8627 9.12659C95.2509 9.10971 95.5885 9.09284 95.8754 9.07596ZM87.3184 9.07596V9.58229C86.7784 9.59916 86.3648 9.66667 86.0779 9.78482C85.8079 9.88608 85.6222 10.0886 85.521 10.3924C85.4197 10.6793 85.3691 11.135 85.3691 11.7595V24.3164C85.3691 24.924 85.4197 25.3797 85.521 25.6835C85.6222 25.9873 85.8079 26.1899 86.0779 26.2911C86.3648 26.3924 86.7784 26.4599 87.3184 26.4937V27C86.8965 26.9662 86.3564 26.9494 85.6982 26.9494C85.04 26.9325 84.3733 26.924 83.6982 26.924C82.9387 26.924 82.2298 26.9325 81.5716 26.9494C80.9134 26.9494 80.3733 26.9662 79.9514 27V26.4937C80.5083 26.4599 80.9218 26.3924 81.1919 26.2911C81.4788 26.1899 81.6644 25.9873 81.7488 25.6835C81.8501 25.3797 81.9007 24.924 81.9007 24.3164V11.7595C81.9007 11.135 81.8501 10.6793 81.7488 10.3924C81.6644 10.0886 81.4788 9.88608 81.1919 9.78482C80.9218 9.66667 80.5083 9.59916 79.9514 9.58229V9.07596C80.3733 9.09284 80.9134 9.10971 81.5716 9.12659C82.2298 9.14347 82.9387 9.15191 83.6982 9.15191C84.3733 9.15191 85.04 9.14347 85.6982 9.12659C86.3564 9.10971 86.8965 9.09284 87.3184 9.07596Z" fill="currentColor"></path>
<path className="" d="M73.1478 9L79.2997 24.8734C79.5528 25.481 79.8229 25.9029 80.1098 26.1392C80.4136 26.3586 80.6921 26.4768 80.9452 26.4936V27C80.5064 26.9662 79.9748 26.9493 79.3503 26.9493C78.7427 26.9325 78.1182 26.924 77.4769 26.924C76.7343 26.924 76.0423 26.9325 75.4009 26.9493C74.7596 26.9493 74.2364 26.9662 73.8313 27V26.4936C74.6921 26.4599 75.2406 26.3164 75.4769 26.0633C75.7132 25.7932 75.6625 25.2278 75.325 24.3671L70.8693 12.3418L71.325 11.7848L67.2744 22.2911C66.9537 23.135 66.7596 23.827 66.6921 24.3671C66.6246 24.9072 66.6668 25.3291 66.8187 25.6329C66.9706 25.9367 67.2237 26.1561 67.5782 26.2911C67.9326 26.4093 68.3714 26.4768 68.8946 26.4936V27C68.3545 26.9662 67.8313 26.9493 67.325 26.9493C66.8187 26.9325 66.3124 26.924 65.806 26.924C65.4347 26.924 65.0803 26.9325 64.7427 26.9493C64.4221 26.9493 64.1351 26.9662 63.882 27V26.4936C64.2533 26.4093 64.6161 26.1899 64.9706 25.8354C65.3419 25.481 65.6963 24.865 66.0339 23.9873L71.8313 9C72.0339 9.01688 72.2533 9.02532 72.4896 9.02532C72.7258 9.02532 72.9453 9.01688 73.1478 9ZM75.3503 19.7088V20.2152H67.806L68.0592 19.7088H75.3503Z" fill="currentColor"></path>
<path className="" d="M63.806 9.07596C63.7385 9.76794 63.6879 10.443 63.6541 11.1013C63.6372 11.7426 63.6288 12.2321 63.6288 12.5696C63.6288 12.8903 63.6372 13.2025 63.6541 13.5063C63.671 13.7933 63.6879 14.038 63.7047 14.2405H63.1225C62.9874 13.076 62.768 12.1646 62.4642 11.5063C62.1604 10.8312 61.7469 10.3587 61.2237 10.0886C60.7174 9.80169 60.0507 9.65823 59.2237 9.65823H57.8313C57.325 9.65823 56.9368 9.70043 56.6668 9.78482C56.3967 9.86921 56.2111 10.0464 56.1098 10.3165C56.0254 10.5865 55.9832 11 55.9832 11.557V24.519C55.9832 25.0591 56.0254 25.4726 56.1098 25.7595C56.2111 26.0295 56.3967 26.2067 56.6668 26.2911C56.9368 26.3755 57.325 26.4177 57.8313 26.4177H59.2744C60.1351 26.4177 60.8524 26.2574 61.4263 25.9367C62.017 25.616 62.4896 25.0928 62.844 24.3671C63.1984 23.6245 63.46 22.6118 63.6288 21.3291H64.2111C64.1604 21.8523 64.1351 22.5274 64.1351 23.3544C64.1351 23.7089 64.1436 24.2236 64.1604 24.8987C64.1942 25.5738 64.2448 26.2743 64.3123 27C63.4516 26.9662 62.4811 26.9494 61.401 26.9494C60.3208 26.9325 59.3588 26.924 58.5149 26.924C58.1267 26.924 57.6119 26.924 56.9706 26.924C56.3461 26.924 55.6626 26.9325 54.9199 26.9494C54.1942 26.9494 53.4516 26.9578 52.6921 26.9747C51.9495 26.9747 51.2406 26.9831 50.5655 27V26.4937C51.1225 26.4599 51.536 26.3924 51.806 26.2911C52.0929 26.1899 52.2786 25.9873 52.363 25.6835C52.4643 25.3797 52.5149 24.924 52.5149 24.3164V11.7595C52.5149 11.135 52.4643 10.6793 52.363 10.3924C52.2786 10.0886 52.0929 9.88608 51.806 9.78482C51.536 9.66667 51.1225 9.59916 50.5655 9.58229V9.07596C51.2406 9.09284 51.9495 9.10971 52.6921 9.12659C53.4516 9.12659 54.1942 9.13503 54.9199 9.15191C55.6626 9.15191 56.3461 9.15191 56.9706 9.15191C57.6119 9.15191 58.1267 9.15191 58.5149 9.15191C59.2913 9.15191 60.1689 9.15191 61.1478 9.15191C62.1436 9.13503 63.0296 9.10971 63.806 9.07596ZM59.7301 17.7342C59.7301 17.7342 59.7301 17.8186 59.7301 17.9873C59.7301 18.1561 59.7301 18.2405 59.7301 18.2405H55.2237C55.2237 18.2405 55.2237 18.1561 55.2237 17.9873C55.2237 17.8186 55.2237 17.7342 55.2237 17.7342H59.7301ZM60.4642 14.3924C60.3967 15.3544 60.363 16.0886 60.363 16.5949C60.3799 17.1013 60.3883 17.5654 60.3883 17.9873C60.3883 18.4093 60.3967 18.8734 60.4136 19.3797C60.4305 19.8861 60.4727 20.6202 60.5402 21.5823H59.9579C59.8735 21.0084 59.7385 20.4684 59.5529 19.962C59.3841 19.4388 59.1056 19.0253 58.7174 18.7215C58.3461 18.4008 57.806 18.2405 57.0972 18.2405V17.7342C57.6204 17.7342 58.0423 17.6245 58.363 17.4051C58.7005 17.1857 58.9706 16.9072 59.1731 16.5696C59.3756 16.2152 59.5275 15.8523 59.6288 15.481C59.7469 15.0928 59.8313 14.73 59.882 14.3924H60.4642Z" fill="currentColor"></path>
<path className="" d="M36.1569 9.07596C36.5788 9.09284 37.1189 9.10971 37.7772 9.12659C38.4354 9.14347 39.0936 9.15191 39.7518 9.15191C40.5788 9.15191 41.3637 9.14347 42.1063 9.12659C42.8489 9.10971 43.3721 9.10127 43.6759 9.10127C45.9375 9.10127 47.6168 9.56541 48.7139 10.4937C49.8278 11.4219 50.3847 12.6203 50.3847 14.0886C50.3847 14.6962 50.275 15.3376 50.0556 16.0127C49.8362 16.6709 49.4565 17.2869 48.9164 17.8608C48.3763 18.4177 47.6337 18.8734 46.6885 19.2278C45.7603 19.5823 44.5704 19.7595 43.1189 19.7595H40.5873V19.2532H42.8658C43.929 19.2532 44.7392 19.0253 45.2961 18.5696C45.87 18.1139 46.2497 17.5148 46.4354 16.7722C46.6379 16.0295 46.7392 15.2278 46.7392 14.3671C46.7392 12.7637 46.4776 11.5654 45.9544 10.7722C45.4312 9.97891 44.5282 9.58229 43.2455 9.58229C42.5535 9.58229 42.0978 9.72575 41.8784 10.0127C41.6759 10.2996 41.5746 10.8819 41.5746 11.7595V24.2152C41.5746 24.8565 41.6421 25.3375 41.7772 25.6582C41.9122 25.9789 42.1738 26.1899 42.562 26.2911C42.9501 26.3924 43.5071 26.4515 44.2328 26.4683V27C43.7434 26.9662 43.1105 26.9494 42.3341 26.9494C41.5577 26.9325 40.7561 26.924 39.9291 26.924C39.2202 26.924 38.5367 26.9325 37.8784 26.9494C37.2202 26.9494 36.6464 26.9662 36.1569 27V26.4937C36.7139 26.4599 37.1274 26.3924 37.3974 26.2911C37.6843 26.1899 37.87 25.9873 37.9544 25.6835C38.0556 25.3797 38.1063 24.924 38.1063 24.3164V11.7595C38.1063 11.135 38.0556 10.6793 37.9544 10.3924C37.87 10.0886 37.6843 9.88608 37.3974 9.78482C37.1274 9.66667 36.7139 9.59916 36.1569 9.58229V9.07596Z" fill="currentColor"></path>
</svg><svg className="" fill="none" height="36" viewbox="0 0 115 36" width="115" xmlns="http://www.w3.org/2000/svg">
<path className="" d="M15.0752 24.6667H14.1152V31.3333H15.4485V24.9333L15.0752 24.6667Z" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5"></path>
<path className="" d="M18.5952 9.42666C17.9819 10.2933 17.8352 11.1467 17.8085 11.5067C17.6485 11.5733 15.3952 12.4933 15.0085 14.68C14.7685 16.0533 15.3952 17.12 15.6085 17.4533C15.3819 17.6133 13.2752 19.2267 13.4619 21.6533C13.5552 22.9067 14.2219 23.7733 14.4619 24.0533C14.6619 24.28 14.8752 24.4933 15.1019 24.6667H4.11521L9.40854 18H6.78188L12.1152 11.3333H9.44854L14.7819 4.66666L18.5952 9.42666Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M23.4485 26H22.1152V31.3333H23.4485V26Z" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5"></path>
<path className="" d="M32.0619 20.6267C31.7552 18.8 30.2752 17.8 29.9819 17.6C30.9019 16.32 31.0085 14.7067 30.2619 13.4533C29.4752 12.1333 28.0485 11.7467 27.7419 11.6667C27.6885 11.3333 27.2085 8.81334 24.9419 7.77334C24.7819 7.70668 23.9952 7.34668 22.9019 7.33334C22.5685 7.33334 20.5952 7.33334 19.1152 8.80001C18.0619 9.85334 17.8485 11.0667 17.7952 11.52C17.6352 11.5867 15.3819 12.5067 14.9952 14.6933C14.7552 16.0667 15.3819 17.1333 15.5952 17.4533C15.3685 17.6267 13.2619 19.2267 13.4485 21.6533C13.5419 22.9067 14.2085 23.7733 14.4485 24.0533C16.0485 25.96 18.7285 26.04 21.0219 25.9867C22.5819 25.9467 24.1552 25.9067 25.7152 25.8533C26.4619 25.8267 27.2219 25.84 27.9685 25.7333C30.4352 25.4133 32.4885 23.1733 32.0619 20.6267Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M22.7819 12.6667V26" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M26.7819 19.3333L22.7819 24.6667" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M18.7819 15.3333L22.7819 20.6667" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M108.675 26.8208L104.578 19.7027H102.35V26.8208H99.4314V9.0256H105.577C106.942 9.0256 108.094 9.26458 109.033 9.74253C109.989 10.2205 110.697 10.8606 111.158 11.6629C111.636 12.4652 111.875 13.3613 111.875 14.3514C111.875 15.5121 111.534 16.5704 110.851 17.5263C110.185 18.4652 109.153 19.1053 107.753 19.4467L112.157 26.8208H108.675ZM102.35 17.3727H105.577C106.669 17.3727 107.488 17.0996 108.035 16.5533C108.598 16.0071 108.88 15.2731 108.88 14.3514C108.88 13.4296 108.606 12.7127 108.06 12.2006C107.514 11.6714 106.686 11.4068 105.577 11.4068H102.35V17.3727Z" fill="currentColor"></path>
<path className="" d="M85.9269 9.0256V20.3684C85.9269 21.7169 86.2768 22.7326 86.9766 23.4154C87.6936 24.0982 88.6836 24.4396 89.9468 24.4396C91.227 24.4396 92.217 24.0982 92.9169 23.4154C93.6338 22.7326 93.9923 21.7169 93.9923 20.3684V9.0256H96.9112V20.3172C96.9112 21.7681 96.5954 22.9972 95.9638 24.0043C95.3323 25.0114 94.4873 25.7625 93.429 26.2575C92.3707 26.7525 91.2014 27 89.9212 27C88.6409 27 87.4717 26.7525 86.4133 26.2575C85.3721 25.7625 84.5442 25.0114 83.9297 24.0043C83.3152 22.9972 83.0079 21.7681 83.0079 20.3172V9.0256H85.9269Z" fill="currentColor"></path>
<path className="" d="M81.3576 9.0256V11.4068H76.6208V26.8208H73.7019V11.4068H68.9394V9.0256H81.3576Z" fill="currentColor"></path>
<path className="" d="M64.0166 23.1849H56.5657L55.2854 26.8208H52.2385L58.614 9H61.9938L68.3694 26.8208H65.2968L64.0166 23.1849ZM63.1972 20.8037L60.3039 12.5334L57.385 20.8037H63.1972Z" fill="currentColor"></path>
<path className="" d="M50.7354 26.8208H47.8165L39.0341 13.532V26.8208H36.1152V9H39.0341L47.8165 22.2632V9H50.7354V26.8208Z" fill="currentColor"></path>
</svg>
<svg className="" fill="none" height="36" viewbox="0 0 110 36" width="110" xmlns="http://www.w3.org/2000/svg">
<path className="" d="M18.3918 31.3333C25.7556 31.3333 31.7251 25.3638 31.7251 18C31.7251 10.6362 25.7556 4.66666 18.3918 4.66666C11.028 4.66666 5.05847 10.6362 5.05847 18C5.05847 25.3638 11.028 31.3333 18.3918 31.3333Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M21.4716 12.6667H15.3116L12.2316 18L15.3116 23.3333H21.4716L24.5516 18L21.4716 12.6667Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M15.3118 23.3333L7.67175 10.0933" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M12.2316 18.0001L15.3116 12.6667L19.885 4.74673" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M24.5516 17.9867L16.8983 31.2533" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M29.1252 25.9067L21.4719 12.6667" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M21.4719 23.3333H6.17859" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M30.605 12.6667H15.3117" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M101.294 23.4526V17.3212V12.1314H103.089L102.914 16.073H103.286C103.418 15.2117 103.622 14.4672 103.899 13.8394C104.177 13.2117 104.542 12.7226 104.994 12.3723C105.461 12.0219 106.038 11.8467 106.724 11.8467C106.87 11.8467 107.038 11.8613 107.228 11.8905C107.418 11.9051 107.637 11.9562 107.885 12.0438L107.775 14.1241C107.556 14.0365 107.337 13.9781 107.118 13.9489C106.899 13.9051 106.688 13.8832 106.483 13.8832C105.914 13.8832 105.418 14.0657 104.994 14.4307C104.586 14.7956 104.242 15.292 103.965 15.9197C103.688 16.5328 103.461 17.2263 103.286 18V23.4526H101.294Z" fill="currentColor"></path>
<path className="" d="M93.1713 23.7372C91.9158 23.7372 90.9669 23.2993 90.3246 22.4234C89.6823 21.5475 89.3611 20.2409 89.3611 18.5037V12.1314H91.3757V18.3285C91.3757 19.5547 91.5655 20.4672 91.945 21.0657C92.3246 21.6642 92.9231 21.9635 93.7406 21.9635C94.1932 21.9635 94.5947 21.854 94.945 21.635C95.31 21.4161 95.6312 21.1022 95.9085 20.6934C96.1859 20.2847 96.4122 19.8029 96.5874 19.2482C96.7626 18.6788 96.8793 18.0657 96.9377 17.4088V12.1314H98.9304V18.7226V23.4526H97.1567L97.3319 19.8175H97.0034C96.8136 20.7226 96.5436 21.4672 96.1932 22.0511C95.8574 22.6204 95.4341 23.0438 94.9231 23.3212C94.4122 23.5986 93.8282 23.7372 93.1713 23.7372Z" fill="currentColor"></path>
<path className="" d="M86.0306 23.6715C84.9795 23.6715 84.1985 23.3868 83.6875 22.8175C83.1766 22.2482 82.9211 21.3795 82.9211 20.2117V13.7956H81.2131L81.235 12.1314H82.308C82.6729 12.1314 82.9357 12.0657 83.0963 11.9343C83.2715 11.8029 83.3737 11.5839 83.4029 11.2773L83.6218 9.56931H84.8481V12.1314H88.1109V13.8175H84.8481V20.1679C84.8481 20.7518 84.9795 21.1752 85.2423 21.4379C85.5197 21.6861 85.9138 21.8102 86.4248 21.8102C86.7021 21.8102 86.9868 21.7737 87.2788 21.7007C87.5853 21.6277 87.8846 21.4817 88.1766 21.2628V23.2555C87.7532 23.416 87.3591 23.5255 86.9941 23.5839C86.6437 23.6423 86.3226 23.6715 86.0306 23.6715Z" fill="currentColor"></path>
<path className="" d="M74.2335 23.4526V17.3212V12.1314H76.0291L75.8539 16.073H76.2262C76.3575 15.2117 76.5619 14.4672 76.8393 13.8394C77.1167 13.2117 77.4816 12.7226 77.9342 12.3723C78.4013 12.0219 78.978 11.8467 79.6641 11.8467C79.8101 11.8467 79.978 11.8613 80.1678 11.8905C80.3576 11.9051 80.5765 11.9562 80.8247 12.0438L80.7152 14.1241C80.4962 14.0365 80.2773 13.9781 80.0583 13.9489C79.8393 13.9051 79.6276 13.8832 79.4232 13.8832C78.8539 13.8832 78.3576 14.0657 77.9342 14.4307C77.5254 14.7956 77.1824 15.292 76.905 15.9197C76.6276 16.5328 76.4013 17.2263 76.2262 18V23.4526H74.2335Z" fill="currentColor"></path>
<path className="" d="M67.8294 23.7372C66.9389 23.7372 66.1505 23.6059 65.4644 23.3431C64.7929 23.0657 64.2235 22.6715 63.7564 22.1606C63.2892 21.6496 62.9389 21.0438 62.7053 20.3431C62.4717 19.6423 62.3549 18.854 62.3549 17.9781C62.3549 17.0876 62.4717 16.2701 62.7053 15.5255C62.9389 14.781 63.2819 14.1314 63.7345 13.5766C64.187 13.0219 64.7418 12.5985 65.3987 12.3066C66.0557 12 66.8075 11.8467 67.6542 11.8467C68.3987 11.8467 69.0776 11.9708 69.6907 12.219C70.3038 12.4671 70.8221 12.854 71.2454 13.3796C71.6834 13.8905 72.0119 14.5401 72.2308 15.3285C72.4498 16.1022 72.5374 17.0146 72.4936 18.0657L63.6031 18.1314V16.7737L71.2892 16.708L70.6104 17.6277C70.6834 16.708 70.5958 15.9489 70.3476 15.3504C70.0995 14.7518 69.7418 14.2993 69.2746 13.9927C68.8221 13.6861 68.2819 13.5328 67.6542 13.5328C66.9827 13.5328 66.3914 13.708 65.8805 14.0584C65.3695 14.4088 64.9754 14.9124 64.698 15.5693C64.4352 16.2263 64.3038 17.0219 64.3038 17.9562C64.3038 19.2993 64.6031 20.3285 65.2016 21.0438C65.8002 21.7445 66.6907 22.0949 67.8732 22.0949C68.3257 22.0949 68.7199 22.0438 69.0557 21.9416C69.406 21.8248 69.698 21.6715 69.9316 21.4818C70.1797 21.2774 70.3768 21.0438 70.5228 20.781C70.6834 20.5183 70.8075 20.2336 70.8951 19.927L72.6469 20.3431C72.5155 20.8686 72.3111 21.3431 72.0338 21.7664C71.771 22.1752 71.4352 22.5256 71.0265 22.8175C70.6323 23.1095 70.1724 23.3358 69.6469 23.4964C69.1214 23.6569 68.5155 23.7372 67.8294 23.7372Z" fill="currentColor"></path>
<path className="" d="M50.7189 27V17.8029V12.1314H52.5146L52.4051 15.0438H52.7335C52.9087 14.4015 53.1496 13.8467 53.4562 13.3796C53.7773 12.8978 54.1861 12.5255 54.6824 12.2628C55.1788 11.9854 55.7773 11.8467 56.4781 11.8467C57.5146 11.8467 58.3759 12.1095 59.062 12.635C59.7627 13.146 60.2883 13.854 60.6387 14.7591C60.989 15.6496 61.1642 16.6642 61.1642 17.8029C61.1642 18.9416 60.989 19.9562 60.6387 20.8467C60.2883 21.7372 59.77 22.4453 59.0839 22.9708C58.4124 23.4818 57.5876 23.7372 56.6095 23.7372C55.8211 23.7372 55.1642 23.5767 54.6386 23.2555C54.1277 22.9197 53.7189 22.4818 53.4124 21.9416C53.1058 21.4015 52.8649 20.8394 52.6897 20.2555H52.3175C52.4051 20.6496 52.4781 21.0584 52.5365 21.4818C52.5948 21.9051 52.6386 22.3285 52.6678 22.7518C52.7116 23.1606 52.7335 23.5548 52.7335 23.9343V27H50.7189ZM56.1058 22.0073C56.7481 22.0073 57.2883 21.8248 57.7262 21.4599C58.1642 21.0803 58.5 20.5694 58.7335 19.927C58.9671 19.2847 59.0839 18.562 59.0839 17.7591C59.0839 16.8978 58.9598 16.1606 58.7116 15.5474C58.4781 14.9343 58.1423 14.4599 57.7043 14.1241C57.2664 13.7737 56.7335 13.5985 56.1058 13.5985C55.5657 13.5985 55.0839 13.7226 54.6605 13.9708C54.2518 14.2044 53.9014 14.5255 53.6094 14.9343C53.3175 15.3285 53.0912 15.7664 52.9306 16.2482C52.7846 16.7153 52.7116 17.1825 52.7116 17.6496V17.9124C52.7116 18.2044 52.7554 18.5329 52.843 18.8978C52.9306 19.2482 53.062 19.6058 53.2372 19.9708C53.4124 20.3358 53.6313 20.6715 53.8941 20.9781C54.1715 21.2847 54.4927 21.5329 54.8576 21.7226C55.2226 21.9124 55.6386 22.0073 56.1058 22.0073Z" fill="currentColor"></path>
<path className="" d="M36.3918 23.4526L41.4721 9H44.4064L49.4867 23.4526H47.3188L43.0706 10.6861H42.8079L38.5597 23.4526H36.3918ZM38.8005 20.2117V18.6788H47.3188V20.2117H38.8005Z" fill="currentColor"></path>
</svg><svg className="" fill="none" height="36" viewbox="0 0 92 36" width="92" xmlns="http://www.w3.org/2000/svg">
<path className="" d="M18.8834 4.66666C17.4168 4.66666 16.2168 5.86666 16.2168 7.33332V11.56L18.8834 14.2267L21.5501 11.56V7.33332C21.5501 6.59999 21.2568 5.93332 20.7634 5.43999C20.2834 4.95999 19.6168 4.66666 18.8834 4.66666Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" stroke-miterlimit="10" strokeWidth="1.5"></path>
<path className="" d="M16.2168 24.44V28.6666C16.2168 30.1333 17.4168 31.3333 18.8834 31.3333C20.3501 31.3333 21.5501 30.1333 21.5501 28.6666V24.44L18.8834 21.7733L16.2168 24.44Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" stroke-miterlimit="10" strokeWidth="1.5"></path>
<path className="" d="M12.4434 15.3333H8.21675C6.75008 15.3333 5.55008 16.5333 5.55008 18C5.55008 19.4667 6.75008 20.6667 8.21675 20.6667H12.4434L15.1101 18L12.4434 15.3333Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" stroke-miterlimit="10" strokeWidth="1.5"></path>
<path className="" d="M29.5502 15.3333H25.3235L22.6569 18L25.3235 20.6667H29.5502C30.2835 20.6667 30.9502 20.3733 31.4302 19.88C31.9235 19.4 32.2169 18.7333 32.2169 18C32.2169 16.5333 31.0169 15.3333 29.5502 15.3333Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" stroke-miterlimit="10" strokeWidth="1.5"></path>
<path className="" d="M28.3101 23.6533L22.6568 18L18.8834 21.7733L24.5368 27.4267C25.5768 28.4667 27.2701 28.4667 28.3101 27.4267C29.3501 26.3867 29.3501 24.6933 28.3101 23.6533Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" stroke-miterlimit="10" strokeWidth="1.5"></path>
<path className="" d="M13.2301 8.57329C12.7101 8.05329 12.0301 7.78662 11.3367 7.78662C10.6567 7.78662 9.97673 8.05329 9.45673 8.57329C8.41673 9.61329 8.41673 11.3066 9.45673 12.3466L15.1101 18L18.8834 14.2266L13.2301 8.57329Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" stroke-miterlimit="10" strokeWidth="1.5"></path>
<path className="" d="M9.45673 27.4266C8.41673 26.3866 8.41673 24.6933 9.45673 23.6533L24.5367 8.5733C25.5767 7.5333 27.2701 7.5333 28.3101 8.5733C29.3501 9.6133 29.3501 11.3066 28.3101 12.3466L13.2301 27.4266C12.1901 28.4666 10.4967 28.4666 9.45673 27.4266Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" stroke-miterlimit="10" strokeWidth="1.5"></path>
<path className="" d="M84.6756 27C84.3509 27 83.9855 26.9675 83.5796 26.9025C83.1899 26.8376 82.8164 26.7564 82.4591 26.659C82.1181 26.5615 81.8502 26.456 81.6554 26.3423C81.5417 26.2774 81.4524 26.2124 81.3874 26.1475C81.3387 26.0663 81.3144 25.9364 81.3144 25.7578L81.1926 21.9337C81.1926 21.6089 81.2981 21.4465 81.5092 21.4465C81.6878 21.4465 81.8096 21.5927 81.8746 21.885L82.0938 22.8105C82.6459 25.1326 83.677 26.2936 85.1871 26.2936C85.9991 26.2936 86.6405 26.0257 87.1114 25.4898C87.5985 24.9377 87.8421 24.1664 87.8421 23.1759C87.8421 22.2178 87.5904 21.3247 87.087 20.4966C86.5999 19.6685 85.8123 18.8484 84.7244 18.0365C83.5227 17.1759 82.6459 16.3478 82.0938 15.5521C81.5417 14.7402 81.2656 13.8796 81.2656 12.9702C81.2656 11.7686 81.6797 10.8106 82.5079 10.0961C83.3522 9.36536 84.424 9 85.723 9C86.2102 9 86.6811 9.05683 87.1357 9.1705C87.5904 9.28417 87.9639 9.44655 88.2562 9.65764C88.3536 9.7226 88.4267 9.79567 88.4754 9.87686C88.5403 9.95805 88.5728 10.0636 88.5728 10.1935L88.6459 13.8714C88.6459 14.1475 88.5403 14.2855 88.3292 14.2855C88.1831 14.2855 88.0775 14.1718 88.0126 13.9445L87.8421 13.3356C87.4848 12.0852 87.1114 11.1759 86.7217 10.6076C86.3482 10.023 85.8042 9.73072 85.0897 9.73072C84.3915 9.73072 83.8313 9.95805 83.4091 10.4127C83.0031 10.8674 82.8001 11.5088 82.8001 12.3369C82.8001 13.0352 83.0275 13.7009 83.4821 14.3342C83.9368 14.9675 84.7325 15.7388 85.8692 16.6482C87.087 17.6387 87.972 18.5967 88.5241 19.5223C89.0924 20.4316 89.3766 21.4059 89.3766 22.4452C89.3766 23.3545 89.1736 24.1502 88.7677 24.8322C88.3617 25.5142 87.8015 26.05 87.087 26.4398C86.3888 26.8132 85.585 27 84.6756 27Z" fill="currentColor"></path>
<path className="" d="M75.2893 27C74.4124 27 73.633 26.797 72.951 26.3911C72.2852 25.9689 71.7656 25.3924 71.3921 24.6617C71.0186 23.9147 70.8319 23.0622 70.8319 22.1042V10.8512C70.8319 10.5102 70.7751 10.2828 70.6614 10.1692C70.564 10.0555 70.3448 9.96619 70.0038 9.90124L69.4435 9.80381C69.2324 9.75509 69.1269 9.65766 69.1269 9.51152C69.1269 9.3329 69.2487 9.24359 69.4923 9.24359H73.9496C74.1932 9.24359 74.315 9.34102 74.315 9.53588C74.315 9.68202 74.2257 9.77133 74.047 9.80381L73.3894 9.90124C73.0646 9.94995 72.8454 10.0393 72.7318 10.1692C72.6181 10.2991 72.5613 10.5345 72.5613 10.8755V22.1286C72.5613 23.3627 72.8373 24.3207 73.3894 25.0027C73.9415 25.6847 74.7128 26.0257 75.7033 26.0257C76.7101 26.0257 77.4895 25.6847 78.0416 25.0027C78.5937 24.3207 78.8698 23.3627 78.8698 22.1286V11.3627C78.8698 10.4858 78.5937 9.99866 78.0416 9.90124L77.384 9.80381C77.2054 9.77133 77.1161 9.68202 77.1161 9.53588C77.1161 9.34102 77.2378 9.24359 77.4814 9.24359H80.9889C81.2324 9.24359 81.3542 9.3329 81.3542 9.51152C81.3542 9.65766 81.2487 9.75509 81.0376 9.80381L80.4774 9.90124C79.9253 9.99866 79.6492 10.4777 79.6492 11.3383V22.1042C79.6492 23.5819 79.2514 24.7673 78.4557 25.6603C77.66 26.5534 76.6046 27 75.2893 27Z" fill="currentColor"></path>
<path className="" d="M58.2703 26.7808C58.108 26.7808 57.9943 26.7483 57.9293 26.6834C57.8644 26.6184 57.8319 26.5534 57.8319 26.4885C57.8319 26.3261 57.9537 26.2206 58.1973 26.1719L58.587 26.0988C59.1066 26.0014 59.5126 25.8471 59.8048 25.636C60.1134 25.4087 60.3732 25.0514 60.5843 24.5643L63.02 18.7673C63.1012 18.5886 63.1499 18.41 63.1661 18.2314C63.1986 18.0365 63.1661 17.8336 63.0687 17.6225L60.0241 10.8755C59.8617 10.502 59.6831 10.2504 59.4882 10.1205C59.3096 9.99055 59.1147 9.90936 58.9036 9.87688L58.4408 9.80381C58.181 9.75509 58.0511 9.64954 58.0511 9.48716C58.0511 9.32478 58.1891 9.24359 58.4652 9.24359H62.7521C63.0444 9.24359 63.1905 9.32478 63.1905 9.48716C63.1905 9.64954 63.0525 9.75509 62.7764 9.80381L62.3136 9.87688C62.0376 9.92559 61.8427 10.023 61.7291 10.1692C61.6316 10.3153 61.656 10.5508 61.8021 10.8755L64.1161 16.0636C64.181 16.2098 64.2541 16.2828 64.3353 16.2828C64.4327 16.2828 64.5139 16.2098 64.5789 16.0636L66.6736 11.1191C66.8197 10.7619 66.8684 10.502 66.8197 10.3397C66.771 10.161 66.6086 10.0393 66.3326 9.97431L65.6506 9.82816C65.3745 9.76321 65.2365 9.64954 65.2365 9.48716C65.2365 9.32478 65.3826 9.24359 65.6749 9.24359H69.4016C69.5802 9.24359 69.6939 9.27607 69.7426 9.34102C69.8075 9.38973 69.84 9.45469 69.84 9.53588C69.84 9.69826 69.7182 9.80381 69.4747 9.85252L69.0849 9.92559C68.679 9.99055 68.338 10.1448 68.0619 10.3884C67.7859 10.6157 67.5423 10.973 67.3312 11.4601L65.0173 16.8674C64.9361 17.0785 64.8874 17.2815 64.8711 17.4763C64.8711 17.6712 64.928 17.8823 65.0416 18.1096L68.1837 25.1489C68.3623 25.5386 68.541 25.7903 68.7196 25.9039C68.9144 26.0176 69.1093 26.0907 69.3042 26.1231L69.7669 26.1962C70.043 26.2449 70.181 26.3586 70.181 26.5372C70.181 26.6996 70.0349 26.7808 69.7426 26.7808H65.4314C65.1391 26.7808 64.9929 26.6834 64.9929 26.4885C64.9929 26.3424 65.1228 26.2449 65.3826 26.1962L65.8698 26.1231C66.1458 26.0907 66.3407 26.0014 66.4544 25.8552C66.568 25.6928 66.5599 25.4574 66.43 25.1489L63.9212 19.571C63.8563 19.4249 63.7751 19.3518 63.6776 19.3518C63.5965 19.3518 63.5234 19.4249 63.4584 19.571L61.2663 24.9053C61.1201 25.2625 61.0714 25.5304 61.1201 25.7091C61.1688 25.8714 61.3312 25.9851 61.6073 26.0501L62.2893 26.1962C62.5653 26.2612 62.7034 26.3748 62.7034 26.5372C62.7034 26.6996 62.5572 26.7808 62.2649 26.7808H58.2703Z" fill="currentColor"></path>
<path className="" d="M49.2578 26.7808C49.0142 26.7808 48.8925 26.6915 48.8925 26.5129C48.8925 26.3667 48.998 26.2693 49.2091 26.2206L49.7693 26.1231C50.1103 26.0582 50.3295 25.9689 50.427 25.8552C50.5406 25.7415 50.5975 25.5142 50.5975 25.1732V10.8512C50.5975 10.5102 50.5406 10.2828 50.427 10.1692C50.3295 10.0555 50.1103 9.96619 49.7693 9.90124L49.2091 9.80381C48.998 9.75509 48.8925 9.65766 48.8925 9.51152C48.8925 9.3329 49.0142 9.24359 49.2578 9.24359H57.4418C57.8153 9.24359 58.0183 9.43033 58.0508 9.80381L58.2456 13.8471C58.2781 14.1232 58.1726 14.2612 57.929 14.2612C57.7341 14.2612 57.6205 14.1556 57.588 13.9445L57.4418 13.0433C57.2632 11.9391 56.8979 11.1353 56.3458 10.632C55.7937 10.1123 55.0305 9.85252 54.0562 9.85252C53.4067 9.85252 52.952 9.94995 52.6922 10.1448C52.4486 10.3234 52.3268 10.6482 52.3268 11.1191V16.9892C52.3268 17.2165 52.4405 17.3302 52.6678 17.3302H53.8857C54.6002 17.3302 55.0386 16.9486 55.201 16.1854L55.4445 14.9675C55.5095 14.724 55.6475 14.6184 55.8586 14.6509C56.0372 14.6834 56.1265 14.8214 56.1265 15.065V20.5453C56.1265 20.7889 56.0372 20.9269 55.8586 20.9594C55.6475 20.9919 55.5095 20.8863 55.4445 20.6428L55.201 19.4493C55.0873 18.8809 54.9249 18.5074 54.7138 18.3288C54.519 18.1502 54.2348 18.0609 53.8613 18.0609H52.6678C52.4405 18.0609 52.3268 18.1746 52.3268 18.4019V24.5156C52.3268 25.1001 52.473 25.5223 52.7653 25.7821C53.0738 26.0419 53.6259 26.1719 54.4215 26.1719C55.2335 26.1719 55.9236 25.9689 56.4919 25.5629C57.0765 25.1407 57.4824 24.3613 57.7098 23.2246L58.0021 21.7876C58.0508 21.5765 58.1726 21.4709 58.3674 21.4709C58.5947 21.4709 58.6922 21.6089 58.6597 21.885L58.4161 26.2206C58.3837 26.594 58.1807 26.7808 57.8072 26.7808H49.2578Z" fill="currentColor"></path>
<path className="" d="M46.943 27C46.6831 27 46.4639 26.8051 46.2853 26.4154L39.6845 12.0934C39.6358 11.9797 39.5708 11.931 39.4896 11.9472C39.4247 11.9635 39.3922 12.0366 39.3922 12.1665V24.6861C39.3922 25.1407 39.4653 25.4898 39.6114 25.7334C39.7738 25.9608 40.058 26.1069 40.4639 26.1718L40.7806 26.2206C40.9917 26.2368 41.0972 26.3342 41.0972 26.5129C41.0972 26.6915 40.9754 26.7808 40.7319 26.7808H37.2488C37.0052 26.7808 36.8834 26.6915 36.8834 26.5129C36.8834 26.3342 36.989 26.2368 37.2001 26.2206L37.5167 26.1718C37.9389 26.1069 38.2312 25.9608 38.3936 25.7334C38.556 25.4898 38.6371 25.1407 38.6371 24.6861V10.8512C38.6371 10.5102 38.5803 10.2828 38.4666 10.1692C38.3692 10.0555 38.1338 9.96619 37.7603 9.90124L37.2001 9.80381C36.989 9.75509 36.8834 9.65766 36.8834 9.51152C36.8834 9.3329 37.0052 9.24359 37.2488 9.24359H39.855C40.261 9.24359 40.5451 9.42221 40.7075 9.77945L46.4558 22.4208C46.5045 22.5183 46.5614 22.5589 46.6263 22.5426C46.7075 22.5264 46.7481 22.4696 46.7481 22.3721V11.3383C46.7481 10.8187 46.6588 10.4615 46.4802 10.2666C46.3015 10.0555 46.0742 9.92559 45.7982 9.87688L45.3597 9.80381C45.1486 9.75509 45.0431 9.65766 45.0431 9.51152C45.0431 9.3329 45.1649 9.24359 45.4084 9.24359H48.8915C49.1351 9.24359 49.2569 9.3329 49.2569 9.51152C49.2569 9.69014 49.1513 9.78757 48.9402 9.80381L48.6236 9.85252C48.2014 9.91747 47.9091 10.0555 47.7467 10.2666C47.5844 10.4615 47.5032 10.8187 47.5032 11.3383V26.2206C47.5032 26.5129 47.4463 26.7158 47.3327 26.8295C47.2352 26.9432 47.1053 27 46.943 27Z" fill="currentColor"></path>
</svg>
<svg className="" fill="none" height="36" viewbox="0 0 104 36" width="104" xmlns="http://www.w3.org/2000/svg">
<path className="" d="M5.42813 24.6667C5.42813 17.3067 11.4015 11.3333 18.7615 11.3333C26.1215 11.3333 32.0948 17.3067 32.0948 24.6667" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M13.4281 24.6667C13.4281 21.72 15.8148 19.3333 18.7615 19.3333C21.7081 19.3333 24.0948 21.72 24.0948 24.6667" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M9.42813 24.6667C9.42813 19.5067 13.6015 15.3333 18.7615 15.3333C23.9215 15.3333 28.0948 19.5067 28.0948 24.6667" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
<path className="" d="M95.8008 26.9918C94.5611 26.9918 93.4867 26.7191 92.5776 26.1736C91.6685 25.6282 90.9633 24.8651 90.4619 23.8844C89.966 22.9036 89.7181 21.7576 89.7181 20.4463C89.7181 19.135 89.966 17.9863 90.4619 17C90.9633 16.0138 91.6685 15.248 92.5776 14.7025C93.4867 14.1571 94.5611 13.8843 95.8008 13.8843C97.0404 13.8843 98.1148 14.1571 99.0239 14.7025C99.933 15.248 100.635 16.0138 101.131 17C101.633 17.9863 101.883 19.135 101.883 20.4463C101.883 21.7576 101.633 22.9036 101.131 23.8844C100.635 24.8651 99.933 25.6282 99.0239 26.1736C98.1148 26.7191 97.0404 26.9918 95.8008 26.9918ZM95.8173 24.5951C96.4895 24.5951 97.0515 24.4105 97.5032 24.0414C97.955 23.6667 98.2911 23.1653 98.5115 22.5372C98.7374 21.9091 98.8504 21.2094 98.8504 20.4381C98.8504 19.6612 98.7374 18.9587 98.5115 18.3306C98.2911 17.697 97.955 17.1929 97.5032 16.8182C97.0515 16.4436 96.4895 16.2562 95.8173 16.2562C95.1286 16.2562 94.5556 16.4436 94.0983 16.8182C93.6465 17.1929 93.3076 17.697 93.0817 18.3306C92.8614 18.9587 92.7512 19.6612 92.7512 20.4381C92.7512 21.2094 92.8614 21.9091 93.0817 22.5372C93.3076 23.1653 93.6465 23.6667 94.0983 24.0414C94.5556 24.4105 95.1286 24.5951 95.8173 24.5951Z" fill="currentColor"></path>
<path className="" d="M85.3591 26.7438V14.0496H88.3508V26.7438H85.3591ZM86.8632 12.2479C86.3894 12.2479 85.9816 12.0909 85.64 11.7769C85.2985 11.4573 85.1277 11.0744 85.1277 10.6281C85.1277 10.1763 85.2985 9.79339 85.64 9.47934C85.9816 9.15978 86.3894 9 86.8632 9C87.3425 9 87.7502 9.15978 88.0863 9.47934C88.4279 9.79339 88.5987 10.1763 88.5987 10.6281C88.5987 11.0744 88.4279 11.4573 88.0863 11.7769C87.7502 12.0909 87.3425 12.2479 86.8632 12.2479Z" fill="currentColor"></path>
<path className="" d="M76.5743 26.967C75.5771 26.967 74.6845 26.7108 73.8966 26.1984C73.1087 25.686 72.4861 24.9422 72.0288 23.967C71.5715 22.9917 71.3429 21.8072 71.3429 20.4132C71.3429 19.0028 71.5743 17.8127 72.0371 16.843C72.5054 15.8678 73.1363 15.1322 73.9297 14.6364C74.7231 14.135 75.6074 13.8843 76.5826 13.8843C77.3264 13.8843 77.9379 14.011 78.4173 14.2645C78.8966 14.5124 79.2768 14.8127 79.5578 15.1653C79.8388 15.5124 80.0564 15.8402 80.2107 16.1488H80.3346V9.81818H83.3346V26.7438H80.3925V24.7438H80.2107C80.0564 25.0524 79.8333 25.3802 79.5413 25.7273C79.2492 26.0689 78.8636 26.3609 78.3842 26.6033C77.9049 26.8457 77.3016 26.967 76.5743 26.967ZM77.409 24.5124C78.0426 24.5124 78.5826 24.3416 79.0289 24C79.4751 23.6529 79.814 23.1708 80.0454 22.5537C80.2768 21.9366 80.3925 21.2176 80.3925 20.3967C80.3925 19.5758 80.2768 18.8623 80.0454 18.2562C79.8195 17.6501 79.4834 17.1791 79.0371 16.843C78.5963 16.5069 78.0536 16.3388 77.409 16.3388C76.7423 16.3388 76.1859 16.5124 75.7396 16.8595C75.2933 17.2066 74.9572 17.686 74.7313 18.2975C74.5054 18.9091 74.3925 19.6088 74.3925 20.3967C74.3925 21.1901 74.5054 21.8981 74.7313 22.5207C74.9627 23.1378 75.3016 23.6254 75.7479 23.9835C76.1996 24.3361 76.7534 24.5124 77.409 24.5124Z" fill="currentColor"></path>
<path className="" d="M66.959 21.405V14.0496H69.9508V26.7438H67.0499V24.4876H66.9177C66.6312 25.1984 66.1601 25.7796 65.5045 26.2314C64.8543 26.6832 64.0527 26.9091 63.0995 26.9091C62.2676 26.9091 61.532 26.7245 60.8929 26.3554C60.2593 25.9807 59.7634 25.438 59.4053 24.7273C59.0472 24.011 58.8681 23.146 58.8681 22.1322V14.0496H61.8598V21.6694C61.8598 22.4738 62.0802 23.113 62.521 23.5868C62.9618 24.0606 63.5403 24.2975 64.2565 24.2975C64.6973 24.2975 65.1243 24.1901 65.5375 23.9752C65.9508 23.7603 66.2896 23.4408 66.5541 23.0165C66.824 22.5868 66.959 22.0496 66.959 21.405Z" fill="currentColor"></path>
<path className="" d="M57.5297 14.0496V16.3636H50.2322V14.0496H57.5297ZM52.0338 11.0083H55.0256V22.9256C55.0256 23.3278 55.0862 23.6364 55.2074 23.8513C55.3341 24.0606 55.4994 24.2039 55.7033 24.281C55.9071 24.3582 56.133 24.3967 56.3809 24.3967C56.5683 24.3967 56.7391 24.3829 56.8933 24.3554C57.0531 24.3278 57.1743 24.3031 57.257 24.281L57.7611 26.6199C57.6013 26.675 57.3727 26.7356 57.0752 26.8017C56.7831 26.8678 56.425 26.9064 56.0008 26.9174C55.2515 26.9394 54.5765 26.8265 53.976 26.5785C53.3754 26.3251 52.8988 25.9339 52.5462 25.405C52.1991 24.8761 52.0283 24.2149 52.0338 23.4215V11.0083Z" fill="currentColor"></path>
<path className="" d="M46.5714 14.4711C46.4943 13.7493 46.1692 13.1873 45.5962 12.7851C45.0287 12.3829 44.2904 12.1818 43.3813 12.1818C42.7422 12.1818 42.194 12.2782 41.7367 12.4711C41.2794 12.6639 40.9295 12.9256 40.6871 13.2562C40.4447 13.5868 40.3207 13.9642 40.3152 14.3884C40.3152 14.741 40.3951 15.0468 40.5549 15.3058C40.7202 15.5647 40.9433 15.7851 41.2243 15.9669C41.5053 16.1432 41.8166 16.292 42.1582 16.4132C42.4998 16.5344 42.8441 16.6364 43.1912 16.719L44.778 17.1157C45.4171 17.2645 46.0315 17.4656 46.621 17.719C47.216 17.9724 47.7477 18.292 48.216 18.6777C48.6899 19.0634 49.0645 19.5289 49.34 20.0744C49.6155 20.6198 49.7532 21.259 49.7532 21.9917C49.7532 22.9835 49.4998 23.8568 48.9929 24.6116C48.486 25.3609 47.7532 25.9477 46.7945 26.3719C45.8414 26.7906 44.6871 27 43.3317 27C42.0149 27 40.8717 26.7962 39.902 26.3884C38.9378 25.9807 38.183 25.3857 37.6375 24.6033C37.0976 23.8209 36.8056 22.8678 36.7615 21.7438H39.778C39.8221 22.3333 40.0039 22.8237 40.3235 23.2149C40.643 23.6061 41.059 23.8981 41.5714 24.0909C42.0893 24.2838 42.6678 24.3802 43.3069 24.3802C43.9736 24.3802 44.5576 24.281 45.059 24.0827C45.5659 23.8788 45.9626 23.5978 46.2491 23.2397C46.5356 22.876 46.6816 22.4518 46.6871 21.9669C46.6816 21.5262 46.5521 21.1625 46.2987 20.876C46.0452 20.584 45.6899 20.3416 45.2326 20.1488C44.7808 19.9504 44.2518 19.7741 43.6458 19.6198L41.7202 19.124C40.3262 18.7658 39.2243 18.2231 38.4144 17.4959C37.61 16.7631 37.2078 15.7906 37.2078 14.5785C37.2078 13.5813 37.4777 12.708 38.0177 11.9587C38.5631 11.2094 39.3042 10.6281 40.2408 10.2149C41.1775 9.79613 42.2381 9.58676 43.4226 9.58676C44.6237 9.58676 45.6761 9.79613 46.5797 10.2149C47.4888 10.6281 48.2023 11.2038 48.7202 11.9421C49.2381 12.6749 49.5053 13.5179 49.5218 14.4711H46.5714Z" fill="currentColor"></path>
</svg>
</div></div></div></div></div></section>
<div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
<div className="rounded-2xl border p-4 border-white/10 bg-white/5">
<div className="flex items-center gap-2">
<svg className="lucide lucide-sparkles w-4 h-4 text-white/80" data-lucide="sparkles" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M11.017 2.814a1 1 0 0 1 1.966 0l1.051 5.558a2 2 0 0 0 1.594 1.594l5.558 1.051a1 1 0 0 1 0 1.966l-5.558 1.051a2 2 0 0 0-1.594 1.594l-1.051 5.558a1 1 0 0 1-1.966 0l-1.051-5.558a2 2 0 0 0-1.594-1.594l-5.558-1.051a1 1 0 0 1 0-1.966l5.558-1.051a2 2 0 0 0 1.594-1.594z"></path><path d="M20 2v4"></path><path d="M22 4h-4"></path><circle cx="4" cy="20" r="2"></circle></svg>
<h3 className="text-xl font-semibold tracking-tight font-sans">Навыки</h3>
</div>
<div className="mt-3 flex flex-wrap gap-2">
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>Design Systems</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>Product Strategy</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>Prototyping</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>Accessibility</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>React</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>TypeScript</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>Next.js</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>Framer Motion</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>Data Visualization</span>
<span className="px-2.5 py-1 text-xs rounded-md border font-sans bg-white/10 border-white/10" style={{}}>Performance</span>
</div>
</div>
<div className="rounded-2xl border p-4 border-white/10 bg-white/5">
<div className="flex items-center gap-2">
<svg className="lucide lucide-briefcase w-4 h-4 text-white/80" data-lucide="briefcase" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M16 20V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path><rect height="14" rx="2" width="20" x="2" y="6"></rect></svg>
<h3 className="text-xl font-semibold tracking-tight font-sans">Опыт</h3>
</div>
<ul className="mt-3 space-y-3">
<li className="flex items-start gap-3">
<svg className="lucide lucide-building-2 w-4 h-4 mt-0.5 text-white/70" data-lucide="building-2" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M10 12h4"></path><path d="M10 8h4"></path><path d="M14 21v-3a2 2 0 0 0-4 0v3"></path><path d="M6 10H4a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2"></path><path d="M6 21V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v16"></path></svg>
<div className="">
<p className="text-sm font-medium tracking-tight font-sans" style={{}}>Senior Product Designer • Atlas Analytics</p>
<p className="text-xs font-sans text-white/60" style={{}}>2022 — 2024 · Led design system and multi-tenant analytics</p>
</div>
</li>
<li className="flex items-start gap-3">
<svg className="lucide lucide-building-2 w-4 h-4 mt-0.5 text-white/70" data-lucide="building-2" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M10 12h4"></path><path d="M10 8h4"></path><path d="M14 21v-3a2 2 0 0 0-4 0v3"></path><path d="M6 10H4a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2"></path><path d="M6 21V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v16"></path></svg>
<div className="">
<p className="text-sm font-medium tracking-tight font-sans" style={{}}>Product Designer • Tempo Health</p>
<p className="text-xs font-sans text-white/60" style={{}}>2019 — 2022 · Care workflows, mobile companion app</p>
</div>
</li>
<li className="flex items-start gap-3">
<svg className="lucide lucide-building-2 w-4 h-4 mt-0.5 text-white/70" data-lucide="building-2" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M10 12h4"></path><path d="M10 8h4"></path><path d="M14 21v-3a2 2 0 0 0-4 0v3"></path><path d="M6 10H4a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2"></path><path d="M6 21V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v16"></path></svg>
<div className="">
<p className="text-sm font-medium tracking-tight font-sans" style={{}}>Frontend Engineer • Nexus Commerce</p>
<p className="text-xs font-sans text-white/60" style={{}}>2017 — 2019 · Storefront UI, performance, A/B testing</p>
</div>
</li>
</ul>
</div>
<div className="rounded-2xl border p-4 border-white/10 bg-white/5">
<div className="flex items-center gap-2">
<svg className="lucide w-4 h-4 text-white/80" data-lucide="timeline" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" width="24" xmlns="http://www.w3.org/2000/svg"></svg>
<h3 className="text-xl font-semibold tracking-tight font-sans" style={{}}>Timeline</h3>
</div>
<ol className="mt-3">
<li className="relative pl-6 pb-4 border-l border-white/10">
<span className="absolute left-[-5px] top-1.5 w-2 h-2 rounded-full bg-white"></span>
<p className="text-xs font-sans text-white/60" style={{}}>2025</p>
<p className="text-sm font-medium tracking-tight font-sans" style={{}}>Independent — Designer &amp; Engineer</p>
</li>
<li className="relative pl-6 pb-4 border-l border-white/10">
<span className="absolute left-[-5px] top-1.5 w-2 h-2 rounded-full bg-white/80"></span>
<p className="text-xs font-sans text-white/60" style={{}}>2022 — 2024</p>
<p className="text-sm font-medium tracking-tight font-sans" style={{}}>Senior Product Designer — Atlas Analytics</p>
</li>
<li className="relative pl-6 pb-4 border-l border-white/10">
<span className="absolute left-[-5px] top-1.5 w-2 h-2 rounded-full bg-white/70"></span>
<p className="text-xs font-sans text-white/60" style={{}}>2019 — 2022</p>
<p className="text-sm font-medium tracking-tight font-sans" style={{}}>Product Designer — Tempo Health</p>
</li>
<li className="relative pl-6 border-l border-white/10">
<span className="absolute left-[-5px] top-1.5 w-2 h-2 rounded-full bg-white/60"></span>
<p className="text-xs font-sans text-white/60" style={{}}>2017 — 2019</p>
<p className="text-sm font-medium tracking-tight font-sans" style={{}}>Frontend Engineer — Nexus Commerce</p>
</li>
</ol>
</div>
</div>
<p className="mt-6 text-center text-[11px] font-sans text-white/60" style={{}}>© <span className="font-sans" id="year" style={{}}>2025</span> Jordan Lee — Available for freelance &amp; contracts</p>
</div>
</div>
</section><section className="sm:pt-24 md:pt-28 pb-20 relative">

<div className="pointer-events-none absolute inset-0 -z-10">
<div className="absolute left-1/2 -translate-x-1/2 -top-20 w-[48rem] h-[48rem] rounded-full bg-[radial-gradient(ellipse_at_center,rgba(99,102,241,0.22)_0%,transparent_60%)] blur-3xl">
</div>
<div className="absolute right-0 bottom-0 w-[36rem] h-[36rem] rounded-full bg-[radial-gradient(ellipse_at_bottom_right,rgba(168,85,247,0.18)_0%,transparent_60%)] blur-3xl">
</div>
</div>
<div className="max-w-7xl mr-auto ml-auto pr-6 pl-6">

<div className="text-center">
<div className="inline-flex gap-2 shadow-neutral-500/20 border rounded-full mr-auto ml-auto pt-1.5 pr-4 pb-1.5 pl-1.5 shadow-sm items-center bg-white/5 border-white/10">
<span className="inline-flex items-center justify-center w-5 h-5 rounded-full ring-1 bg-neutral-600 ring-white/20">
<svg className="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" strokeWidth="2" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path className="" d="M12 8V4H8"></path><rect className="" height="12" rx="2" width="16" x="4" y="8"></rect><path className="" d="M2 14h2"></path><path className="" d="M20 14h2"></path><path className="" d="M15 13v2"></path><path className="" d="M9 13v2"></path></svg>
</span>
<span className="text-xs font-medium font-sans text-white/80">AI Portfolio</span>
</div>
<h2 className="mt-6 text-4xl sm:text-6xl md:text-7xl leading-[1.06] tracking-tighter font-sans font-semibold">
        Intelligent Solutions
        <span className="font-semibold text-white/80">Delivered with Precision.</span>
</h2>
<p className="mx-auto mt-4 max-w-2xl text-base sm:text-lg font-sans text-white/70">
        Our AI-powered projects showcase how applied intelligence transforms businesses, delivering measurable results
        and breakthrough innovations.
      </p>
<div className="mt-8">
<a className="hidden sm:inline-flex items-center gap-2 hover:from-neutral-500 transition-colors shadow-blue-500/25 text-sm font-medium bg-gradient-to-l rounded-full pt-2 pr-4 pb-2 pl-4 shadow-md font-sans hover:to-neutral-700 text-white from-neutral-600 to-neutral-800" href="#">View
          AI Solutions<svg className="lucide lucide-arrow-right w-4 h-4" data-lucide="arrow-right" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg></a>
</div>
</div>

<section className="bg-black max-w-max rounded-3xl mt-8 pt-6 pr-6 pb-6 pl-6">

<div className="flex items-center justify-between mb-3">
<p className="text-xs font-medium text-neutral-500 uppercase tracking-wide" style={{}}>/Portfolio</p>
<span className="text-xs font-medium text-neutral-500" style={{}}>(02)</span>
</div>
<div className="flex gap-6 items-start justify-between mb-8">
<div className="max-w-3xl">
<h2 className="text-4xl md:text-6xl font-light tracking-tight text-neutral-100" style={{}}>Latest moments from my
        portfolio.</h2>
<p className="mt-4 text-sm md:text-base text-neutral-400" style={{}}>Portraits, editorials, and landscapes by Suzy
        Liu—minimal, calm, and intentional storytelling through light.</p>
</div>
<a className="hidden sm:inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition ring-1 bg-neutral-900 ring-neutral-800 hover:bg-neutral-800 text-neutral-100" href="#" style={{}}>
      View all projects
      <svg className="lucide lucide-plus h-4 w-4" data-lucide="plus" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="M12 5v14"></path></svg>
</a>
</div>

<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">

<article className="lg:col-span-2 rounded-3xl p-1 ring-1 bg-neutral-950 ring-neutral-800">
<div className="relative overflow-hidden rounded-2xl">
<img alt="Minimal landscape in soft gradient light" className="h-[360px] md:h-[460px] w-full object-cover" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/a96798fa-9905-42b3-9d61-fec4d29fbe06_800w.jpg"/>

<span className="absolute right-3 top-3 rounded-full px-3 py-1 text-xs font-medium shadow-sm backdrop-blur bg-black/90 text-neutral-200" style={{}}>Editorial</span>

<div className="pointer-events-none absolute inset-x-0 bottom-0">
<div className="h-40 bg-gradient-to-t to-transparent from-neutral-100/90 via-neutral-100/40"></div>
</div>
<div className="absolute left-4 right-4 bottom-4">
<p className="text-xs font-medium text-black/80" style={{}}>May 30, 2025</p>
<h3 className="mt-1 text-xl md:text-2xl font-light tracking-tight text-black" style={{}}>The Power of Restraint in
            Photography</h3>
<p className="mt-2 text-sm text-black/80" style={{}}>How simplicity, negative space, and patient framing amplify
            emotion and create timeless images.</p>
</div>
</div>
</article>

<article className="rounded-3xl p-1 ring-1 bg-neutral-950 ring-neutral-800">
<div className="overflow-hidden rounded-2xl relative">
<img alt="Motion portrait in soft tones" className="h-[240px] md:h-[260px] w-full object-cover" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/361b93b3-4faa-4d90-a64d-f5de4921a4d7_800w.jpg"/>
<span className="absolute right-3 top-3 rounded-full px-3 py-1 text-xs font-medium shadow-sm backdrop-blur bg-black/90 text-neutral-200" style={{}}>Portrait</span>
</div>
<div className="px-4 pt-4 pb-6">
<p className="text-xs font-medium text-neutral-500" style={{}}>May 23, 2025</p>
<h4 className="mt-1 text-lg font-semibold tracking-tight text-neutral-100" style={{}}>Designing for Calm: Motion in
          Portraits</h4>
<p className="mt-2 text-sm text-neutral-400" style={{}}>Exploring blur, minimal palettes, and subtle pacing to shape
          the viewer's emotional response.</p>
</div>
</article>

<article className="ring-1 rounded-3xl pt-1 pr-1 pb-1 pl-1 ring-neutral-800 bg-neutral-950">
<div className="overflow-hidden rounded-2xl relative">
<img alt="Studio black-and-white portrait with geometric light" className="h-[240px] md:h-[260px] w-full object-cover" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/cee814f4-9f14-4c82-896a-b36587194633_800w.jpg"/>
<span className="absolute right-3 top-3 rounded-full px-3 py-1 text-xs font-medium shadow-sm backdrop-blur bg-black/90 text-neutral-200" style={{}}>Studio</span>
</div>
<div className="px-4 pt-4 pb-6">
<p className="text-xs font-medium text-neutral-500" style={{}}>May 16, 2025</p>
<h4 className="mt-1 text-lg font-semibold tracking-tight text-neutral-100" style={{}}>Building a Timeless Aesthetic
        </h4>
<p className="mt-2 text-sm text-neutral-400" style={{}}>Creating images that transcend trends by focusing on light,
          form, and honest expression.</p>
</div>
</article>
<article className="ring-1 rounded-3xl pt-1 pr-1 pb-1 pl-1 ring-neutral-800 bg-neutral-950">
<div className="overflow-hidden rounded-2xl relative">
<img alt="Studio black-and-white portrait with geometric light" className="h-[240px] md:h-[260px] w-full object-cover" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/88cc054f-a996-4a0c-8fec-3721e3ac4e25_800w.jpg"/>
<span className="absolute right-3 top-3 rounded-full px-3 py-1 text-xs font-medium shadow-sm backdrop-blur bg-black/90 text-neutral-200" style={{}}>Studio</span>
</div>
<div className="px-4 pt-4 pb-6">
<p className="text-xs font-medium text-neutral-500" style={{}}>May 16, 2025</p>
<h4 className="mt-1 text-lg font-semibold tracking-tight text-neutral-100" style={{}}>Building a Timeless Aesthetic
        </h4>
<p className="mt-2 text-sm text-neutral-400" style={{}}>Creating images that transcend trends by focusing on light,
          form, and honest expression.</p>
</div>
</article>
<article className="ring-1 rounded-3xl pt-1 pr-1 pb-1 pl-1 ring-neutral-800 bg-neutral-950">
<div className="overflow-hidden rounded-2xl relative">
<img alt="Studio black-and-white portrait with geometric light" className="h-[240px] md:h-[260px] w-full object-cover" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/779c923e-bcc8-49c7-9328-661a04d3f208_800w.jpg"/>
<span className="absolute right-3 top-3 rounded-full px-3 py-1 text-xs font-medium shadow-sm backdrop-blur bg-black/90 text-neutral-200" style={{}}>Studio</span>
</div>
<div className="px-4 pt-4 pb-6">
<p className="text-xs font-medium text-neutral-500" style={{}}>May 16, 2025</p>
<h4 className="mt-1 text-lg font-semibold tracking-tight text-neutral-100" style={{}}>Building a Timeless Aesthetic
        </h4>
<p className="mt-2 text-sm text-neutral-400" style={{}}>Creating images that transcend trends by focusing on light,
          form, and honest expression.</p>
</div>
</article>
</div>

<div className="mt-6 sm:hidden">
<a className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition ring-1 w-full justify-center bg-neutral-900 ring-neutral-800 hover:bg-neutral-800 text-neutral-100" href="#" style={{}}>
      View all projects
      <svg className="lucide lucide-plus h-4 w-4" data-lucide="plus" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="M12 5v14"></path></svg>
</a>
</div>
</section>

<section className="text-sm bg-black max-w-max rounded-none mt-8 pt-6 pr-6 pb-6 pl-6">

<div className="flex items-center justify-between mb-3 text-sm rounded-none">
<p className="font-medium text-neutral-500 uppercase tracking-wide text-sm rounded-none" style={{}}>/Portfolio</p>
<span className="font-medium text-neutral-500 text-sm rounded-none" style={{}}>(02)</span>
</div>
<div className="flex gap-6 items-start justify-between mb-8 text-sm rounded-none">
<div className="max-w-3xl text-sm rounded-none">
<h2 className="md:text-6xl font-light tracking-tight text-sm rounded-none text-neutral-100" style={{}}>Latest moments from my
        portfolio.</h2>
<p className="mt-4 md:text-base text-sm rounded-none text-neutral-400" style={{}}>Portraits, editorials, and landscapes by Suzy
        Liu—minimal, calm, and intentional storytelling through light.</p>
</div>
</div>

<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-sm rounded-none">

<article className="lg:col-span-2 p-1 ring-1 text-sm rounded-none bg-neutral-950 ring-neutral-800">
<div className="relative overflow-hidden text-sm rounded-none">
<img alt="Minimal landscape in soft gradient light" className="h-[360px] md:h-[460px] w-full object-cover text-sm rounded-none" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/a96798fa-9905-42b3-9d61-fec4d29fbe06_800w.jpg"/>

<span className="absolute right-3 top-3 px-3 py-1 font-medium shadow-sm backdrop-blur text-sm rounded-none bg-black/90 text-neutral-200" style={{}}>Editorial</span>

<div className="pointer-events-none absolute inset-x-0 bottom-0 text-sm rounded-none">
<div className="h-40 bg-gradient-to-t to-transparent text-sm rounded-none from-neutral-100/90 via-neutral-100/40"></div>
</div>
<div className="absolute left-4 right-4 bottom-4 text-sm rounded-none">
<p className="font-medium text-sm rounded-none text-black/80" style={{}}>May 30, 2025</p>
<h3 className="mt-1 md:text-2xl font-light tracking-tight text-sm rounded-none text-black" style={{}}>The Power of Restraint in
            Photography</h3>
<p className="mt-2 text-sm rounded-none text-black/80" style={{}}>How simplicity, negative space, and patient framing amplify
            emotion and create timeless images.</p>
</div>
</div>
</article>

<article className="p-1 ring-1 text-sm rounded-none bg-neutral-950 ring-neutral-800">
<div className="overflow-hidden relative text-sm rounded-none">
<img alt="Motion portrait in soft tones" className="h-[240px] md:h-[260px] w-full object-cover text-sm rounded-none" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/361b93b3-4faa-4d90-a64d-f5de4921a4d7_800w.jpg"/>
<span className="absolute right-3 top-3 px-3 py-1 font-medium shadow-sm backdrop-blur text-sm rounded-none bg-black/90 text-neutral-200" style={{}}>Portrait</span>
</div>
<div className="px-4 pt-4 pb-6 text-sm rounded-none">
<p className="font-medium text-neutral-500 text-sm rounded-none" style={{}}>May 23, 2025</p>
<h4 className="mt-1 font-semibold tracking-tight text-sm rounded-none text-neutral-100" style={{}}>Designing for Calm: Motion in
          Portraits</h4>
<p className="mt-2 text-sm rounded-none text-neutral-400" style={{}}>Exploring blur, minimal palettes, and subtle pacing to shape
          the viewer's emotional response.</p>
</div>
</article>

<article className="ring-1 pt-1 pr-1 pb-1 pl-1 text-sm rounded-none ring-neutral-800 bg-neutral-950">
<div className="overflow-hidden relative text-sm rounded-none">
<img alt="Studio black-and-white portrait with geometric light" className="h-[240px] md:h-[260px] w-full object-cover text-sm rounded-none" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/cee814f4-9f14-4c82-896a-b36587194633_800w.jpg"/>
<span className="absolute right-3 top-3 px-3 py-1 font-medium shadow-sm backdrop-blur text-sm rounded-none bg-black/90 text-neutral-200" style={{}}>Studio</span>
</div>
<div className="px-4 pt-4 pb-6 text-sm rounded-none">
<p className="font-medium text-neutral-500 text-sm rounded-none" style={{}}>May 16, 2025</p>
<h4 className="mt-1 font-semibold tracking-tight text-sm rounded-none text-neutral-100" style={{}}>Building a Timeless Aesthetic
        </h4>
<p className="mt-2 text-sm rounded-none text-neutral-400" style={{}}>Creating images that transcend trends by focusing on light,
          form, and honest expression.</p>
</div>
</article>
<article className="ring-1 pt-1 pr-1 pb-1 pl-1 text-sm rounded-none ring-neutral-800 bg-neutral-950">
<div className="overflow-hidden relative text-sm rounded-none">
<img alt="Studio black-and-white portrait with geometric light" className="h-[240px] md:h-[260px] w-full object-cover text-sm rounded-none" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/88cc054f-a996-4a0c-8fec-3721e3ac4e25_800w.jpg"/>
<span className="absolute right-3 top-3 px-3 py-1 font-medium shadow-sm backdrop-blur text-sm rounded-none bg-black/90 text-neutral-200" style={{}}>Studio</span>
</div>
<div className="px-4 pt-4 pb-6 text-sm rounded-none">
<p className="font-medium text-neutral-500 text-sm rounded-none" style={{}}>May 16, 2025</p>
<h4 className="mt-1 font-semibold tracking-tight text-sm rounded-none text-neutral-100" style={{}}>Building a Timeless Aesthetic
        </h4>
<p className="mt-2 text-sm rounded-none text-neutral-400" style={{}}>Creating images that transcend trends by focusing on light,
          form, and honest expression.</p>
</div>
</article>
<article className="ring-1 pt-1 pr-1 pb-1 pl-1 text-sm rounded-none ring-neutral-800 bg-neutral-950">
<div className="overflow-hidden relative text-sm rounded-none">
<img alt="Studio black-and-white portrait with geometric light" className="h-[240px] md:h-[260px] w-full object-cover text-sm rounded-none" loading="lazy" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/779c923e-bcc8-49c7-9328-661a04d3f208_800w.jpg"/>
<span className="absolute right-3 top-3 px-3 py-1 font-medium shadow-sm backdrop-blur text-sm rounded-none bg-black/90 text-neutral-200" style={{}}>Studio</span>
</div>
<div className="px-4 pt-4 pb-6 text-sm rounded-none">
<p className="font-medium text-neutral-500 text-sm rounded-none" style={{}}>May 16, 2025</p>
<h4 className="mt-1 font-semibold tracking-tight text-sm rounded-none text-neutral-100" style={{}}>Building a Timeless Aesthetic
        </h4>
<p className="mt-2 text-sm rounded-none text-neutral-400" style={{}}>Creating images that transcend trends by focusing on light,
          form, and honest expression.</p>
</div>
</article>
</div>

<div className="mt-6 sm:hidden text-sm rounded-none">
<a className="inline-flex items-center gap-2 px-4 py-2 font-medium transition ring-1 w-full justify-center text-sm rounded-none bg-neutral-900 ring-neutral-800 hover:bg-neutral-800 text-neutral-100" href="#" style={{}}>
      View all projects
      <svg className="lucide lucide-plus h-4 w-4 text-sm rounded-none" data-lucide="plus" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="M12 5v14"></path></svg>
</a>
</div>
</section>
</div><div className="max-w-7xl mr-auto ml-auto pr-6 pl-6">

<div className="text-center">
<div className="inline-flex gap-2 shadow-neutral-500/20 border rounded-full mr-auto ml-auto pt-1.5 pr-4 pb-1.5 pl-1.5 shadow-sm items-center bg-white/5 border-white/10">
<span className="inline-flex items-center justify-center w-5 h-5 rounded-full ring-1 bg-neutral-600 ring-white/20">
<svg className="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" strokeWidth="2" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path className="" d="M12 8V4H8"></path><rect className="" height="12" rx="2" width="16" x="4" y="8"></rect><path className="" d="M2 14h2"></path><path className="" d="M20 14h2"></path><path className="" d="M15 13v2"></path><path className="" d="M9 13v2"></path></svg>
</span>
<span className="text-xs font-medium font-sans text-white/80">AI Portfolio</span>
</div>
<h2 className="mt-6 text-4xl sm:text-6xl md:text-7xl leading-[1.06] tracking-tighter font-sans font-semibold">
        Intelligent Solutions
        <span className="font-semibold text-white/80">Delivered with Precision.</span>
</h2>
<p className="mx-auto mt-4 max-w-2xl text-base sm:text-lg font-sans text-white/70">
        Our AI-powered projects showcase how applied intelligence transforms businesses, delivering measurable results
        and breakthrough innovations.
      </p>
<div className="mt-8">
<a className="hidden sm:inline-flex items-center gap-2 hover:from-neutral-500 transition-colors shadow-blue-500/25 text-sm font-medium bg-gradient-to-l rounded-full pt-2 pr-4 pb-2 pl-4 shadow-md font-sans hover:to-neutral-700 text-white from-neutral-600 to-neutral-800" href="#">View
          AI Solutions<svg className="lucide lucide-arrow-right w-4 h-4" data-lucide="arrow-right" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg></a>
</div>
</div>

<section className="sm:px-8 max-w-7xl mr-auto ml-auto pr-6 pl-6">
</section>

<div className="grid grid-cols-1 lg:grid-cols-4 gap-4 bg-[#000000] border rounded-3xl mt-10 pt-4 pr-4 pb-4 pl-4 border-slate-50/10">

<div className="group relative overflow-hidden sm:p-6 hover:bg-[radial-gradient(circle_at_top,var(--tw-gradient-stops))] transition-all duration-300 border rounded-2xl pt-5 pr-5 pb-5 pl-5 hover:from-neutral-400/30 hover:to-neutral-400/0 hover:border-slate-50/20 bg-white/5 border-white/10">
<div className="pointer-events-none absolute inset-0 opacity-[0.07]" style={{backgroundImage: 'radial-gradient(circle at 1px 1px, rgba(255,255,255,0.9) 1px, transparent 1px)', backgroundSize: '14px 14px'}}>
</div>
<div className="flex items-center gap-3">
<span className="inline-flex items-center justify-center rounded-full ring-1 px-2.5 py-1 text-xs font-sans bg-white/5 ring-white/15 text-white/80">2024</span>
<p className="text-sm font-sans text-white/70">FinanceAI Pro</p>
</div>
<h3 className="sm:text-2xl text-xl tracking-tight mt-3 font-sans font-semibold">Intelligent Trading Bot</h3>
<ul className="mt-4 grid grid-cols-1 gap-2">
<li className="inline-flex items-center gap-2 text-sm font-sans text-white/80">
<span className="inline-flex w-5 h-5 items-center justify-center ring-1 bg-neutral-500/10 rounded-full ring-neutral-300/20">
<svg className="w-3.5 h-3.5 text-indigo-300" fill="none" stroke="currentColor" strokeWidth="2" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path className="" d="m20 6-11 11-5-5"></path></svg>
</span>
            Algorithmic Trading
          </li>
<li className="inline-flex items-center gap-2 text-sm font-sans text-white/80">
<span className="inline-flex w-5 h-5 items-center justify-center ring-1 bg-neutral-500/10 rounded-full ring-neutral-300/20">
<svg className="w-3.5 h-3.5 text-indigo-300" fill="none" stroke="currentColor" strokeWidth="2" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path className="" d="m20 6-11 11-5-5"></path></svg>
</span>
            Risk Management
          </li>
<li className="inline-flex items-center gap-2 text-sm font-sans text-white/80">
<span className="inline-flex w-5 h-5 items-center justify-center ring-1 bg-neutral-500/10 rounded-full ring-neutral-300/20">
<svg className="w-3.5 h-3.5 text-indigo-300" fill="none" stroke="currentColor" strokeWidth="2" viewbox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path className="" d="m20 6-11 11-5-5"></path></svg>
</span>
            Real-time Analysis
          </li>
</ul>
<div className="mt-4 flex flex-wrap gap-2">
<span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs ring-1 font-sans bg-white/5 text-white/70 ring-white/10">FinTech</span>
<span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs ring-1 font-sans bg-white/5 text-white/70 ring-white/10">Automation</span>
</div>
</div>

<div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-3 gap-4">
<div className="relative overflow-hidden md:col-span-2 border rounded-2xl bg-white/5 border-white/10">
<img alt="Trading Dashboard" className="w-full h-full opacity-90 hover:opacity-100 transition object-cover grayscale" src="https://cdn.midjourney.com/05ee647a-401b-40dd-985c-f81f5fcc0ac4/0_0.png?w=800&amp;q=80" style={{}}/>
</div>
<div className="relative overflow-hidden border rounded-2xl bg-white/5 border-white/10">
<img alt="Smart Shopping Interface" className="w-full h-full opacity-90 hover:opacity-100 transition object-cover grayscale" src="https://cdn.midjourney.com/7519434b-08d7-48f3-baf1-6ee3ec8ab7b0/0_0.png?w=800&amp;q=80"/>
</div>
</div>
</div>
</div>
</section><div className="flex md:hidden mt-4 items-center justify-between" style={{display: 'none'}}>
<button className="rounded-xl px-4 py-2 text-sm ring-1 font-sans bg-white/5 text-zinc-200 ring-white/10" style={{}}>
            Contact
          </button>
<button className="rounded-xl px-4 py-2 text-sm ring-1 font-sans bg-white text-black ring-black/10" style={{}}>
            Join waitlist
          </button>
</div>


<section className="animate-fade-in opacity-0 max-w-7xl mr-auto ml-auto pr-6 pl-6" style={{animationDelay: '1s'}}>
</section><section className="max-w-7xl mt-10 mr-auto mb-20 ml-auto pr-6 pl-6">
<div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 space-y-6 gap-x-6 gap-y-6" id="gallery" style={{-Cols: '4'}}>

<div className="gallery-item group overflow-hidden transition-all duration-300 break-inside-avoid border rounded-2xl mb-6 relative hover:border-white/20 bg-white/5 border-white/10">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/ecd8a16e-643b-416c-b628-c9fde1f4f660_800w.webp"/>
<div className="group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent opacity-0 absolute top-0 right-0 bottom-0 left-0 from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/f7db323b-b62c-48e9-ba21-8ed554e7d2a8_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans">
              CyberVision
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full object-cover group-hover:scale-110 transition-transform duration-700" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/48a28117-7409-4626-895f-4839a0dec5ec_800w.webp" style={{aspectRatio: '3/4'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/386aed4d-4457-47f0-9edf-23b08539d1e4_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              NeonDreams
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/5aa0ce1a-38d3-4eb1-9b04-be1f2c8edf8b_800w.jpg" style={{aspectRatio: '2/3'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border border-white/20" src="https://i.pravatar.cc/40?img=3"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              QuantumArt
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/b0ba932f-461e-435d-bdf9-a0e9136b08e0_800w.jpg"/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/5865c193-61b7-47c8-899a-c6fa35826688_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              VoxelStudio
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/a3c9a9ca-5bde-46fb-b5fa-c3a22ee34a3f_800w.webp" style={{aspectRatio: '3/4'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/a858baac-5596-4de9-8229-7a3c2cd9a05c_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              FluxCanvas
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item group overflow-hidden transition-all duration-300 break-inside-avoid border rounded-2xl mb-6 relative hover:border-white/20 bg-white/5 border-white/10">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/3c4f1dea-f838-465a-bfd2-8fbbf334a6bc_800w.webp" style={{aspectRatio: '2/3'}}/>
<div className="group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent opacity-0 absolute top-0 right-0 bottom-0 left-0 from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/625569f6-2546-4071-9e1a-1a5d8f130ba3_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              ChromaMind
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/95221975-49b7-4db6-a8fe-15f9a78f1303_800w.webp"/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/99ffb7cb-ea18-479c-93b7-7dda9534f513_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              PulseCreate
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/ed239d63-c20e-4e7d-b60a-fa07e64a867a_800w.jpg" style={{aspectRatio: '3/4'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/dc92b0c1-b36d-4071-a25e-0ff83ab8cc7a_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              ZenithLab
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/048e0a68-a97c-46dd-aed4-734f98009a4c_800w.webp" style={{aspectRatio: '2/3'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/5d577f83-6ff9-4ae7-b5cb-ccfd2ba39458_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              PixelForge
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/5f268bed-1724-4f96-9a60-a6bf8a0406d6_800w.webp"/>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/c8b41f45-708f-4821-8903-b4825f2f3237_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              ArtisanAI
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/e0e6b5ac-24f8-446f-8b42-e4891f8f181f_800w.webp" style={{aspectRatio: '3/4'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/e8252582-b1a9-4c58-a483-9b55d2a9eb29_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              VisionaryArt
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/25928c2a-a773-4535-906c-da729deae044_800w.webp" style={{aspectRatio: '2/3'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border border-white/20" src="https://i.pravatar.cc/40?img=12"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              SynthWave
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/0d9b3fd8-338f-430e-86d7-21702b5f5d29_800w.webp"/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/e9542191-4df3-4ba8-85c1-2b738ef7c3d2_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              LuminaArts
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/6b2a6ec7-ff19-482c-9f75-2569234343b5_800w.jpg" style={{aspectRatio: '3/4'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border border-white/20" src="https://i.pravatar.cc/40?img=14"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              DreamScape
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/9c4f47dd-cdd9-48fb-ac75-7df83023df8c_800w.jpg" style={{aspectRatio: '2/3'}}/>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/4608706c-4ba5-47d2-aab2-e92a101cee39_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              EtherealArt
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/2cd506d0-dd27-4384-a2f9-785ea63b5a91_800w.webp"/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/6f74f270-3a34-48bf-af87-e0e7f4b87bb8_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              CosmicCanvas
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/d4814233-7ad0-4e94-beb2-6f61b78cd196_800w.webp" style={{aspectRatio: '3/4'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/b5cf61fd-d2fd-4bc9-995d-c404cfba9937_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              NovaStudio
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/17355c23-5710-4c89-bd15-5b7ed9da596c_800w.webp" style={{aspectRatio: '2/3'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/3aa0ef45-8616-4670-ad2b-4a676949289c_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              AuroraDesign
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/762ce684-abe6-4cba-8859-3d870a3b39e8_800w.webp"/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border border-white/20" src="https://i.pravatar.cc/40?img=19"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              SpectraLab
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
<div className="gallery-item relative group overflow-hidden rounded-2xl border transition-all duration-300 break-inside-avoid mb-6 bg-white/5 border-white/10 hover:border-white/20">
<img alt="" className="w-full group-hover:scale-110 transition-transform duration-700 object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/7dd59aaa-8029-485e-a5f1-82f2ba5cad29_800w.webp" style={{aspectRatio: '3/4'}}/>
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t via-transparent to-transparent from-black/70">
</div>
<div className="absolute top-3 left-3 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<img alt="" className="w-6 h-6 rounded-full border object-cover border-white/20" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/7dd0000d-1655-4086-b7f0-f7de221f6d9f_320w.webp"/>
<span className="text-xs font-light tracking-normal font-sans" style={{}}>
              InfinityArts
            </span>
</div>
<div className="absolute bottom-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"></path>
</svg>
</button>
<button className="p-2 rounded-full transition-colors duration-200 bg-black/50 hover:bg-black/70">
<svg className="w-4 h-4" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
<path d="M12 15V3"></path>
<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
<path d="m7 10 5 5 5-5"></path>
</svg>
</button>
</div>
</div>
</div>
</section><footer className="bg-[#0b0b16] border-t mt-32 border-white/10">
<div className="md:py-24 max-w-6xl mr-auto ml-auto pt-16 pb-16">
<div className="mb-12">
<div className="flex items-center gap-3 mb-6">
<span className="uppercase text-xs font-medium tracking-widest font-sans text-white/50" style={{}}>Services</span>
<div className="h-px flex-1 bg-white/10"></div>
</div>
<h2 className="md:text-4xl text-3xl tracking-tight mb-4 font-sans font-semibold text-white">Как я работаю</h2>
<p className="lg:text-lg leading-relaxed text-base mb-8 font-sans text-cyan-300" style={{}}>From initial concept
      to final deployment, I provide comprehensive design and development services tailored to your needs.</p>
</div>
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">

<div className="group relative transition-all duration-500 overflow-hidden border rounded-2xl hover:bg-white/10 bg-white/5 border-white/10">

<div className="relative overflow-hidden h-80 border rounded-xl bg-cyan-900/90 border-white/10">

<div className="pointer-events-none absolute right-2 top-12 bg-gradient-to-br to-transparent invisible w-[68%] h-[68%] border rounded-xl rotate-[8deg] from-white/5 border-white/5" style={{boxShadow: '0 20px 80px rgba(0,0,0,0.4)'}}></div>

<div className="relative z-10 mx-auto mt-6 w-[88%] h-[76%] rounded-xl border shadow-[0_32px_80px_rgba(0,0,0,0.7)] bg-cyan-950 border-white/15">

<div className="flex items-center justify-between p-4 border-b border-white/5">
<div className="flex items-center gap-2">
<div className="bg-red-500/80 w-3 h-3 rounded-full"></div>
<div className="w-3 h-3 rounded-full bg-yellow-500/80"></div>
<div className="w-3 h-3 rounded-full bg-cyan-500/80"></div>
</div>
<p className="text-xs font-sans text-white/60" style={{}}>Strategy Board</p>
</div>

<div className="px-5 pt-4">
<div className="space-y-3">
<div className="h-2 w-8 rounded-full bg-blue-400/60"></div>
<div className="h-2 w-32 rounded-full bg-white/20"></div>
<div className="h-10 w-full rounded-lg border flex items-center px-3 border-white/10 bg-white/5">
<div className="w-6 h-2 rounded-full" style={{background: 'linear-gradient(90deg, #FF6A1A 0%, #FF8A3A 100%)'}}>
</div>
</div>
<div className="grid grid-cols-3 gap-2 mt-4">
<div className="h-12 rounded-lg border bg-white/5 border-white/10"></div>
<div className="h-12 rounded-lg border bg-blue-400/10 border-blue-400/20"></div>
<div className="h-12 rounded-lg border bg-white/5 border-white/10"></div>
</div>
</div>
</div>

<div className="absolute right-4 bottom-6 opacity-90 text-blue-400">
</div>
</div>
</div>

<div className="pt-6 pr-6 pb-6 pl-6">
<div className="flex mb-4 items-center justify-between">
<span className="text-lg font-semibold tracking-tight font-sans text-blue-400" style={{}}>01</span>
</div>
<h3 className="text-2xl tracking-tight mb-3 font-sans font-semibold text-white" style={{}}>Strategy &amp; Research</h3>
<p className="leading-relaxed text-sm font-sans text-white/60" style={{}}>I start by understanding your goals,
          researching your market, and crafting a strategic foundation for success.</p>
</div>
</div>

<div className="group relative rounded-2xl border transition-all duration-500 overflow-hidden border-white/10 bg-white/5 hover:bg-white/10">

<div className="relative overflow-hidden h-80 border rounded-xl bg-cyan-900/90 border-white/10">


<div className="relative z-10 w-[88%] h-[76%] border rounded-xl mt-6 mr-auto ml-auto shadow-[0_32px_80px_rgba(0,0,0,0.7)] bg-cyan-950 border-white/15">

<div className="flex border-b pt-4 pr-4 pb-4 pl-4 items-center justify-between border-white/5">
<div className="flex items-center gap-2">
<div className="w-3 h-3 rounded-full bg-red-500/80"></div>
<div className="w-3 h-3 rounded-full bg-yellow-500/80"></div>
<div className="w-3 h-3 rounded-full bg-cyan-500/80"></div>
</div>
<p className="text-xs font-sans text-white/60" style={{}}>Design System</p>
</div>

<div className="px-5 pt-4">
<div className="space-y-4">

<div className="flex items-center gap-3">
<div className="w-4 h-4 rounded-full" style={{background: 'linear-gradient(135deg, #FF6A1A 0%, #FF8A3A 100%)'}}></div>
<div className="w-4 h-4 rounded-full bg-blue-400"></div>
<div className="w-4 h-4 rounded-full bg-indigo-400"></div>
<div className="w-4 h-4 rounded-full bg-white/20"></div>
</div>

<div className="space-y-3">
<div className="h-8 w-24 rounded-lg" style={{background: 'linear-gradient(90deg, #FF6A1A 0%, #FF8A3A 100%)'}}>
</div>
<div className="h-6 w-full rounded-lg bg-white/10"></div>
<div className="flex gap-2">
<div className="h-4 w-16 rounded-full bg-white/20"></div>
<div className="h-4 w-12 rounded-full bg-blue-400/30"></div>
</div>
</div>
</div>
</div>

<div className="absolute right-5 bottom-5 opacity-90 text-indigo-400">
</div>
</div>
</div>

<div className="p-6">
<div className="flex items-center justify-between mb-4">
<span className="text-lg font-semibold tracking-tight font-sans text-indigo-400" style={{}}>02</span>
</div>
<h3 className="text-2xl tracking-tight mb-3 font-sans font-semibold text-white" style={{}}>Design &amp; Prototyping</h3>
<p className="leading-relaxed text-sm font-sans text-white/60" style={{}}>Creating beautiful, functional designs
          with interactive prototypes that bring your vision to life.</p>
</div>
</div>

<div className="group relative rounded-2xl border transition-all duration-500 overflow-hidden border-white/10 bg-white/5 hover:bg-white/10">

<div className="relative h-80 rounded-xl border overflow-hidden bg-cyan-900/90 border-white/10">


<div className="relative z-10 w-[88%] h-[76%] border rounded-xl mt-6 mr-auto ml-auto shadow-[0_32px_80px_rgba(0,0,0,0.7)] bg-cyan-950 border-white/15">

<div className="flex border-b pt-4 pr-4 pb-4 pl-4 items-center justify-between border-white/5">
<div className="flex items-center gap-2">
<div className="w-3 h-3 rounded-full bg-red-500/80"></div>
<div className="w-3 h-3 rounded-full bg-yellow-500/80"></div>
<div className="w-3 h-3 rounded-full bg-cyan-500/80"></div>
</div>
<p className="text-xs font-sans text-white/60" style={{}}>development.js</p>
</div>

<div className="px-5 pt-4 space-y-3">
<div className="flex items-center gap-2">
<span className="text-xs font-sans text-blue-400" style={{}}>const</span>
<div className="h-2 w-16 rounded-full bg-indigo-400/60"></div>
<div className="h-2 w-8 rounded-full bg-white/30"></div>
</div>
<div className="flex items-center gap-2">
<div className="h-2 w-4 rounded-full bg-purple-400/60"></div>
<div className="h-2 w-20 rounded-full bg-white/20"></div>
</div>
<div className="pl-4 space-y-2">
<div className="h-8 w-full rounded-lg bg-gradient-to-r border from-blue-400/10 to-indigo-400/10 border-blue-400/20">
</div>
<div className="flex gap-3">
<div className="h-2 w-12 rounded-full" style={{background: 'linear-gradient(90deg, #FF6A1A 0%, #FF8A3A 100%)'}}></div>
<div className="h-2 w-6 rounded-full bg-blue-400/60"></div>
</div>
</div>

<div className="flex items-center gap-2 mt-4 px-3 py-2 rounded-lg border bg-blue-400/10 border-blue-400/20">
<div className="w-2 h-2 rounded-full bg-blue-400"></div>
<span className="text-xs font-sans text-blue-400" style={{}}>Build successful</span>
</div>
</div>

<div className="absolute right-4 bottom-6 opacity-90 text-purple-400">
</div>
</div>
</div>

<div className="p-6">
<div className="flex items-center justify-between mb-4">
<span className="text-lg font-semibold tracking-tight font-sans text-purple-400" style={{}}>03</span>
</div>
<h3 className="text-2xl tracking-tight mb-3 font-sans font-semibold text-white" style={{}}>Development &amp; Launch</h3>
<p className="leading-relaxed text-sm font-sans text-white/60" style={{}}>Building high-performance, responsive
          experiences with clean code and seamless deployment.</p>
</div>
</div>
</div>
</div><div className="sm:px-6 sm:mt-10 max-w-7xl mt-8 mr-auto mb-16 ml-auto pr-4 pl-4">
<div className="overflow-hidden sm:p-8 border rounded-[40px] pt-6 pr-6 pb-6 pl-6 relative shadow-[0_8px_30px_rgba(0,0,0,0.18)] text-white bg-cyan-950 border-white/10">
<div className="absolute inset-0 pointer-events-none">
<div className="absolute inset-0 bg-[radial-gradient(1200px_600px_at_20%_-20%,rgba(255,255,255,0.06),transparent_60%)]"></div>
<div className="absolute inset-0 bg-[radial-gradient(1200px_600px_at_80%_120%,rgba(255,255,255,0.05),transparent_60%)]"></div>
<div className="absolute inset-0 bg-[radial-gradient(#ffffff0d_1px,transparent_1px)] [background-size:20px_20px] opacity-[0.15]"></div>
</div>
<div className="relative">
<h2 className="text-[16vw] sm:text-[12vw] lg:text-[9vw] leading-[0.9] font-semibold tracking-tighter font-geist">
<span className="block font-sans font-semibold">Ready to build</span>
<span className="block font-sans font-semibold text-white/60">something extraordinary?</span>
</h2>
<div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">

<div className="">
<p className="text-sm font-sans text-white/60">Get Started</p>
<a className="mt-2 inline-flex items-center gap-3 text-xl sm:text-2xl font-medium tracking-tight font-geist text-white" href="mailto:hello@studiomeridian.co">
<svg className="lucide lucide-mail w-5 h-5 stroke-[1.5] flex-shrink-0" data-lucide="mail" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="m22 7-8.991 5.727a2 2 0 0 1-2.009 0L2 7"></path><rect height="16" rx="2" width="20" x="2" y="4"></rect></svg>
<span className="break-all font-sans font-semibold">hello@studiomeridian.co</span>
</a>
</div>

<div className="">
<p className="text-sm font-sans text-white/60">Schedule a Call</p>
<a className="inline-flex items-center gap-2 text-sm font-medium tracking-tight border rounded-full mt-2 pt-3 pr-5 pb-3 pl-5 transition-colors duration-200 hover:bg-white/90 text-indigo-900 bg-white border-white/10" href="#contact">
<svg className="lucide lucide-calendar" data-lucide="calendar" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M8 2v4"></path><path d="M16 2v4"></path><rect height="18" rx="2" width="18" x="3" y="4"></rect><path d="M3 10h18"></path></svg>
<span className="font-sans">Book a Meeting</span>
</a>
</div>

<div className="">
<p className="text-sm font-sans text-white/60">Follow Along</p>
<div className="flex flex-wrap gap-3 mt-2 items-center">
<a className="inline-flex items-center gap-2 text-sm font-medium tracking-tight border rounded-full pt-3 pr-4 pb-3 pl-4 transition-colors duration-200 text-indigo-900 bg-white border-white/10 hover:bg-white/90" href="#">
<svg className="lucide lucide-twitter" data-lucide="twitter" fill="none" height="16" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"></path></svg>
<span className="font-sans">2,843</span>
</a>
<a className="inline-flex items-center justify-center w-12 h-12 rounded-full border transition-colors duration-200 bg-white text-indigo-900 border-white/10 hover:bg-white/90" href="#">
<svg className="lucide lucide-instagram" data-lucide="instagram" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><rect height="20" rx="5" ry="5" width="20" x="2" y="2"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" x2="17.51" y1="6.5" y2="6.5"></line></svg>
</a>
<a className="inline-flex items-center justify-center w-12 h-12 rounded-full border transition-colors duration-200 bg-white text-indigo-900 border-white/10 hover:bg-white/90" href="#">
<svg className="lucide lucide-dribbble" data-lucide="dribbble" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10"></circle><path d="M19.13 5.09C15.22 9.14 10 10.44 2.25 10.94"></path><path d="M21.75 12.84c-6.62-1.41-12.14 1-16.38 6.32"></path><path d="M8.56 2.75c4.37 6 6 9.42 8 17.72"></path></svg>
</a>
<a className="inline-flex items-center justify-center w-12 h-12 border rounded-full transition-colors duration-200 text-indigo-900 bg-white border-white/10 hover:bg-white/90" href="#">
<svg className="lucide lucide-behance" data-lucide="behance" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="M2.5 17a24.12 24.12 0 0 1 0-10 2 2 0 0 1 1.4-1.4 49.56 49.56 0 0 1 16.2 0A2 2 0 0 1 21.5 7a24.12 24.12 0 0 1 0 10 2 2 0 0 1-1.4 1.4 49.55 49.55 0 0 1-16.2 0A2 2 0 0 1 2.5 17"></path><path d="m10 15 5-3-5-3z"></path></svg>
</a>
<a className="inline-flex items-center justify-center w-12 h-12 rounded-full border transition-colors duration-200 bg-white text-indigo-900 border-white/10 hover:bg-white/90" href="#">
<svg className="lucide lucide-linkedin" data-lucide="linkedin" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect height="12" width="4" x="2" y="9"></rect><circle cx="4" cy="4" r="2"></circle></svg>
</a>
</div>
</div>
</div>
<div className="mt-8 border-t border-white/10"></div>
<div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">

<div className="">
<p className="text-sm font-sans text-white/60">Explore</p>
<div className="mt-3 grid grid-cols-2 gap-2 text-sm">
<a className="font-medium tracking-tight hover:underline font-sans" href="#work">Our Work</a>
<a className="font-medium tracking-tight hover:underline font-sans" href="#services">What We Do</a>
<a className="font-medium tracking-tight hover:underline font-sans" href="#pricing">Investment</a>
<a className="font-medium tracking-tight hover:underline font-sans" href="#blog">Insights</a>
</div>
</div>

<div className="">
<p className="text-sm font-sans text-white/60">Fine Print</p>
<div className="mt-3 grid grid-cols-1 gap-2 text-sm">
<a className="font-medium tracking-tight hover:underline font-sans" href="#">Terms &amp; Conditions</a>
<a className="font-medium tracking-tight hover:underline font-sans" href="#">Privacy Notice</a>
</div>
</div>
</div>
<p className="mt-6 text-center text-xs font-sans text-white/70">© <span className="font-sans" id="year">2025</span> Studio Meridian — Crafted with care in San Francisco</p>
</div>
</div>
</div>
</footer>


    </>
  );
}

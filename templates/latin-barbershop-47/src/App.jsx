...
gtag('config', 'G-2M6V79H761');
        ...
        const toggleBtn = document.getElementById('toggle-more-services');
        const hiddenGrid = document.getElementById('more-services-grid');
        const toggleIcon = document.getElementById('more-services-icon');

        toggleBtn.addEventListener('click', () => {
            hiddenGrid.classList.toggle('hidden');
            if(!hiddenGrid.classList.contains('hidden')) {
                toggleIcon.style.transform = 'rotate(180deg)';
                setTimeout(() => hiddenGrid.classList.add('is-visible'), 50);
            } else {
        ...
        function updateStatus() {
            ...
            let isOpen = false;
            if (day >= 1 && day <= 5 && hour >= 10 && hour < 21) isOpen = true; // Mon-Fri 10am-9pm
            if ((day === 0 || day === 6) && hour >= 10 && hour < 19) isOpen = true; // Sat-Sun 10am-7pm
        ...
        const servicesData = [
            { id: 's1', category: 'Cortes', name: "Men's Haircut", price: 35, duration: '45m' },
            { id: 's2', category: 'Cortes', name: "Women's Haircut", price: 40, duration: '1h' },
            { id: 's3', category: 'VIP', name: "VIP Treatment", price: 60, duration: '1h30' },
            { id: 's4', category: 'Cabello y Barba', name: "Hair & Beard", price: 45, duration: '1h' },
            { id: 's5', category: 'Barba', name: "Beard Trim", price: 25, duration: '30m' },
            { id: 's6', category: 'Faciales', name: "Basic Facial", price: 15, duration: '15m' },
        ];
        ...
        function generateDates() {
            ...
            for(let i=0; i<8; i++) {
                ...
                `;
                btn.onclick = () => selectDate(fullDate, btn, isWeekend, `${dayName}, ${month} ${dayNum}`);
                grid.appendChild(btn);
            }
        }
        generateDates();

        function selectDate(dateStr, el, isWeekend, displayStr) {
        ...
        function generateTimes(isWeekend) {
        ...
        function resetBooking() {
            bookingState = { step: 1, barber: 'Angel Tamayo', service: null, date: null, time: null };
...
<a className="flex flex-col justify-center group relative" href="#">
...
<span className="text-[#CC1F1F]">LIONS</span> STYLE
                    </span>
...
<a className="hover:text-white hover:drop-shadow-[0_0_8px_rgba(0,207,207,0.5)] transition-all" href="#barber">Our Barber</a>
...
<span className="text-base text-[#AAAAAA] font-medium">Montreal, QC</span>
</div>
<h1 className="font-heading text-6xl md:text-8xl lg:text-9xl tracking-tight font-semibold text-white mb-6 uppercase leading-[0.9]">
                Fierce <span className="text-[#CC1F1F] drop-shadow-[0_0_15px_rgba(204,31,31,0.4)]">Precision</span><br/>
                Premium <span className="text-[#00CFCF] drop-shadow-[0_0_15px_rgba(0,207,207,0.4)]">Style</span>
</h1>
<p className="text-xl md:text-2xl text-[#AAAAAA] mb-10 max-w-2xl mx-auto font-medium">
                Más que un corte — es tu estilo. Premium Latin grooming designed to unleash your inner lion.
            </p>
...
<h2 className="font-heading text-5xl md:text-6xl tracking-tight font-semibold mb-6 uppercase">
                        The Latin <span className="text-[#CC1F1F]">Standard</span>
</h2>
<p className="text-lg md:text-xl text-[#AAAAAA] mb-6 leading-relaxed">
                        We are a Latin barbershop in the heart of Montreal, blending tradition with a fierce modern style. Our space is designed for the modern man who values precision, atmosphere, and a flawless finish.
                    </p>
<p className="text-lg md:text-xl text-[#AAAAAA] leading-relaxed border-l-2 border-[#00CFCF] pl-4">
                        Our team of skilled barbers is dedicated to making you look and feel your best, offering everything from classic cuts to modern fades and precise beard alignments.
                    </p>
</div>
...
<img alt="Barber Tools" className="relative rounded-xl w-full h-[500px] object-cover border border-[#00CFCF]/30 grayscale group-hover:grayscale-0 transition-all duration-700 shadow-[0_0_30px_rgba(0,0,0,0.8)]" src="https://images.unsplash.com/photo-1503951914875-452162b0f3f1?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=1470&amp;q=80"/>
</div>
</div>
</div>
</section>
...
<div className="absolute -inset-1 bg-gradient-to-r from-[#CC1F1F] to-[#00CFCF] rounded-xl blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
<img alt="Angel Tamayo" className="relative rounded-xl w-full h-[600px] object-cover object-top border border-[#161616] grayscale-[50%] group-hover:grayscale-0 transition-all duration-700" src="https://images.unsplash.com/photo-1622286342621-4bd786c2447c?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=1470&amp;q=80"/>
</div>
...
<span className="text-sm font-semibold uppercase tracking-wider">Master Barber</span>
</div>
<h2 className="font-heading text-5xl md:text-7xl tracking-tight font-semibold mb-2">Angel Tamayo</h2>
<h3 className="text-2xl text-[#00CFCF] font-medium mb-8">Barber Angel</h3>
<p className="text-lg md:text-xl text-[#AAAAAA] mb-10 leading-relaxed border-l-2 border-[#CC1F1F] pl-6">
                        "Angel brings precision and passion to every cut. Specializing in men's cuts, fades, and beard work, he'll craft the perfect look tailored specifically to your features and lifestyle."
                    </p>
<a className="inline-flex items-center gap-3 bg-white text-black font-semibold rounded-md px-8 py-4 text-lg hover:bg-gray-200 hover:shadow-[0_0_20px_rgba(255,255,255,0.3)] transition-all group" href="#booking" onclick="selectBarber('Angel Tamayo')">
                        Book with Angel
...
<h2 className="font-heading text-5xl md:text-6xl tracking-tight font-semibold mb-4 uppercase">Our <span className="text-[#CC1F1F]">Services</span></h2>
<p className="text-xl text-[#00CFCF] tracking-widest uppercase text-sm font-semibold">Nuestros Servicios</p>
</div>
<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 fade-in-section">

<div className="space-y-6">
<h3 className="font-heading text-3xl tracking-tight font-semibold text-white/90 border-b border-[#00CFCF]/20 pb-4 uppercase">Haircut / Cortes</h3>
...
<div>
<h4 className="text-xl font-semibold text-white group-hover:text-[#CC1F1F] transition-colors">Men's Haircut</h4>
<p className="text-base text-[#AAAAAA] mt-1">~45 min</p>
</div>
<div className="text-right">
<span className="text-sm text-[#AAAAAA] mr-1">Starting at</span>
<span className="text-2xl font-semibold text-[#00CFCF] group-hover:text-white transition-colors">$35</span>
</div>
</div>
<div className="group bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] hover:shadow-[0_0_15px_rgba(204,31,31,0.15)] transition-all flex justify-between items-center">
<div>
<h4 className="text-xl font-semibold text-white group-hover:text-[#CC1F1F] transition-colors">Student Men's Cut</h4>
<p className="text-base text-[#AAAAAA] mt-1">Mon-Tue only, ID required • ~45 min</p>
</div>
<div className="text-2xl font-semibold text-[#00CFCF] group-hover:text-white transition-colors">$30</div>
</div>
<div className="group bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] hover:shadow-[0_0_15px_rgba(204,31,31,0.15)] transition-all flex justify-between items-center">
<div>
<h4 className="text-xl font-semibold text-white group-hover:text-[#CC1F1F] transition-colors">Boy under 12</h4>
<p className="text-base text-[#AAAAAA] mt-1">~45 min</p>
</div>
<div className="text-right">
<span className="text-sm text-[#AAAAAA] mr-1">Starting at</span>
<span className="text-2xl font-semibold text-[#00CFCF] group-hover:text-white transition-colors">$25</span>
</div>
</div>
<div className="group bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] hover:shadow-[0_0_15px_rgba(204,31,31,0.15)] transition-all flex justify-between items-center">
<div>
<h4 className="text-xl font-semibold text-white group-hover:text-[#CC1F1F] transition-colors">Women's Haircut</h4>
<p className="text-base text-[#AAAAAA] mt-1">~1 hr</p>
</div>
<div className="text-right">
<span className="text-sm text-[#AAAAAA] mr-1">Starting at</span>
<span className="text-2xl font-semibold text-[#00CFCF] group-hover:text-white transition-colors">$40</span>
</div>
</div>
<div className="group bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] hover:shadow-[0_0_15px_rgba(204,31,31,0.15)] transition-all flex justify-between items-center">
<div>
<h4 className="text-xl font-semibold text-white group-hover:text-[#CC1F1F] transition-colors">Girl under 12</h4>
<p className="text-base text-[#AAAAAA] mt-1">~1 hr</p>
</div>
<div className="text-right">
<span className="text-sm text-[#AAAAAA] mr-1">Starting at</span>
<span className="text-2xl font-semibold text-[#00CFCF] group-hover:text-white transition-colors">$30</span>
</div>
</div>
<div className="group bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] hover:shadow-[0_0_15px_rgba(204,31,31,0.15)] transition-all flex justify-between items-center">
<div>
<h4 className="text-xl font-semibold text-white group-hover:text-[#CC1F1F] transition-colors">Women's Cut + Blowout</h4>
<p className="text-base text-[#AAAAAA] mt-1">~1h 30min</p>
</div>
<div className="text-right">
<span className="text-sm text-[#AAAAAA] mr-1">Starting at</span>
<span className="text-2xl font-semibold text-[#00CFCF] group-hover:text-white transition-colors">$70</span>
</div>
</div>
</div>
</div>
...
<div className="space-y-6">
<h3 className="font-heading text-3xl tracking-tight font-semibold text-white/90 border-b border-[#00CFCF]/20 pb-4 uppercase">Combos &amp; VIP</h3>
<div className="space-y-4">
...
<div>
<h4 className="text-xl font-semibold text-[#CC1F1F]">VIP Treatment</h4>
<p className="text-base text-[#AAAAAA] mt-1">Beard alignment + haircut + facial cleanse</p>
<p className="text-sm text-[#AAAAAA] opacity-70 mt-1">~1h 30min</p>
</div>
<div className="text-3xl font-semibold text-white pl-4">$60</div>
</div>
<div className="group bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] hover:shadow-[0_0_15px_rgba(204,31,31,0.15)] transition-all flex justify-between items-center">
<div>
<h4 className="text-xl font-semibold text-white group-hover:text-[#CC1F1F] transition-colors">Hair &amp; Beard</h4>
<p className="text-base text-[#AAAAAA] mt-1">~1 hr</p>
</div>
<div className="text-right">
<span className="text-sm text-[#AAAAAA] mr-1">Starting at</span>
<span className="text-2xl font-semibold text-[#00CFCF] group-hover:text-white transition-colors">$45</span>
</div>
</div>
</div>
</div>
...
<div className="space-y-6">
<h3 className="font-heading text-3xl tracking-tight font-semibold text-white/90 border-b border-[#00CFCF]/20 pb-4 uppercase">Grooming</h3>
<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
<div className="bg-[#161616] p-5 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] transition-colors flex flex-col justify-between group">
<h4 className="text-lg font-semibold text-white mb-2 group-hover:text-[#CC1F1F] transition-colors">Beard Trim</h4>
<div className="flex justify-between items-end">
<span className="text-sm text-[#AAAAAA]">~30 min</span>
<span className="text-xl font-semibold text-[#00CFCF]">from $25</span>
</div>
</div>
<div className="bg-[#161616] p-5 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] transition-colors flex flex-col justify-between group">
<h4 className="text-lg font-semibold text-white mb-2 group-hover:text-[#CC1F1F] transition-colors">Mustache</h4>
<div className="flex justify-between items-end">
<span className="text-sm text-[#AAAAAA]">~15 min</span>
<span className="text-xl font-semibold text-[#00CFCF]">$10</span>
</div>
</div>
<div className="bg-[#161616] p-5 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] transition-colors flex flex-col justify-between group">
<h4 className="text-lg font-semibold text-white mb-2 group-hover:text-[#CC1F1F] transition-colors">Basic Facial</h4>
<div className="flex justify-between items-end">
<span className="text-sm text-[#AAAAAA]">~15 min</span>
<span className="text-xl font-semibold text-[#00CFCF]">from $15</span>
</div>
</div>
<div className="bg-[#161616] p-5 rounded-xl border border-[#CC1F1F]/40 bg-gradient-to-br from-[#161616] to-[#1a0505] flex flex-col justify-between">
<h4 className="text-lg font-semibold text-[#CC1F1F] mb-2">Intensive Facial</h4>
<div className="flex justify-between items-end">
<span className="text-sm text-[#AAAAAA]">~30 min</span>
<span className="text-xl font-semibold text-white">$35</span>
</div>
</div>
</div>
</div>
...
<button className="inline-flex items-center gap-2 text-[#00CFCF] font-medium text-lg hover:text-[#CC1F1F] hover:drop-shadow-[0_0_8px_rgba(204,31,31,0.5)] transition-all" id="toggle-more-services">
                    View More Services (Color, Perms, Waxing)
                    <iconify-icon className="transition-transform duration-300" height="20" icon="solar:alt-arrow-down-linear" id="more-services-icon" style={{strokeWidth: '1.5'}} width="20"></iconify-icon>
</button>
</div>

<div className="hidden mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 fade-in-section" id="more-services-grid">
<div className="bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] transition-colors">
<h4 className="text-xl font-semibold text-white mb-4 uppercase">Perms / Rizados</h4>
<ul className="space-y-3">
<li className="flex justify-between text-lg"><span className="text-[#AAAAAA]">+ Long/Med Hair</span> <span className="font-medium text-[#00CFCF]">$150</span></li>
<li className="flex justify-between text-lg"><span className="text-[#AAAAAA]">+ Short Hair</span> <span>from <span className="font-medium text-[#00CFCF]">$135</span></span></li>
</ul>
</div>
<div className="bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] transition-colors">
<h4 className="text-xl font-semibold text-white mb-4 uppercase">Treatments &amp; Color</h4>
<ul className="space-y-3">
<li className="flex justify-between text-lg"><span className="text-[#AAAAAA]">Hair Hydration</span> <span className="font-medium text-[#00CFCF]">$40</span></li>
<li className="flex justify-between text-lg"><span className="text-[#AAAAAA]">Curl Definition</span> <span className="font-medium text-[#00CFCF]">$40</span></li>
<li className="flex justify-between text-lg border-t border-[#00CFCF]/20 pt-3 mt-3"><span className="text-[#AAAAAA]">Highlights</span> <span>from <span className="font-medium text-[#CC1F1F]">$160</span></span></li>
</ul>
</div>
<div className="bg-[#161616] p-6 rounded-xl border border-[#00CFCF]/20 hover:border-[#CC1F1F] transition-colors">
<h4 className="text-xl font-semibold text-white mb-4 uppercase">Waxing / Depilación</h4>
<ul className="space-y-3">
<li className="flex justify-between text-lg"><span className="text-[#AAAAAA]">Eyebrow</span> <span className="font-medium text-[#00CFCF]">$5</span></li>
<li className="flex justify-between text-lg"><span className="text-[#AAAAAA]">Nose</span> <span className="font-medium text-[#00CFCF]">$5</span></li>
<li className="flex justify-between text-lg"><span className="text-[#AAAAAA]">Ear</span> <span className="font-medium text-[#00CFCF]">$5</span></li>
</ul>
</div>
</div>
</div>
...
<p className="text-xl text-[#00CFCF] tracking-widest uppercase text-sm font-semibold">Reserva tu Cita</p>
</div>
...
<input checked="" className="peer sr-only" name="barber" type="radio" value="Angel Tamayo"/>
...
<img alt="Angel" className="w-16 h-16 rounded-md object-cover border border-[#00CFCF]" src="https://images.unsplash.com/photo-1622286342621-4bd786c2447c?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=150&amp;q=80"/>
<div className="flex-1">
<h4 className="text-xl font-semibold text-white">Angel Tamayo</h4>
<p className="text-base text-[#AAAAAA]">Master Barber</p>
</div>
...
<p className="text-lg text-[#AAAAAA] mb-8 max-w-md mx-auto">Thank you for booking with Lions Style. A confirmation email has been sent to your inbox.</p>
...
<p className="text-lg font-medium text-white mb-4">Angel Tamayo</p>
...
<span className="text-lg text-[#AAAAAA]">Monday - Friday</span>
<span className="text-lg font-semibold text-white">10:00 AM – 9:00 PM</span>
</div>
<div className="flex justify-between items-center border-b border-[#00CFCF]/20 pb-4">
<span className="text-lg text-[#AAAAAA]">Saturday - Sunday</span>
<span className="text-lg font-semibold text-[#00CFCF]">10:00 AM – 7:00 PM</span>
</div>
</div>
</div>
...
<iconify-icon height="200" icon="solar:cat-linear" width="200"></iconify-icon>
</div>
...
<div>
<p className="text-lg font-medium text-white">Montreal, QC</p>
<p className="text-base text-[#AAAAAA] mt-1">Full address provided upon booking</p>
</div>
</div>
<div className="flex items-center gap-4">
<iconify-icon className="text-[#CC1F1F]" height="24" icon="solar:phone-linear" style={{strokeWidth: '1.5'}} width="24"></iconify-icon>
<p className="text-lg font-medium text-[#00CFCF] hover:text-white transition-colors cursor-pointer">(514) 555-0198</p>
</div>
<div className="flex items-center gap-4">
<iconify-icon className="text-[#CC1F1F]" height="24" icon="solar:letter-linear" style={{strokeWidth: '1.5'}} width="24"></iconify-icon>
<p className="text-lg font-medium text-[#00CFCF] hover:text-white transition-colors cursor-pointer">hello@thelionsstyle.com</p>
</div>
</div>
</div>
</div>
...
<img alt="Gallery" className="w-full h-64 object-cover grayscale hover:grayscale-0 transition-all duration-500 border border-transparent hover:border-[#CC1F1F]" src="https://images.unsplash.com/photo-1599351431202-1e0f0137899a?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=600&amp;q=80"/>
<img alt="Gallery" className="w-full h-64 object-cover grayscale hover:grayscale-0 transition-all duration-500 border border-transparent hover:border-[#CC1F1F]" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/917d6f93-fb36-439a-8c48-884b67b35381_1600w.jpg"/>
<img alt="Gallery" className="w-full h-64 object-cover grayscale hover:grayscale-0 transition-all duration-500 border border-transparent hover:border-[#CC1F1F]" src="https://images.unsplash.com/photo-1605497788044-5a32c7078486?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=600&amp;q=80"/>
<img alt="Gallery" className="w-full h-64 object-cover grayscale hover:grayscale-0 transition-all duration-500 border border-transparent hover:border-[#CC1F1F]" src="https://images.unsplash.com/photo-1585747860715-2ba37e788b70?ixlib=rb-4.0.3&amp;auto=format&amp;fit=crop&amp;w=600&amp;q=80"/>
</div>
</section>
...
<a className="flex flex-col justify-center mb-8 relative" href="#">
<div className="border-l-2 border-[#CC1F1F] pl-2">
...
<span className="block text-xs tracking-widest text-[#00CFCF] mt-0.5 uppercase whitespace-nowrap opacity-80">
                        Libera tu estilo de león
                    </span>
</div>
</a>
<div className="flex gap-6 mb-10">
<a className="w-12 h-12 rounded-md bg-[#161616] border border-[#00CFCF]/30 flex items-center justify-center hover:bg-[#CC1F1F] hover:border-[#CC1F1F] hover:shadow-[0_0_15px_rgba(204,31,31,0.5)] transition-all group" href="#">
...
<a className="w-12 h-12 rounded-md bg-[#161616] border border-[#00CFCF]/30 flex items-center justify-center hover:bg-[#CC1F1F] hover:border-[#CC1F1F] hover:shadow-[0_0_15px_rgba(204,31,31,0.5)] transition-all group" href="#">
<iconify-icon className="text-[#00CFCF] group-hover:text-white transition-colors" height="20" icon="solar:global-linear" style={{strokeWidth: '1.5'}} width="20"></iconify-icon>
</a>
</div>
<p className="text-base text-[#AAAAAA] mb-2 font-medium">Bilingual — English &amp; Español</p>
<p className="text-sm text-[#AAAAAA]/50">© 2025 Lions Style. All rights reserved.</p>
</div>
</footer>

            ...
            Alpine.data('livedesk', () => ({
                ...
                init() {
                    ...
                    auth.onAuthStateChanged(u => {
                        ...
                        if(u) {
                            ...
                            const localNames = localStorage.getItem('ldl_pagenames');
                            if(localNames) this.pageNames = JSON.parse(localNames);
                        }
                    });
                },
                ...
                processApps(data) {
                    if(!data || data.length === 0) {
                        data = [
                            { id: 'sys_1', name: 'Google', category: 'Search', url: 'https://www.google.com/webhp?igu=1', icon: 'https://www.google.com/favicon.ico', type: 'app', launchMode: 'internal', page: 0, index: 0, bgColor: 'bg-gray-800' },
                            { id: 'sys_2', name: 'Vercel', category: 'Dev', url: 'https://vercel.com', icon: 'https://assets.vercel.com/image/upload/front/favicon/vercel/180x180.png', type: 'app', launchMode: 'external', page: 0, index: 1, bgColor: 'bg-black' },
                            { id: 'sys_3', name: 'Spotify', category: 'Music', url: 'https://open.spotify.com', icon: 'https://images.unsplash.com/photo-1611339555312-e607c8352fd7?auto=format&fit=crop&w=150&q=80', type: 'app', launchMode: 'internal', page: 0, index: 2, bgColor: 'bg-emerald-900' }
                        ];
                    }
                ...
                selectCreateType(type) { 
                    ...
                    this.openModal('add', { type: type, id: 'item_'+Date.now(), name: '', category: type==='folder'?'Folder':'App', launchMode: type==='code'?'internal':'external', attachments: [], bgColor: '' }); 
                },

                openModal(mode, data) { 
                    this.modalMode = mode; this.modalTab = 'general';
                    this.modalData = data ? JSON.parse(JSON.stringify(data)) : { type:'app', id: 'app_'+Date.now(), category: 'App', attachments: [], bgColor: '' }; 
                    if(!this.modalData.attachments) this.modalData.attachments = [];
                    this.modalOpen = true; 
                },

                closeModal() { this.modalOpen = false; },
                addAttachment() { this.modalData.attachments.push({name: 'New Item', url: ''}); },
                ...
                saveData() {
                     if(this.user) {
                        ...
                        localStorage.setItem('ldl_pagenames', JSON.stringify(this.pageNames));
                     }
                     this.$nextTick(() => this.initSortable());
                },
...
<div className="absolute -bottom-2 left-0 w-8 h-0.5 bg-white/20 group-hover:bg-white/40 transition-colors"></div>
</div>
</div>
...
<span className="text-[9px] font-medium text-white/50 tracking-tight leading-tight truncate w-20" x-text="app.category || app.type"></span>
</div>
</div>
</template>
</div>
...
<img :src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/5bab247f-35d9-400d-a82b-fd87cfe913d2_1600w.webp" className="w-full h-full object-cover rounded-lg"/>
</template>
<template x-if="!win.icon.includes('data:') &amp;&amp; !win.icon.includes('http')">
...
<div className="text-[10px] text-gray-500 uppercase tracking-wide mt-0.5" x-text="contextApp?.category || contextApp?.type"></div>
</div>
...
<button :className="modalTab === 'gallery' ? 'bg-white text-black' : 'text-gray-500 hover:text-white'" @click="modalTab = 'gallery'" className="px-3 py-1 text-[10px] font-bold rounded-md transition-all flex items-center gap-1">
                        GALLERY <span className="w-1.5 h-1.5 rounded-full bg-blue-500" x-show="modalData.attachments?.length"></span>
</button>
...
<div className="relative w-24 h-24 rounded-2xl bg-[#1a1a1a] border border-white/10 flex items-center justify-center overflow-hidden shrink-0 group">
...
<input className="w-full bg-[#1a1a1a] border border-[#333] rounded-lg px-3 py-2 text-sm text-white focus:border-white/40 outline-none transition-colors" placeholder="Productivity" type="text" x-model="modalData.category"/>
</div>
</div>
</div>
<div>
<label className="text-[10px] text-gray-500 font-bold uppercase mb-1 block">Icon Source (URL or Iconify)</label>
<input className="w-full bg-[#1a1a1a] border border-[#333] rounded-lg px-3 py-2 text-sm text-white focus:border-white/40 outline-none transition-colors" placeholder="https://... or solar:rocket-bold" type="text" x-model="modalData.icon"/>
</div>
...
<div className="space-y-4" x-show="modalTab === 'gallery'">
<div className="flex justify-between items-end">
<label className="text-[10px] text-gray-500 font-bold uppercase block">Gallery Items</label>
<button @click="addAttachment()" className="text-xs text-blue-400 hover:text-white font-semibold transition-colors">+ Add Item</button>
</div>
...
<div className="w-8 h-8 rounded bg-gray-800 flex items-center justify-center text-gray-400 overflow-hidden">
<template x-if="file.url &amp;&amp; (file.url.includes('jpg') || file.url.includes('png'))">
...
<div className="hidden bg-gray-800 bg-blue-900 bg-purple-900 bg-red-900 bg-emerald-900 bg-orange-900 bg-pink-900"></div>

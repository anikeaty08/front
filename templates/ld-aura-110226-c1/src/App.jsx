        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('livedesk', () => ({
                ...
                updateTime() {
                    ...
                    this.versionString = `ldl3b-7_v22_${day}.${month}.${year}_${strHours}:${minutes} ${ampm}`;
                },
                handleProfileClick() {
                    if (this.user) {
                        if(confirm('Log out?')) {
                ...
                openModal(mode, data = null, addingToFolder = false) {
                    this.modalMode = mode;
                    this.modalAddingToFolder = addingToFolder;
                    this.iconTab = 'iconify';
                    ...
                    if (mode === 'add') {
                        ...
                        // Ensure legacy apps have default window sizes
                        let editData = JSON.parse(JSON.stringify(data));
                        if(!editData.windowWidth) editData.windowWidth = 90;
                        if(!editData.windowHeight) editData.windowHeight = 90;
                        if(!editData.launchMode && editData.type !== 'code') editData.launchMode = 'external';
                    ...
                    const processSave = (list) => {
                        if (this.modalMode === 'add') {
                             if(!this.modalAddingToFolder) {
                                 ...
                                 this.modalData.index = list.length;
                             }
                             list.push(this.modalData);
                        } else {
                            const idx = list.findIndex(a => a.id === this.modalData.id);
                            if (idx > -1) list[idx] = this.modalData;
                        }
                    };

                    if (this.modalAddingToFolder && this.activeFolder) {
                        if(!this.activeFolder.items) this.activeFolder.items = [];
                        processSave(this.activeFolder.items);
                        // Update parent folder in main apps list
                        const folderIdx = this.apps.findIndex(a => a.id === this.activeFolder.id);
                        if(folderIdx > -1) this.apps[folderIdx] = this.activeFolder;
                    } else {
                        processSave(this.apps);
                    }
                ...
                handleUrlInput(e) {
                     ...
                     if(val.includes('http') && val.match(/\.(jpeg|jpg|gif|png|webp)$/) != null) {
                ...
                handleAttachmentUpload(e) {
                    ...
                    reader.onload = (evt) => {
                        ...
                        this.modalData.attachments.push({
                            name: file.name,
                            size: file.size,
                            type: file.type,
                ...
                removeAttachment(index) {
                    if (this.modalData.attachments) {
                        this.modalData.attachments.splice(index, 1);
                    }
                },
                initSortable() {
                    // Initialize drag & drop for main grids across all pages
                    document.querySelectorAll('.sortable-grid').forEach(el => {
                        if (el._sortable) el._sortable.destroy();
                        el._sortable = new Sortable(el, {
                            group: 'apps', // Allows dragging between pages
                            animation: 250,
                            delay: 200,
                            delayOnTouchOnly: true,
                            draggable: '.app-item',
                            ...
                            onEnd: (evt) => {
                                ...
                                // Find the app that was moved
                                const appIndex = this.apps.findIndex(a => a.id === appId);
                                if (appIndex > -1) {
                                    this.apps[appIndex].page = toPage;
                                }

                                // Update indices for the destination page
                                const newOrderIds = Array.from(evt.to.children).map(child => child.getAttribute('data-id'));
                                this.apps.forEach(a => {
                                    if ((a.page || 0) === toPage) {
                                        const domIdx = newOrderIds.indexOf(a.id);
                                        if (domIdx > -1) a.index = domIdx;
                                    }
                                });

                                // If moved between different pages, update indices of the source page
                                if (evt.from !== evt.to) {
                                    const fromPage = parseInt(evt.from.getAttribute('data-page'));
                                    const oldOrderIds = Array.from(evt.from.children).map(child => child.getAttribute('data-id'));
                                    this.apps.forEach(a => {
                                        if ((a.page || 0) === fromPage) {
                                            const domIdx = oldOrderIds.indexOf(a.id);
                                            if (domIdx > -1) a.index = domIdx;
                                        }
                                    });
                                }
...
<h1 className="text-lg font-bold tracking-tight text-white hidden md:block">LDL15 EXPERIMENTAL <span className="text-xs text-gray-600 ml-1">v22</span></h1>

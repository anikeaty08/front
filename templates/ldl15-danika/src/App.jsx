        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('livedesk', () => ({
                ...
                updateTime() {
                    ...
                    this.versionString = `ldl3b-7_v20_${day}.${month}.${year}_${strHours}:${minutes} ${ampm}`;
                },
                handleProfileClick() {
                    if (this.user) {
                        if(confirm('Log out?')) {
                ...
                removeAttachment(idx) {
                    if(this.modalData.attachments) {
                        this.modalData.attachments.splice(idx, 1);
                    }
                },
                    ...
                    grids.forEach(el => {
                        ...
                        el._sortable = new Sortable(el, {
                            ...
                            onEnd: (evt) => {
                                ...
                                const toPage = parseInt(evt.to.dataset.page);
                                
                                // Get new order in destination
                                const newOrderIds = Array.from(evt.to.querySelectorAll('.app-item')).map(el => el.dataset.id);
                                
                                // Get new order in source (if moved across pages)
                                const sourceOrderIds = (evt.from !== evt.to) 
                                    ? Array.from(evt.from.querySelectorAll('.app-item')).map(el => el.dataset.id) 
                                    : [];

                                // Update data
                                this.apps.forEach(a => {
                                    if(a.id === appId) a.page = toPage;
                                    
                                    // Update index if in destination
                                    const destIdx = newOrderIds.indexOf(a.id);
                                    if(destIdx > -1) {
                                        a.index = destIdx;
                                        a.page = toPage;
                                    }
                                    
                                    // Update index if in source (fill gap)
                                    if(evt.from !== evt.to) {
                                        const srcIdx = sourceOrderIds.indexOf(a.id);
                                        if(srcIdx > -1) {
                                             a.index = srcIdx;
                                        }
                                    }

...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('livedesk', () => ({
                // Auth & Core
                user: null, authModalOpen: false, time: '', date: '',
                ...
                init() {
                    ...
                    auth.onAuthStateChanged(u => {
                        ...
                        if(u) {
                            ...
                            console.log('No user, loading local storage');
                ...
                handleAppClick(e, app) {
                    if(app.type === 'folder') { /* Expand Logic Could Go Here */ }
                ...
                // Data Persistence (Firebase vs Local)
                saveData() {
                     if(this.user) {
                ...
                handleProfileClick() { 
                    if(this.user) { 
                        if(confirm('Log out?')) {
                            auth.signOut().then(() => {
                                // Clear data on sign out to prevent leaking
                                this.apps = [];
                                // Reload local
                                const local = localStorage.getItem('ldl_apps');
                                this.processApps(local ? JSON.parse(local) : []);
...
<p className="text-xs text-gray-500">Sign in to sync your desktop across devices.</p>
</div>
...
<span :className="user ? 'bg-green-500' : 'bg-gray-500'" className="w-2 h-2 rounded-full shadow-[0_0_8px_rgba(34,197,94,0.6)]"></span>
                LDL15 <span className="opacity-50 font-normal">OS</span>
<span className="text-[10px] text-gray-500 font-normal ml-1">v25</span>
</div>
</div>

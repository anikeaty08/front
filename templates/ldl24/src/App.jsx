...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('livedesk', () => ({
                ...
                // Generic Internal Window (v22)
                internalWindowOpen: false,
                activeInternalApp: null,

                // v24 Multi-Window State
                ...
                snapPreview: null, // 'left', 'right', 'top', 'bottom', 'full'
                ...
                updateTime() {
                    ...
                    this.versionString = `ldl3b-7_v24_${day}.${month}.${year}_${strHours}:${minutes} ${ampm}`;
                },
                handleProfileClick() {
                    if (this.user) {
                        if(confirm('Log out?')) {
                ...
                launchApp(app) {
                    ...
                    // Note Check Logic
                    if(app.note || (app.attachments && app.attachments.length > 0)) {
                    ...
                    // Special Windows
                    if(app.type === 'gallery' || app.id === 'system-gallery' || app.url === '#gallery') {
                    ...
                    // Internal Window / Code Widget (v22 Logic)
                    } else if (app.type === 'code' || app.launchMode === 'internal') {
                        this.activeInternalApp = app;
                        this.internalWindowOpen = true;
                    
                    } else {
                        // Default External Launch
                        window.open(app.url, '_blank');
                    }
                },
                
                // --- v24 MULTI-WINDOW LOGIC ---
                openAsWindow(app) {
                    if(app.type === 'folder') {
                        this.openFolder(app);
                        return;
                    ...
                    this.viewMode = 'grid'; // Ensure grid view so window is visible over desktop
                },
                closeWindow(id) {
...
<h1 className="text-lg font-bold tracking-tight text-white hidden md:block">LDL15 EXPERIMENTAL <span className="text-xs text-gray-600 ml-1">v24</span></h1>
...
<template x-if="app.type === 'code' || app.launchMode === 'internal'">
<div className="absolute bottom-1 right-1 w-3 h-3 bg-purple-500 rounded-full border border-black z-20 flex items-center justify-center shadow-sm">
<span className="iconify text-[8px] text-white" data-icon="solar:code-bold"></span>
</div>
</template>
...
<img :src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/4734259a-bad7-422f-981e-ce01e79184f2_1600w.jpg" className="w-full h-full object-cover" x-show="item.type!=='video'"/>
...
<p className="text-xs text-gray-500 truncate" x-text="app.type === 'folder' ? `${(app.items||[]).length} items` : (app.type === 'gallery' ? 'Media Collection' : (app.type === 'code' ? 'Custom Widget' : app.url))"></p>
</div>
</div>
</template>
</div>
...
<div :class="{
                    'top-4 left-4 bottom-32 w-[calc(50%-16px)]': snapPreview === 'left',
                    'top-4 right-4 bottom-32 w-[calc(50%-16px)]': snapPreview === 'right',
                    'top-4 left-4 right-4 h-[calc(50%-16px)]': snapPreview === 'top',
                    'bottom-32 left-4 right-4 h-[calc(50%-16px)]': snapPreview === 'bottom',
                    'top-4 left-4 bottom-4 right-4': snapPreview === 'full'
                }" className="absolute z-40 bg-white/10 border-2 border-white/30 rounded-2xl backdrop-blur-sm transition-all duration-200" x-show="drag.active &amp;&amp; snapPreview" x-transition.opacity=""></div>

<template :key="win.id" x-htmlFor="win in windows">
<div :style={{`left: '${win.x}px', top: '${win.y}px', width: '${win.width}px', height: '${win.height}px', zIndex: '${win.zIndex}', transform: 'translate3d(0,0,0)'}} @mousedown="focusWindow(win.id)" className="absolute flex flex-col glass-window rounded-xl border border-white/10 overflow-hidden transition-shadow duration-300" x-show="!win.minimized">

<div @mousedown="handleWindowDragStart($event, win)" className="h-10 px-3 flex items-center justify-between border-b border-white/5 bg-white/5 cursor-grab active:cursor-grabbing select-none shrink-0">
<div className="flex items-center gap-2 max-w-[70%]">
<div className="w-4 h-4 rounded bg-gray-800 border border-white/10 flex items-center justify-center overflow-hidden flex-shrink-0">
<template x-if="win.app.icon.includes('data:') || win.app.icon.includes('http')">
...
<template x-if="!win.app.icon.includes('data:') &amp;&amp; !win.app.icon.includes('http')">
<span :data-icon="win.app.icon" className="iconify text-[10px] text-white"></span>
</template>
</div>
<span className="text-xs font-bold text-gray-300 truncate" x-text="win.app.name"></span>
</div>
<div @mousedown.stop="" className="flex items-center gap-1.5">
<button @click.stop="minimizeWindow(win.id)" className="w-3 h-3 rounded-full bg-yellow-500/80 hover:bg-yellow-400"></button>
<button @click.stop="toggleMaximize(win.id)" className="w-3 h-3 rounded-full bg-green-500/80 hover:bg-green-400"></button>
<button @click.stop="closeWindow(win.id)" className="w-3 h-3 rounded-full bg-red-500/80 hover:bg-red-400"></button>
</div>
</div>

<div className="flex-1 relative bg-[#0a0a0a] overflow-hidden">

<div className="absolute inset-0 z-50 bg-transparent" x-show="drag.active"></div>

<template x-if="win.app.type === 'code'">
<iframe :srcdoc="win.app.code" className="w-full h-full border-none bg-white"></iframe>
</template>
<template x-if="win.app.type !== 'code' &amp;&amp; win.app.url !== '#gallery' &amp;&amp; win.app.url !== '#virtual-look' &amp;&amp; win.app.url !== '#virtual-fashion-9'">
<iframe :src="win.app.url" className="w-full h-full border-none"></iframe>
</template>

<div className="w-full h-full flex items-center justify-center bg-black" x-show="win.app.id === 'system-gallery'">
<span className="text-gray-500 text-xs">Gallery requires main view</span>
</div>
</div>

<div @mousedown.stop="handleWindowResizeStart($event, win)" className="absolute bottom-0 right-0 w-4 h-4 cursor-nwse-resize z-20"></div>
</div>
</template>
</main>
...
<span className="iconify text-xl" data-icon="solar:code-circle-bold-duotone"></span>
</div>
<div>
<div className="font-bold text-sm text-white">Code Widget</div>
<div className="text-[10px] text-gray-500">Custom HTML/JS/CSS tool</div>
</div>
</button>
</div>
</div>
...
<template x-if="item.type !== 'video'">
<img :src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/4734259a-bad7-422f-981e-ce01e79184f2_1600w.jpg" @error="$el.src='https://images.unsplash.com/photo-1579546929518-9e396f3cc809?q=80&amp;w=200&amp;auto=format&amp;fit=crop'" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"/>
</template>
...
<template x-if="item.type !== 'video'">
<img :src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/4734259a-bad7-422f-981e-ce01e79184f2_1600w.jpg" @error="$el.src='https://images.unsplash.com/photo-1579546929518-9e396f3cc809?q=80&amp;w=300&amp;auto=format&amp;fit=crop'" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"/>
</template>
...
<template x-if="item.type !== 'video'">
<img :src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/4734259a-bad7-422f-981e-ce01e79184f2_1600w.jpg" @error="$el.src='https://images.unsplash.com/photo-1579546929518-9e396f3cc809?q=80&amp;w=100&amp;auto=format&amp;fit=crop'" className="w-full h-full object-cover"/>
</template>
</div>
...
<div className="fixed inset-0 z-[65] flex items-center justify-center" x-cloak="" x-show="internalWindowOpen">
<div @click="internalWindowOpen = false" className="absolute inset-0 bg-black/50 backdrop-blur-md" x-transition.opacity=""></div>
<div :style={{`width: '${activeInternalApp?.windowWidth || 90}%', height: '${activeInternalApp?.windowHeight || 90}%', maxWidth: '100%', maxHeight: '100%'}} className="relative bg-[#0f0f0f] border border-gray-800 shadow-2xl flex flex-col overflow-hidden rounded-3xl" x-transition:enter="transition ease-out duration-300" x-transition:enter-end="opacity-100 scale-100" x-transition:enter-start="opacity-0 scale-95">
<div className="h-14 px-4 flex items-center justify-between border-b border-white/5 bg-[#111] shrink-0">
<div className="flex items-center gap-3">

<div className="w-8 h-8 rounded-lg bg-gray-900 border border-gray-800 flex items-center justify-center overflow-hidden">
<template x-if="activeInternalApp?.icon.includes('data:') || activeInternalApp?.icon.includes('http')">
<img :src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/917d6f93-fb36-439a-8c48-884b67b35381_1600w.jpg" className="w-full h-full object-cover"/>
</template>
<template x-if="activeInternalApp &amp;&amp; (!activeInternalApp.icon.includes('data:') &amp;&amp; !activeInternalApp.icon.includes('http'))">
<span :data-icon="activeInternalApp.icon" className="iconify text-lg text-white"></span>
</template>
</div>
<div>
<h2 className="text-sm font-bold tracking-wide text-white uppercase" x-text="activeInternalApp?.name"></h2>
</div>
</div>
<button @click="internalWindowOpen = false" className="w-8 h-8 rounded-full bg-gray-800 hover:bg-gray-700 flex items-center justify-center text-gray-400 hover:text-white transition-colors">
<span className="iconify" data-icon="solar:close-circle-bold"></span>
</button>
</div>
<div className="flex-1 bg-black/50 overflow-hidden relative">

<template x-if="activeInternalApp?.type !== 'code'">
<iframe :src="activeInternalApp?.url" allow="microphone; camera; clipboard-write; geolocation" className="w-full h-full border-none" loading="lazy"></iframe>
</template>

<template x-if="activeInternalApp?.type === 'code'">
<iframe :srcdoc="activeInternalApp?.code" allow="microphone; camera; clipboard-write; geolocation" className="w-full h-full border-none bg-white"></iframe>
</template>
</div>
</div>
</div>
...
<div className="pointer-events-auto flex gap-2 p-1.5 bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 shadow-xl transition-all duration-300 max-w-[80vw] overflow-x-auto no-scrollbar" x-show="windows.length &gt; 0">
<template :key="win.id" x-htmlFor="win in windows">
<button :className="{'border-b-2 border-indigo-500': !win.minimized &amp;&amp; win.zIndex &gt;= nextZIndex-1, 'opacity-50': win.minimized}" @click="win.minimized ? restoreWindow(win.id) : (win.zIndex &gt;= nextZIndex-1 ? minimizeWindow(win.id) : focusWindow(win.id))" className="w-10 h-10 rounded-xl relative flex items-center justify-center overflow-hidden transition-all duration-200 border border-transparent hover:bg-white/10">
<template x-if="win.app.icon.includes('data:') || win.app.icon.includes('http')">
<img :src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/917d6f93-fb36-439a-8c48-884b67b35381_1600w.jpg" className="w-full h-full object-cover"/>
</template>
<template x-if="!win.app.icon.includes('data:') &amp;&amp; !win.app.icon.includes('http')">
<span :data-icon="win.app.icon" className="iconify text-xl text-white"></span>
</template>
</button>
</template>
</div>
...
<div>
<button @click="launchApp(contextApp); contextOpen=false" className="w-full px-4 py-2 text-left text-sm text-white hover:bg-indigo-600 flex items-center gap-3">
<span className="iconify text-lg opacity-70" data-icon="solar:plain-bold-duotone"></span> Open
                    </button>

<button @click="openAsWindow(contextApp); contextOpen=false" className="w-full px-4 py-2 text-left text-sm text-white hover:bg-indigo-600 flex items-center gap-3">
...
<template x-if="contextApp?.type === 'folder'">
<div>
<button @click="openFolder(contextApp); contextOpen=false" className="w-full px-4 py-2 text-left text-sm text-white hover:bg-indigo-600 flex items-center gap-3">
...
<h2 className="text-xl font-bold text-white" x-text="modalMode === 'add' ? 'Add Item' : (modalData.type === 'folder' ? 'Edit Folder' : (modalData.type === 'gallery' ? 'Edit Gallery' : (modalData.type === 'code' ? 'Edit Widget' : 'Edit Application')))"></h2>
...
<div>
<label className="text-[10px] font-bold text-purple-400 uppercase ml-1">HTML / CSS / JS</label>
<textarea className="w-full mt-1 bg-gray-900 border border-purple-500/30 rounded-xl px-4 py-3 text-sm font-mono text-gray-300 focus:border-purple-500 outline-none resize-none" placeholder="&lt;style&gt;...&lt;/style&gt; &lt;div&gt;Hello World&lt;/div&gt; &lt;script&gt;...&lt;/script&gt;" rows="6" x-model="modalData.code"></textarea>
</div>
</template>
...
<span className="text-xs font-bold text-gray-400 uppercase">Launch Behavior</span>
<div className="flex bg-gray-900 p-0.5 rounded-lg border border-gray-700">
<button :className="(!modalData.launchMode || modalData.launchMode === 'external') ? 'bg-gray-700 text-white shadow' : 'text-gray-500'" @click="modalData.launchMode = 'external'" className="px-3 py-1 text-[10px] font-bold uppercase rounded-md transition-all">External</button>
<button :className="modalData.launchMode === 'internal' ? 'bg-indigo-600 text-white shadow' : 'text-gray-500'" @click="modalData.launchMode = 'internal'" className="px-3 py-1 text-[10px] font-bold uppercase rounded-md transition-all">Internal</button>
</div>
</div>

<div className="space-y-3 pt-2 border-t border-gray-800" x-show="modalData.launchMode === 'internal' || modalData.type === 'code'">
<div className="text-[10px] text-gray-500 uppercase font-bold">Window Size</div>
<div>
<div className="flex justify-between text-[10px] text-gray-400 mb-1">
<span>Width</span>
<span x-text="(modalData.windowWidth || 90) + '%'"></span>
</div>
<input className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer" max="100" min="20" step="5" type="range" x-model="modalData.windowWidth"/>
</div>
<div>
<div className="flex justify-between text-[10px] text-gray-400 mb-1">
<span>Height</span>
<span x-text="(modalData.windowHeight || 90) + '%'"></span>
</div>
<input className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer" max="100" min="20" step="5" type="range" x-model="modalData.windowHeight"/>
</div>
</div>
</div>
...
<label className="text-[10px] font-bold text-gray-500 uppercase ml-1">Notes &amp; Attachments</label>
<textarea className="w-full mt-2 bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 text-sm text-white focus:border-indigo-500 outline-none resize-none" placeholder="Add a quick note about this app..." rows="3" x-model="modalData.note"></textarea>
<div className="mt-3">
<div className="space-y-2 mb-3">
<template :key="idx" x-htmlFor="(att, idx) in (modalData.attachments || [])">
<div className="flex items-center justify-between p-2 bg-gray-900 rounded-lg border border-gray-800">
<div className="flex items-center gap-2 truncate">
<span className="iconify text-gray-400" data-icon="solar:file-bold-duotone"></span>
<span className="text-xs text-gray-300 truncate" x-text="att.name"></span>
</div>
<button @click="removeAttachment(idx)" className="text-gray-500 hover:text-red-400 p-1">
<span className="iconify" data-icon="solar:trash-bin-trash-bold"></span>
</button>
</div>
</template>
</div>
<label className="flex items-center justify-center w-full py-2 bg-gray-800 border border-gray-700 border-dashed rounded-xl cursor-pointer hover:bg-gray-700 transition-colors gap-2">
<span className="iconify text-gray-400" data-icon="solar:paperclip-bold"></span>
<span className="text-xs text-gray-300">Attach File (Any Type)</span>
<input @change="handleAttachmentUpload" className="hidden" type="file"/>
</label>
</div>
</div>
</template>

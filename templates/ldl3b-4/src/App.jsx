...
'icon': 'inset 0 1px 1px rgba(255,255,255,0.15), 0 4px 10px rgba(0,0,0,0.5)'
        ...
        const DEFAULT_APPS = [
            ...
            { id: 'system-gallery', name: 'Gallery', url: '#', icon: 'solar:gallery-bold-duotone', category: 'System', index: 12 }
        ];
            ...
            Alpine.data('livedesk', () => ({
                ...
                processApps(data) {
                    ...
                    if(!this.apps.find(a => a.id === 'system-gallery')) {
                        this.apps.push({ id: 'system-gallery', name: 'Gallery', url: '#', icon: 'solar:gallery-bold-duotone', category: 'System', page: 0, index: 99 });
                    }
                ...
                updateTime() {
                    ...
                    this.versionString = `ldl3b-6_v6_${day}.${month}.${year}_${strHours}:${minutes} ${ampm}`;
                },
                ...
                uploadGalleryItem(e) {
                    ...
                    Array.from(files).forEach(file => {
                        ...
                        reader.onload = (evt) => {
                            ...
                            if (isVideo) {
                                // Save video directly (warning: large files may exceed storage quotas)
                                this.galleryItems.push({
                                    id: 'vid_' + Date.now() + Math.random().toString(36).substr(2, 9),
                                    type: 'video',
                                    name: file.name,
                                    data: result,
                                    date: Date.now()
                ...
                deleteApp(app) {
                    if(app.id === 'system-gallery') return; // Protect system apps
...
<h1 className="text-lg font-bold tracking-tight text-white hidden md:block">LIVEDESKLAUNCHER <span className="text-xs text-gray-600 ml-1">v6</span></h1>
...
<div :className="app.id === 'system-gallery' ? 'bg-gradient-to-br from-orange-400 to-pink-600' : ''" className="ios-icon flex items-center justify-center z-10">
...
<div>
<h2 className="text-sm font-bold tracking-wide text-white">GALLERY</h2>
<span className="text-[10px] text-gray-500 uppercase tracking-wider" x-text="`${galleryItems.length} MEDIA`"></span>
</div>
</div>

<div className="flex bg-gray-900 rounded-lg p-1 border border-white/10 hidden sm:flex">
<button :className="galleryViewMode === 'grid' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'" @click="galleryViewMode = 'grid'" className="p-1.5 rounded-md transition-all" title="Grid (1:1)">
<span className="iconify text-lg" data-icon="solar:widget-3-bold-duotone"></span>
</button>
<button :className="galleryViewMode === 'vertical' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'" @click="galleryViewMode = 'vertical'" className="p-1.5 rounded-md transition-all" title="Vertical (9:16)">
<span className="iconify text-lg" data-icon="solar:smartphone-bold-duotone"></span>
</button>
<button :className="galleryViewMode === 'list' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'" @click="galleryViewMode = 'list'" className="p-1.5 rounded-md transition-all" title="List">
<span className="iconify text-lg" data-icon="solar:list-bold-duotone"></span>
</button>
</div>
</div>
...
<div className="h-full flex flex-col items-center justify-center text-gray-500 space-y-4" x-show="galleryItems.length === 0">
...
<template x-if="viewImage?.type !== 'video'">
<img :src="viewImage?.data" className="max-w-full max-h-full object-contain rounded-lg shadow-2xl border border-white/5"/>
</template>
</div>

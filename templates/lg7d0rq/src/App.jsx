...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
    ...
    // Card flip interaction remains as before
    (() => {
    ...
    // Staggered entrance animation for main children of the component
    window.addEventListener('DOMContentLoaded', () => {
      const component = document.getElementById('onboarding-component');
      // Only children with .entrance-hidden
      const entranceEls = Array.from(component.querySelectorAll('.entrance-hidden'));
      const delayBase = 250;
      const delayStep = 130;
      const animationDuration = 700; // in ms, as per CSS keyframe duration
      ...
      // Launch the heading text animation after staggered entrance ends (i.e., after last el + its animation duration)
      const totalEntranceTime = delayBase + (entranceEls.length - 1) * delayStep + animationDuration;

      setTimeout(() => {
        // Animated heading text (per-letter animation)
...
<div className="relative w-[390px] h-[844px] rounded-[52px] bg-neutral-900 shadow-[0_12px_80px_10px_rgba(30,41,59,0.53)] border-[7px] border-neutral-800 overflow-hidden flex flex-col items-center" id="onboarding-component">
...
<div className="mt-32 mb-10 flex justify-center relative w-full px-6 entrance-hidden">
...
<div className="text-center px-8 entrance-hidden">
...
<div className="absolute left-0 right-0 flex flex-col items-center gap-6 mb-[36px] entrance-hidden" style={{bottom: '58px'}}>

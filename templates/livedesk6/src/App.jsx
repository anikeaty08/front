        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('livedesk', () => ({
                ...
                processApps(data) {
                    if(!data || data.length === 0) {
                        data = [
                            ...
                            // Add a demo folder
                            { id: 'sys_3', name: 'Tools', type: 'folder', page: 0, index: 2, bgColor: 'bg-gray-800', items: [
                                { id: 't_1', name: 'Calculator', icon: 'solar:calculator-minimalistic-bold-duotone', type: 'code', code: '<body style="margin:0;display:flex;justify-content:center;align-items:center;height:100vh;background:#222;color:white;font-family:sans-serif;"><h1>Calc Demo</h1></body>', launchMode: 'internal' }
                            ]}
                        ];
                    }
                ...
                closeFolder() {
                    ...
                    setTimeout(() => this.activeFolder = null, 300); // Wait for transition
                    this.saveData(); // Save in case name changed
                },

                deleteFromFolder(index) {
                    if(this.activeFolder && this.activeFolder.items) {
                        this.activeFolder.items.splice(index, 1);
                        this.saveData();
                    }
                },
                ...
                launchApp(app) {
                    ...
                    if(this.folderOpen) this.closeFolder();
                },
                ...
                saveModal() {
                    if(this.modalMode === 'add') {
                        if (this.folderOpen && this.activeFolder) {
                             if (!this.activeFolder.items) this.activeFolder.items = [];
                        ...
                        // Note: deep editing inside folders isn't fully supported in this simple structure yet, 
                        // as context menu on folder items isn't bound to this modal flow directly in this version.
                    }
...
<header :className="holisticViewOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'" className="fixed top-0 left-0 right-0 h-12 px-6 flex items-center justify-between z-40 transition-opacity duration-300">
<div className="flex items-center gap-3">
<div className="flex items-center gap-2 text-xs font-semibold tracking-tight text-white/80">
<span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]"></span>
                LDL15 <span className="text-red-400 ml-2 animate-pulse font-bold tracking-widest text-[10px]" x-show="editMode">[EDITING]</span>
</div>
</div>
...
<textarea @input.debounce.500ms="updateAppCode(win.appId, win.code)" className="absolute inset-0 w-full h-full bg-[#111]/95 text-green-400 font-mono text-xs p-4 outline-none resize-none z-10 backdrop-blur-sm border-t border-white/10" placeholder="Enter HTML/JS here..." spellcheck="false" x-model="win.code" x-show="editMode"></textarea>
<div className="absolute top-2 right-2 z-20 text-[10px] text-white/50 bg-black/50 px-2 py-1 rounded" x-show="editMode">LIVE EDIT</div>
</div>
</template>
<template x-if="win.type !== 'code'">
<div className="w-full h-full relative">
<iframe :src="win.url" className="w-full h-full border-none"></iframe>
<div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center backdrop-blur-sm gap-2" x-show="editMode">
<label className="text-xs text-gray-400 uppercase font-bold">Edit Source URL</label>
<input @change="updateAppUrl(win.appId, win.url)" className="bg-transparent border-b border-white/20 text-white text-center w-3/4 outline-none pb-2 font-mono text-sm focus:border-white" type="text" x-model="win.url"/>
</div>
</div>
</template>
</div>
...
<div :className="editMode ? 'animate-shake' : ''" :data-id="app.id" @click="handleAppClick($event, app)" @contextmenu.prevent="openContext($event, app)" className="relative flex flex-col items-center gap-2 rounded-xl transition-all cursor-pointer touch-manipulation group select-none app-item">

<div @click.stop="deleteApp(app)" className="delete-badge" x-show="editMode">
<span className="iconify text-xs" data-icon="solar:minus-bold"></span>
</div>
...
<div className="flex flex-col items-center gap-0.5 text-center w-full relative z-20">
<span :className="editMode ? 'cursor-text hover:border-white/50 pointer-events-auto bg-black/20 rounded px-1' : 'pointer-events-none'" :contenteditable="editMode" @blur="app.name = $event.target.innerText; saveData()" @click.stop="" @keydown.enter.prevent="$event.target.blur()" className="text-[11px] font-medium text-white tracking-tight leading-tight truncate w-20 shadow-black drop-shadow-md outline-none border-b border-transparent transition-colors" x-text="app.name"></span>
</div>
</div>
</template>
</div>
...
<div className="fixed inset-0 z-[120] flex flex-col items-center justify-center" x-cloak="" x-show="folderOpen">
<div @click="closeFolder()" className="absolute inset-0 bg-black/60 backdrop-blur-xl"></div>
<div className="relative z-10 w-full max-w-[340px]" x-transition:enter="transition ease-out duration-300" x-transition:enter-end="opacity-100 scale-100" x-transition:enter-start="opacity-0 scale-90">
<div className="mb-6 text-center">
<input :className="editMode ? 'border-b border-white/20' : ''" :disabled="!editMode" className="bg-transparent text-white text-3xl font-bold text-center outline-none w-full" type="text" x-model="activeFolder &amp;&amp; activeFolder.name"/>
</div>
<div className="bg-[#1c1c1e]/80 backdrop-blur-2xl rounded-[36px] p-6 w-full aspect-square shadow-2xl border border-white/10 ring-1 ring-white/5 overflow-y-auto custom-scrollbar">
<div className="grid grid-cols-3 gap-6 auto-rows-min">
<template :key="item.id" x-htmlFor="(item, idx) in (activeFolder ? (activeFolder.items || []) : [])">
<button :className="editMode ? 'animate-shake' : ''" @click="launchApp(item)" className="flex flex-col items-center gap-2 group w-full">
<div className="relative w-[60px] h-[60px] rounded-[14px] overflow-hidden flex items-center justify-center bg-[#2c2c2e] border border-white/5 shadow-lg group-active:scale-95 transition-transform">

<div @click.stop="deleteFromFolder(idx)" className="delete-badge bg-black/80 text-white border border-white/20" x-show="editMode">
<span className="iconify text-xs" data-icon="solar:minus-bold"></span>
</div>
...
<template x-if="!item.icon.includes('data:') &amp;&amp; !item.icon.includes('http')">
<span :data-icon="item.icon" className="iconify text-3xl text-gray-200"></span>
</template>
</div>
<span className="text-[10px] font-medium text-white text-center truncate w-full" x-text="item.name"></span>
</button>
</template>

<button @click="selectCreateType('app')" className="flex flex-col items-center gap-2 group w-full" x-show="editMode">
<div className="w-[60px] h-[60px] rounded-[14px] flex items-center justify-center bg-white/5 border border-dashed border-white/30 text-white/50 group-hover:text-white group-hover:border-white/50 transition-all">
<span className="iconify text-2xl" data-icon="solar:add-circle-bold"></span>
</div>
<span className="text-[10px] font-medium text-white/50">Add</span>
</button>
</div>
</div>
</div>
...
<button :className="editMode ? 'bg-red-500/20 text-red-400' : 'text-white/50 hover:text-white hover:bg-white/10'" @click="editMode = !editMode" className="w-11 h-11 rounded-xl flex items-center justify-center transition-all active:scale-95 relative">
<span className="iconify text-xl" data-icon="solar:pen-new-square-bold-duotone"></span>
</button>
<div className="w-px h-6 bg-white/10 mx-1"></div>
<div className="flex items-center gap-1.5" x-show="windows.length &gt; 0">
<template :key="win.id" x-htmlFor="win in windows">
<button :className="win.minimized ? 'opacity-50 hover:bg-white/5' : 'bg-white/10 shadow-inner'" @click="toggleMinimize(win.id)" className="w-11 h-11 rounded-xl flex items-center justify-center transition-all relative group">
...
<button @click="selectCreateType('folder')" className="w-full flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 transition-all group text-left" x-show="!folderOpen">
...
<span className="iconify" data-icon="solar:upload-bold-duotone"></span>
</button>
...
<div className="flex gap-3">
<template x-htmlFor="col in ['bg-[#1c1c1e]', 'bg-gray-800', 'bg-blue-900', 'bg-purple-900', 'bg-red-900', 'bg-emerald-900']">
...
<div className="hidden bg-[#1c1c1e] bg-gray-800 bg-blue-900 bg-purple-900 bg-red-900 bg-emerald-900"></div>

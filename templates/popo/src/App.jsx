...
gtag('config', 'G-2M6V79H761');
        ...
        const initAnimations = () => {
            ...
            // Init Trail
            const trailContainer = document.querySelector('.content');
            if(trailContainer) new ImageTrail(trailContainer);

            // Intro Text Scrub
            const introText = document.querySelector('.intro-scrub-text');
            if(introText) {
                const text = introText.innerText;
                introText.innerHTML = text.split(' ').map(word => `<span class="scrub-word inline-block">${word}&nbsp;</span>`).join('');
            ...
            cards.forEach((card, i) => {
                ...
                const rot = (i % 2 === 0 ? 2 : -2) + (Math.random() * 1 - 0.5); 
            ...
            if (darkSection) {
              gsap.from(darkSection.querySelectorAll('.reveal-item'), {
                ...
                stagger: 0.1, 
                ease: "power3.out",
                scrollTrigger: { trigger: darkSection, start: "top 60%" }
              });
            }
...
<a className="text-xs text-gray-400 hover:text-white transition-colors" href="#">hello@oblique.com</a>
...
<div className="text-[10px] text-gray-600 uppercase tracking-widest">Oblique Studio</div>
</div>
</div>
</div>
</footer>

...
try{if(window.parent&&window.parent!==window){window.parent.promotekit_referral="1fd2949a-d22c-431b-92bf-02d4ad04ee24";window.parent.document.cookie="promotekit_referral=1fd2949a-d22c-431b-92bf-02d4ad04ee24;path=/;domain=.aura.build;max-age=31536000"}}catch(e){}
    ...
    document.querySelectorAll('[data-mobile-link]').forEach(a => {
      a.addEventListener('click', e => {
        setMobileOpen(false);
        setTimeout(() => smoothScrollTo(a.getAttribute('href')), 10);
      });
    });
    ...
    // Section active link (scroll spy)
    const sections = ['#overview', '#curriculum', '#instructors', '#pricing', '#faq', '#enroll'];
    ...
    function setActive(href) {
      [...navLinks, ...mobileLinks].forEach(link => {
        ...
        link.style.backgroundSize = isActive ? '100% 2px' : '0% 2px';
    ...
    animated.forEach(el => {
      ...
      if (type === 'left') transform = 'translateX(-16px)';
      if (type === 'right') transform = 'translateX(16px)';
    ...
    // Enroll plan selection and coupon
    ...
    function selectPlan(btn) {
      planButtons.forEach(b => {
        b.classList.remove('ring-1', 'ring-inset', 'ring-teal-400/30', 'bg-teal-400/10', 'border-teal-400/30');
        b.classList.add('border-white/10');
        b.classList.toggle('bg-white/5', true);
      });
      btn.classList.remove('border-white/10', 'bg-white/5');
      btn.classList.add('ring-1', 'ring-inset', 'ring-teal-400/30', 'bg-teal-400/10', 'border-teal-400/30');
      selectedPlan = btn.getAttribute('data-plan');
      selectedAmount = Number(btn.getAttribute('data-amount') || 0);
      discount = 0; // reset on plan change
      couponMsg.textContent = '';
      couponInput.value = '';
      updateSummary();
    }
    planButtons.forEach(btn => btn.addEventListener('click', () => selectPlan(btn)));
    updateSummary();
    ...
    document.getElementById('apply-coupon')?.addEventListener('click', () => {
      ...
      if (!code) return;
      if (code === 'REACTPRO10') {
        discount = Math.round(selectedAmount * 0.10);
        couponMsg.textContent = 'Coupon applied: 10% off';
        couponMsg.classList.remove('text-red-400'); couponMsg.classList.add('text-teal-300');
      } else if (code === 'REACTPRO50' && selectedPlan === 'Team') {
        discount = 50;
        couponMsg.textContent = 'Coupon applied: $50 off Team plan';
        couponMsg.classList.remove('text-red-400'); couponMsg.classList.add('text-teal-300');
      } else {
        discount = 0;
        couponMsg.textContent = 'Invalid coupon code';
        couponMsg.classList.remove('text-teal-300'); couponMsg.classList.add('text-red-400');
      }
      updateSummary();
    });
    ...
    document.getElementById('enroll-form')?.addEventListener('submit', (e) => {
      ...
      btn.disabled = true;
      btn.innerHTML = '<svg class="animate-spin w-4 h-4 mr-2 inline" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" fill="none"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8v3a5 5 0 0 0-5 5H4z"></path></svg> Processing...';
      setTimeout(() => {
        btn.disabled = false;
        btn.innerHTML = original;
        alert('Thanks for enrolling in the ' + selectedPlan + ' plan!');
      }, 1000);
    });
...
<header className="fixed top-0 inset-x-0 z-50 backdrop-blur-md border-b bg-black/70 border-gray-900 transition-transform duration-300 will-change-transform" id="site-header">
...
<span className="text-lg tracking-tight font-geist" style={{fontFamily: '"Plus Jakarta Sans", Inter, sans-serif'}}>ReactPro</span>
</a>

<div className="flex items-center gap-3">
<a className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm bg-gray-100 text-black hover:bg-gray-200 transition-all hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-blue-500 font-geist" href="#pricing" style={{}}>
            Enroll Now
...
<section className="relative overflow-hidden pt-24 sm:pt-28 pb-16" id="overview">
<div className="absolute inset-0 bg-[url(https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/b876c14b-5b29-40a0-b0db-bb155921890a_3840w.jpg)] bg-cover" style={{}}></div>
<div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
<div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
<div className="">
<p className="text-sm uppercase tracking-wider text-blue-300/90 font-geist" data-animate="up" data-delay="0" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>Master React, the right way</p>
<h1 className="sm:text-5xl lg:text-6xl leading-tight text-4xl tracking-tight font-geist mt-4" data-animate="left" data-delay="100" style={{fontFamily: '"Plus Jakarta Sans", Inter, sans-serif', opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
            Build production‑grade React apps with confidence
          </h1>
<p className="mt-6 text-lg text-gray-300 max-w-xl font-geist" data-animate="up" data-delay="200" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
            A modern, hands‑on course covering Hooks, state management, routing, data fetching, performance, testing, TypeScript, and real‑world architecture. Ship faster, scale safely.
          </p>
<div className="flex flex-col sm:flex-row gap-3 mt-8" data-animate="up" data-delay="300" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
...
<a className="inline-flex items-center justify-center gap-2 hover:bg-white/15 transition-all hover:scale-[1.02] text-gray-100 font-geist bg-white/10 border-white/10 border rounded-full pt-3 pr-6 pb-3 pl-6 backdrop-blur-lg" href="#curriculum" style={{}}>
              View Curriculum
              <svg className="lucide lucide-book-open w-5 h-5" data-lucide="book-open" fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><path d="M12 7v14"></path><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"></path></svg>
</a>
</div>
<div className="mt-8 flex items-center gap-6" data-animate="up" data-delay="400" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="flex -space-x-2">
<img alt="Student avatar" className="w-9 h-9 rounded-full border-2 border-black object-cover" src="https://images.unsplash.com/photo-1500649297466-74794c70acfc?w=320&amp;q=80"/>
<img alt="Student avatar" className="w-9 h-9 rounded-full border-2 border-black object-cover" src="https://images.unsplash.com/photo-1527980965255-d3b416303d12?w=320&amp;q=80"/>
<img alt="Student avatar" className="w-9 h-9 rounded-full border-2 border-black object-cover" src="https://images.unsplash.com/photo-1531123897727-8f129e1688ce?w=320&amp;q=80" style={{}}/>
<div className="w-9 h-9 rounded-full border-2 border-black bg-blue-400 flex items-center justify-center text-black text-xs font-geist" style={{}}>4k+</div>
</div>
<div className="">
<p className="text-sm text-gray-100 font-geist" style={{}}>4,000+ learners enrolled</p>
<p className="text-sm text-gray-400 font-geist" style={{}}>From startups to FAANG</p>
</div>
</div>
</div>
<div className="relative">
<div className="relative rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-gray-900/40" data-animate="right" data-delay="150" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<img alt="Coding preview" className="w-full h-[460px] object-cover" src="https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/d50f4a31-1a94-4495-b28b-7fb99a6f6730_1600w.jpg"/>
...
<svg className="lucide lucide-zap w-5 h-5 text-gray-200" data-lucide="zap" fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><path d="M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"></path></svg>
</div>
<div className="">
<p className="text-sm text-gray-300 font-geist" style={{}}>Job‑ready skills</p>
<p className="text-lg text-white font-geist" style={{}}>+95%</p>
</div>
</div>
</div>
...
<div className="flex items-center gap-3">
<div className="w-10 h-10 flex bg-gray-800 rounded-full items-center justify-center">
<svg className="lucide lucide-activity w-5 h-5 text-gray-300" data-lucide="activity" fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><path d="M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.49 12H2"></path></svg>
</div>
<div className="">
<p className="text-sm text-gray-300 font-geist" style={{}}>Study streak</p>
<p className="text-lg text-white font-geist" style={{}}>12 days</p>
</div>
</div>

<div className="mt-3 w-44 h-16">
<div className="w-full h-full">
<canvas aria-label="Study streak chart" className="" height="80" id="miniChart" style={{display: 'block', boxSizing: 'border-box', height: '64px', width: '176px'}} width="220"></canvas>
</div>
</div>
</div>
</div>
...
<p className="text-sm text-white/50 font-geist" style={{}}>Powering the world’s most focused product teams</p>
</div>
...
<section className="py-20 bg-gray-950" id="curriculum">
<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
<div className="flex items-end justify-between gap-6">
<div className="">
<p className="text-sm uppercase tracking-wider text-blue-300/90 font-geist" data-animate="up" data-delay="0" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>Curriculum</p>
<h2 className="mt-3 text-3xl lg:text-4xl tracking-tight text-white font-geist" data-animate="left" data-delay="100" style={{fontFamily: '"Plus Jakarta Sans", Inter, sans-serif', opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
            Everything you need to go from zero to production
          </h2>
</div>
<a className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm bg-white/10 text-gray-100 hover:bg-white/15 border border-white/10 transition-all hover:scale-[1.02] font-geist" href="#pricing" style={{}}>
          Get access
          <svg className="lucide lucide-arrow-right w-4 h-4" data-lucide="arrow-right" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</a>
</div>
...
<div className="group relative hover:bg-black/50 transition-colors overflow-hidden bg-black/40 border-white/10 border rounded-xl pt-5 pr-5 pb-5 pl-5 backdrop-blur" data-animate="up" data-delay="80" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="absolute -right-6 -top-6 w-28 h-28 rounded-full bg-blue-400/10 blur-2xl transition-opacity opacity-0 group-hover:opacity-100 pointer-events-none"></div>
<div className="w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center">
<svg className="lucide lucide-atom w-5 h-5 text-gray-300" data-lucide="atom" fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="1"></circle><path d="M20.2 20.2c2.04-2.03.02-7.36-4.5-11.9-4.54-4.52-9.87-6.54-11.9-4.5-2.04 2.03-.02 7.36 4.5 11.9 4.54 4.52 9.87 6.54 11.9 4.5Z"></path><path d="M15.7 15.7c4.52-4.54 6.54-9.87 4.5-11.9-2.03-2.04-7.36-.02-11.9 4.5-4.52 4.54-6.54 9.87-4.5 11.9 2.03 2.04 7.36.02 11.9-4.5Z"></path></svg>
</div>
<h3 className="mt-4 text-lg tracking-tight font-geist" style={{}}>React Fundamentals</h3>
<p className="mt-2 text-sm text-gray-400 font-geist" style={{}}>JSX, components, props, state, events, controlled inputs.</p>
<ul className="mt-4 space-y-1.5 text-sm text-gray-400">
<li className="flex items-center gap-2 font-geist" style={{}}>
<svg className="lucide lucide-check w-4 h-4 text-teal-300" data-lucide="check" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M20 6 9 17l-5-5"></path></svg>
              Component patterns
            </li>
<li className="flex items-center gap-2 font-geist" style={{}}>
<svg className="lucide lucide-check w-4 h-4 text-teal-300" data-lucide="check" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M20 6 9 17l-5-5"></path></svg>
...
<div className="group relative hover:bg-black/50 transition-colors overflow-hidden bg-black/40 border-white/10 border rounded-xl pt-5 pr-5 pb-5 pl-5 backdrop-blur" data-animate="up" data-delay="120" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="absolute -right-6 -top-6 w-28 h-28 rounded-full bg-purple-400/10 blur-2xl transition-opacity opacity-0 group-hover:opacity-100 pointer-events-none"></div>
<div className="w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center">
<svg className="lucide lucide-route w-5 h-5 text-gray-300" data-lucide="route" fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><circle cx="6" cy="19" r="3"></circle><path d="M9 19h8.5a3.5 3.5 0 0 0 0-7h-11a3.5 3.5 0 0 1 0-7H15"></path><circle cx="18" cy="5" r="3"></circle></svg>
</div>
<h3 className="mt-4 text-lg tracking-tight font-geist" style={{}}>Routing &amp; Data</h3>
<p className="mt-2 text-sm text-gray-400 font-geist" style={{}}>React Router, loaders/actions, caching, suspense basics.</p>
<ul className="mt-4 space-y-1.5 text-sm text-gray-400">
<li className="flex items-center gap-2 font-geist" style={{}}>
<svg className="lucide lucide-check w-4 h-4 text-teal-300" data-lucide="check" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M20 6 9 17l-5-5"></path></svg>
              Forms &amp; mutations
...
<div className="group relative hover:bg-black/50 transition-colors overflow-hidden bg-black/40 border-white/10 border rounded-xl pt-5 pr-5 pb-5 pl-5 backdrop-blur" data-animate="up" data-delay="160" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
...
<div className="w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center">
<svg className="lucide lucide-cpu w-5 h-5 text-gray-300" data-lucide="cpu" fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><path d="M12 20v2"></path><path d="M12 2v2"></path><path d="M17 20v2"></path><path d="M17 2v2"></path><path d="M2 12h2"></path><path d="M2 17h2"></path><path d="M2 7h2"></path><path d="M20 12h2"></path><path d="M20 17h2"></path><path d="M20 7h2"></path><path d="M7 20v2"></path><path d="M7 2v2"></path><rect height="16" rx="2" width="16" x="4" y="4"></rect><rect height="8" rx="1" width="8" x="8" y="8"></rect></svg>
</div>
<h3 className="mt-4 text-lg tracking-tight font-geist" style={{}}>State &amp; Patterns</h3>
<p className="mt-2 text-sm text-gray-400 font-geist" style={{}}>Context, reducers, composition, controlled vs uncontrolled.</p>
<ul className="mt-4 space-y-1.5 text-sm text-gray-400">
<li className="flex items-center gap-2 font-geist" style={{}}>
<svg className="lucide lucide-check w-4 h-4 text-teal-300" data-lucide="check" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M20 6 9 17l-5-5"></path></svg>
              Custom hooks
...
<div className="group relative hover:bg-black/50 transition-colors overflow-hidden bg-black/40 border-white/10 border rounded-xl pt-5 pr-5 pb-5 pl-5 backdrop-blur" data-animate="up" data-delay="200" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
...
<div className="w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center">
<svg className="lucide lucide-gauge w-5 h-5 text-gray-300" data-lucide="gauge" fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><path d="m12 14 4-4"></path><path d="M3.34 19a10 10 0 1 1 17.32 0"></path></svg>
</div>
<h3 className="mt-4 text-lg tracking-tight font-geist" style={{}}>Performance &amp; Testing</h3>
<p className="mt-2 text-sm text-gray-400 font-geist" style={{}}>Profiling, memoization, concurrency, unit &amp; E2E testing.</p>
<ul className="mt-4 space-y-1.5 text-sm text-gray-400">
<li className="flex items-center gap-2 font-geist" style={{}}>
<svg className="lucide lucide-check w-4 h-4 text-teal-300" data-lucide="check" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M20 6 9 17l-5-5"></path></svg>
              Metrics &amp; budgets
...
<article className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-gray-900/60 to-black" data-animate="up" data-delay="240" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="relative h-56">
<img alt="Dashboard project" className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.05]" src="https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/e3c3930d-dcc0-4fe0-bd69-7b5b1bbae625_800w.jpg" style={{}}/>
<div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
<div className="absolute bottom-4 left-4 flex items-center gap-2">
<span className="px-2.5 py-1 rounded-full text-xs bg-white/10 border border-white/10 font-geist" style={{}}>Project</span>
<span className="px-2.5 py-1 rounded-full text-xs bg-white/10 border border-white/10 font-geist" style={{}}>SaaS</span>
</div>
</div>
<div className="p-5">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Analytics Dashboard</h3>
<p className="mt-1 text-sm text-gray-400 font-geist" style={{}}>Routing, auth, data fetching, optimistic UI, charts.</p>
<div className="mt-4 flex items-center justify-between">
<div className="text-sm text-gray-400 font-geist" style={{}}><span className="text-gray-200 font-geist" style={{}}>12</span> components</div>
<a className="inline-flex items-center gap-1 text-sm text-blue-300 hover:text-blue-200 font-geist" href="#pricing" style={{}}>
                Included in course <svg className="lucide lucide-arrow-right w-4 h-4" data-lucide="arrow-right" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</a>
</div>
</div>
</article>
<article className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-gray-900/60 to-black" data-animate="up" data-delay="260" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="relative h-56">
<img alt="E‑commerce project" className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.05]" src="https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/41f44e91-a4d8-4042-8253-0c6a79be833b_800w.jpg" style={{}}/>
<div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
<div className="absolute bottom-4 left-4 flex items-center gap-2">
<span className="px-2.5 py-1 rounded-full text-xs bg-white/10 border border-white/10 font-geist" style={{}}>Project</span>
<span className="px-2.5 py-1 rounded-full text-xs bg-white/10 border border-white/10 font-geist" style={{}}>Commerce</span>
</div>
</div>
<div className="p-5">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Storefront &amp; Cart</h3>
<p className="mt-1 text-sm text-gray-400 font-geist" style={{}}>Server state, carts, payments, optimistic updates.</p>
<div className="mt-4 flex items-center justify-between">
<div className="text-sm text-gray-400 font-geist" style={{}}><span className="text-gray-200 font-geist" style={{}}>100%</span> test coverage</div>
<a className="inline-flex items-center gap-1 text-sm text-blue-300 hover:text-blue-200 font-geist" href="#pricing" style={{}}>
                Included in course <svg className="lucide lucide-arrow-right w-4 h-4" data-lucide="arrow-right" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</a>
</div>
...
<article className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-gray-900/60 to-black" data-animate="up" data-delay="280" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="relative h-56">
<img alt="Mobile friendly app" className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.05]" src="https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/f41cd27f-b37b-47b1-9488-5df62ac4c335_800w.jpg" style={{}}/>
<div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
<div className="absolute bottom-4 left-4 flex items-center gap-2">
<span className="px-2.5 py-1 rounded-full text-xs bg-white/10 border border-white/10 font-geist" style={{}}>Project</span>
<span className="px-2.5 py-1 rounded-full text-xs bg-white/10 border border-white/10 font-geist" style={{}}>PWA</span>
</div>
</div>
<div className="p-5">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Offline‑first Notes</h3>
<p className="mt-1 text-sm text-gray-400 font-geist" style={{}}>IndexedDB, background sync, caching strategies.</p>
<div className="mt-4 flex items-center justify-between">
<div className="text-sm text-gray-400"><span className="text-gray-200 font-geist" style={{}}>LCP</span> <span className="text-gray-200 font-geist" style={{}}>1.6s</span></div>
<a className="inline-flex items-center gap-1 text-sm text-blue-300 hover:text-blue-200 font-geist" href="#pricing" style={{}}>
                Included in course <svg className="lucide lucide-arrow-right w-4 h-4" data-lucide="arrow-right" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</a>
</div>
...
<section className="pt-20 pb-20" id="instructors">
<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
<div className="flex items-end justify-between gap-6">
<div className="">
<p className="text-sm uppercase tracking-wider text-blue-300/90 font-geist" data-animate="up" data-delay="0" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>Meet your instructors</p>
<h2 className="mt-3 text-3xl lg:text-4xl tracking-tight text-white font-geist" data-animate="left" data-delay="100" style={{fontFamily: '"Plus Jakarta Sans", Inter, sans-serif', opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
            Senior engineers who ship at scale
          </h2>
</div>
<a className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm bg-gray-100 text-black hover:bg-gray-200 transition-all hover:scale-[1.02] font-geist" href="#pricing" style={{}}>
          Start learning
          <svg className="lucide lucide-handshake w-4 h-4" data-lucide="handshake" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="m11 17 2 2a1 1 0 1 0 3-3"></path><path d="m14 14 2.5 2.5a1 1 0 1 0 3-3l-3.88-3.88a3 3 0 0 0-4.24 0l-.88.88a1 1 0 1 1-3-3l2.81-2.81a5.79 5.79 0 0 1 7.06-.87l.47.28a2 2 0 0 0 1.42.25L21 4"></path><path d="m21 3 1 11h-2"></path><path d="M3 3 2 14l6.5 6.5a1 1 0 1 0 3-3"></path><path d="M3 4h8"></path></svg>
</a>
</div>
<div className="mt-10 grid md:grid-cols-2 lg:grid-cols-3 gap-6">

<div className="rounded-2xl border border-white/10 bg-black/40 overflow-hidden" data-animate="up" data-delay="80" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<img alt="Instructor" className="w-full h-48 object-cover" src="https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/321cc5b6-c611-4dd6-82df-a7ea47211580_800w.jpg" style={{}}/>
<div className="p-5">
<div className="flex items-center justify-between">
<div className="">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Alex Johnson</h3>
<p className="text-sm text-gray-400 font-geist" style={{}}>Frontend Lead • 10+ yrs</p>
</div>
<div className="flex items-center gap-2 text-gray-400">
<a aria-label="GitHub" className="hover:text-gray-200" href="#">
<svg className="lucide lucide-github w-4.5 h-4.5" data-lucide="github" fill="none" height="19" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="19" xmlns="http://www.w3.org/2000/svg"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path></svg>
</a>
<a aria-label="Twitter" className="hover:text-gray-200" href="#">
<svg className="lucide lucide-twitter w-4.5 h-4.5" data-lucide="twitter" fill="none" height="19" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="19" xmlns="http://www.w3.org/2000/svg"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"></path></svg>
</a>
</div>
</div>
<p className="text-sm text-gray-400 font-geist mt-3">Previously at large-scale SaaS, focuses on performance and DX.</p>
</div>
</div>
<div className="rounded-2xl border border-white/10 bg-black/40 overflow-hidden" data-animate="up" data-delay="120" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<img alt="Instructor" className="w-full h-48 object-cover" src="https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/e828f9ca-da5b-489e-8b41-aae701acb7de_800w.jpg" style={{}}/>
<div className="p-5">
<div className="flex items-center justify-between">
<div className="">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Priya Kaur</h3>
<p className="text-sm text-gray-400 font-geist" style={{}}>Staff Engineer • 8+ yrs</p>
</div>
<div className="flex items-center gap-2 text-gray-400">
<a aria-label="GitHub" className="hover:text-gray-200" href="#">
<svg className="lucide lucide-github w-4.5 h-4.5" data-lucide="github" fill="none" height="19" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="19" xmlns="http://www.w3.org/2000/svg"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path></svg>
</a>
<a aria-label="LinkedIn" className="hover:text-gray-200" href="#">
<svg className="lucide lucide-linkedin w-4.5 h-4.5" data-lucide="linkedin" fill="none" height="19" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="19" xmlns="http://www.w3.org/2000/svg"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect height="12" width="4" x="2" y="9"></rect><circle cx="4" cy="4" r="2"></circle></svg>
</a>
</div>
</div>
<p className="mt-3 text-sm text-gray-400 font-geist" style={{}}>Leads architecture for high-traffic apps, loves TypeScript and testing.</p>
</div>
</div>
<div className="rounded-2xl border border-white/10 bg-black/40 overflow-hidden" data-animate="up" data-delay="160" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<img alt="Instructor" className="w-full h-48 object-cover" src="https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/566d4b5d-ce71-4ae8-9976-c268fe32108a_800w.jpg" style={{}}/>
<div className="p-5">
<div className="flex items-center justify-between">
<div className="">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Miguel Santos</h3>
<p className="text-sm text-gray-400 font-geist" style={{}}>Principal • 12+ yrs</p>
</div>
<div className="flex items-center gap-2 text-gray-400">
<a aria-label="GitHub" className="hover:text-gray-200" href="#">
<svg className="lucide lucide-github w-4.5 h-4.5" data-lucide="github" fill="none" height="19" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="19" xmlns="http://www.w3.org/2000/svg"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path></svg>
</a>
<a aria-label="YouTube" className="hover:text-gray-200" href="#">
<svg className="lucide lucide-youtube w-4.5 h-4.5" data-lucide="youtube" fill="none" height="19" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="19" xmlns="http://www.w3.org/2000/svg"><path className="" d="M2.5 17a24.12 24.12 0 0 1 0-10 2 2 0 0 1 1.4-1.4 49.56 49.56 0 0 1 16.2 0A2 2 0 0 1 21.5 7a24.12 24.12 0 0 1 0 10 2 2 0 0 1-1.4 1.4 49.55 49.55 0 0 1-16.2 0A2 2 0 0 1 2.5 17"></path><path className="" d="m10 15 5-3-5-3z"></path></svg>
</a>
</div>
</div>
<p className="mt-3 text-sm text-gray-400 font-geist" style={{}}>Focuses on UX architecture and scalable component systems.</p>
</div>
</div>
</div>
</div>
...
<section className="bg-gray-950 bg-[url(https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/7416d619-3268-423e-8109-676f678b7806_3840w.jpg)] bg-cover pt-20 pb-20" id="pricing">
<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
<div className="flex items-end justify-between gap-6">
<div className="">
<p className="text-sm uppercase tracking-wider text-blue-300/90 font-geist" data-animate="up" data-delay="0" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>Pricing</p>
<h2 className="mt-3 text-3xl lg:text-4xl tracking-tight text-white font-geist" data-animate="left" data-delay="100" style={{fontFamily: '"Plus Jakarta Sans", Inter, sans-serif', opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
            Simple, transparent plans
          </h2>
<p className="mt-3 text-gray-400 font-geist" data-animate="up" data-delay="200" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>Lifetime access, updates included. 14‑day refund guarantee.</p>
</div>
<a className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm bg-blue-400 text-black hover:bg-blue-300 transition-all hover:scale-[1.02] font-geist" href="#enroll" style={{}}>
          Proceed to enroll
          <svg className="lucide lucide-credit-card w-4 h-4" data-lucide="credit-card" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><rect height="14" rx="2" width="20" x="2" y="5"></rect><line x1="2" x2="22" y1="10" y2="10"></line></svg>
</a>
</div>
<div className="mt-10 grid md:grid-cols-2 lg:grid-cols-3 gap-6">

<div className="rounded-2xl border border-white/10 bg-gradient-to-b from-gray-950 to-black p-6" data-animate="up" data-delay="80" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="flex items-center justify-between">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Starter</h3>
<span className="text-xs px-2 py-1 rounded-full bg-white/10 border border-white/10 font-geist" style={{}}>Self‑paced</span>
</div>
<p className="mt-1 text-sm text-gray-400 font-geist" style={{}}>Core curriculum &amp; projects</p>
<div className="mt-5 flex items-baseline gap-1">
<span className="text-3xl font-geist" style={{}}>$149</span><span className="text-sm text-gray-400 font-geist" style={{}}>one‑time</span>
</div>
...
<li className="flex items-center gap-2 text-gray-500 font-geist" style={{}}>
<svg className="lucide lucide-minus w-4 h-4" data-lucide="minus" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path></svg>
              Community &amp; certificate
            </li>
</ul>
<a className="mt-6 inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 rounded-full text-sm bg-white/10 text-gray-100 hover:bg-white/15 border border-white/10 transition-all font-geist" href="#enroll" style={{}}>
            Choose Starter
          </a>
</div>

<div className="rounded-2xl border border-blue-400/30 bg-gradient-to-b from-gray-900/80 to-black p-6 relative" data-animate="up" data-delay="120" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="absolute -top-3 right-4 px-2 py-1 rounded-full text-xs bg-blue-400 text-black font-geist" style={{}}>Best value</div>
<div className="flex items-center justify-between">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Pro</h3>
<span className="text-xs px-2 py-1 rounded-full bg-white/10 border border-white/10 font-geist" style={{}}>Certificate</span>
</div>
<p className="mt-1 text-sm text-gray-300 font-geist" style={{}}>Everything in Starter, plus community, code reviews, and cert.</p>
<div className="mt-5 flex items-baseline gap-1">
<span className="text-3xl font-geist" style={{}}>$299</span><span className="text-sm text-gray-400 font-geist" style={{}}>one‑time</span>
</div>
<ul className="mt-5 space-y-2 text-sm text-gray-300">
<li className="flex items-center gap-2 font-geist" style={{}}>
<svg className="lucide lucide-check w-4 h-4 text-teal-300" data-lucide="check" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M20 6 9 17l-5-5"></path></svg>
...
<a className="mt-6 inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 rounded-full text-sm bg-blue-400 text-black hover:bg-blue-300 transition-all font-geist" href="#enroll" style={{}}>
            Choose Pro
          </a>
</div>

<div className="rounded-2xl border border-white/10 bg-gradient-to-b from-gray-950 to-black p-6" data-animate="up" data-delay="160" style={{opacity: '1', transform: 'none', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
<div className="flex items-center justify-between">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Team</h3>
<span className="text-xs px-2 py-1 rounded-full bg-white/10 border border-white/10 font-geist" style={{}}>Up to 10 seats</span>
</div>
<p className="mt-1 text-sm text-gray-400 font-geist" style={{}}>Team license, reporting, Q&amp;A session.</p>
<div className="mt-5 flex items-baseline gap-1">
<span className="text-3xl font-geist" style={{}}>$1490</span><span className="text-sm text-gray-400 font-geist" style={{}}>one‑time</span>
</div>
<ul className="mt-5 space-y-2 text-sm text-gray-300">
<li className="flex items-center gap-2 font-geist" style={{}}>
<svg className="lucide lucide-check w-4 h-4 text-teal-300" data-lucide="check" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M20 6 9 17l-5-5"></path></svg>
...
<div className="">
<p className="text-sm uppercase tracking-wider text-blue-300/90 font-geist" data-animate="up" data-delay="0" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>FAQ</p>
<h2 className="mt-3 text-3xl lg:text-4xl tracking-tight text-white font-geist" data-animate="left" data-delay="100" style={{fontFamily: '"Plus Jakarta Sans", Inter, sans-serif', opacity: '0', transform: 'translateX(-16px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>
            Answers to common questions
          </h2>
<p className="mt-4 text-gray-400 font-geist" data-animate="up" data-delay="200" style={{opacity: '0', transform: 'translateY(12px)', transition: 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)'}}>Can’t find what you’re looking for? Email us: team@reactpro.dev</p>

<div className="mt-8 grid sm:grid-cols-2 gap-4">
<div className="p-5 rounded-xl border border-white/10 bg-black/40">
<div className="flex items-center gap-3">
<div className="w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center">
<svg className="lucide lucide-clock w-5 h-5 text-gray-300" data-lucide="clock" fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><path d="M12 6v6l4 2"></path><circle cx="12" cy="12" r="10"></circle></svg>
</div>
<div className="">
<p className="text-base font-geist" style={{}}>Time to finish</p>
<p className="text-sm text-gray-400 font-geist" style={{}}>4–6 weeks</p>
</div>
</div>
</div>
<div className="p-5 rounded-xl border border-white/10 bg-black/40">
...
<div className="p-6 rounded-2xl border border-white/10 bg-gradient-to-b from-gray-950 to-black">
<div className="divide-y divide-white/10">
<div className="py-4">
<button className="faq-toggle w-full flex items-center justify-between text-left">
<span className="text-sm text-gray-100 font-geist" style={{}}>Is this course beginner friendly?</span>
<span className="ml-4 w-6 h-6 flex items-center justify-center rounded-md bg-white/5 border border-white/10">
<svg className="lucide lucide-chevron-down chevron-down w-4 h-4 transition-transform duration-300" data-lucide="chevron-down" fill="none" height="16" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg"><path d="m6 9 6 6 6-6"></path></svg>
</span>
</button>
<div className="faq-content mt-3 hidden text-sm text-gray-400 leading-relaxed font-geist" id="faq-1-content" style={{}}>
                Yes. We start from the fundamentals and quickly ramp up to advanced topics with lots of hands‑on practice. If you’ve written a bit of JavaScript, you’ll be comfortable.
              </div>
</div>
<div className="py-4">
<button aria-controls="faq-2-content" aria-expanded="false" className="faq-toggle w-full flex items-center justify-between text-left">
<span className="text-sm text-gray-100 font-geist" style={{}}>Do I need prior React or TypeScript experience?</span>
<span className="ml-4 w-6 h-6 flex items-center justify-center rounded-md bg-white/5 border border-white/10">
<svg className="lucide lucide-chevron-down chevron-down w-4 h-4 transition-transform duration-300" data-lucide="chevron-down" fill="none" height="16" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg"><path d="m6 9 6 6 6-6"></path></svg>
</span>
</button>
<div className="faq-content mt-3 hidden text-sm text-gray-400 leading-relaxed font-geist" id="faq-2-content" style={{}}>
                No prior React experience is required. We teach React from first principles. TypeScript is optional but recommended—TypeScript primers are included when used.
              </div>
</div>
<div className="py-4">
<button aria-controls="faq-3-content" aria-expanded="false" className="faq-toggle w-full flex items-center justify-between text-left">
<span className="text-sm text-gray-100 font-geist" style={{}}>How does the certificate work?</span>
<span className="ml-4 w-6 h-6 flex items-center justify-center rounded-md bg-white/5 border border-white/10">
<svg className="lucide lucide-chevron-down chevron-down w-4 h-4 transition-transform duration-300" data-lucide="chevron-down" fill="none" height="16" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg"><path d="m6 9 6 6 6-6"></path></svg>
</span>
</button>
<div className="faq-content mt-3 hidden text-sm text-gray-400 leading-relaxed font-geist" id="faq-3-content" style={{}}>
                Pro learners unlock a graded capstone. Pass the capstone and receive a verifiable certificate link you can share on LinkedIn and with employers.
              </div>
</div>
<div className="py-4">
<button aria-controls="faq-4-content" aria-expanded="false" className="faq-toggle w-full flex items-center justify-between text-left">
<span className="text-sm text-gray-100 font-geist" style={{}}>Will the course be updated as React evolves?</span>
<span className="ml-4 w-6 h-6 flex items-center justify-center rounded-md bg-white/5 border border-white/10">
<svg className="lucide lucide-chevron-down chevron-down w-4 h-4 transition-transform duration-300" data-lucide="chevron-down" fill="none" height="16" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg"><path d="m6 9 6 6 6-6"></path></svg>
</span>
</button>
<div className="faq-content mt-3 hidden text-sm text-gray-400 leading-relaxed font-geist" id="faq-4-content" style={{}}>
                Yes. Your purchase includes lifetime updates. We track major React releases and add new modules, examples, and patterns as features land.
              </div>
</div>
<div className="py-4">
<button aria-controls="faq-5-content" aria-expanded="false" className="faq-toggle w-full flex items-center justify-between text-left">
<span className="text-sm text-gray-100 font-geist" style={{}}>Can my company pay or get an invoice?</span>
<span className="ml-4 w-6 h-6 flex items-center justify-center rounded-md bg-white/5 border border-white/10">
<svg className="lucide lucide-chevron-down chevron-down w-4 h-4 transition-transform duration-300" data-lucide="chevron-down" fill="none" height="16" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg"><path d="m6 9 6 6 6-6"></path></svg>
</span>
</button>
<div className="faq-content mt-3 hidden text-sm text-gray-400 leading-relaxed font-geist" id="faq-5-content" style={{}}>
                Absolutely. Use the Team plan for up to 10 seats, or contact us for a custom quote. We provide VAT/GST invoices and can onboard via PO for larger teams.
              </div>
</div>
</div>
</div>
...
<a className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-blue-400 text-black hover:bg-blue-300 transition-all hover:scale-[1.02] font-geist" href="#enroll" style={{}}>
          Enroll in minutes
          <svg className="lucide lucide-arrow-right w-4 h-4" data-lucide="arrow-right" fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
</a>
</div>
...
<section className="bg-gray-950 bg-[url(https://hoirqrkdgbmvpwutwuwj-all.supabase.co/storage/v1/object/public/assets/assets/9c0f0815-332d-4eb6-858d-d5e96d001df2_3840w.jpg)] bg-cover pt-20 pb-20" id="enroll">
<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
<div className="grid lg:grid-cols-2 gap-6">
<div className="rounded-2xl border border-white/10 bg-gradient-to-b from-gray-900/70 to-black p-6">
<div className="flex items-center justify-between">
<h3 className="text-lg tracking-tight font-geist" style={{}}>Secure checkout</h3>
<span className="text-xs px-2 py-1 rounded-full bg-white/10 border border-white/10 font-geist" style={{}}>SSL 256‑bit</span>
</div>
<p className="mt-1 text-sm text-gray-400 font-geist" style={{}}>Complete your enrollment. No hidden fees.</p>
<form className="mt-6 space-y-4" id="enroll-form">
<div>
<label className="block text-sm mb-1 font-geist" htmlFor="plan" style={{}}>Plan</label>
<div className="grid grid-cols-3 gap-2">
<button className="plan-btn col-span-1 px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-sm hover:bg-white/10 font-geist" data-amount="149" data-plan="Starter" style={{}} type="button">Starter</button>
<button className="plan-btn col-span-1 px-3 py-2 rounded-lg border border-blue-400/30 bg-blue-400/10 text-sm hover:bg-blue-400/20 ring-1 ring-inset ring-blue-400/30 font-geist" data-amount="299" data-plan="Pro" style={{}} type="button">Pro</button>
<button className="plan-btn col-span-1 px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-sm hover:bg-white/10 font-geist" data-amount="1490" data-plan="Team" style={{}} type="button">Team</button>
</div>
</div>
<div className="">
<label className="block text-sm mb-1 font-geist" htmlFor="email" style={{}}>Email</label>
<input className="w-full px-3 py-2 rounded-lg bg-black/40 border border-white/10 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500" id="email" name="email" placeholder="you@company.com" required="" type="email"/>
</div>
<div className="grid grid-cols-2 gap-3">
<div>
<label className="block text-sm mb-1 font-geist" htmlFor="name" style={{}}>Full name</label>
<input className="w-full px-3 py-2 rounded-lg bg-black/40 border border-white/10 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500" id="name" name="name" placeholder="Jane Doe" required="" type="text"/>
</div>
<div className="">
<label className="block text-sm mb-1 font-geist" htmlFor="country" style={{}}>Country</label>
<select className="w-full px-3 py-2 rounded-lg bg-black/40 border border-white/10 focus:outline-none focus:ring-2 focus:ring-blue-500" id="country" name="country">
<option className="font-geist" style={{}}>United States</option>
<option className="font-geist" style={{}}>Canada</option>
<option className="font-geist" style={{}}>United Kingdom</option>
<option className="font-geist" style={{}}>Germany</option>
<option className="font-geist" style={{}}>India</option>
<option className="font-geist" style={{}}>Australia</option>
<option className="font-geist" style={{}}>Other</option>
</select>
</div>
</div>
<div className="">
<label className="block text-sm mb-1 font-geist" htmlFor="coupon" style={{}}>Coupon code</label>
<div className="flex gap-2">
<input className="w-full px-3 py-2 rounded-lg bg-black/40 border border-white/10 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500" id="coupon" name="coupon" placeholder="REACTPRO10" type="text"/>
<button className="px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-sm hover:bg-white/10 font-geist" id="apply-coupon" style={{}} type="button">Apply</button>
</div>
<p className="mt-1 text-xs text-gray-500" id="coupon-msg"></p>
</div>
<div className="pt-4 border-t border-white/10">
<dl className="text-sm space-y-2">
<div className="flex items-center justify-between">
<dt className="text-gray-400 font-geist" style={{}}>Selected plan</dt>
<dd className="text-gray-200 font-geist" id="summary-plan" style={{}}>Pro</dd>
</div>
<div className="flex items-center justify-between">
<dt className="text-gray-400 font-geist" style={{}}>Subtotal</dt>
<dd className="text-gray-200 font-geist" id="summary-subtotal" style={{}}>$299</dd>
</div>
<div className="flex items-center justify-between">
<dt className="text-gray-400 font-geist" style={{}}>Discount</dt>
<dd className="text-blue-300 font-geist" id="summary-discount" style={{}}>-$0</dd>
</div>
<div className="flex items-center justify-between font-medium pt-2 border-t border-white/10">
<dt className="font-geist" style={{}}>Total</dt>
<dd className="font-geist" id="summary-total" style={{}}>$299</dd>
</div>
</dl>
<button className="mt-4 inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 rounded-full text-sm bg-blue-400 text-black hover:bg-blue-300 transition-all font-geist" style={{}} type="submit">
                Pay securely
                <svg className="lucide lucide-lock w-4 h-4" data-lucide="lock" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><rect height="11" rx="2" ry="2" width="18" x="3" y="11"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
</button>
<p className="mt-3 text-xs text-gray-500 text-center font-geist" style={{}}>By continuing you agree to the Terms and Privacy Policy.</p>
</div>
</form>
</div>
<div className="border-white/10 border rounded-2xl pt-6 pr-6 pb-6 pl-6 backdrop-blur-xl grayscale-0">
<h3 className="text-lg tracking-tight font-geist" style={{}}>What you’ll get</h3>
...
<div className="flex items-center gap-3">
<img alt="Student" className="w-10 h-10 rounded-full object-cover border border-white/10" src="https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&amp;w=96&amp;auto=format&amp;fit=crop" style={{}}/>
<div className="">
<p className="text-sm text-gray-200 font-geist" style={{}}>“This course helped me refactor a legacy app and ship features twice as fast.”</p>
<p className="text-xs text-gray-500 mt-1 font-geist" style={{}}>— Jamie, Frontend Engineer</p>
</div>
</div>
</div>
<div className="mt-6 grid sm:grid-cols-2 gap-3">
<div className="flex items-center gap-2 text-sm text-gray-400 font-geist" style={{}}>
<svg className="lucide lucide-refresh-ccw w-4 h-4" data-lucide="refresh-ccw" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M21 12a9 9 0 0 0-9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path><path d="M3 12a9 9 0 0 0 9 9 9.75 9.75 0 0 0 6.74-2.74L21 16"></path><path d="M16 16h5v5"></path></svg>
              14‑day money back
            </div>
<div className="flex items-center gap-2 text-sm text-gray-400 font-geist" style={{}}>
<svg className="lucide lucide-shield w-4 h-4" data-lucide="shield" fill="none" height="18" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewbox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"></path></svg>
...
<p className="mt-3 text-sm text-gray-400 max-w-md font-geist" style={{}}>A modern, practical path to mastering React. Learn the patterns, tools, and architecture used by high‑performing teams.</p>
</div>
<div>
<h4 className="text-sm font-geist" style={{}}>Explore</h4>
<ul className="mt-3 space-y-2 text-sm text-gray-400">
<li><a className="hover:text-gray-200 font-geist" href="#curriculum" style={{}}>Curriculum</a></li>
<li><a className="hover:text-gray-200 font-geist" href="#instructors" style={{}}>Instructors</a></li>
<li><a className="hover:text-gray-200 font-geist" href="#pricing" style={{}}>Pricing</a></li>
<li><a className="hover:text-gray-200 font-geist" href="#faq" style={{}}>FAQ</a></li>
</ul>
</div>
<div className="">
<h4 className="text-sm font-geist" style={{}}>Contact</h4>
<ul className="mt-3 space-y-2 text-sm text-gray-400">
<li><a className="hover:text-gray-200 font-geist" href="mailto:team@reactpro.dev" style={{}}>team@reactpro.dev</a></li>
<li className="flex items-center gap-3">
<a className="hover:text-gray-200 font-geist" href="#" style={{}}>Twitter</a>
<span className="text-gray-700 font-geist" style={{}}>•</span>
<a className="hover:text-gray-200 font-geist" href="#" style={{}}>GitHub</a>
</li>
</ul>
</div>
</div>
<div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4">
<p className="text-xs text-gray-500 font-geist" style={{}}>© <span className="font-geist" id="year" style={{}}>2025</span> React Pro Course. All rights reserved.</p>
<div className="flex items-center gap-4 text-xs text-gray-500">
<a className="hover:text-gray-300 font-geist" href="#" style={{}}>Terms</a>
<a className="hover:text-gray-300 font-geist" href="#" style={{}}>Privacy</a>
<a className="inline-flex items-center gap-1 hover:text-gray-300 font-geist" href="#overview" style={{}}>
            Back to top

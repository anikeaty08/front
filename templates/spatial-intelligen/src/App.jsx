...
gtag('config', 'G-2M6V79H761');
        ...
        (function() {
            const tunnel = document.getElementById('telemetry-tunnel');
            const scene = document.getElementById('telemetry-scene');
            const title = document.getElementById('telemetry-title');
            const text = "Navigate your architecture in absolute clarity.";
            
            // GSAP Title Reveal
            title.innerHTML = text.split(' ').map(w => `<span class="inline-block overflow-hidden pb-1"><span class="t-word inline-block translate-y-full opacity-0">${w}</span></span>`).join(' ');
            
            const tl = gsap.timeline({delay: 0.5});
            tl.to('.t-word', {y: 0, opacity: 1, duration: 1, stagger: 0.1, ease: 'power4.out'})
              .to(['#telemetry-subtitle', '#telemetry-actions'], {opacity: 1, y: 0, duration: 0.8}, "-=0.5");

            // 3D Engine
            const generators = {
                chart: () => `<div class="flex items-center justify-between mb-3"><span class="text-[10px] text-neutral-400">Latency</span><iconify-icon icon="solar:chart-square-bold-duotone" class="text-blue-400"></iconify-icon></div><div class="flex items-end gap-1 h-12">${Array.from({length:6}).map(()=>`<div class="flex-1 bg-blue-500/20 rounded-t-sm" style="height:${Math.random()*100}%"></div>`).join('')}</div>`,
                stats: () => `<div class="text-[10px] text-neutral-500 mb-1">Nodes Active</div><div class="text-2xl font-mono text-white">1,024</div><div class="mt-2 text-[9px] text-emerald-400 bg-emerald-400/10 px-1.5 py-0.5 rounded w-fit">+12.5%</div>`,
                status: () => `<div class="flex justify-between items-center mb-4"><span class="text-[10px] text-neutral-200">DB Sync</span><div class="h-1.5 w-1.5 rounded-full bg-indigo-500 animate-pulse shadow-[0_0_8px_#6366f1]"></div></div><div class="h-1 w-full bg-white/5 rounded-full overflow-hidden"><div class="h-full bg-indigo-500 w-[85%]"></div></div>`
            };
            ...
            // Parallax
            document.getElementById('telemetry-viewport').addEventListener('mousemove', (e) => {
                const rect = e.currentTarget.getBoundingClientRect();
                const x = (e.clientX - rect.left) / rect.width;
                const y = (e.clientY - rect.top) / rect.height;
                gsap.to(scene, { perspectiveOrigin: `${45 + x*10}% ${45 + y*10}%`, duration: 2, ease: 'power2.out' });
            });
        })();
        ...
        (() => {
          const section = document.getElementById("h-scroll-section");
          const track = document.getElementById("h-scroll-track");
          const progress = document.getElementById("h-scroll-progress");
        ...
        document.addEventListener("DOMContentLoaded", (event) => {
                ...
                const maskedTargets = document.querySelectorAll('.masked-reveal-target');

                maskedTargets.forEach(target => {
                    // Split text into words and wrap
                    const text = target.innerText;
                    target.innerHTML = ''; // Clear original
                ...
                // Hero Animation Timeline (runs on load)
                ...
                // We use a timeline linked to the scroll position of the inner container
                const expandTl = gsap.timeline({
                    scrollTrigger: {
                        trigger: container,
                        start: "top 60%", // Start animating when container is 60% down the viewport
                        end: "bottom 80%", // End when bottom of container is near bottom
                        scrub: 1, // Smooth scrubbing
                    }
                });

                // Level 1 Node fades in
                expandTl.to('.seq-node:nth-of-type(1)', { opacity: 1, y: 0, duration: 1 })

                // Level 1 to 2 Vertical Line draws
                .to('.seq-line:nth-of-type(1)', { scaleY: 1, duration: 1.5, ease: "none" })

                // Level 2 Horizontal Line draws out from center
                .to('.seq-line-h:nth-of-type(1)', { scaleX: 1, duration: 1, ease: "power1.inOut" }, "-=0.5")

                // Level 2 Nodes fade in
                .to('.seq-node:nth-of-type(2), .seq-node:nth-of-type(3)', { opacity: 1, y: 0, duration: 1, stagger: 0.2 }, "-=0.2")

                // Level 2 to 3 Vertical Lines draw down
                .to('.seq-line:nth-of-type(2), .seq-line:nth-of-type(3)', { scaleY: 1, duration: 1.5, ease: "none", stagger: 0.1 })

                // Level 3 Horizontal Line draws
                .to('.seq-line-h:nth-of-type(2)', { scaleX: 1, duration: 1, ease: "power1.inOut" }, "-=0.5")

                // Level 3 Complex Nodes fade in
      ...
      // Horizontal scrolling section logic
      const hSection = document.getElementById('h-scroll-section');
      const hTrack = document.getElementById('h-scroll-track');
      const hProgress = document.getElementById('h-scroll-progress');

      window.addEventListener('scroll', () => {
          if(!hSection || !hTrack || !hProgress) return;

          const rect = hSection.getBoundingClientRect();
          if (rect.top <= 0 && rect.bottom >= window.innerHeight) {
              const scrollProgress = Math.abs(rect.top) / (rect.height - window.innerHeight);
              const maxScroll = hTrack.scrollWidth - window.innerWidth;
              hTrack.style.transform = `translate3d(-${scrollProgress * maxScroll}px, 0, 0)`;
              hProgress.style.width = `${scrollProgress * 100}%`;
          } else if (rect.top > 0) {
              hTrack.style.transform = `translate3d(0, 0, 0)`;
              hProgress.style.width = `0%`;
          } else if (rect.bottom < window.innerHeight) {
              const maxScroll = hTrack.scrollWidth - window.innerWidth;
              hTrack.style.transform = `translate3d(-${maxScroll}px, 0, 0)`;
      ...
      counters.forEach(counter => {
          ...
          let current = target - 0.5; // Start slightly below

          setInterval(() => {
              // Add tiny jitter to make it look live
...
<div className="flex gap-3 bg-center text-white bg-[url(https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/53e40404-d9d0-4f28-9e82-7f2f45062936_1600w.png)] bg-cover pt-6 pr-12 pb-6 pl-12 gap-x-3 gap-y-3 items-center">
</div>
<div className="hidden md:flex items-center gap-8 text-xs font-mono text-neutral-400">
<a className="hover:text-white transition-colors duration-300 flex items-center gap-1.5 group" href="#">
...
<span className="">System: Online</span>
<span className="text-neutral-300">Ping: 12ms</span>
</div>
...
<section className="overflow-hidden selection:bg-white/20 selection:text-white flex flex-col text-[#ededed] font-sans bg-[#0a0a0a] w-screen h-screen relative" id="nexus-engine-hero">
<div className="w-full max-w-[1400px] mx-auto min-h-[800px] flex flex-col relative border-x border-white/5 bg-[#030303] overflow-hidden" style={{fontFamily: '\'Inter\', sans-serif', background: 'radial-gradient(ellipse at top, rgba(255,255,255,0.02) 0%, transparent 100%)'}}>





<div className="absolute -top-1.5 -left-1.5 w-3 h-3 text-white/20 z-50"><svg fill="none" stroke="currentColor" strokeWidth="1.5" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4"></path>
</svg></div>
<div className="absolute -top-1.5 -right-1.5 w-3 h-3 text-white/20 z-50"><svg fill="none" stroke="currentColor" strokeWidth="1.5" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4"></path>
</svg></div>
<div className="absolute -bottom-1.5 -left-1.5 w-3 h-3 text-white/20 z-50"><svg fill="none" stroke="currentColor" strokeWidth="1.5" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4"></path>
</svg></div>
<div className="absolute -bottom-1.5 -right-1.5 w-3 h-3 text-white/20 z-50"><svg fill="none" stroke="currentColor" strokeWidth="1.5" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4"></path>
</svg></div>
<div className="w-full max-w-[1400px] mx-auto min-h-[800px] flex flex-col relative border-x border-white/5 bg-[#030303] overflow-hidden" style={{fontFamily: '\'Inter\', sans-serif', background: 'radial-gradient(ellipse at top, rgba(255,255,255,0.02) 0%, transparent 100%)'}}>
...
<div className="absolute inset-0 z-0 pointer-events-none flex items-center justify-center overflow-hidden opacity-80" id="telemetry-scene" style={{perspective: '900px'}}>
<div className="relative w-full h-full" id="telemetry-tunnel" style={{transformStyle: 'preserve-3d'}}></div>
</div>

<main className="relative z-40 flex-1 flex flex-col items-center justify-center text-center px-6 w-full max-w-5xl mx-auto">
<a className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-normal text-gray-300 mb-8 hover:bg-white/[0.08] transition-all backdrop-blur-md" href="#" style={{background: 'linear-gradient(#111, #111) padding-box, linear-gradient(90deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.02) 100%) border-box', border: '1px solid transparent'}}>
<span className="flex h-1.5 w-1.5 relative">
<span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
<span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-blue-500"></span>
</span>
                    Core Infrastructure v3.0 deployed
                    <iconify-icon className="text-xs text-gray-400" icon="solar:alt-arrow-right-bold-duotone"></iconify-icon>
</a>
<h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-normal tracking-tight text-white mb-6 leading-[1.05]" id="telemetry-title" style={{textShadow: '0 4px 40px rgba(0,0,0,0.5)'}}></h1>
<p className="text-sm md:text-base text-neutral-400 mb-10 max-w-2xl leading-relaxed opacity-0" id="telemetry-subtitle">
...
<div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto opacity-0" id="telemetry-actions">
...
<iconify-icon className="text-lg" icon="solar:rocket-bold-duotone"></iconify-icon> Initialize Instance
                    </button>
<button className="w-full sm:w-auto px-8 py-4 rounded-xl bg-neutral-900/40 text-white font-normal text-sm hover:bg-neutral-800 transition-all backdrop-blur-md flex items-center justify-center gap-2.5" style={{background: 'linear-gradient(rgba(20,20,20,0.4), rgba(20,20,20,0.4)) padding-box, linear-gradient(180deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.02) 100%) border-box', border: '1px solid transparent'}}>
                        API Reference
                    </button>
</div>
</main>
...
<iconify-icon className="text-2xl text-neutral-300 group-hover:text-white" icon="solar:box-minimalistic-linear"></iconify-icon>
</div>
...
<p className="text-sm text-neutral-400 font-light leading-relaxed mb-8 flex-1">
                  Constructing multi-dimensional web planes. Moving beyond flat
                  coordinate systems into true Z-axis awareness.
                </p>
...
<div className="w-12 h-12 rounded-xl bg-black border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-500 shadow-inner">
<iconify-icon className="text-2xl text-neutral-300 group-hover:text-white" icon="solar:cpu-bolt-linear"></iconify-icon>
</div>
...
<p className="text-sm text-neutral-400 font-light leading-relaxed mb-8 flex-1 relative z-10">
                  Embedded deterministic models enabling predictive rendering
                  and autonomous state resolution.
                </p>
...
<span className="text-[10px] font-mono text-neutral-500 uppercase ml-2">
                    Processing
                  </span>
</div>
</div>
...
<div className="flex justify-between items-start mb-8 relative z-10">
<div className="w-12 h-12 rounded-xl bg-black border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-500 shadow-inner">
<iconify-icon className="text-2xl text-neutral-300 group-hover:text-white" icon="solar:code-scan-linear"></iconify-icon>
</div>
<div className="text-[9px] font-mono text-neutral-500 border border-white/10 px-2 py-1 rounded bg-black/50">
                    MOD_03
                  </div>
...
<p className="text-sm text-neutral-400 font-light leading-relaxed mb-8 flex-1 relative z-10">
                  Compiling direct to browser APIs. Stripping abstractions to
                  deliver bare-metal performance for graphic pipelines.
                </p>
<div className="font-mono text-[10px] text-neutral-500 flex flex-col gap-1 relative z-10 mt-auto">
<span className="group-hover:text-white transition-colors">
                    &gt; compile_shaders()
                  </span>
<span className="opacity-50 group-hover:opacity-100 transition-opacity delay-75">
                    &gt; init_webgl_ctx()
                  </span>
<span className="opacity-25 group-hover:opacity-100 transition-opacity delay-150">
                    &gt; OK
                  </span>
</div>
</div>
...
<iconify-icon className="text-neutral-500 text-xl group-hover:text-white transition-colors" icon="solar:chart-square-linear"></iconify-icon>
</div>
...
<div className="flex justify-between items-start">
<iconify-icon className="text-neutral-500 text-xl group-hover:text-white transition-colors" icon="solar:layers-minimalistic-linear"></iconify-icon>
...
<div className="w-24 h-24 border border-white/20 bg-black/80 backdrop-blur-md absolute layer-float-1 flex items-center justify-center text-[10px] font-mono text-white/50">
                L3
              </div>
<div className="w-24 h-24 border border-white/10 bg-white/5 absolute layer-float-2 mt-8 flex items-center justify-center text-[10px] font-mono text-white/30">
                L2
              </div>
<div className="w-24 h-24 border border-white/5 bg-white/[0.02] absolute layer-float-3 mt-16 flex items-center justify-center text-[10px] font-mono text-white/10">
                L1
              </div>
</div>
<div className="">
...
<div className="flex justify-between items-start">
<iconify-icon className="text-neutral-500 text-xl group-hover:text-white transition-colors" icon="solar:shield-check-linear"></iconify-icon>
...
<div className="col-span-1 row-span-1 p-6 rounded-[2rem] bg-[#050505] border border-white/10 relative overflow-hidden group hover:border-white/20 transition-all duration-500 flex flex-col justify-between">
<div className="flex justify-between items-start">
<iconify-icon className="text-neutral-500 text-xl group-hover:text-white transition-colors" icon="solar:radar-linear"></iconify-icon>
</div>
...
<p className="text-sm text-neutral-400 font-light">
                Distributed state synchronization ensuring sub-millisecond data
                availability across all active nodes.
              </p>
</div>
...
<div className="bg-[#000] relative" id="h-scroll-section" style={{height: '2424.3px'}}>
...
<div className="h-full bg-white w-0 transition-all duration-100 ease-out" id="h-scroll-progress" style={{width: '0%'}}></div>
</div>
<div className="flex will-change-transform pr-[20vw] pl-[10vw] gap-x-8 gap-y-8 items-center" id="h-scroll-track" style={{width: 'max-content'}}>

<div className="schema-card schema-card-enhanced w-[85vw] md:w-[450px] aspect-square rounded-[2rem] p-10 flex flex-col relative overflow-hidden flex-shrink-0 group transition-all duration-500 shadow-2xl">
...
<div className="schema-vignette"></div>
<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[170px] h-[170px] rounded-full border border-white/[0.08] schema-ring-spin"></div>
...
<div className="w-3 h-3 rounded-full bg-white shadow-[0_0_10px_rgba(255,255,255,0.75)] schema-pulse-dot"></div>
</div>
<div className="absolute top-1/2 left-[22%] w-[28%] h-px bg-gradient-to-r from-white/0 via-white/20 to-white/0 schema-line-h-center"></div>
<div className="absolute top-1/2 right-[22%] w-[28%] h-px bg-gradient-to-r from-white/0 via-white/20 to-white/0 schema-line-h-center" style={{animationDelay: '1s'}}></div>
<div className="absolute left-1/2 top-[22%] h-[28%] w-px bg-gradient-to-b from-white/0 via-white/20 to-white/0 schema-line-v-center" style={{animationDelay: '2s'}}></div>
<div className="absolute left-1/2 bottom-[22%] h-[28%] w-px bg-gradient-to-b from-white/0 via-white/20 to-white/0 schema-line-v-center" style={{animationDelay: '3s'}}></div>
<div className="schema-node-clean" style={{top: '50%', left: '22%'}}></div>
<div className="schema-node-clean" style={{top: '50%', left: '78%', animationDelay: '0.5s'}}></div>
<div className="schema-node-clean" style={{top: '22%', left: '50%', animationDelay: '1s'}}></div>
<div className="schema-node-clean" style={{top: '78%', left: '50%', animationDelay: '1.5s'}}></div>
</div>
...
<p className="text-sm text-neutral-400 font-light leading-relaxed mb-8">
                Frame-perfect fidelity mapped directly to execution cycles.
                Every pixel shift is calculated and predictable.
              </p>
...
<div className="schema-card schema-card-enhanced w-[85vw] md:w-[450px] aspect-square rounded-[2rem] p-10 flex flex-col relative overflow-hidden flex-shrink-0 group transition-all duration-500 shadow-2xl">
<div className="absolute inset-0 dot-grid opacity-[0.12] pointer-events-none"></div>
<div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.025),transparent_72%)] pointer-events-none"></div>
<div className="schema-layer opacity-90">
<div className="schema-vignette"></div>
<div className="absolute top-[28%] left-[20%] right-[20%] h-px bg-gradient-to-r from-white/0 via-white/22 to-white/0 schema-line-h-center"></div>
<div className="absolute top-[50%] left-[16%] right-[16%] h-px bg-gradient-to-r from-white/0 via-white/28 to-white/0 schema-line-h-center" style={{animationDelay: '1.5s'}}></div>
<div className="absolute top-[72%] left-[20%] right-[20%] h-px bg-gradient-to-r from-white/0 via-white/22 to-white/0 schema-line-h-center" style={{animationDelay: '3s'}}></div>
<div className="absolute top-[28%] left-[64%] w-px h-[44%] bg-gradient-to-b from-white/0 via-white/12 to-white/0 schema-line-v-center"></div>
...
<div className="w-1.5 h-1.5 rounded-full bg-white schema-pulse-dot"></div>
</div>
</div>
<div className="schema-node-clean" style={{top: '28%', left: '20%'}}></div>
<div className="schema-node-clean" style={{top: '50%', left: '16%', animationDelay: '0.5s'}}></div>
<div className="schema-node-clean" style={{top: '72%', left: '20%', animationDelay: '1s'}}></div>
</div>
<span className="text-6xl font-light text-white/[0.05] absolute top-6 right-8 tracking-tighter font-mono group-hover:text-white/[0.08] transition-colors">
              02
            </span>
<div className="flex z-10 bg-white/[0.04] w-12 h-12 border-white/[0.08] border rounded-xl mt-2 mb-auto relative items-center justify-center">
<svg className="text-white" fill="none" height="24" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
...
<p className="text-sm text-neutral-400 font-light leading-relaxed mb-8">
                Coalescing multi-modal inputs into a single authoritative event
                stream with zero-delay processing.
              </p>
<div className="p-4 bg-black border border-white/[0.08] rounded-lg flex justify-between items-center group-hover:border-white/15 transition-colors">
<span className="text-[10px] font-mono text-neutral-500 uppercase">
                  Polling Rate
                </span>
<span className="text-[10px] font-mono text-white">1000 HZ</span>
</div>
</div>
</div>

<div className="schema-card schema-card-enhanced w-[85vw] md:w-[450px] aspect-square rounded-[2rem] p-10 flex flex-col relative overflow-hidden flex-shrink-0 group transition-all duration-500 shadow-2xl">
<div className="absolute inset-0 bg-gradient-to-tr from-white/[0.015] to-transparent pointer-events-none"></div>
<div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.03),transparent_74%)] pointer-events-none"></div>
<div className="schema-layer opacity-95">
<div className="schema-vignette"></div>
<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[220px] h-[220px] rounded-full border border-white/[0.08] schema-ring-spin"></div>
<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[140px] h-[140px] rounded-full border border-white/[0.08] schema-ring-spin-reverse"></div>
<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white schema-pulse-dot-pos"></div>
<div className="absolute top-1/2 left-1/2 -translate-x-1/2 w-[220px] h-px bg-gradient-to-r from-white/0 via-white/18 to-white/0 schema-line-h-center"></div>
<div className="absolute top-1/2 left-1/2 -translate-y-1/2 h-[220px] w-px bg-gradient-to-b from-white/0 via-white/18 to-white/0 schema-line-v-center" style={{animationDelay: '2s'}}></div>
<div className="schema-node-clean" style={{top: '50%', left: '18%'}}></div>
<div className="schema-node-clean" style={{top: '50%', left: '82%', animationDelay: '0.5s'}}></div>
<div className="schema-node-clean" style={{top: '18%', left: '50%', animationDelay: '1s'}}></div>
<div className="schema-node-clean" style={{top: '82%', left: '50%', animationDelay: '1.5s'}}></div>
</div>
<span className="text-6xl font-light text-white/[0.05] absolute top-6 right-8 tracking-tighter font-mono group-hover:text-white/[0.08] transition-colors">
              03
            </span>
...
<p className="text-sm text-neutral-400 font-light leading-relaxed mb-8">
                CRDT-based state reconciliation ensuring all connected endpoints
                reflect truth instantaneously.
              </p>
<div className="p-4 bg-black border border-white/[0.08] rounded-lg flex justify-between items-center group-hover:border-white/15 transition-colors">
<span className="text-[10px] font-mono text-neutral-500 uppercase">
                  Topology
...
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.04] schema-flash"></div>
<div className="w-6 h-6 rounded bg-white shadow-[0_0_14px_rgba(255,255,255,0.75)] schema-flash-1"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.04] schema-flash-2"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.06] schema-flash-3"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.04] schema-flash-2"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.06] schema-flash-1"></div>
<div className="w-6 h-6 rounded bg-white shadow-[0_0_14px_rgba(255,255,255,0.75)] schema-flash-3"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.04] schema-flash"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.06] schema-flash-3"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.04] schema-flash"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.06] schema-flash-2"></div>
<div className="w-6 h-6 rounded bg-white shadow-[0_0_14px_rgba(255,255,255,0.75)] schema-flash-1"></div>
<div className="w-6 h-6 rounded bg-white shadow-[0_0_14px_rgba(255,255,255,0.75)] schema-flash-2"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.06] schema-flash-3"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.04] schema-flash"></div>
<div className="w-6 h-6 rounded border border-white/[0.18] bg-white/[0.06] schema-flash-1"></div>
</div>
</div>
<div className="absolute h-px bg-gradient-to-r from-white/0 via-white/16 to-white/0 top-1/2 left-[10%] right-[10%] schema-line-h-center"></div>
<div className="absolute w-px bg-gradient-to-b from-white/0 via-white/16 to-white/0 left-1/2 top-[12%] bottom-[12%] schema-line-v-center" style={{animationDelay: '1.5s'}}></div>
</div>
<span className="text-6xl font-light text-white/[0.05] absolute top-6 right-8 tracking-tighter font-mono group-hover:text-white/[0.08] transition-colors">
              04
            </span>
...
<p className="text-sm text-neutral-400 font-light leading-relaxed mb-8">
                Dynamic heap management and memory pooling ensuring zero garbage
                collection pauses during peak loads.
              </p>
<div className="p-4 bg-black border border-white/[0.08] rounded-lg flex justify-between items-center group-hover:border-white/15 transition-colors">
<span className="text-[10px] font-mono text-neutral-500 uppercase">
                  Buffer Size
                </span>
<span className="text-[10px] font-mono text-white">2048 MB</span>
</div>
</div>
</div>
...
<div className="absolute inset-[36px] rounded-full border border-white/[0.08] schema-ring-spin-inset"></div>
<div className="absolute inset-[72px] rounded-full border border-white/[0.08] schema-ring-spin-inset-reverse"></div>
<div className="absolute inset-[108px] rounded-full border border-white/[0.12] schema-ring-spin-inset"></div>
<div className="absolute top-1/2 left-1/2 w-3 h-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white schema-pulse-dot-pos"></div>
<div className="absolute inset-0 schema-ring-spin-inset">
<div className="absolute top-0 left-1/2 w-2 h-2 -translate-x-1/2 rounded-full bg-white/90 shadow-[0_0_10px_rgba(255,255,255,0.6)]"></div>
</div>
<div className="absolute inset-0 schema-ring-spin-inset-reverse">
<div className="absolute bottom-0 left-1/2 w-2 h-2 -translate-x-1/2 rounded-full bg-white/90 shadow-[0_0_10px_rgba(255,255,255,0.6)]"></div>
</div>
<div className="absolute inset-0 schema-ring-spin-inset" style={{animationDuration: '15s'}}>
<div className="absolute top-1/2 left-0 w-2 h-2 -translate-y-1/2 rounded-full bg-white/90 shadow-[0_0_10px_rgba(255,255,255,0.6)]"></div>
</div>
<div className="absolute inset-0 schema-ring-spin-inset-reverse" style={{animationDuration: '18s'}}>
<div className="absolute top-1/2 right-0 w-2 h-2 -translate-y-1/2 rounded-full bg-white/90 shadow-[0_0_10px_rgba(255,255,255,0.6)]"></div>
</div>
<div className="absolute top-1/2 left-[12%] right-[12%] h-px bg-gradient-to-r from-white/0 via-white/16 to-white/0 schema-line-h-center"></div>
<div className="absolute left-1/2 top-[12%] bottom-[12%] w-px bg-gradient-to-b from-white/0 via-white/16 to-white/0 schema-line-v-center" style={{animationDelay: '2s'}}></div>
</div>
</div>
</div>
<span className="text-6xl font-light text-white/[0.05] absolute top-6 right-8 tracking-tighter font-mono group-hover:text-white/[0.08] transition-colors">
...
<p className="text-sm text-neutral-400 font-light leading-relaxed mb-8">
                Instantaneous state broadcast to edge nodes worldwide using
                hyper-optimized binary protocols.
              </p>
<div className="p-4 bg-black border border-white/[0.08] rounded-lg flex justify-between items-center group-hover:border-white/15 transition-colors">
<span className="text-[10px] font-mono text-neutral-500 uppercase">
                  Target Latency
                </span>
<span className="text-[10px] font-mono text-white">~12 MS</span>
</div>
</div>
</div>
</div>
...
<style>
        ...
        .schema-node-clean {
          ...
          animation: schemaNodePulse 2s infinite alternate;
        }

        @keyframes schemaNodePulse {
          0% { transform: translate(-50%, -50%) scale(0.8); opacity: 0.5; }
          100% { transform: translate(-50%, -50%) scale(1.2); opacity: 1; box-shadow: 0 0 15px rgba(255,255,255,0.8); }
        }
        .schema-ring-spin { animation: schemaRingSpin 20s linear infinite; }
        .schema-ring-spin-reverse { animation: schemaRingSpin 25s linear infinite reverse; }
        @keyframes schemaRingSpin {
          0% { transform: translate(-50%, -50%) rotate(0deg); }
          100% { transform: translate(-50%, -50%) rotate(360deg); }
        }
        .schema-box-spin { animation: schemaBoxSpin 15s linear infinite; }
        @keyframes schemaBoxSpin {
          0% { transform: translate(-50%, -50%) rotate(0deg); }
          100% { transform: translate(-50%, -50%) rotate(90deg); }
        }
        .schema-line-h-center { animation: schemaLineHCenter 4s ease-in-out infinite alternate; transform-origin: center; }
        .schema-line-v-center { animation: schemaLineVCenter 4s ease-in-out infinite alternate; transform-origin: center; }
        @keyframes schemaLineHCenter { 0% { transform: scaleX(0); opacity: 0; } 100% { transform: scaleX(1); opacity: 1; } }
        @keyframes schemaLineVCenter { 0% { transform: scaleY(0); opacity: 0; } 100% { transform: scaleY(1); opacity: 1; } }
        .schema-flash { animation: schemaFlash 2s infinite alternate; }
        .schema-flash-1 { animation: schemaFlash 2s infinite alternate 0.5s; }
        .schema-flash-2 { animation: schemaFlash 2s infinite alternate 1s; }
        .schema-flash-3 { animation: schemaFlash 2s infinite alternate 1.5s; }
        @keyframes schemaFlash {
          0%, 40% { opacity: 0.3; background: rgba(255,255,255,0.04); box-shadow: none; }
          100% { opacity: 1; background: #fff; box-shadow: 0 0 14px rgba(255,255,255,0.75); }
        }
        .schema-pulse-dot { animation: schemaPulseDot 1.5s ease-in-out infinite alternate; }
        @keyframes schemaPulseDot { 0% { transform: scale(0.8); opacity: 0.5; } 100% { transform: scale(1.5); opacity: 1; box-shadow: 0 0 20px rgba(255,255,255,1); } }
        .schema-pulse-dot-pos { animation: schemaPulseDotPos 1.5s ease-in-out infinite alternate; }
        @keyframes schemaPulseDotPos {
          0% { transform: translate(-50%, -50%) scale(0.8); opacity: 0.5; }
          100% { transform: translate(-50%, -50%) scale(1.5); opacity: 1; box-shadow: 0 0 20px rgba(255,255,255,1); }
        }
        .schema-ring-spin-inset { animation: schemaRingSpinInset 20s linear infinite; }
        .schema-ring-spin-inset-reverse { animation: schemaRingSpinInset 25s linear infinite reverse; }
        @keyframes schemaRingSpinInset { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
      </style>
<style>
        ...
        .schema-card-enhanced > span.text-6xl {
          color: rgba(255, 255, 255, 0.12) !important;
        }
        .schema-card-enhanced:hover > span.text-6xl {
...
<h2 className="text-[10vw] font-bold uppercase tracking-tighter text-white mx-8">
              DEPLOYMENTS
            </h2>
<h2 className="text-[10vw] font-bold uppercase tracking-tighter text-transparent mx-8" style={{WebkitTextStroke: '1px rgba(255,255,255,0.8)'}}>
              DEPLOYMENTS
            </h2>
<h2 className="text-[10vw] font-bold uppercase tracking-tighter text-white mx-8">
              DEPLOYMENTS
...
<a className="group/proj block relative rounded-[2rem] overflow-hidden aspect-[4/3] border border-white/10 hover:border-white/30 transition-all duration-500 shadow-2xl bg-[#050505] p-2" href="#">
...
<img alt="Work 1" className="w-full h-full object-cover filter grayscale opacity-40 group-hover/proj:opacity-80 group-hover/proj:scale-105 transition-all duration-700 mix-blend-luminosity" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/variants/468eeec1-e10c-4305-ad63-50cff0e023dc/1600w.png"/>
...
<a className="group/proj block relative rounded-[2rem] overflow-hidden aspect-[4/3] border border-white/10 hover:border-white/30 transition-all duration-500 shadow-2xl md:mt-24 bg-[#050505] p-2" href="#">
<div className="absolute top-6 right-6 z-20 bg-black/50 backdrop-blur-md border border-white/20 text-white font-mono text-[10px] px-3 py-1 rounded-full opacity-0 group-hover/proj:opacity-100 translate-y-2 group-hover/proj:translate-y-0 transition-all duration-300 flex items-center gap-2">
<div className="w-1.5 h-1.5 rounded-full bg-white animate-pulse"></div>
              LIVE
            </div>
<div className="relative w-full h-full rounded-[1.5rem] overflow-hidden bg-neutral-900">
<img alt="Work 2" className="w-full h-full object-cover filter grayscale opacity-40 group-hover/proj:opacity-80 group-hover/proj:scale-105 transition-all duration-700 mix-blend-luminosity" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/variants/823f2b22-afcd-4c1d-b3f6-c025e505e98c/1600w.jpg"/>
<div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent flex flex-col justify-end p-8">
<div className="translate-y-4 group-hover/proj:translate-y-0 transition-transform duration-500">
<span className="text-[10px] font-mono font-bold uppercase tracking-widest text-neutral-400 mb-2 block opacity-0 group-hover/proj:opacity-100 transition-opacity duration-500">
                    AI Platform
...
<div className="absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-white/[0.03] to-transparent pointer-events-none"></div>
...
<span className="text-[10px] font-mono uppercase tracking-[0.22em] text-white/90">
            Unified System
          </span>
</div>
<h2 className="md:text-5xl text-4xl font-semibold text-white tracking-tight mb-4">
    Data Convergence
  </h2>
<p className="max-w-md mx-auto text-sm text-neutral-400 font-light leading-relaxed">
    Multiple scattered elements, fragmented workflows, and disparate
    interfaces are pulled into a singular cognitive core. Complexity
    compressed into absolute control.
  </p>
</div>
...
<div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] border-white/20 border-solid rounded-full pointer-events-none" style={{animation: 'radar-pull-anim 4s cubic-bezier(0.5, 0, 0.8, 1) infinite', animationDelay: '0s'}}></div>
<div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] border-white/20 border-solid rounded-full pointer-events-none" style={{animation: 'radar-pull-anim 4s cubic-bezier(0.5, 0, 0.8, 1) infinite', animationDelay: '1.33s'}}></div>
<div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] border-white/20 border-solid rounded-full pointer-events-none" style={{animation: 'radar-pull-anim 4s cubic-bezier(0.5, 0, 0.8, 1) infinite', animationDelay: '2.66s'}}></div>
...
<svg className="animate-pulse" fill="black" height="32" stroke="black" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="32" xmlns="http://www.w3.org/2000/svg">
...
<style>
        @keyframes core-pulse-anim {
          ...
          50% {
            ...
            box-shadow: 0 0 80px rgba(255,255,255,0.7), inset 0 0 40px rgba(255,255,255,0.3);
          }
        }
        ...
        @keyframes gravity-well-anim {
          0% {
            transform: translate(calc(-50% + var(--tx)), calc(-50% + var(--ty))) scale(1.1) rotate(calc(var(--rot) * -1));
            opacity: 0;
            filter: blur(4px);
          }
          15% {
            opacity: 1;
            filter: blur(0px);
            transform: translate(calc(-50% + var(--tx)), calc(-50% + var(--ty))) scale(1) rotate(0deg);
          }
          60% {
            transform: translate(calc(-50% + var(--tx)*0.6), calc(-50% + var(--ty)*0.6)) scale(0.85) rotate(var(--rot));
            opacity: 1;
            filter: blur(0px);
          }
          85% {
            transform: translate(calc(-50% + var(--tx)*0.2), calc(-50% + var(--ty)*0.2)) scale(0.4) rotate(calc(var(--rot) * 2));
            opacity: 0.8;
            filter: blur(2px);
          }
          100% {
            transform: translate(-50%, -50%) scale(0) rotate(calc(var(--rot) * 3));
            opacity: 0;
            filter: blur(8px);
          }
        }
...
<style>
        /* Base typography configuration matching the premium aesthetic */
        body {
        ...
        /* Top Progressive Blur Skill Snippet */
        .gradient-blur {
          position: fixed;
          z-index: 40;
          inset: 0 0 auto 0;
        ...
        /* Smooth hiding for scroll elements */
        .initially-hidden {
          opacity: 0;
          visibility: hidden;
        }
...
<svg className="absolute -top-[12px] -left-[12px] w-6 h-6 text-neutral-800" fill="none" stroke="currentColor" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1"></path>
</svg>
<svg className="absolute -top-[12px] -right-[12px] w-6 h-6 text-neutral-800" fill="none" stroke="currentColor" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1"></path>
</svg>

<header className="pt-40 pb-24 px-6 md:px-12 relative z-10 flex flex-col items-center text-center w-full overflow-hidden">
<div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-neutral-800 bg-neutral-900/30 text-xs font-normal text-neutral-400 mb-8 opacity-0" id="hero-badge">
<span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></span>
            System initialization complete
          </div>
<h1 className="text-4xl md:text-6xl lg:text-7xl font-medium tracking-tight text-white w-full max-w-[1100px] leading-[1.06] masked-reveal-target break-words whitespace-normal"><span className="mask-word"><span className="mask-word-inner">Architecturethatscalesorganicallywithyourcomplexity.</span></span></h1>
<p className="mt-8 text-base md:text-lg text-neutral-400 max-w-2xl font-normal opacity-0" id="hero-desc">
            Begin with a single core. As data flows increase, the system
            progressively reveals deeper layers, connections, and autonomous
            sub-routines. Scroll to initiate expansion.
          </p>
</header>

<div className="mx-4 md:mx-10 mb-32 p-6 md:p-16 border border-neutral-800/60 rounded-3xl bg-neutral-950/40 relative z-10 overflow-hidden" id="expansion-container">

<svg className="absolute top-4 left-4 w-4 h-4 text-neutral-700" fill="none" stroke="currentColor" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
</svg>
<svg className="absolute top-4 right-4 w-4 h-4 text-neutral-700" fill="none" stroke="currentColor" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
</svg>
<svg className="absolute bottom-4 left-4 w-4 h-4 text-neutral-700" fill="none" stroke="currentColor" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
</svg>
<svg className="absolute bottom-4 right-4 w-4 h-4 text-neutral-700" fill="none" stroke="currentColor" viewbox="0 0 24 24">
<path d="M12 4v16m8-8H4" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5"></path>
</svg>
...
<iconify-icon className="text-2xl text-neutral-300" icon="solar:cpu-bolt-bold-duotone"></iconify-icon>
</div>
<h3 className="text-lg font-medium tracking-tight text-neutral-100">
                    Primary Core
                  </h3>
<p className="mt-2 text-sm text-neutral-500 font-normal">
                    Single point of origin. Awaiting data saturation to expand
                    network topography.
                  </p>
</div>
</div>
...
<div className="w-[1px] h-32 bg-gradient-to-b from-neutral-700 to-neutral-800 z-10 seq-line" style={{transformOrigin: 'top', transform: 'scaleY(0)'}}></div>

<div className="w-full relative z-20 mt-[-1px]">

<div className="absolute top-0 left-1/4 right-1/4 h-[1px] bg-neutral-800 seq-line-h" style={{transformOrigin: 'center', transform: 'scaleX(0)'}}></div>
<div className="flex flex-col md:flex-row justify-center gap-8 md:gap-32 pt-8">

<div className="w-full max-w-sm seq-node opacity-0" style={{transform: 'translateY(20px)'}}>
<div className="p-[1px] rounded-2xl bg-gradient-to-br from-neutral-700/50 via-neutral-800/20 to-transparent w-full">
<div className="bg-neutral-900/90 backdrop-blur-sm h-full w-full rounded-[15px] p-6 text-left border border-white/5">
<iconify-icon className="text-xl text-neutral-400 mb-3" icon="solar:database-bold-duotone"></iconify-icon>
<h3 className="text-base font-medium tracking-tight text-neutral-200">
                        Data Persistence
                      </h3>
<p className="mt-1 text-xs text-neutral-500 font-normal leading-relaxed">
                        State is separated from execution. Local cache clusters
                        begin forming to handle increased read throughput.
                      </p>
</div>
</div>
</div>
...
<div className="w-full max-w-sm seq-node opacity-0" style={{transform: 'translateY(20px)'}}>
<div className="p-[1px] rounded-2xl bg-gradient-to-bl from-neutral-700/50 via-neutral-800/20 to-transparent w-full">
<div className="bg-neutral-900/90 backdrop-blur-sm h-full w-full rounded-[15px] p-6 text-left border border-white/5">
<iconify-icon className="text-xl text-neutral-400 mb-3" icon="solar:network-bold-duotone"></iconify-icon>
<h3 className="text-base font-medium tracking-tight text-neutral-200">
                        Load Distribution
                      </h3>
<p className="mt-1 text-xs text-neutral-500 font-normal leading-relaxed">
                        Traffic is dynamically routed. Edge nodes are deployed
                        to minimize latency for incoming external requests.
                      </p>
</div>
</div>
</div>
...
<div className="w-[1px] h-32 bg-gradient-to-b from-neutral-800 to-neutral-800/50 seq-line" style={{transformOrigin: 'top', transform: 'scaleY(0)'}}></div>
<div className="w-[1px] h-32 bg-gradient-to-b from-neutral-800 to-neutral-800/50 seq-line hidden md:block" style={{transformOrigin: 'top', transform: 'scaleY(0)'}}></div>
</div>

<div className="w-full relative z-20 mt-[-1px]">

<div className="absolute top-0 left-[10%] right-[10%] h-[1px] bg-neutral-800/50 seq-line-h" style={{transformOrigin: 'center', transform: 'scaleX(0)'}}></div>
<div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-8 w-full max-w-5xl mx-auto px-4 md:px-0">

<div className="w-full seq-node opacity-0" style={{transform: 'translateY(20px)'}}>
...
<iconify-icon className="text-lg text-neutral-400" icon="solar:shield-keyhole-bold-duotone"></iconify-icon>
<span className="text-[10px] font-medium tracking-wide text-neutral-500 uppercase px-2 py-0.5 rounded bg-neutral-900 border border-neutral-800">
                            Active
                          </span>
</div>
...
<div className="h-full bg-neutral-500 w-[99%]"></div>
</div>
</div>
</div>
</div>
...
<div>
<div className="flex justify-between items-center mb-3">
<iconify-icon className="text-lg text-neutral-400" icon="solar:widget-add-bold-duotone"></iconify-icon>
<span className="flex items-center gap-1 text-[10px] font-medium tracking-wide text-neutral-500 uppercase px-2 py-0.5 rounded bg-neutral-900 border border-neutral-800">
...
<div className="mt-4 pt-4 border-t border-neutral-800/50 flex gap-1">

<div className="flex-1 h-6 bg-neutral-800/30 rounded flex items-end p-[1px]">
<div className="w-full bg-neutral-600 rounded-[2px]" style={{height: '40%'}}></div>
</div>
<div className="flex-1 h-6 bg-neutral-800/30 rounded flex items-end p-[1px]">
<div className="w-full bg-neutral-600 rounded-[2px]" style={{height: '70%'}}></div>
</div>
<div className="flex-1 h-6 bg-neutral-800/30 rounded flex items-end p-[1px]">
<div className="w-full bg-neutral-500 rounded-[2px]" style={{height: '90%'}}></div>
</div>
<div className="flex-1 h-6 bg-neutral-800/30 rounded flex items-end p-[1px]">
<div className="w-full bg-neutral-600 rounded-[2px]" style={{height: '60%'}}></div>
</div>
<div className="flex-1 h-6 bg-neutral-800/30 rounded flex items-end p-[1px]">
<div className="w-full bg-neutral-600 rounded-[2px]" style={{height: '85%'}}></div>
</div>
</div>
</div>
</div>
...
<div>
<div className="flex justify-between items-center mb-3">
<iconify-icon className="text-lg text-neutral-400" icon="solar:chart-square-bold-duotone"></iconify-icon>
<span className="text-[10px] font-medium tracking-wide text-neutral-500 uppercase px-2 py-0.5 rounded bg-neutral-900 border border-neutral-800">
                            Live
                          </span>
</div>
...
<p className="mt-1 text-xs text-neutral-600 font-normal">
                          Aggregating system logs &amp; traces.
                        </p>
</div>
<div className="mt-4 pt-4 border-t border-neutral-800/50">
...
<div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-neutral-950/40 to-transparent pointer-events-none z-30"></div>
</div>
</div>
</main>
...
<style>
        ...
        .typewriter-text {
          ...
          width: 19ch;
          animation: type 5s steps(19, end) infinite, blink 0.8s step-end infinite;
        }
      </style>
...
<p className="text-neutral-400 max-w-lg font-light leading-relaxed mb-10">
        Instantiate a new spatial computing node. Connect to the global mesh
        network with deterministic state synchronization.
      </p>
...
<p className="text-sm text-neutral-500 max-w-sm font-light leading-relaxed mt-2">
            A purely deterministic ecosystem designed for scale. Construct,
            monitor, and deploy with tactile precision across the global mesh
            network.
          
            Platform
          
          <a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Engine</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Studio</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Research</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Network Map</a>
</p></div>
<div className="flex flex-col gap-4">
<h4 className="text-[10px] font-mono font-medium text-white uppercase tracking-widest mb-2">
            Resources
          </h4>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Documentation</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">API Reference</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Changelog</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">System Status</a>
</div>
<div className="flex flex-col gap-4">
<h4 className="text-[10px] font-mono font-medium text-white uppercase tracking-widest mb-2">
            Company
          </h4>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">About</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Blog</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Careers</a>
<a className="text-sm text-neutral-500 hover:text-white transition-colors duration-200" href="#">Contact</a>
</div>
</div>
...
<a className="hover:text-white transition-colors duration-300 flex items-center gap-1.5" href="#">
...
<a className="hover:text-white transition-colors duration-300 flex items-center gap-1.5" href="#">
<svg className="lucide lucide-github" fill="none" height="14" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewbox="0 0 24 24" width="14" xmlns="http://www.w3.org/2000/svg">
...
<span>© 2024 Aura_SYS.</span>
<span className="px-2 py-0.5 border border-white/10 rounded">
            v4.0.0
          </span>
</div>
</div>

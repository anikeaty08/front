...
})(window,document,'script','dataLayer','GTM-PS5GBKN');
...
(function(){var r=document.getElementById("impact-rotator-aura-emjhjknib23wc6rtb");var s=document.getElementById("impact-rotator-sizer-aura-emjhjknib23wc6rtb");if(!r||!s)return;var words=["un arbre est planté","un repas est distribué","une personne dormira au chaud","une plage est nettoyée"];var i=0;var fade=220;var hold=2000;function setText(t){r.textContent=t;s.textContent=t;var w=s.getBoundingClientRect().width;r.style.minWidth=Math.ceil(w)+"px";}setText(words[i]);setInterval(function(){r.style.transition="opacity "+fade+"ms ease";r.style.opacity="0";window.setTimeout(function(){i=(i+1)%words.length;setText(words[i]);r.style.opacity="1";},fade+40);},hold+fade+40);})();


        (function() {
            const metrics = [
                { val: "123 553", label: "Arbres plantés à Madagascar et en Tanzanie" },
                { val: "87 815 m²", label: "Forêts protégées en France" },
                { val: "110 250", label: "Paniers repas distribués aux personnes dans le besoin" },
                { val: "1 001 400 cm²", label: "Récifs dépollués en Méditerranée" }
            ];
            
            let currentIndex = 0;
            const container = document.getElementById('impact-rotator-container');
            const valueElement = document.getElementById('impact-counter-value');
            const labelElement = document.getElementById('impact-counter-label');
            const dotsContainer = document.getElementById('impact-dots');
            
            if (!container || !valueElement || !labelElement || !dotsContainer) return;
            ...
            setInterval(() => {
                ...
                setTimeout(() => {
                    ...
                    valueElement.textContent = metrics[currentIndex].val;
                    labelElement.textContent = metrics[currentIndex].label;
                    
                    // Update Dots
                    Array.from(dots).forEach((dot, index) => {
                        if (index === currentIndex) {
                            dot.className = "w-4 h-1.5 rounded-full bg-[#06818E] transition-all duration-500";
                        } else {
                            dot.className = "w-1.5 h-1.5 rounded-full bg-[#334669]/10 transition-all duration-500";
                        }
                    });
    ...
    (function() {
      const slidesData = [
        { 
          type: "Environnement",
          location: "Kenya", 
          title: "Restaurer une mangrove au Kenya", 
          subtitle: "Pour 15 000 pas, un arbre planté"
        },
        { 
          type: "Environnement",
          location: "France", 
          title: "Protéger les forêts françaises", 
          subtitle: "Pour 15 000 pas, c'est 1m2 de forêt protégé"
        },
        { 
          type: "Environnement",
          location: "France", 
          title: "Retirer les filets fantômes en Méditerranée", 
          subtitle: "Pour 15 000 pas, c'est 500 cm2 de récifs dépollués"
        },
        { 
          type: "Lutte contre la précarité",
          location: "France", 
          title: "Offrir des paniers d'invendus aux plus précaires", 
          subtitle: "Pour 15 000 pas, c'est 5 paniers distribués"
        },
        { 
          type: "Environnement",
          location: "International", 
          title: "Soutenir les agricultrices face au changement climatique", 
          subtitle: "Pour 15 000 pas, c'est 9 jours de formation"
        }
      ];

      let currentSlideIndex = 0;
      const slideElements = document.querySelectorAll('#treely-main-carousel .carousel-main-slide');
      const titleEl = document.getElementById('main-carousel-title');
      const subtitleEl = document.getElementById('main-carousel-subtitle');
      const locationEl = document.getElementById('main-carousel-location');
      const typeEl = document.getElementById('main-carousel-type');
      const dotsContainer = document.getElementById('main-carousel-dots');

      if (!titleEl || !subtitleEl || !locationEl || !typeEl || !dotsContainer || slideElements.length === 0) return;
      ...
      function updateSlide(index) {
        ...
        locationEl.textContent = slidesData[index].location;
        typeEl.textContent = slidesData[index].type;
      ...
      setInterval(function() {
        const nextSlide = (currentSlideIndex + 1) % slidesData.length;
...
<noscript><iframe height="0" src="https://www.googletagmanager.com/ns.html?id=GTM-PS5GBKN" style={{display: 'none', visibility: 'hidden'}} width="0"></iframe></noscript>
...
<h1 className="md:text-7xl lg:text-8xl leading-[1.1] text-5xl font-extrabold text-[#334669] tracking-tight max-w-5xl mr-auto mb-6 ml-auto">Team building : <br className="hidden md:block"/> transformez vos actions <span className="text-[#06818E]">en impact.</span></h1>

<p className="md:text-xl leading-relaxed text-lg font-medium text-[#858E96] max-w-2xl mr-auto mb-10 ml-auto">Votre équipe a 10 jours pour marcher. Tous les 15 000 pas, <span className="relative inline-block align-baseline"><span className="text-[#334669] font-semibold" id="impact-rotator-aura-emjhjknib23wc6rtb" style={{minWidth: '180px'}}>un arbre est planté</span><span className="invisible absolute left-0 top-0 whitespace-nowrap font-semibold" id="impact-rotator-sizer-aura-emjhjknib23wc6rtb">un arbre est planté</span></span>.</p>
...
<div className="flex flex-col group transition-all duration-500 items-center justify-center bg-white/60 backdrop-blur-xl rounded-2xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:border-[#06818E]/20 p-6 w-full max-w-[360px] mx-auto relative overflow-hidden">

<div className="flex items-center gap-2 mb-3 opacity-90 group-hover:opacity-100 transition-opacity">
<span className="relative flex h-2 w-2">
<span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
<span className="relative inline-flex rounded-full h-2 w-2 bg-[#06818E]"></span>
</span>
<span className="text-[11px] uppercase leading-none font-bold text-[#06818E] tracking-widest">l'impact de Treely en 2025</span>
</div>

<div className="flex flex-col items-center justify-center w-full transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] transform" id="impact-rotator-container">
<div className="relative inline-block text-center py-2">
<span className="md:text-4xl tabular-nums leading-none block text-3xl font-bold text-[#334669] tracking-tight" id="impact-counter-value">523 553</span>
</div>
<p className="text-sm font-normal text-[#858E96] mt-2 text-center leading-snug min-h-[40px] flex items-center justify-center" id="impact-counter-label">
            Arbres plantés à Madagascar et en Tanzanie
        </p>
</div>

<div className="flex gap-1.5 mt-5" id="impact-dots">
<div className="w-4 h-1.5 rounded-full bg-[#06818E] transition-all duration-500"></div>
<div className="w-1.5 h-1.5 rounded-full bg-[#334669]/10 transition-all duration-500"></div>
<div className="w-1.5 h-1.5 rounded-full bg-[#334669]/10 transition-all duration-500"></div>
<div className="w-1.5 h-1.5 rounded-full bg-[#334669]/10 transition-all duration-500"></div>
</div>
...
<p className="leading-relaxed text-sm font-medium text-[#F4F0FE]/70" style={{}}>Données complètes exportables : taux d'engagement, arbres plantés et classement équipes.</p>
</div>
</div>
</div>
</div>
...
<p className="leading-relaxed text-lg font-normal text-[#858E96] mb-8" style={{}}>Vos collaborateurs n'ont qu'à installer l'app Treely. Pendant les 10 jours du défi, l'app recueille leurs pas et les convertis en action à impact. À chaque fois qu'un participant fait 15 000 pas, nous lançons une action.</p>
...
<div className="lg:sticky lg:top-32 order-2 lg:order-1 relative">

<div className="overflow-hidden aspect-[4/5] group transform transition-all duration-500 hover:shadow-3xl bg-slate-900 ring-white/10 ring-1 rounded-[2.5rem] relative shadow-2xl">

<div className="absolute inset-0 bg-[#334669]">
<div className="w-full h-full relative" id="treely-main-carousel">

<div className="carousel-main-slide transition-opacity duration-1000 ease-in-out absolute top-0 right-0 bottom-0 left-0">
<img alt="Slide 1" className="w-full h-full object-cover z-30" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/449fde6e-bbd5-46e1-a6f4-8b064729fd79_1600w.png" style={{animation: 'ken-burns-anim 20s ease-out infinite alternate'}}/>
</div>
<div className="carousel-main-slide transition-opacity duration-1000 ease-in-out absolute top-0 right-0 bottom-0 left-0">
<img alt="Slide 2" className="z-30 w-full h-full object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/4c0955fb-8b32-4365-be82-7774a757ddb3_1600w.jpg" style={{animation: 'ken-burns-anim 20s ease-out infinite alternate'}}/>
</div>
<div className="carousel-main-slide transition-opacity duration-1000 ease-in-out z-0 opacity-0 absolute top-0 right-0 bottom-0 left-0">
<img alt="Slide 3" className="w-full h-full object-cover" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/6b600cf2-057e-4b56-8a37-74528657b4e5_1600w.png" style={{animation: 'ken-burns-anim 20s ease-out infinite alternate'}}/>
</div>
<div className="carousel-main-slide transition-opacity duration-1000 ease-in-out z-0 opacity-0 absolute top-0 right-0 bottom-0 left-0">
<img alt="Slide 4" className="w-full h-full object-cover z-30" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/f7808fa9-15fd-4768-a001-6dcb75c949ad_1600w.png" style={{animation: 'ken-burns-anim 20s ease-out infinite alternate'}}/>
</div>
<div className="carousel-main-slide transition-opacity duration-1000 ease-in-out opacity-0 z-0 absolute inset-0">
<img alt="Slide 5" className="w-full h-full object-cover z-30" src="https://hoirqrkdgbmvpwutwuwj.supabase.co/storage/v1/object/public/assets/assets/6ee650c7-74ce-4901-901b-f7288f315666_1600w.jpg" style={{animation: 'ken-burns-anim 20s ease-out infinite alternate'}}/>
</div>

<div className="absolute inset-0 bg-gradient-to-t from-[#0f172a]/95 via-[#0f172a]/30 to-transparent z-20 pointer-events-none"></div>

<div className="flex flex-col md:p-10 z-30 pt-8 pr-8 pb-12 pl-8 absolute top-0 right-0 bottom-0 left-0 justify-end">
<div className="w-full max-w-lg">

<div className="flex flex-wrap gap-2 mb-5 items-center">

<span className="inline-flex items-center text-[10px] md:text-xs ring-inset uppercase shadow-[#06818E]/20 font-bold text-white tracking-widest bg-[#06818E] ring-white/20 ring-1 rounded-full pt-1 pr-3 pb-1 pl-3 shadow-lg backdrop-blur-md" id="main-carousel-type">Environnement</span>

<span className="inline-flex items-center gap-1.5 text-[10px] md:text-xs ring-inset uppercase font-bold text-white tracking-widest bg-white/10 ring-white/20 ring-1 rounded-full pt-1 pr-3 pb-1 pl-3 shadow-sm backdrop-blur-md">
<svg className="text-[#359D9E]" fill="none" height="12" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" viewbox="0 0 24 24" width="12" xmlns="http://www.w3.org/2000/svg"><path className="" d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"></path><circle className="" cx="12" cy="10" r="3"></circle></svg>
<span className="" id="main-carousel-location">France</span>
</span>
</div>

<div className="space-y-2 mb-8">
<h3 className="md:text-4xl lg:text-[2.75rem] leading-[1.1] text-3xl font-bold text-white tracking-tight drop-shadow-lg" id="main-carousel-title">Protéger les forêts françaises</h3>
<p className="md:text-lg leading-relaxed text-base font-medium text-white/90 max-w-sm drop-shadow-md" id="main-carousel-subtitle">Pour 15 000 pas, c'est 1m2 de forêt protégé</p>
</div>

<div className="flex items-center gap-2" id="main-carousel-dots">
<button aria-label="Go to slide 1" className="h-1 rounded-full transition-all duration-500 ease-out" data-slide="0"></button>
<button aria-label="Go to slide 2" className="hover:bg-white/60 transition-all duration-500 ease-out h-1 rounded-full" data-slide="1"></button>
<button aria-label="Go to slide 3" className="h-1 rounded-full bg-white/30 w-4 hover:bg-white/60 transition-all duration-500 ease-out" data-slide="2"></button>
<button aria-label="Go to slide 4" className="h-1 rounded-full bg-white/30 w-4 hover:bg-white/60 transition-all duration-500 ease-out" data-slide="3"></button>
<button aria-label="Go to slide 5" className="h-1 rounded-full bg-white/30 w-4 hover:bg-white/60 transition-all duration-500 ease-out" data-slide="4"></button>
</div>
</div>
</div>
</div>
...
<div className="-z-10 -left-6 w-full h-full border-[#334669]/10 border rounded-[2.5rem] absolute top-6"></div>
</div>

<div className="order-1 lg:order-2 pt-4 relative">

<div className="absolute left-[28px] top-10 bottom-12 w-[2px] bg-slate-200"></div>

<div className="flex gap-8 group mb-14 relative">
<div className="relative z-10 shrink-0">
<div className="flex shadow-[#06818E]/20 transition-transform group-hover:scale-110 duration-300 text-white bg-[#06818E] w-14 h-14 ring-white ring-8 rounded-2xl shadow-xl items-center justify-center">
...
<h3 className="group-hover:text-[#06818E] transition-colors text-xl font-semibold text-[#334669] mb-2">
                  Après le défi
                </h3>
<p className="text-[#858E96] leading-relaxed font-normal">
...
<form action="https://hook.eu1.make.com/5gz2sgykff33qjbt983hx4ike7sd5j3m" className="z-10 relative space-y-5" method="POST">

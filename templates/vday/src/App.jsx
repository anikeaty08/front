...
...transformStyleUtilities,
        ...
        function calculateLove() {
            ...
            setTimeout(() => {
                ...
                if (isPair('win', 'victoria') || isPair('tom', 'mary') || isPair('jim', 'harla') || isPair('james', 'maliha') || isPair('ben', 'reya') || isPair('jessica', 'erik') || isPair('laramie', 'michael') || isPair('travis', 'lindsay')) {
                    score = 100;
                    message = isPair('win', 'victoria') ? "Congratulations! You've got your perfect match!! ❤️" : "It's a 100% Match! The stars have aligned! ✨";
                }
                    ...
                    score = Math.floor(Math.random() * 79);
                    message = score < 30 ? "A bit rocky, but love finds a way." : (score < 60 ? "There is definitely a spark!" : "Looking good together!");
                }
        ...
        function spawnButterflies() {
            ...
            const card = document.getElementById('valentines-card');
            ...
            for (let i = 0; i < 16; i++) {
                const butterfly = document.createElement('iconify-icon');
                butterfly.setAttribute('icon', 'ph:butterfly'); 
                butterfly.classList.add('butterfly', 'text-rose-400', 'butterfly-wing');
                butterfly.setAttribute('width', Math.random() * 20 + 20);
                ...
                const destX = Math.cos(angle) * (window.innerWidth / 2) + (Math.random() * 200 - 100);
                const destY = Math.sin(angle) * (window.innerHeight / 2) - 300;

                const animation = butterfly.animate([
                    { transform: `translate(-50%, -50%) rotate(0deg) scale(0)`, opacity: 0 },
                    { transform: `translate(-50%, -50%) rotate(${Math.random() * 30 - 15}deg) scale(1)`, opacity: 1, offset: 0.1 },
                    { transform: `translate(${destX}px, ${destY}px) rotate(${Math.random() * 90 - 45}deg) scale(0.8)`, opacity: 0 }
                ], {
                ...
                animation.onfinish = () => butterfly.remove();
            }
        }

        function burstLove(element) {
            const container = document.getElementById('effect-container');
            const rect = element.getBoundingClientRect();
            const centerX = rect.left + rect.width / 2;
            const centerY = rect.top + rect.height / 2;
            
            const icons = ['solar:heart-bold', 'solar:heart-linear', 'solar:rose-linear', 'solar:rose-linear'];
            
            for(let i=0; i<25; i++) {
                const el = document.createElement('iconify-icon');
                const icon = icons[Math.floor(Math.random() * icons.length)];
                el.setAttribute('icon', icon);
                el.classList.add('love-particle', 'text-rose-500');
                ...
                const moveY = (Math.sin(angle) * velocity) - 150; // Tend upwards
                
                const animation = el.animate([
                    { transform: 'translate(-50%, -50%) scale(0) rotate(0deg)', opacity: 0 },
                    { transform: 'translate(-50%, -50%) scale(1.2) rotate(10deg)', opacity: 1, offset: 0.2 },
                    { transform: `translate(calc(-50% + ${moveX}px), calc(-50% + ${moveY}px)) scale(0.5) rotate(${Math.random()*90}deg)`, opacity: 0 }
                ], {
...
<div className="fixed inset-0 pointer-events-none z-50 overflow-hidden w-full h-full" id="effect-container"></div>

<div className="fixed inset-0 pointer-events-none z-0 opacity-40 overflow-hidden">
<div className="absolute top-[10%] left-[5%] text-rose-200 animate-float" style={{animationDelay: '0s'}}><iconify-icon icon="solar:heart-bold" width="64"></iconify-icon></div>
<div className="absolute top-[30%] right-[10%] text-rose-200 animate-float" style={{animationDelay: '2s'}}><iconify-icon icon="solar:heart-bold" width="48"></iconify-icon></div>
<div className="absolute bottom-[20%] left-[15%] text-rose-200 animate-float" style={{animationDelay: '4s'}}><iconify-icon icon="solar:heart-angle-bold" width="56"></iconify-icon></div>
</div>
...
<div className="w-8 h-8 rounded-full bg-rose-600 text-white flex items-center justify-center text-xs font-semibold tracking-tighter shadow-lg shadow-rose-200">
                W+V
            </div>
</div>
<div className="hidden md:flex items-center gap-6 text-sm text-slate-500 font-medium">
<a className="hover:text-rose-600 transition-colors" href="#story">Our Story</a>
<a className="hover:text-rose-600 transition-colors" href="#details">The Dream</a>
<a className="hover:text-rose-600 transition-colors" href="#poem">For You</a>
</div>
...
<iconify-icon className="text-rose-500 animate-pulse" icon="solar:music-note-linear" strokeWidth="1.5" width="16"></iconify-icon>
<span className="text-xs font-medium text-slate-600 group-hover:text-rose-600 transition-colors">Playlist</span>
</a>
</nav>
...
<div className="inline-flex uppercase text-xs font-semibold text-rose-600 tracking-wide bg-rose-100 border-rose-200 border rounded-full pt-1.5 pr-4 pb-1.5 pl-4 shadow-md gap-x-2 gap-y-2 items-center">
<iconify-icon className="text-rose-600" icon="solar:heart-bold" width="24"></iconify-icon>
...
<h1 className="md:text-8xl lg:text-9xl leading-[0.9] text-gradient-animate text-6xl font-medium tracking-tight font-serif-custom max-w-5xl mr-auto mb-2 ml-auto">HAPPY<br/>VALENTINE'S<br/>DAY !!</h1>
</div>
...
<div className="text-sm font-medium text-slate-600 bg-slate-100 max-w-[85%] rounded-2xl pt-2.5 pr-4 pb-2.5 pl-4">Hey Victoria! 👋</div>
</div>
...
<span className="text-xs text-rose-300 font-semibold pr-1 flex items-center gap-1">
                        Read <iconify-icon icon="solar:check-read-linear" width="12"></iconify-icon>
</span>
</div>
<div className="flex justify-center py-6">
<span className="text-xs text-slate-400 uppercase tracking-widest font-semibold bg-slate-50 px-3 py-1 rounded-full">Where it all began</span>
</div>
</div>
...
<h2 className="text-3xl font-serif-custom italic text-slate-900 tracking-tight mb-4 inline-flex items-center gap-3">
                    Our Timeline
...
<h3 className="text-xl font-semibold text-slate-900 tracking-tight">The Connection</h3>
<p className="text-slate-500 mt-2 font-light">June 15th. The digital spark that lit the fire.</p>
</div>
...
<div className="md:w-1/2 md:pl-12 order-3 md:order-3">
<h3 className="text-xl font-semibold text-slate-900 tracking-tight">Getting to know each other</h3>
<p className="text-slate-500 mt-2 font-light">Bonding over cats (Burizer &amp; Ivy), deep talks, and shared laughter.</p>
</div>
</div>

<div className="relative flex flex-col md:flex-row items-center gap-8 group">
<div className="md:w-1/2 md:text-right md:pr-12 order-2 md:order-1">
<h3 className="text-xl font-semibold text-slate-900 tracking-tight">The Forever</h3>
<p className="text-slate-500 mt-2 font-light">Building our dream life. Creating our sanctuary.</p>
</div>
...
<iconify-icon icon="solar:home-heart-linear" strokeWidth="1.5" width="22"></iconify-icon>
</div>
...
<div className="">
<h3 className="text-3xl md:text-4xl font-serif-custom text-white leading-tight mb-2 drop-shadow-md tracking-tight">
                                The Gothic Victorian
                            </h3>
<p className="text-slate-200 text-sm md:text-base font-light max-w-md drop-shadow">
                                High ceilings, intricate spires, dark wood, and a garden for the cats. A forever sanctuary designed for Victoria's brilliant soul.
                            </p>
</div>
<div className="flex gap-6 pt-4 border-t border-white/10">
<div className="flex flex-col">
<span className="text-xs text-rose-200/80 uppercase tracking-wider">Style</span>
<span className="text-sm text-white font-medium">Victorian Gothic</span>
</div>
<div className="flex flex-col">
<span className="text-xs text-rose-200/80 uppercase tracking-wider">Residents</span>
<span className="text-sm font-medium text-white">Win, Victoria, mini us, Cats</span>
</div>
<div className="flex flex-col">
<span className="text-xs text-rose-200/80 uppercase tracking-wider">Status</span>
<span className="text-sm text-rose-400 font-medium">Manifesting</span>
</div>
</div>
</div>
</div>
...
<iconify-icon className="text-rose-50 absolute top-4 right-4" icon="solar:quote-up-square-linear" strokeWidth="1.5" width="80"></iconify-icon>
<div className="relative z-10">
<p className="text-lg font-serif-custom italic text-slate-800 leading-relaxed">
                            "I am just happy to linger in her light. She is the main character."
                        </p>
<div className="mt-4 flex items-center gap-2">
<div className="w-6 h-px bg-rose-400"></div>
<span className="text-xs font-semibold text-rose-500 uppercase tracking-widest">Win</span>
</div>
</div>
</div>
...
<div className="relative z-10">
<h4 className="text-lg font-semibold text-slate-900 tracking-tight">Burizer &amp; Ivy</h4>
...
<iconify-icon className="text-rose-400 mb-6" height="42" icon="solar:list-heart-minimalistic-linear" strokeWidth="1.5" width="42"></iconify-icon>
<h2 className="text-4xl md:text-5xl font-serif-custom italic text-slate-900 mb-10 tracking-tight">Win &amp; Victoria</h2>
<div className="font-serif-custom text-xl md:text-2xl text-slate-700 leading-loose space-y-10">
<p className="">
                        Two names that echo, almost the same,<br/>
                        Win and Victoria, a mirrored flame.<br/>
<span className="text-rose-500">March and May</span>, the fifteenth aligned,<br/>
                        Two hearts marked by a secret design.
                    </p>
<p>
                        Your days apart, yet bound by thread,<br/>
                        A rhythm of fate the stars have spread.<br/>
                        Not chance, but a whisper the cosmos knew,<br/>
                        That one day my steps would lead me to you.
                    </p>
<p>
                        Victoria — victory, radiant and true,<br/>
                        And Win, who finds his triumph in you.<br/>
                        Together, a story the calendar sings,<br/>
                        Of love that awakens in simplest things.
                    </p>
<p>
                        So here’s my vow, both tender and bold:<br/>
                        To cherish the warmth your green eyes hold.<br/>
                        For names may rhyme and dates may align,<br/>
...
<span className="uppercase text-xs font-semibold text-rose-500 tracking-widest mb-2">Our Playlist</span>
<h2 className="text-3xl font-serif-custom text-slate-900 italic flex items-center gap-3 tracking-tight">
                Songs for Us 
...
<h3 className="text-2xl font-semibold text-slate-900 mb-1 fade-text tracking-tight" id="player-title">Smile</h3>
<p className="text-slate-500 font-medium fade-text" id="player-artist">Uncle Kracker</p>
</div>
...
<span className="w-8 text-right tabular-nums" id="current-time">0:00</span>
...
<h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Playlist</h3>
<div className="flex items-center gap-1 text-xs text-rose-500 font-semibold bg-rose-50 px-2 py-1 rounded-full border border-rose-100">
<iconify-icon icon="solar:infinity-bold"></iconify-icon> Auto-play
                    </div>
</div>
...
<h4 className="text-sm font-semibold text-slate-900 truncate">Smile</h4>
...
<div className="flex-1 min-w-0">
<h4 className="truncate text-sm font-semibold text-slate-900">God is a Weapon</h4>
...
<div className="flex-1 min-w-0">
<h4 className="text-sm font-semibold text-slate-900 truncate">Every Woman in the World</h4>
...
<div className="flex-1 min-w-0">
<h4 className="truncate text-sm font-semibold text-slate-900">I Lied</h4>
...
<div className="flex-1 min-w-0">
<h4 className="text-sm font-semibold text-slate-900 truncate">Perfect Two</h4>
...
<div className="flex-1 min-w-0">
<h4 className="truncate text-sm font-semibold text-slate-900">Keeping Me Up All Night</h4>
...
<div className="flex-1 min-w-0">
<h4 className="text-sm font-semibold text-slate-900 truncate">One Thing</h4>
...
<span className="uppercase text-xs font-semibold tracking-widest bg-white/20 border-white/30 border rounded-md pt-1 pr-2 pb-1 pl-2 backdrop-blur-sm">Bloom</span>
</div>
</div>
...
<span className="text-xs font-semibold uppercase tracking-widest bg-white/20 backdrop-blur-sm px-2 py-1 rounded-md border border-white/30">Sweet</span>
</div>
</div>
...
<p className="font-serif-custom italic text-2xl md:text-3xl text-rose-600 leading-tight">
                        "For you,<br/> from Win,<br/> with Love"
                    </p>
...
<div className="bg-white rounded-[2.5rem] p-8 shadow-[0_20px_60px_-15px_rgba(225,29,72,0.15)] border border-rose-100 relative overflow-hidden">
<div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-rose-400 via-rose-500 to-rose-400"></div>
...
<h2 className="text-2xl font-serif-custom italic text-slate-900 mb-2 tracking-tight">Love Compatibility</h2>
<p className="text-sm text-slate-500">Calculate the odds of your forever</p>
</div>
<div className="space-y-4 relative z-10">
<div className="">
<label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5 ml-2">Name 1</label>
...
<div className="">
<label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5 ml-2">Name 2</label>
...
<button className="w-full bg-rose-600 hover:bg-rose-700 text-white font-semibold py-4 rounded-xl shadow-lg shadow-rose-200 transition-all active:scale-95 flex justify-center items-center gap-2 mt-4 group" onclick="calculateLove()">
...
<div className="text-6xl font-black text-rose-600 tracking-tighter mb-2" id="calc-score">0%</div>
<p className="text-lg font-serif-custom italic text-slate-800" id="calc-message">Measuring vibes...</p>
...
<div className="max-w-md mx-auto relative h-[500px] w-full cursor-pointer group" id="valentines-card" onclick="toggleCard(this)">
<div className="text-center mb-8 absolute -top-16 left-0 right-0 transition-opacity duration-500 group-[.card-open]:opacity-0">
<span className="inline-flex items-center gap-2 uppercase animate-bounce text-xs font-semibold text-rose-500 tracking-widest bg-white border-rose-100 border rounded-full pt-2 pr-4 pb-2 pl-4 shadow-sm">
<iconify-icon icon="solar:hand-stars-linear" strokeWidth="1.5"></iconify-icon> Tap to Open
                </span>
</div>
...
<h3 className="font-serif-custom italic text-2xl text-slate-900 mb-6 tracking-tight">My Dearest Victoria,</h3>
<p className="text-slate-600 leading-relaxed mb-6 font-medium">
                            Every day with you feels like this holiday. Thank you for the life you bring to my days. You're my best friend, and my home. In a world of chaos, you are my calm.
                        </p>
...
<span className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Forever Yours, Win</span>
</div>
</div>
</div>
...
<iconify-icon className="mb-6 drop-shadow-md animate-heartbeat" icon="solar:letter-heart-linear" strokeWidth="1.5" width="80"></iconify-icon>
<h2 className="font-serif-custom italic text-4xl mb-2 tracking-tight">For Victoria</h2>
<p className="text-xs font-semibold uppercase tracking-[0.2em] text-rose-100">Open Me</p>
...
<div className="relative group cursor-pointer active:scale-95 transition-transform" onclick="burstLove(this)">
<div className="absolute -inset-4 bg-gradient-to-r from-rose-400 to-rose-600 rounded-full blur opacity-20 group-hover:opacity-40 transition duration-1000 pointer-events-none"></div>
<div className="w-48 h-48 md:w-64 md:h-64 relative transition-transform duration-500 hover:scale-105 heart-mask">
...
<span className="bg-white/90 backdrop-blur text-rose-600 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest shadow-lg">Click Me</span>
</div>
</div>
<p className="text-sm italic text-rose-500 font-serif-custom mt-4">Victoria ❤️ Win</p>
</div>
</section>
...
<div className="flex shadow-rose-200 text-sm font-semibold text-white tracking-tighter bg-gradient-to-tr from-rose-500 to-rose-600 w-12 h-12 rounded-full shadow-lg items-center justify-center animate-pulse">
...
<p className="text-base font-medium text-slate-900 mb-2">Building a Gothic Victorian life together.</p>
<p className="text-xs text-slate-400">© 2025-2026 Win &amp; Victoria. Happy Valentine's Day.</p>
</div>
</footer>

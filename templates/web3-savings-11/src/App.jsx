...
xxs: '0.65rem',
        ...
        // --- STATE MANAGEMENT ---
        const state = {
            view: 'login', // login, dashboard, group-detail, create-group
            user: null,
            groups: [],
            activeGroupId: null,
            loading: false
        ...
        const LoginView = () => `
            <div class="flex flex-col items-center justify-center h-full bg-zinc-50 px-4">
                <div class="w-full max-w-sm bg-white border border-zinc-200 shadow-sm rounded-xl p-8 text-center">
                    ...
                    <h1 class="text-xl font-semibold tracking-tight text-zinc-900 mb-2">Welcome to Circa</h1>
                    <p class="text-sm text-zinc-500 mb-8">Rotating savings circles on-chain.</p>
                    
                    <button id="btn-login" class="w-full bg-zinc-900 hover:bg-zinc-800 text-white font-medium text-sm py-2.5 rounded-lg transition-all flex items-center justify-center gap-2 group">
                    ...
                    <div class="bg-white border border-zinc-200 rounded-lg p-3 shadow-sm">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1 overflow-hidden">
                                <p class="text-xs font-medium text-zinc-900 truncate">${state.user.address}</p>
                                <p class="text-xs text-zinc-500 truncate">${state.user.balance}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            return `
                <div class="max-w-4xl mx-auto space-y-8">
                    <div>
                        ...
                        <p class="text-sm text-zinc-500">Welcome back. You have ${pendingGroups.length} pending contributions.</p>
                    </div>
        ...
        const GroupDetailView = () => {
            ...
            const isMeRecipient = currentRecipient.address === MOCK_USER.address.substring(0, 10); // Simple check
        ...
        const actions = {
            ...
            pay: async (groupId) => {
                ...
                try {
                    ...
                    const me = group.members.find(m => m.name === "You"); // Simplified
...
<div className="flex-1 flex flex-col h-full overflow-hidden relative" id="app">
